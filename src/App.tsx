...
function ProtectedLayout() {
  ...
  const doSignOut = async () => { const pending = await technicianOfflineService.pending(); if (pending.length && !window.confirm(`Hay ${pending.length} cambios técnicos pendientes de sincronizar. Si sales, seguirán guardados en este dispositivo. ¿Quieres cerrar sesión?`)) return; setAlertsOpen(false); await signOut(); navigate('/', { replace: true }); };
...
function SyncButton({ workOrderId, checkId }: { workOrderId?: string; checkId?: string }) {
  ...
  return <div className="sync-box"><button className="primary" disabled={!pending.length || syncing} onClick={sync}>{syncing ? 'Sincronizando...' : `Sincronizar (${pending.length})`}</button>{message && <p>{message}</p>}</div>;
}
