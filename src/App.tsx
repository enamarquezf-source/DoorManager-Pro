...
import { quoteLineTypes, quoteStatuses, quoteTypes, quotesService } from './services/quotesService';
...
function navForWorkspace(workspace: Workspace) {
  const superadmin = [{ id: 'inicio', label: 'Inicio', path: '/app/superadmin', icon: ShieldAlert }, { id: 'usuarios', label: 'Usuarios', path: '/app/superadmin/usuarios', icon: UsersRound }, { id: 'crear-usuario', label: 'Crear usuario', path: '/app/superadmin/usuarios/nuevo', icon: UserRound }, { id: 'roles', label: 'Roles y permisos', path: '/app/superadmin/roles', icon: Settings }, { id: 'clientes', label: 'Clientes', path: '/app/superadmin/clientes', icon: Building2 }, { id: 'centros', label: 'Centros', path: '/app/superadmin/centros', icon: Factory }, { id: 'equipos', label: 'Equipos', path: '/app/superadmin/equipos', icon: Warehouse }, { id: 'partes', label: 'Partes', path: '/app/superadmin/partes', icon: ClipboardList }, { id: 'presupuestos', label: 'Presupuestos', path: '/app/modulos/presupuestos', icon: FileText }, { id: 'checks', label: 'Checks', path: '/app/superadmin/checks', icon: ClipboardCheck }, { id: 'plantillas', label: 'Plantillas de checks', path: '/app/superadmin/plantillas', icon: PackageCheck }, { id: 'sincronizacion', label: 'Sincronización', path: '/app/superadmin/sincronizacion', icon: Truck }, { id: 'auditoria', label: 'Auditoría', path: '/app/superadmin/auditoria', icon: FileText }, { id: 'configuracion', label: 'Configuración', path: '/app/superadmin/configuracion', icon: Settings }];
  const sat = [{ id: 'inicio', label: 'Inicio', path: '/app/inicio', icon: Home }, { id: 'planificacion', label: 'Planificación', path: '/app/modulos/planificacion', icon: CalendarClock }, { id: 'tecnicos', label: 'Técnicos', path: '/app/modulos/tecnicos', icon: UsersRound }, { id: 'clientes', label: 'Clientes', path: '/app/clientes', icon: Building2 }, { id: 'centros', label: 'Centros', path: '/app/centros', icon: Factory }, { id: 'equipos', label: 'Equipos', path: '/app/equipos', icon: Warehouse }, { id: 'expedientes', label: 'Expedientes', path: '/app/expedientes', icon: FileText }, { id: 'partes', label: 'Partes', path: '/app/partes', icon: ClipboardList }, { id: 'presupuestos', label: 'Presupuestos', path: '/app/modulos/presupuestos', icon: FileText }, { id: 'checks', label: 'Checks', path: '/app/checks', icon: ClipboardCheck }, { id: 'deficiencias', label: 'Deficiencias', path: '/app/deficiencias', icon: ShieldAlert }, { id: 'documentos', label: 'Documentación', path: '/app/documentos', icon: FileText }, { id: 'avisos', label: 'Avisos', path: '/app/avisos', icon: Bell }, { id: 'plantillas', label: 'Plantillas', path: '/app/plantillas', icon: PackageCheck }];
...
function QuotesModule() {
  ...
  const [search, setSearch] = useState('');
  const [creating, setCreating] = useState(false);
  const { data, loading, error, reload } = useLoad(() => quotesService.list(search), [search], [] as any[]);
  return <ListPage title="Presupuestos" summary="Presupuestos de instalación, reparación y mantenimiento con costes, venta y margen." search={search} setSearch={setSearch} action={<button className="primary" onClick={() => setCreating(true)}>Crear presupuesto</button>} loading={loading} error={error} retry={reload} empty={!data.length}><div className="grid half">{data.map((quote: any) => <Card key={quote.id} title={`${quote.code} · ${quote.title}`} action={<Link to={`/app/modulos/presupuestos/${quote.id}`}>Abrir</Link>}><InfoGrid items={[[ 'Tipo', displayStatus(quote.quote_type ?? 'reparacion') ], [ 'Estado', displayStatus(quote.status) ], [ 'Cliente', quote.clients?.legal_name ?? '-' ], [ 'Base venta', `${Number(quote.subtotal_sale ?? quote.subtotal ?? 0).toLocaleString('es-ES')} €` ], [ 'Impuestos', `${Number(quote.tax_amount ?? 0).toLocaleString('es-ES')} €` ], [ 'Total', `${Number(quote.total_amount ?? quote.total ?? 0).toLocaleString('es-ES')} €` ], [ 'Margen estimado', `${Number(quote.estimated_margin ?? 0).toLocaleString('es-ES')} €` ], [ 'Validez', quote.valid_until ?? '-' ]]} /></Card>)}</div>{creating && <QuoteForm onClose={() => setCreating(false)} onSaved={(id: string) => { setCreating(false); reload(); navigate(`/app/modulos/presupuestos/${id}`); }} />}</ListPage>;
}
...
function QuoteLineForm({ quoteId, initial, onClose, onSaved }: { quoteId: string; initial?: any; onClose: () => void; onSaved: () => void }) {
  ...
  const [error, setError] = useState('');
  const set = (key: string, value: any) => setValues((current) => ({ ...current, [key]: value }));
  const selectMaterial = (id: string) => { const material = materials.data.find((item: any) => item.id === id); setValues((current) => ({ ...current, material_id: id || null, description: material?.description ?? current.description, unit: material?.unit ?? current.unit, unit_price: material?.unit_price ?? current.unit_price })); };
