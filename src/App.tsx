...
import { canAccessModule, canAccessRoute, canAssignTechnician, canCreateAlert, canCreateCheck, canCreateWorkOrder, canEditWorkOrder, canExecuteWorkOrder, canRole, canViewWorkOrder, isSuperadmin } from './auth/permissions';
...
function SuperadminTemplates() {
  const { data, loading, error, reload } = useLoad(() => superadminService.templates(), [], [] as any[]);
  const toggle = async (template: any) => { await superadminService.toggleTemplate(template.id, !template.active); reload(); };
  return <section className="page"><Breadcrumb items={['Propietario DMP', 'Plantillas de checks']} /><div className="page-head"><div><h2>Plantillas de checks</h2><p>Vista global para detectar tipos de equipo sin plantilla y activar/desactivar plantillas.</p></div></div><StateBlock loading={loading} error={error} retry={reload} empty={!data.length}><div className="grid half">{data.map((template: any) => <Card key={template.id} title={template.name} action={<button onClick={() => toggle(template)}>{template.active ? 'Desactivar' : 'Activar'}</button>}><InfoGrid items={[[ 'Tipo compatible', template.equipment_types?.name ?? 'Global' ], [ 'Versión', template.version ], [ 'Estado', template.active ? 'Activo' : 'Inactivo' ], [ 'Bloques', String(template.check_template_sections?.length ?? 0) ], [ 'Componentes', String((template.check_template_sections ?? []).reduce((sum: number, section: any) => sum + (section.check_template_items?.length ?? 0), 0)) ], [ 'Creación', formatDate(template.created_at) ], [ 'Última modificación', formatDate(template.updated_at) ]]} /><div className="actions"><button disabled>Crear plantilla</button><button disabled>Editar plantilla</button><button disabled>Duplicar plantilla</button><button disabled>Añadir bloques/componentes</button></div><p className="large-note">Edición estructural preparada para RPC segura; no se hace con privilegios elevados en frontend.</p></Card>)}</div></StateBlock></section>;
}

function SuperadminSync() {
  const { pending, summary, reload } = useOfflineQueue();
  return <section className="page"><Breadcrumb items={['Propietario DMP', 'Sincronización']} /><Hero title="Sincronización" subtitle="Estado local visible para el usuario actual. Los errores globales requieren backend o tabla agregada." tone="warn" /><Card title="Resumen local"><InfoGrid items={[[ 'Pendientes', summary.pending ], [ 'Fallidos', summary.failed ], [ 'Sincronizados', 'No persistido localmente' ], [ 'Último intento', pending[0]?.updatedAt ? formatDate(pending[0].updatedAt) : '-' ]]} /><button onClick={reload}>Actualizar</button></Card><CompactRows rows={pending.map((item) => [displayStatus(item.status), `Motivo: ${item.error ?? '-'} · Usuario/técnico: sesión actual · Parte: ${item.workOrderId ?? '-'} · Check: ${item.checkId ?? '-'}`, item.status === 'failed' ? 'danger' : 'warn', '/app/pendientes'])} empty="Sin pendientes locales visibles." /></section>;
}
...
function EquipmentPage() { const { workspace } = useAuth(); const baseRoute = workspace === 'superadmin' ? '/app/superadmin/equipos' : '/app/equipos'; const [search, setSearch] = useState(''); const { data, loading, error, reload } = useLoad(() => equipmentService.list(search), [search], [] as any[]); const [creating, setCreating] = useState(false); return <ListPage title="Equipos" summary="Inventario conectado a public.equipment, tipos y componentes." search={search} setSearch={setSearch} action={<button className="primary" onClick={() => setCreating(true)}>Crear equipo</button>} loading={loading} error={error} retry={reload} empty={!data.length}><div className="grid half">{data.map((item) => <Card key={item.id} title={item.code} action={<Link to={`${baseRoute}/${item.id}`}>Abrir</Link>}><InfoGrid items={[[ 'Tipo', item.equipment_types?.name ?? '-' ], [ 'Cliente', item.clients?.legal_name ?? '-' ], [ 'Centro', item.sites?.name ?? '-' ], [ 'Ubicación', item.internal_location ?? '-' ], [ 'Marca/modelo', `${item.brand ?? '-'} ${item.model ?? ''}` ], [ 'Estado', displayStatus(item.status) ], [ 'Próxima revisión', item.next_review_date ?? '-' ]]} /></Card>)}</div>{creating && <EquipmentForm onClose={() => setCreating(false)} onSaved={() => { setCreating(false); reload(); }} />}</ListPage>; }
...
function WorkOrdersPage() {
  ...
  const changeFilter = (next: string) => { setFilter(next); const updated = new URLSearchParams(params); ['estado', 'prioridad', 'filtro', 'fecha'].forEach((key) => updated.delete(key)); if (next === 'sin-asignar' || next === 'checks-pendientes') updated.set('filtro', next); else if (next === 'en-curso') updated.set('estado', 'en-curso'); else if (next === 'finalizados') updated.set('estado', 'realizado'); else if (next === 'pendientes') updated.set('estado', 'pendiente'); else if (next === 'urgentes') updated.set('prioridad', 'critica'); else if (next === 'hoy') updated.set('fecha', 'hoy'); setParams(updated, { replace: true }); };
  ...
  return <ListPage title="Partes" summary="Partes SAT con técnicos, comerciales, checks y carga de trabajo asociada." search={search} setSearch={setSearch} action={canCreateWorkOrder(profile) ? <button className="primary" onClick={() => setCreating(true)}>Crear parte</button> : null} loading={loading} error={error} retry={reload} empty={!rows.length}><div className="filters sat-filters"><FormSelect label="Filtro" value={filter} onChange={changeFilter} options={[['todos','Todos'],['sin-asignar','Sin asignar'],['checks-pendientes','Checks pendientes'],['en-curso','En curso'],['finalizados','Finalizados'],['pendientes','Pendientes operativos'],['urgentes','Alta o crítica'],['hoy','Hoy']].map(([value, label]) => ({ value, label }))} /><FormSelect label="Técnico o comercial" value={person} onChange={setPerson} options={people.map((name) => ({ value: name, label: name }))} /><button className="link-button" onClick={clearFilters}>Limpiar filtros</button></div><div className="sat-work-list">{rows.map((work: any) => <SatWorkOrderCard key={work.id} work={work} />)}</div>{workspace === 'sat' && rows.length === 0 && <p className="large-note">No hay partes para este filtro. Cambia el filtro o la búsqueda.</p>}{creating && <WorkOrderForm onClose={() => setCreating(false)} onSaved={() => { setCreating(false); reload(); }} />}</ListPage>;
}
...
function CheckDetailPage({ forcedId }: { forcedId?: string } = {}) {
  ...
  const sectionStatus = (zone: CheckBlockId) => {
    ...
    return normalizeCheckStatus(result?.result ?? 'Sin revisar');
  };
...
function CheckBlockPageLegacy() {
  ...
  useEffect(() => {
    ...
    technicianOfflineService.sectionState(id, blockId).then((local) => {
      if (local) {
        ...
        setObservations(existing.observations ?? ''); setIntervention(''); setSeverity('Leve'); setComponents([]); setPhotos([]);
      } else {
        setStatus('Sin revisar'); setConfirmedStatus('Sin revisar'); setObservations(''); setIntervention(''); setSeverity('Leve'); setComponents([]); setPhotos([]);
      }
      setLocalLoaded(true);
  ...
  return <section className="check-mobile block-page"><button className="link-button sticky-back" onClick={() => navigate(`/app/checks/${id}`)}><ChevronLeft size={16} /> Volver a la puerta</button><header><p className="eyebrow">Detalle del bloque</p><h2>{zone.name}</h2><Badge tone={severityForStatus(status)}>{displayStatus(status)}</Badge></header><div className="status-grid">{checkStatuses.map((item) => <button type="button" key={item} className={status === item ? 'active' : ''} onClick={() => selectStatus(item)}>{item}</button>)}</div>{status === 'Sin revisar' && <p className="large-note">Selecciona un estado. No se guardará hasta pulsar Confirmar selección.</p>}{needsDetail && <Card title="Observación e intervención"><label>Observación<textarea value={observations} onChange={(event) => { setObservations(event.target.value); setSaveState('idle'); }} /></label><label>Intervención<textarea value={intervention} onChange={(event) => { setIntervention(event.target.value); setSaveState('idle'); }} placeholder="Intervención realizada si aplica" /></label></Card>}{needsDetail && <Card title="Incidencia del bloque"><div className="component-select">{zone.components.map((component) => <label key={component}><input type="checkbox" checked={components.includes(component)} onChange={() => { toggleComponent(component); setSaveState('idle'); }} /> {component}</label>)}</div><FormSelect label="Gravedad" value={severity} onChange={(value) => { setSeverity(value); setSaveState('idle'); }} options={['Leve','Media','Alta','Critica'].map((value) => ({ value, label: displayStatus(value) }))} /><div className="photo-strip"><button type="button" onClick={() => { setPhotos((current) => [...current, `Foto local ${current.length + 1}`]); setSaveState('idle'); }}>Añadir foto</button></div>{photos.length > 0 && <div className="photo-list">{photos.map((photo) => <span key={photo}>{photo}<button type="button" onClick={() => { setPhotos((current) => current.filter((item) => item !== photo)); setSaveState('idle'); }}>Quitar</button></span>)}</div>}</Card>}<div className="confirm-bar"><button className="primary wide big" disabled={saving || !hasChanges} onClick={save}>{saveState === 'saving' ? 'Guardando' : saveState === 'saved' ? 'Guardado en dispositivo' : saveState === 'error' ? 'Reintentar' : 'Confirmar selección'}</button>{status !== confirmedStatus && <p>Selección pendiente: {displayStatus(status)}</p>}{saveState === 'error' && <p className="form-error">Error al guardar en el dispositivo.</p>}</div></section>;
}
...
function ModulePage() { const { moduleId = '' } = useParams(); const { workspace } = useAuth(); if (moduleId === 'tecnicos') return <TechniciansModule />; if (moduleId === 'comerciales') return <CommercialProfilesModule />; if (moduleId === 'ventas') return <SalesModule mode="ventas" />; if (moduleId === 'oportunidades') return <SalesModule mode="oportunidades" />; if (moduleId === 'presupuestos') return <SalesModule mode="presupuestos" />; if (moduleId === 'operaciones') return <OperationsModule />; if (moduleId === 'informes') return <ManagementPage />; if (moduleId === 'personal') return <TechniciansModule />; if (moduleId === 'rentabilidad') return <ProfitabilityModule />; const meta = moduleMeta[moduleId] ?? { title: 'Módulo en preparación', description: 'Este módulo tendrá su propia pantalla y no redirige a otro módulo incorrecto.', links: [] }; return <section className="page"><Breadcrumb items={[workspaceTitles[workspace], meta.title]} /><Hero title={meta.title} subtitle={meta.description} tone="info" /><Card title="Estado del módulo"><p className="large-note">Pantalla en preparación. No se muestran cifras simuladas ni se sustituye por otro módulo no relacionado.</p><div className="actions">{meta.links.map((link: any) => <Link key={link.to} to={link.to}>{link.label}</Link>)}</div></Card></section>; }
...
function NotFound() {
  ...
  if (superadminClientMatch) return <SuperadminGuard><SuperadminClientDetail id={superadminClientMatch[1]} /></SuperadminGuard>;
  ...
  if (superadminWorkOrderMatch) return <SuperadminGuard><SuperadminWorkOrderDetail id={superadminWorkOrderMatch[1]} /></SuperadminGuard>;
...
function SuperadminClientDetail({ id }: { id: string }) {
  const { data, loading, error, reload } = useLoad(() => clientsService.get(id), [id], null as any);
  if (loading || error || !data) return <StateBlock loading={loading} error={error} retry={reload} empty={!data} />;
  return <section className="page"><BackButton /><Hero title={data.legal_name} subtitle={`${data.code} · ${data.status}`} tone={severityForStatus(data.status)} /><div className="grid two-one"><Card title="Datos del cliente"><InfoGrid items={[[ 'Código', data.code ], [ 'Razón social', data.legal_name ], [ 'Nombre comercial', data.trade_name ?? '-' ], [ 'NIF', data.tax_id ?? '-' ], [ 'Dirección', data.address ?? '-' ], [ 'Localidad', data.city ?? '-' ], [ 'Provincia', data.province ?? '-' ], [ 'CP', data.postal_code ?? '-' ], [ 'País', data.country ?? '-' ], [ 'Teléfono', data.phone ?? '-' ], [ 'Correo', data.email ?? '-' ], [ 'Observaciones', data.notes ?? '-' ]]} /></Card><Card title="Contactos"><CompactRows rows={(data.client_contacts ?? []).map((item: any) => [fullName(item), `${item.role ?? '-'} · ${item.email ?? '-'} · ${item.phone ?? '-'}`, item.is_primary ? 'ok' : 'info', `/app/superadmin/clientes/${data.id}`])} empty="Sin contactos." /></Card></div><Related title="Relaciones" groups={[[ 'Centros', data.sites, '/app/superadmin/centros'], ['Equipos', data.equipment, '/app/superadmin/equipos'], ['Expedientes', data.cases, '/app/expedientes'], ['Partes', data.work_orders, '/app/superadmin/partes']]} /></section>;
}

function SuperadminSiteDetail({ id }: { id: string }) {
  const { data, loading, error, reload } = useLoad(() => sitesService.get(id), [id], null as any);
  if (loading || error || !data) return <StateBlock loading={loading} error={error} retry={reload} empty={!data} />;
  return <section className="page"><BackButton /><Hero title={data.name} subtitle={`${data.code} · ${data.clients?.legal_name ?? ''}`} tone="maintenance" /><Card title="Datos del centro"><InfoGrid items={[[ 'Cliente', data.clients?.legal_name ?? '-' ], [ 'Dirección', data.address ?? '-' ], [ 'Localidad', data.city ?? '-' ], [ 'Provincia', data.province ?? '-' ], [ 'CP', data.postal_code ?? '-' ], [ 'Horario', data.schedule ?? '-' ], [ 'Requisitos de acceso', data.access_requirements?.description ?? '-' ], [ 'Observaciones', data.notes ?? '-' ]]} /></Card><Related title="Relaciones" groups={[[ 'Equipos instalados', data.equipment, '/app/superadmin/equipos'], ['Expedientes', data.cases, '/app/expedientes'], ['Partes', data.work_orders, '/app/superadmin/partes']]} /></section>;
}
...
function SuperadminEquipmentDetail({ id }: { id: string }) {
  ...
  if (loading || error || !data) return <StateBlock loading={loading} error={error} retry={reload} empty={!data} />;
  return <section className="page"><BackButton /><Hero title={`${data.code} · ${data.equipment_types?.name ?? 'Equipo'}`} subtitle={`${data.clients?.legal_name ?? ''} · ${data.sites?.name ?? ''}`} tone={severityForStatus(data.status)} /><Card title="Identificación"><InfoGrid items={[[ 'Código', data.code ], [ 'Cliente', data.clients?.legal_name ?? '-' ], [ 'Centro', data.sites?.name ?? '-' ], [ 'Ubicación', data.internal_location ?? '-' ], [ 'Fabricante', data.brand ?? '-' ], [ 'Modelo', data.model ?? '-' ], [ 'Serie', data.serial_number ?? '-' ], [ 'Estado', displayStatus(data.status) ], [ 'Criticidad', displayStatus(data.criticality) ]]} /></Card><div className="grid half"><Card title="Componentes"><CompactRows rows={(data.equipment_components ?? []).map((component: any) => [component.component_type, `${component.brand ?? '-'} ${component.model ?? ''} · ${component.status}`, severityForStatus(component.status), `/app/superadmin/equipos/${id}`])} empty="Sin componentes." /></Card><Card title="Historial"><CompactRows rows={history.data.map((item: any) => [displayStatus(item.event_type), `${formatDate(item.event_at)} · ${item.summary ?? ''} · ${item.detail ?? ''}`, severityForStatus(item.detail), item.event_type === 'parte' ? '/app/superadmin/partes' : `/app/superadmin/equipos/${id}`])} empty="Sin historial." /></Card></div></section>;
}
...
function SuperadminWorkOrderDetail({ id }: { id: string }) {
  ...
  if (loading || error || !data) return <StateBlock loading={loading} error={error} retry={reload} empty={!data} />;
  return <section className="page"><BackButton /><Hero title={`${data.code} · ${data.title}`} subtitle={`${data.clients?.legal_name ?? ''} · ${data.sites?.name ?? ''}`} tone={severityForStatus(data.status)} /><Card title="Detalle del parte"><InfoGrid items={[[ 'Estado', displayStatus(data.status) ], [ 'Prioridad', displayStatus(data.priority) ], [ 'Cliente', data.clients?.legal_name ?? '-' ], [ 'Centro', data.sites?.name ?? '-' ], [ 'Equipo', data.primary_equipment?.code ?? '-' ], [ 'Fecha', data.scheduled_date ?? '-' ], [ 'Hora', data.scheduled_time ?? '-' ], [ 'Origen', data.origin ?? '-' ], [ 'Descripción', data.description ?? '-' ]]} /></Card><Card title="Asignaciones y checks"><CompactRows rows={[...(data.assignments ?? []).map((item: any) => [fullName(item.profiles), `${item.assignment_date ?? '-'} · ${item.planned_start_time ?? 'Sin hora'} · ${displayStatus(item.status)}`, 'info', '/app/superadmin/partes']), ...(data.checks ?? []).map((check: any) => [check.code, `${displayStatus(check.status)} · ${displayStatus(check.global_result)}`, severityForStatus(check.global_result), '/app/superadmin/checks'])]} empty="Sin asignaciones ni checks vinculados." /></Card></section>;
}
...
function GlobalSearch({ query, setQuery }: { query: string; setQuery: (value: string) => void }) {
  ...
  useEffect(() => {
    ...
    if (!trimmed) { setData([]); setLoading(false); return; }
    setLoading(true);
  ...
  return <div className="search-wrap"><label className="search"><Search size={17} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={workspace === 'tecnico' ? 'Buscar mis trabajos y checks...' : 'Buscar cliente, equipo, expediente, parte...'} /></label>{trimmed && <div className="search-results">{loading && <p>Buscando...</p>}{!loading && data.slice(0, 8).map((item) => <button key={`${item.route}-${item.id}`} onClick={() => open(item)}><Badge tone="info">{item.kind ?? 'Resultado'}</Badge><span>{item.title}</span><small>{item.subtitle ?? 'Registro Supabase'}</small></button>)}{!loading && !data.length && <p>Sin resultados.</p>}</div>}</div>;
}
...
function SuperadminUserForm({ initial, onClose, onSaved }: any) {
  ...
  return <ModalForm title={initial?.id ? 'Editar usuario' : 'Crear usuario'} onClose={onClose} onSubmit={submit} saving={saving} error={error} submitLabel={initial?.id ? 'Guardar usuario' : 'Crear perfil'}><p className="large-note">No se guarda ni se muestra ninguna contraseña. Para alta Auth real usa invitación o creación en Supabase Auth mediante backend seguro y enlaza auth_user_id.</p><div className="form-grid"><label>Nombre *<input value={values.first_name ?? ''} onChange={(event) => set('first_name', event.target.value)} required /></label><label>Apellidos *<input value={values.last_name ?? ''} onChange={(event) => set('last_name', event.target.value)} required /></label><label>Email *<input type="email" value={values.email ?? ''} onChange={(event) => set('email', event.target.value)} required /></label><label>Teléfono<input value={values.phone ?? ''} onChange={(event) => set('phone', event.target.value)} /></label><label>Usuario/login opcional<input value={values.username ?? ''} onChange={(event) => set('username', event.target.value)} disabled placeholder="No persistido en profiles actual" /></label><FormSelect label="Rol principal" value={values.primary_area} onChange={(value) => set('primary_area', value)} required options={roleOptions.map((value) => ({ value, label: value === 'superadmin' ? 'Propietario DMP' : value }))} /><FormSelect label="Estado" value={String(values.active)} onChange={(value) => set('active', value)} options={[{ value: 'true', label: 'Activo' }, { value: 'false', label: 'Inactivo' }]} /><FormSelect label="Contraseña inicial o invitación" value={values.invitation_mode} onChange={(value) => set('invitation_mode', value)} options={[{ value: 'invite', label: 'Enviar invitación segura' }, { value: 'backend-password', label: 'Contraseña temporal vía backend' }]} /></div><Card title="Roles adicionales"><div className="component-select">{roleOptions.map((role) => <label key={role}><input type="checkbox" checked={values.roles.includes(role)} onChange={() => toggleRole(role)} /> {role === 'superadmin' ? 'Propietario DMP' : role}</label>)}</div></Card><Card title="Vinculaciones"><p className="large-note">Vincular con técnico o comercial se resuelve usando el mismo perfil DMP y sus roles. Si se crea una tabla específica de vínculos, debe gestionarse desde RPC segura.</p></Card><p className="state-warning"><ShieldAlert size={16} />Contraseña protegida. No se mostrará después de crear, no se guarda en tablas públicas y no se registra en consola.</p></ModalForm>;
}
