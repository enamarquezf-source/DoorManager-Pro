...
import { canAccessModule, canAccessRoute, canArchiveEntity, canAssignTechnician, canCreateAlert, canCreateCheck, canCreateWorkOrder, canEditWorkOrder, canExecuteCheck, canExecuteWorkOrder, canManageCheck, canManageWorkOrderAssignments, canManageWorkOrderMaterials, canManageWorkOrderStatus, canManageWorkOrderTime, canPermanentlyDeleteEntity, canRestoreEntity, canRole, canViewCheck, canViewWorkOrder, canViewWorkOrderCosts, isSuperadmin, normalizedRoleNames, profileWorkspaces } from './auth/permissions';
...
function WorkOrderDetailPageV2({ forcedId }: { forcedId?: string } = {}) {
  ...
  const [mode, setMode] = useState<'edit' | 'assign' | 'check' | 'time' | 'material' | null>(null);
  const [message, setMessage] = useState('');
  const [actionError, setActionError] = useState('');
  ...
  return <section className="page work-detail"><BackButton /><div className="work-summary"><div><p className="eyebrow">Ficha completa del parte</p><h2>{data.code} · {data.title}</h2><p>{data.clients?.legal_name ?? 'Cliente no informado'} · {data.sites?.name ?? 'Centro no informado'} · {data.primary_equipment?.code ?? 'Sin equipo'} · {equipmentTypeName(data.primary_equipment) ?? 'Tipo de equipo no disponible'}</p></div><Badge tone={severityForStatus(data.status)}>{displayStatus(data.status)}</Badge></div><WorkOrderStatusSelector workOrder={data} onChanged={() => operationalChanged("Estado actualizado y persistido.")} onError={setActionError} /><div className="actions work-actions"><button onClick={() => setMode('edit')} disabled={!canEditWorkOrder(profile)}>Editar datos</button>{manageAllowed && <button className="primary" onClick={() => setMode('assign')}>Gestionar asignaciones</button>}{canCreateCheck(profile) && <button onClick={() => setMode('check')}>Crear/abrir check</button>}<button onClick={() => setMode("time")} disabled={!canManageWorkOrderTime(profile, data)}>Añadir horas</button><button onClick={() => setMode("material")} disabled={!canManageWorkOrderMaterials(profile, data)}>Añadir material</button><SyncButton workOrderId={data.id} onSynced={reload} />{data.main_equipment_id && <Link to={`/app/equipos/${data.main_equipment_id}`}>Abrir/corregir equipo</Link>}</div>{message && <p className="success-note">{message}</p>}{actionError && <p className="form-error">{actionError}</p>}<AssignmentsCard workOrder={data} canManage={manageAllowed} onManage={() => setMode('assign')} onChanged={changed} /><div className="stats-grid"><Card title="Resumen"><InfoGrid items={[[ 'Estado', displayStatus(data.status) ], [ 'Prioridad', displayStatus(data.priority) ], [ 'Técnico principal', fullName(data.primary_technician) ], [ 'Apoyos', (data.support_technicians ?? []).map(fullName).join(', ') || '-' ], [ 'Fecha/hora', `${data.scheduled_date ?? '-'} ${data.scheduled_time ?? ''}` ], [ 'Resultado', summary.result ?? '-' ]]}/></Card><Card title="Contadores"><InfoGrid items={[[ 'Fotos', String((data.photos ?? []).length) ], [ 'Firmas', String((data.signatures ?? []).length) ], [ 'Checks', String((data.checks ?? []).length) ], [ 'Materiales', String((data.materials ?? []).length) ], [ 'Última sincronización', summary.lastSync ?? '-' ]]}/></Card></div><WorkOrderTimeCard workOrder={data} onChanged={() => operationalChanged("Horas actualizadas.")} /><WorkOrderMaterialsCard workOrder={data} onChanged={() => operationalChanged("Materiales actualizados.")} /><WorkProgress history={data.status_history ?? []} current={data.status} /><div className="grid two-one"><Card title="Resumen de intervención"><InfoGrid items={[[ 'Diagnóstico', summary.diagnosis ?? '-' ], [ 'Trabajo realizado', summary.work ?? '-' ], [ 'Observaciones', summary.observations ?? '-' ], [ 'Resultado', summary.result ?? '-' ], [ 'Cliente', data.clients?.legal_name ?? '-' ], [ 'Centro', data.sites?.name ?? '-' ], [ 'Equipo', data.primary_equipment?.code ?? '-' ], [ 'Tipo equipo', equipmentTypeName(data.primary_equipment) ?? '-' ]]}/></Card><Card title="Actividad"><ActivityTimeline events={activity} formatDate={formatDate} /></Card></div><Card title="Checks asociados"><div className="check-summary-list">{(data.checks ?? []).map((check: any) => <CheckSummaryCard key={check.id} check={check} />)}{!(data.checks ?? []).length && <p className="large-note">Sin checks asociados.</p>}</div></Card><MediaGallery photos={data.photos ?? []} signatures={data.signatures ?? []} /><div className="grid half"><Card title="Materiales"><CompactRows rows={(data.materials ?? []).map((item: any) => [item.materials?.description ?? item.description ?? 'Material', `${item.quantity ?? 1} · ${displayStatus(item.status)}`, severityForStatus(item.status), `/app/partes/${data.id}`])} empty="Sin materiales." /></Card><Card title="Notas"><Timeline items={(data.notes ?? []).map((note: any) => `${formatDate(note.created_at)} · ${fullName(note.profiles) || 'Usuario'} · ${note.note}`)} /></Card></div>{mode === 'edit' && <WorkOrderForm initial={data} onClose={() => setMode(null)} onSaved={() => { setMode(null); reload(); }} />}{mode === 'assign' && <AssignmentForm workOrder={data} workOrderId={data.id} companyId={data.company_id} onClose={() => setMode(null)} onSaved={() => { setMode(null); changed(); }} />}{mode === 'check' && <CheckForm initial={{ company_id: data.company_id, work_order_id: data.id, equipment_id: data.main_equipment_id }} onClose={() => setMode(null)} onSaved={() => { setMode(null); reload(); }} />}{mode === 'time' && <WorkOrderTimeForm workOrder={data} onClose={() => setMode(null)} onSaved={() => { setMode(null); operationalChanged("Horas registradas."); }} />}{mode === 'material' && <WorkOrderMaterialForm workOrder={data} onClose={() => setMode(null)} onSaved={() => { setMode(null); operationalChanged("Material registrado."); }} />}</section>;
}
...
function WorkOrderStatusSelector({ workOrder, onChanged, onError }: { workOrder: any; onChanged: () => void; onError: (message: string) => void }) { const { profile } = useAuth(); const [value, setValue] = useState(workOrder.status); const [saving, setSaving] = useState(false); if (!canManageWorkOrderStatus(profile, workOrder)) return null; const change = async (next: string) => { if (next === workOrder.status || saving) return; const currentIndex = workOrderStatuses.indexOf(workOrder.status); const nextIndex = workOrderStatuses.indexOf(next); const needsReason = next === 'Cancelado' || next === 'Cerrado' || nextIndex < currentIndex || ['Pendiente','Devuelto por SAT'].includes(next); const warning = next === 'Cerrado' ? 'Cerrar el parte puede bloquear modificaciones posteriores. ' : next === 'Cancelado' ? 'Cancelar exige justificar el motivo. ' : ''; const reason = needsReason ? window.prompt(`${warning}Motivo del cambio a ${displayStatus(next)}`) : window.prompt(`${warning}Confirma el cambio a ${displayStatus(next)}. Motivo opcional`) ?? ''; if (needsReason && !reason?.trim()) { setValue(workOrder.status); return; } setSaving(true); onError(''); try { await workOrdersService.changeStatus(workOrder.id, next, reason ?? 'Cambio directo de estado'); onChanged(); } catch (err) { setValue(workOrder.status); onError(err instanceof Error ? err.message : 'No se ha podido cambiar el estado.'); } finally { setSaving(false); } }; return <Card title="Estado del parte"><label className="status-selector">Estado:<select value={value} disabled={saving} onChange={(event) => { const next = event.target.value; setValue(next); change(next); }}>{workOrderStatuses.map((status) => <option key={status} value={status}>{displayStatus(status)}</option>)}</select></label><p className="large-note">SAT, Gerencia y superadmin pueden seleccionar cualquier estado. Técnicos y Comercial quedan limitados por Supabase.</p></Card>; }

function WorkOrderTimeCard({ workOrder, onChanged }: { workOrder: any; onChanged: () => void }) { const { profile } = useAuth(); const [editing, setEditing] = useState<any | null>(null); const rows = workOrder.time_entries ?? []; const totals = rows.reduce((acc: Record<string, number>, row: any) => { const name = fullName(row.profiles); acc[name] = (acc[name] ?? 0) + Number(row.duration_minutes ?? 0); return acc; }, {}); const total = rows.reduce((sum: number, row: any) => sum + Number(row.duration_minutes ?? 0), 0); const remove = async (row: any) => { const reason = window.prompt('Motivo para eliminar este registro de horas'); if (!reason) return; await workOrdersService.deleteTimeEntry(row.id, reason); onChanged(); }; return <Card title="Horas trabajadas"><div className="actions"><span className="large-note">Total general: {formatMinutes(total)}</span>{Object.entries(totals).map(([name, minutes]) => <span key={name} className="large-note">{name}: {formatMinutes(Number(minutes))}</span>)}</div><div className="table-card slim"><table><thead><tr><th>Trabajador</th><th>Fecha</th><th>Tramo</th><th>Pausa</th><th>Tipo</th><th>Total</th><th>Trabajo</th><th></th></tr></thead><tbody>{rows.map((row: any) => <tr key={row.id}><td>{fullName(row.profiles)}</td><td>{formatDate(row.work_date)}</td><td>{row.started_at ? `${row.started_at} - ${row.ended_at}` : 'Duración manual'}</td><td>{row.break_minutes ?? 0} min</td><td>{displayStatus(row.hour_type)}</td><td>{formatMinutes(Number(row.duration_minutes ?? 0))}</td><td>{row.description}</td><td>{canManageWorkOrderTime(profile, workOrder, row) && <div className="row-actions"><button onClick={() => setEditing(row)}>Editar</button><button onClick={() => remove(row)}>Eliminar</button></div>}</td></tr>)}</tbody></table></div>{!rows.length && <p className="large-note">Sin horas registradas.</p>}{editing && <WorkOrderTimeForm workOrder={workOrder} initial={editing} onClose={() => setEditing(null)} onSaved={() => { setEditing(null); onChanged(); }} />}</Card>; }

function WorkOrderMaterialsCard({ workOrder, onChanged }: { workOrder: any; onChanged: () => void }) { const { profile } = useAuth(); const [editing, setEditing] = useState<any | null>(null); const rows = workOrder.materials ?? []; const showCosts = canViewWorkOrderCosts(profile); const units = rows.reduce((sum: number, row: any) => sum + Number(row.used_quantity ?? row.quantity ?? 0), 0); const amount = rows.reduce((sum: number, row: any) => sum + Number(row.used_quantity ?? 0) * Number(row.unit_price ?? 0), 0); const remove = async (row: any) => { const reason = window.prompt('Motivo para eliminar este material'); if (!reason) return; await workOrdersService.deleteMaterial(row.id, reason); onChanged(); }; return <Card title="Materiales utilizados"><div className="actions"><span className="large-note">Unidades: {units}</span>{showCosts && <span className="large-note">Total económico: {amount.toFixed(2)} €</span>}</div><div className="table-card slim"><table><thead><tr><th>Material</th><th>Cantidad</th><th>Fecha</th><th>Registrado por</th>{showCosts && <th>Importe</th>}<th>Observaciones</th><th></th></tr></thead><tbody>{rows.map((row: any) => <tr key={row.id}><td>{row.materials?.description ?? row.description ?? 'Material no catalogado'}</td><td>{row.used_quantity ?? row.quantity} {row.unit ?? 'ud'}</td><td>{formatDate(row.used_at ?? row.created_at)}</td><td>{fullName(row.profiles)}</td>{showCosts && <td>{(Number(row.used_quantity ?? 0) * Number(row.unit_price ?? 0)).toFixed(2)} €</td>}<td>{row.notes ?? '-'}</td><td>{canManageWorkOrderMaterials(profile, workOrder, row) && <div className="row-actions"><button onClick={() => setEditing(row)}>Editar</button><button onClick={() => remove(row)}>Eliminar</button></div>}</td></tr>)}</tbody></table></div>{!rows.length && <p className="large-note">Sin materiales registrados.</p>}{editing && <WorkOrderMaterialForm workOrder={workOrder} initial={editing} onClose={() => setEditing(null)} onSaved={() => { setEditing(null); onChanged(); }} />}</Card>; }

function WorkOrderTimeForm({ workOrder, initial, onClose, onSaved }: { workOrder: any; initial?: any; onClose: () => void; onSaved: () => void }) { const [values, setValues] = useState<Record<string, string>>({ id: initial?.id ?? '', work_order_id: workOrder.id, work_date: initial?.work_date ?? new Date().toISOString().slice(0, 10), started_at: initial?.started_at ?? '', ended_at: initial?.ended_at ?? '', hour_type: initial?.hour_type ?? 'normal', break_minutes: String(initial?.break_minutes ?? 0), duration_minutes: initial?.duration_minutes ? String(initial.duration_minutes) : '', description: initial?.description ?? '' }); const [error, setError] = useState(''); const [saving, setSaving] = useState(false); const set = (key: string, value: string) => setValues((current) => ({ ...current, [key]: value })); const submit = async (event: FormEvent) => { event.preventDefault(); const hasRange = values.started_at || values.ended_at; if (hasRange && (!values.started_at || !values.ended_at)) { setError('validacion del formulario: indica inicio y fin, o usa duración manual.'); return; } if (!hasRange && !values.duration_minutes) { setError('validacion del formulario: indica duración manual o tramo horario.'); return; } if (!values.description?.trim()) { setError('validacion del formulario: describe el trabajo realizado.'); return; } setSaving(true); setError(''); try { await workOrdersService.upsertTimeEntry(Object.fromEntries(Object.entries(values).filter(([, value]) => value !== ''))); onSaved(); } catch (err) { setError(err instanceof Error ? err.message : 'respuesta de Supabase: no se han podido registrar las horas.'); } finally { setSaving(false); } }; return <div className="mini-modal" role="dialog" aria-modal="true"><form onSubmit={submit}><h3>{initial ? 'Editar horas' : 'Añadir horas'}</h3><p className="large-note">Ejemplo: 08:00-10:30 con 15 min de pausa, o duración manual 90 si no registras horario.</p><div className="form-grid"><label>Fecha<input type="date" value={values.work_date} onChange={(event) => set('work_date', event.target.value)} required /></label><label>Inicio<input type="time" value={values.started_at ?? ''} onChange={(event) => set('started_at', event.target.value)} /></label><label>Fin<input type="time" value={values.ended_at ?? ''} onChange={(event) => set('ended_at', event.target.value)} /></label><label>Pausa minutos<input type="number" min="0" value={values.break_minutes} onChange={(event) => set('break_minutes', event.target.value)} /></label><label>Duración manual minutos<input type="number" min="1" value={values.duration_minutes ?? ''} onChange={(event) => set('duration_minutes', event.target.value)} placeholder="Solo si no hay inicio/fin" /></label><FormSelect label="Tipo de hora" value={values.hour_type} onChange={(value) => set('hour_type', value)} options={['normal','nocturna','festiva','desplazamiento','otra'].map((value) => ({ value, label: displayStatus(value) }))} /></div><label>Descripción del trabajo<textarea value={values.description ?? ''} onChange={(event) => set('description', event.target.value)} required /></label>{error && <p className="form-error">{error}</p>}<div className="modal-footer"><button type="button" onClick={onClose} disabled={saving}>Cancelar</button><button className="primary" disabled={saving}>{initial ? 'Guardar cambios' : 'Guardar horas'}</button></div></form></div>; }
...
function assignmentRows(workOrder: any) {
  ...
  if (workOrder.responsible || workOrder.current_responsible_id) {
    ...
    rows.push({ id: workOrder.current_responsible_id, person, type: commercial ? 'Comercial' : 'Responsable', assignment_date: workOrder.scheduled_date, planned_start_time: workOrder.scheduled_time, planned_end_time: null, status: workOrder.status, route: commercial ? `/app/modulos/comerciales/${workOrder.current_responsible_id}` : `/app/partes/${workOrder.id}` });
  }
...
function AssignmentsCard({ workOrder, canManage, onManage, onChanged }: { workOrder: any; canManage: boolean; onManage: () => void; onChanged: () => void }) {
  ...
  const unassign = async () => {
    ...
    try { await workOrdersService.unassign(workOrder.id, confirm.id, confirm.reason); setMessage(`${fullName(confirm.person)} desasignado.`); setConfirm(null); onChanged(); }
