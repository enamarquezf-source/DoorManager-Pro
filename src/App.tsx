...
function PurchaseOrderLineForm({ order, materials, initial, onClose, onSaved }: any) {
  ...
  const matching = relations.data.filter((row: any) => row.supplier_id === order.supplier_id);
  const selected = matching.find((row: any) => row.id === values.material_supplier_id);
  const set = (key: string, value: any) => setValues((current) => ({ ...current, [key]: value }));
  const selectMaterial = (value: string) => { const material = materials.find((item: any) => item.id === value); setValues((current) => ({ ...current, material_id: value, material_supplier_id: '', unit_purchase_price: '' })); if (material) set('unit_purchase_price', ''); };
  ...
  return <ModalForm title={initial?.id ? 'Editar línea del pedido' : 'Añadir línea al pedido'} onClose={onClose} onSubmit={submit} saving={saving} error={error}><FormSelect label="Material" value={values.material_id} onChange={selectMaterial} required options={[{ value: '', label: 'Selecciona material' }, ...materials.map((item: any) => ({ value: item.id, label: `${item.code} · ${item.description}` }))]} loading={relations.loading} /><FormSelect label="Relación proveedor-material" value={values.material_supplier_id ?? ''} onChange={selectRelation} options={[{ value: '', label: matching.length ? 'Línea manual con advertencia' : 'Sin relación para este proveedor' }, ...matching.map((row: any) => ({ value: row.id, label: `${row.supplier_reference ?? 'Sin referencia'} · ${row.purchase_unit_price == null ? 'Precio no configurado' : `${row.purchase_unit_price} €`}` }))]} /><div className="form-grid"><label>Cantidad<input type="number" min="0.01" step="0.01" value={values.ordered_quantity} onChange={(event) => set('ordered_quantity', event.target.value)} required /></label><label>Precio acordado<input type="number" min="0" step="0.01" value={values.unit_purchase_price} onChange={(event) => set('unit_purchase_price', event.target.value)} placeholder="Obligatorio al pedir" /></label></div>{values.material_id && !matching.length && <p className="state-warning">El material no tiene relación con este proveedor. Se permite una línea manual; la relación no se creará automáticamente.</p>}{selected && <p className="large-note">Referencia sugerida: {selected.supplier_reference ?? 'Sin referencia'}. El precio queda copiado en el pedido.</p>}</ModalForm>;
}
