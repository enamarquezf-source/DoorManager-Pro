...
function LegacySuperadminUsers() {
  ...
  return <section className="page"><Breadcrumb items={['Propietario DMP', 'Usuarios']} /><div className="page-head"><div><h2>Usuarios</h2><p>Perfiles de la empresa operadora. No se muestran ni se guardan contraseñas.</p></div><Link className="primary" to="/app/superadmin/usuarios/nuevo">Crear usuario</Link></div>{message && <p className="success-note">{message}</p>}<div className="filters local-filters"><label><Search size={16} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar por nombre, email o rol..." /></label><FormSelect label="Área" value={area} onChange={setArea} options={[{ value: 'todas', label: 'Todas' }, ...areaOptions().map((item) => ({ value: normalizeArea(item), label: areaLabel(item) }))]} /><FormSelect label="Estado" value={active} onChange={setActive} options={[{ value: 'todos', label: 'Todos' }, { value: 'activos', label: 'Activos' }, { value: 'inactivos', label: 'Inactivos' }]} /></div><StateBlock loading={loading} error={error} retry={reload} empty={!rows.length}><div className="table-card"><table><thead><tr><th>Nombre</th><th>Apellidos</th><th>Email</th><th>Área/rol principal</th><th>Activo</th><th>Cuenta Auth</th><th>Fechas</th><th>Acciones</th></tr></thead><tbody>{rows.map((user: any) => <tr key={user.id} className={isArchivedRecord('profiles', user) ? 'archived-row' : ''}><td>{user.first_name}<span>{rolesText(user)}</span></td><td>{user.last_name}</td><td>{user.email}</td><td>{areaLabel(user.primary_area)}</td><td>{user.active ? 'Activo' : 'Inactivo'}</td><td><small>{user.auth_user_id ? 'Enlazada' : 'Pendiente de enlace'}</small></td><td>{formatDate(user.created_at)}<span>Actualizado: {formatDate(user.updated_at)}</span>{user.deleted_at && <span>Eliminado: {formatDate(user.deleted_at)}</span>}</td><td><div className="row-actions"><button onClick={() => setEditing(user)}>Ver / editar</button><LifecycleActionPanel entity="profiles" record={user} onChanged={() => { setMessage('Operacion de ciclo de vida registrada.'); reload(); }} /></div></td></tr>)}</tbody></table></div></StateBlock>{editing && <SuperadminProfileForm initial={editing} onClose={() => setEditing(null)} onSaved={() => { setEditing(null); reload(); }} />}</section>;
}

function SuperadminUserCreate() {
  const navigate = useNavigate();
  return <section className="page"><Breadcrumb items={['Propietario DMP', 'Crear usuario']} /><Hero title="Crear usuario" subtitle="Crea el perfil y prepara sus roles. El enlace Auth se completa mediante el canal seguro autorizado." tone="info" /><SuperadminProfileForm onClose={() => navigate('/app/superadmin/usuarios')} onSaved={() => navigate('/app/superadmin/usuarios')} /></section>;
}
...
function SuperadminProfileForm({ initial, onClose, onSaved }: any) {
  ...
  return <ModalForm title={initial?.id ? 'Editar usuario' : 'Crear usuario'} onClose={onClose} onSubmit={submit} saving={saving} error={error} submitLabel={initial?.id ? 'Guardar usuario' : 'Crear perfil'}><p className="large-note">No se guarda ni se muestra ninguna contraseña. El perfil puede quedar pendiente de enlace Auth hasta completar el flujo seguro autorizado.</p><p className="large-note">SAT y Comercial son incompatibles en el mismo perfil. Al seleccionar uno se desmarca el otro.</p><div className="form-grid"><label>Nombre *<input value={values.first_name ?? ''} onChange={(event) => set('first_name', event.target.value)} required /></label><label>Apellidos *<input value={values.last_name ?? ''} onChange={(event) => set('last_name', event.target.value)} required /></label><label>Email *<input type="email" value={values.email ?? ''} onChange={(event) => set('email', event.target.value)} required /></label><label>Teléfono<input value={values.phone ?? ''} onChange={(event) => set('phone', event.target.value)} /></label><FormSelect label="Área/rol principal" value={values.primary_area} onChange={(value) => { set('primary_area', value); if (!values.roles.includes(value)) set('roles', [...values.roles, value]); }} required options={areaOptions().map((value) => ({ value, label: areaLabel(value) }))} /><FormSelect label="Estado" value={String(values.active)} onChange={(value) => set('active', value)} options={[{ value: 'true', label: 'Activo' }, { value: 'false', label: 'Inactivo' }]} /></div><Card title="Roles adicionales"><div className="component-select">{areaOptions().map((role) => <label key={role}><input type="checkbox" checked={values.roles.includes(role)} onChange={() => toggleRole(role)} /> {areaLabel(role)}</label>)}</div></Card><Card title="Acceso Auth"><p className="large-note">El alta, la invitación, el restablecimiento y la contraseña se gestionan fuera del navegador mediante backend seguro. Esta ficha sólo muestra si el perfil está enlazado.</p></Card></ModalForm>;
}
