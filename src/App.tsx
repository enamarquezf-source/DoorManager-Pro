...
function NotFound() {
  ...
  if (materialMatch) return <MaterialDetailPage />;
...
function MaterialDetailPage() {
  ...
  const material = useLoad(() => materialsService.get(id), [id], null as any); const stock = useLoad(() => workOrdersService.warehouseStockCatalog(), [], [] as any[]); const warehouses = useLoad(() => workOrdersService.warehousesCatalog(), [], [] as any[]); const relations = useLoad(() => suppliersService.listMaterialSuppliers(id), [id], [] as any[]); const { profile } = useAuth();
  ...
  if (material.loading || material.error || !material.data) return <section className="page material-detail-page"><BackButton /><StateBlock loading={material.loading} error={material.error} retry={material.reload} empty={!material.data} /></section>;
