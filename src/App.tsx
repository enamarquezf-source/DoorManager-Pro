...
function SuperadminTemplates() {
  const { companyId } = useSuperadminScope();
  const { data, loading, error, reload } = useLoad(() => superadminService.templates(companyId), [companyId], [] as any[]);
...
function CheckForm({ initial, onClose, onSaved }: any) {
  ...
  const templates = useLoad(() => checksService.templates(selectedEquipment?.equipment_type_id, initial?.company_id), [selectedEquipment?.equipment_type_id, initial?.company_id], [] as any[]);
