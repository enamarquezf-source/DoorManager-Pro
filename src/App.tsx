...
function SupplierForm({ initial, onClose, onSaved }: { initial?: any; onClose: () => void; onSaved: () => void }) {
  return <EntityForm title={initial?.id ? 'Editar proveedor' : 'Crear proveedor'} fields={[[ 'name', 'Razón social', true ], [ 'tax_id', 'NIF/CIF' ], [ 'email', 'Correo' ], [ 'phone', 'Teléfono' ]]} initial={{ active: true, ...initial }} errorMessage={initial?.id ? 'No se ha podido modificar el proveedor.' : 'No se ha podido crear el proveedor.'} onClose={onClose} onSubmit={(values: any) => initial?.id ? suppliersService.update(initial.id, values) : suppliersService.create(values)} onSaved={onSaved} />;
}
