import { Component, Suspense, createContext, lazy, useContext, useEffect, useMemo, useRef, useState, type ErrorInfo, type FormEvent, type PointerEvent, type ReactNode } from 'react';
...
const BillingModule = lazy(() => import('./modules/BillingModule').then((module) => ({ default: module.BillingModule })));
const LazyWarrantyBillingDecisionPanel = lazy(() => import('./components/WarrantyBillingDecisionPanel').then((module) => ({ default: module.WarrantyBillingDecisionPanel })));
const LazyEconomicReviewPanel = lazy(() => import('./components/EconomicReviewPanel').then((module) => ({ default: module.EconomicReviewPanel })));

function WarrantyBillingDecisionPanel(props: { workOrder: any; lines: any[]; canDecide: boolean; onChanged: () => void }) {
  return <Suspense fallback={<p className="large-note">Cargando decisiones economicas...</p>}><LazyWarrantyBillingDecisionPanel {...props} /></Suspense>;
}

function EconomicReviewPanel(props: { workOrder: any; profile: any; onChanged: () => void }) {
  return <Suspense fallback={<p className="large-note">Cargando revision economica...</p>}><LazyEconomicReviewPanel {...props} /></Suspense>;
}
...
function ProtectedLayout() {
  ...
  useEffect(() => {
    const loadUnread = () => { if (profile) alertsService.unreadCount().then(setUnread).catch(() => setUnread(0)); };
    loadUnread();
...
function useLoad<T>(loader: (signal: AbortSignal) => Promise<T>, deps: unknown[] = [], empty: T, debounceDependencies = false) {
  ...
  useEffect(() => {
    ...
    const delay = debounceDependencies && loadedRef.current ? 250 : 0;
...
function ClientsPage() {
  ...
  const { data, loading, error, reload } = useLoad(() => clientsService.list(search, scope, archiveFilter), [search, scope, archiveFilter], [] as any[], true);
...
function EquipmentPage() { const { workspace } = useAuth(); const scope = undefined; const baseRoute = workspace === 'superadmin' ? '/app/superadmin/equipos' : '/app/equipos'; const [search, setSearch] = useState(''); const [archiveFilter, setArchiveFilter] = useState<ArchiveFilter>('active'); const { data, loading, error, reload } = useLoad(() => equipmentService.list(search, scope, archiveFilter), [search, scope, archiveFilter], [] as any[], true); const [creating, setCreating] = useState(false); const [purgeTarget, setPurgeTarget] = useState<any>(null); return <><ListPage title="Equipos" search={search} setSearch={setSearch} archiveFilter={archiveFilter} setArchiveFilter={setArchiveFilter} action={<button className="primary" onClick={() => setCreating(true)}>Crear equipo</button>} loading={loading} error={error} retry={reload} empty={!data.length}><div className="record-list">{data.map((item) => <RecordCard key={item.id} archived={isArchivedRecord('equipment', item)} title={<strong>{item.code}</strong>} status={[item.deleted_at ? 'Archivado' : displayStatus(item.status), item.deleted_at ? 'muted' : severityForStatus(item.status)]} to={`${baseRoute}/${item.id}`} actions={<><LifecycleActionPanel entity="equipment" record={item} onChanged={reload} />{entityPurgeCanShowButton('equipment', item, workspace) && <button className="danger-action" onClick={() => setPurgeTarget(item)}>Eliminar definitivamente</button>}</>} meta={<RecordMeta items={[[ 'Tipo', item.equipment_types?.name ?? '-' ], [ 'Cliente', item.clients?.legal_name ?? '-' ], [ 'Centro', item.sites?.name ?? '-' ], [ 'Marca/modelo', `${item.brand ?? '-'} ${item.model ?? ''}` ]]} />} />)}</div>{creating && <EquipmentForm initial={{}} onClose={() => setCreating(false)} onSaved={() => { setCreating(false); reload(); }} />}{purgeTarget && <GenericEntityPurgeModal config={equipmentPurgeConfig} record={purgeTarget} onClose={() => setPurgeTarget(null)} onDeleted={() => { setPurgeTarget(null); reload(); }} />}</ListPage></>; }
...
function PendingMaterialValidationPanel() {
  ...
  const pending = useLoad(() => canValidate ? workOrdersService.pendingMaterialValidations(search) : Promise.resolve([]), [canValidate, search], [] as any[], true);
...
function WorkOrderMaterialForm({ workOrder, initial, onClose, onSaved }: { workOrder: any; initial?: any; onClose: () => void; onSaved: () => void }) {
  ...
  const [search, setSearch] = useState('');
  const [additional, setAdditional] = useState(initial?.source === 'additional');
  const catalog = useLoad(() => workOrdersService.materialsCatalog(search), [search], [] as any[], true);
  const warehouses = useLoad(() => workOrdersService.warehousesCatalog(), [], [] as any[]);
  const warehouseStock = useLoad(() => workOrdersService.warehouseStockCatalog(), [], [] as any[]);
