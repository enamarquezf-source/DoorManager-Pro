...
function CheckDetailPage({ forcedId }: { forcedId?: string } = {}) {
  ...
  const blockHref = (zoneId: CheckBlockId) => workspace === 'superadmin' ? `/app/superadmin/checks/${id}` : `/app/checks/${id}/bloque/${zoneId}`;
  return <section className="check-mobile"><BackButton /><header><p className="eyebrow">Check {template.name}</p><h2>{data.code} · {data.equipment?.code}</h2><p>{reviewed} de {zones.length} bloques revisados</p>{template.placeholder && <p className="large-note">Falta imagen específica de este equipo. Se usa un esquema profesional temporal.</p>}<div className="actions"><button onClick={() => setMode('edit')}>Modificar check</button><SyncButton checkId={id} /></div></header><div className="progress"><span style={{ width: `${Math.min(100, (reviewed / zones.length) * 100)}%` }} /></div><div className={`door-check ${template.placeholder ? 'placeholder' : ''}`} aria-label="Zonas táctiles del equipo">{template.image ? <img src={template.image} alt={template.name} /> : <div className="equipment-placeholder"><Factory size={48} /><strong>{template.name}</strong><span>Imagen específica pendiente</span></div>}{physicalZones.map((zone) => <Link key={zone.id} style={{ ...zone.area, zIndex: zone.zIndex }} className={`hotspot ${severityForStatus(sectionStatus(zone.id))}`} to={blockHref(zone.id)} aria-label={`Revisar ${zone.name}`}><span>{zone.name}</span></Link>)}</div><div className="block-list status-summary" aria-label="Resumen de bloques revisados">{zones.map((zone) => <Link className="check-block-card" to={blockHref(zone.id)} key={zone.id}><div><strong>{zone.name}</strong><small>{zone.area ? 'Zona sobre imagen' : 'Bloque general fuera de imagen'} · {incidences(zone.id)} incidencias · {pending.some((item) => item.blockId === zone.id) ? 'Pendiente de sincronizar' : 'Sincronizado'}</small></div><Badge tone={severityForStatus(sectionStatus(zone.id))}>{displayStatus(sectionStatus(zone.id))}</Badge></Link>)}</div><button className="primary wide big" disabled={!allReviewed} onClick={() => setMode('finish')}>Finalizar check</button>{!allReviewed && <p className="large-note">Para finalizar, todos los bloques, incluido Funcionamiento general, deben estar revisados o marcados como No aplicable.</p>}{mode === 'edit' && <CheckForm initial={data} onClose={() => setMode(null)} onSaved={() => { setMode(null); reload(); }} />}{mode === 'finish' && <ConfirmModal title="Completar check" text="Se finalizará el check en Supabase. Sincroniza antes los bloques pendientes." onCancel={() => setMode(null)} onConfirm={async () => { await checksService.finish(id, globalResult); setMode(null); reload(); }} />}</section>;
}
