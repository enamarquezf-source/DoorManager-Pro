...
function MaterialsModule() {
  ...
  return <>{workspace === 'superadmin' && <section className="page"><SuperadminCompanyScope /></section>}<ListPage title="Materiales" summary="Catálogo real de materiales reutilizable en partes y presupuestos." search={search} setSearch={setSearch} action={<button className="primary" disabled={workspace === 'superadmin' && !companyId} onClick={() => setEditing(workspace === 'superadmin' ? { company_id: companyId } : {})}>Crear material</button>} loading={loading} error={error} retry={reload} empty={!data.length}><div className="grid half">{data.map((item: any) => { const margin = Number(item.price ?? 0) - Number(item.cost ?? 0); return <Card key={item.id} title={`${item.code} · ${item.description}`} action={<button onClick={() => setEditing(item)}>Editar</button>}><InfoGrid items={[[ 'Activo', item.active === false ? 'No' : 'Sí' ], [ 'Fabricante', item.manufacturer ?? '-' ], [ 'Referencia', item.reference ?? '-' ], [ 'Unidad', item.unit ?? 'ud' ], [ 'Coste', `${Number(item.cost ?? 0).toLocaleString('es-ES')} €` ], [ 'Precio venta', `${Number(item.price ?? 0).toLocaleString('es-ES')} €` ], [ 'Margen aprox.', `${margin.toLocaleString('es-ES')} €` ], [ 'Stock mínimo', String(item.minimum_stock ?? 0) ]]} /><div className="actions"><button onClick={() => setEditing(item)}>Editar material</button><button className="danger" onClick={() => setRemoving(item)}>Desactivar</button></div></Card>; })}</div>{editing && <MaterialForm initial={editing.id ? editing : workspace === 'superadmin' ? { company_id: companyId } : undefined} onClose={() => setEditing(null)} onSaved={() => { setEditing(null); reload(); }} />}{removing && <ReasonConfirmModal title="Desactivar material" text={`Desactivar ${removing.code} · ${removing.description}. El material no se borrará de partes ni presupuestos históricos.`} requiredLabel="Motivo de desactivación" onCancel={() => setRemoving(null)} onConfirm={async () => { await materialsService.deactivate(removing.id); setRemoving(null); reload(); }} />}</ListPage></>;
}

function MaterialForm({ initial, onClose, onSaved }: { initial?: any; onClose: () => void; onSaved: () => void }) {
  const [values, setValues] = useState<Record<string, any>>({ description: '', manufacturer: '', reference: '', unit: 'ud', cost: 0, price: 0, minimum_stock: 0, active: true, ...initial });
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState('');
  const set = (key: string, value: any) => setValues((current) => ({ ...current, [key]: value }));
  ...
  return <ModalForm title={initial?.id ? 'Editar material' : 'Crear material'} onClose={onClose} onSubmit={submit} saving={saving} error={error}><p className="large-note">{initial?.id ? 'Edita el catálogo compartido.' : 'El código MAT se generará automáticamente si lo dejas vacío.'}</p><div className="form-grid"><label>Código<input value={values.code ?? ''} onChange={(event) => set('code', event.target.value)} placeholder="Automático" /></label><label>Descripción *<input value={values.description ?? ''} onChange={(event) => set('description', event.target.value)} required /></label><label>Fabricante<input value={values.manufacturer ?? ''} onChange={(event) => set('manufacturer', event.target.value)} /></label><label>Referencia<input value={values.reference ?? ''} onChange={(event) => set('reference', event.target.value)} /></label><label>Unidad<input value={values.unit ?? ''} onChange={(event) => set('unit', event.target.value)} /></label><label>Coste<input type="number" min="0" step="0.01" value={values.cost ?? 0} onChange={(event) => set('cost', event.target.value)} /></label><label>Precio venta<input type="number" min="0" step="0.01" value={values.price ?? 0} onChange={(event) => set('price', event.target.value)} /></label><label>Stock mínimo<input type="number" min="0" step="0.01" value={values.minimum_stock ?? 0} onChange={(event) => set('minimum_stock', event.target.value)} /></label></div><label className="check-consent"><input type="checkbox" checked={values.active !== false} onChange={(event) => set('active', event.target.checked)} /> Material activo</label></ModalForm>;
}
...
function QuoteForm({ initial, onClose, onSaved }: { initial?: any; onClose: () => void; onSaved: (id: string) => void }) {
  ...
  const [values, setValues] = useState<Record<string, any>>({ quote_type: 'reparacion', status: 'Borrador', title: '', description: '', conditions: '', discount_amount: 0, ...initial });
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState('');
  const set = (key: string, value: any) => setValues((current) => ({ ...current, [key]: value }));
...
function QuoteLineForm({ quoteId, initial, onClose, onSaved }: { quoteId: string; initial?: any; onClose: () => void; onSaved: () => void }) {
  ...
  const [values, setValues] = useState<Record<string, any>>({ line_type: 'material', description: '', quantity: 1, unit: 'ud', unit_cost: 0, unit_price: 0, tax_rate: 21, ...initial });
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState('');
  const set = (key: string, value: any) => setValues((current) => ({ ...current, [key]: value }));
  ...
  const submit = async (event: FormEvent) => { event.preventDefault(); setSaving(true); setError(''); try { initial?.id ? await quotesService.updateLine(initial.id, values) : await quotesService.addLine(quoteId, values); onSaved(); } catch (err) { setError(formErrorMessage(err, initial?.id ? 'No se ha podido modificar la línea.' : 'No se ha podido añadir la línea.')); } finally { setSaving(false); } };
  return <ModalForm title={initial?.id ? 'Editar línea' : 'Añadir línea'} onClose={onClose} onSubmit={submit} saving={saving} error={error}><FormSelect label="Tipo" value={values.line_type} onChange={(value) => set('line_type', value)} options={quoteLineTypes.map((value) => ({ value, label: displayStatus(value) }))} />{values.line_type === 'material' && <><label>Buscar material existente<input value={materialSearch} onChange={(event) => setMaterialSearch(event.target.value)} placeholder="Código, descripción, fabricante o referencia" /></label><FormSelect label="Material de catálogo opcional" value={values.material_id ?? ''} onChange={selectMaterial} options={[{ value: '', label: 'Material manual / sin catálogo' }, ...materials.data.map((item: any) => ({ value: item.id, label: `${item.code} · ${item.description} · ${Number(item.price ?? 0).toLocaleString('es-ES')} €` }))]} loading={materials.loading} /></>}<label>Descripción *<input value={values.description ?? ''} onChange={(event) => set('description', event.target.value)} required /></label><div className="form-grid"><label>Cantidad<input type="number" min="0.01" step="0.01" value={values.quantity} onChange={(event) => set('quantity', event.target.value)} required /></label><label>Unidad<input value={values.unit ?? ''} onChange={(event) => set('unit', event.target.value)} /></label><label>Coste unitario<input type="number" min="0" step="0.01" value={values.unit_cost} onChange={(event) => set('unit_cost', event.target.value)} /></label><label>Venta unitaria<input type="number" min="0" step="0.01" value={values.unit_price} onChange={(event) => set('unit_price', event.target.value)} /></label><label>IVA %<input type="number" min="0" step="0.01" value={values.tax_rate} onChange={(event) => set('tax_rate', event.target.value)} /></label><label>Descuento %<input type="number" min="0" max="100" step="0.01" value={values.discount_percent ?? 0} onChange={(event) => set('discount_percent', event.target.value)} /></label></div></ModalForm>;
}
