...
function PurchaseOrderForm({ initial, suppliers, warehouses, onClose, onSaved }: any) {
  const [values, setValues] = useState<Record<string, any>>({ order_date: new Date().toISOString().slice(0, 10), supplier_id: '', destination_warehouse_id: '', supplier_reference: '', notes: '', ...initial });
...
function PurchaseOrderLineForm({ order, materials, initial, onClose, onSaved }: any) {
  ...
  const matching = relations.data.filter((row: any) => row.supplier_id === order.supplier_id && row.active !== false);
  ...
  const set = (key: string, value: any) => setValues((current) => ({ ...current, [key]: value }));
  useEffect(() => { if (!initial?.id && values.material_id && !relations.loading && matching.length && !values.material_supplier_id) { const relation = matching.find((row: any) => row.is_preferred) ?? matching[0]; setValues((current) => ({ ...current, material_supplier_id: relation.id, unit_purchase_price: relation.purchase_unit_price == null ? '' : String(relation.purchase_unit_price) })); } }, [initial?.id, values.material_id, values.material_supplier_id, relations.loading, matching]);
  const selectMaterial = (value: string) => setValues((current) => ({ ...current, material_id: value, material_supplier_id: '', unit_purchase_price: '' }));
...
function PurchaseOrderDetail({ id, suppliers, warehouses, materials, canManage, onChanged }: any) {
  ...
  const data = order.data; const lines = data.purchase_order_lines ?? []; const refresh = () => { order.reload(); receipts.reload(); onChanged(); };
...
function ContextualPurchaseFlow({ material, orders, suppliers, warehouses, onClose }: any) {
  ...
  const preferred = relations.data.find((row: any) => row.active !== false && row.is_preferred) ?? relations.data.find((row: any) => row.active !== false);
  const effectiveSupplierId = mode === 'new' ? supplierId : drafts.find((order: any) => order.id === orderId)?.supplier_id;
  const selectedRelation = relations.data.find((row: any) => row.active !== false && row.supplier_id === effectiveSupplierId) ?? preferred;
  useEffect(() => { if (preferred && !supplierId) setSupplierId(preferred.supplier_id); if (selectedRelation) setPrice(selectedRelation.purchase_unit_price == null ? '' : String(selectedRelation.purchase_unit_price)); }, [preferred?.id, selectedRelation?.id, mode, orderId]);
  const submit = async (event: FormEvent) => { event.preventDefault(); if (mode === 'existing' && !orderId) { setError('Selecciona un borrador editable.'); return; } if (mode === 'new' && !supplierId) { setError('Selecciona un proveedor.'); return; } if (Number(quantity) <= 0) { setError('La cantidad debe ser mayor que cero.'); return; } setSaving(true); setError(''); try { const id = mode === 'new' ? await purchaseOrdersService.create({ supplier_id: supplierId, destination_warehouse_id: warehouseId || null, order_date: new Date().toISOString().slice(0, 10), notes: `Compra iniciada desde ${material.code}` }) : orderId; await purchaseOrdersService.addLine(id, { material_id: material.id, ordered_quantity: quantity, unit_purchase_price: price === '' ? null : price, material_supplier_id: selectedRelation?.supplier_id === effectiveSupplierId ? selectedRelation.id : null }); onClose(); navigate(`/app/modulos/compras?pedido=${id}`); } catch (err) { setError(formErrorMessage(err, 'No se ha podido preparar la compra.')); } finally { setSaving(false); } };
