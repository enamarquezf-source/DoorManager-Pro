...
function QuoteDetailModal({ quoteId, onClose, onChanged }: { quoteId: string; onClose: () => void; onChanged: () => void }) {
  ...
  const changed = () => { setAdding(false); reload(); onChanged(); };
  return <div className="mini-modal" role="dialog" aria-modal="true"><div><StateBlock loading={loading} error={error} retry={reload} empty={!data}>{data && <><h3>{data.code} · {data.title}</h3><InfoGrid items={[[ 'Tipo', displayStatus(data.quote_type) ], [ 'Estado', displayStatus(data.status) ], [ 'Cliente', data.clients?.legal_name ?? '-' ], [ 'Equipo', data.equipment?.code ?? '-' ], [ 'Parte', data.work_orders?.code ?? '-' ], [ 'Coste', `${Number(data.subtotal_cost ?? 0).toLocaleString('es-ES')} €` ], [ 'Venta', `${Number(data.subtotal_sale ?? data.subtotal ?? 0).toLocaleString('es-ES')} €` ], [ 'Total', `${Number(data.total_amount ?? data.total ?? 0).toLocaleString('es-ES')} €` ], [ 'Beneficio estimado', `${Number(data.estimated_margin ?? 0).toLocaleString('es-ES')} €` ]]} /><Card title="Líneas"><div className="table-card"><table><thead><tr><th>Tipo</th><th>Descripción</th><th>Cantidad</th><th>Coste</th><th>Venta</th></tr></thead><tbody>{(data.quote_lines ?? []).filter((line: any) => !line.deleted_at).map((line: any) => <tr key={line.id}><td>{displayStatus(line.line_type ?? 'material')}</td><td>{line.description}</td><td>{Number(line.quantity ?? 0).toLocaleString('es-ES')} {line.unit ?? ''}</td><td>{Number(line.total_cost ?? 0).toLocaleString('es-ES')} €</td><td>{Number(line.total_price ?? line.total ?? 0).toLocaleString('es-ES')} €</td></tr>)}</tbody></table></div>{!(data.quote_lines ?? []).length && <p className="large-note">Sin líneas.</p>}<button className="primary" onClick={() => setAdding(true)}>Añadir línea</button></Card>{adding && <QuoteLineForm quoteId={data.id} onClose={() => setAdding(false)} onSaved={changed} />}</>}</StateBlock><div className="modal-footer"><button onClick={onClose}>Cerrar</button></div></div></div>;
}

function QuoteForm({ onClose, onSaved }: { onClose: () => void; onSaved: (id: string) => void }) {
  const clients = useLoad(() => clientsService.list(''), [], [] as any[]);
  const [values, setValues] = useState<Record<string, any>>({ quote_type: 'reparacion', status: 'Borrador', title: '', description: '' });
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState('');
  const set = (key: string, value: any) => setValues((current) => ({ ...current, [key]: value }));
  const submit = async (event: FormEvent) => { event.preventDefault(); setSaving(true); setError(''); try { const row = await quotesService.create(values); onSaved(row.id); } catch (err) { setError(formErrorMessage(err, 'No se ha podido crear el presupuesto.')); } finally { setSaving(false); } };
  return <ModalForm title="Crear presupuesto" onClose={onClose} onSubmit={submit} saving={saving} error={error}><FormSelect label="Cliente" value={values.client_id} onChange={(value) => set('client_id', value)} required loading={clients.loading} options={clients.data.map((client: any) => ({ value: client.id, label: `${client.code} · ${client.legal_name}` }))} /><FormSelect label="Tipo" value={values.quote_type} onChange={(value) => set('quote_type', value)} options={[['instalacion','Instalación'],['reparacion','Reparación'],['mantenimiento','Mantenimiento']].map(([value, label]) => ({ value, label }))} /><label>Título *<input value={values.title} onChange={(event) => set('title', event.target.value)} required /></label><label>Descripción<textarea value={values.description} onChange={(event) => set('description', event.target.value)} /></label><label>Válido hasta<input type="date" value={values.valid_until ?? ''} onChange={(event) => set('valid_until', event.target.value)} /></label></ModalForm>;
}

function QuoteLineForm({ quoteId, onClose, onSaved }: { quoteId: string; onClose: () => void; onSaved: () => void }) {
  const [values, setValues] = useState<Record<string, any>>({ line_type: 'material', description: '', quantity: 1, unit: 'ud', unit_cost: 0, unit_price: 0, tax_rate: 21 });
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState('');
  const set = (key: string, value: any) => setValues((current) => ({ ...current, [key]: value }));
  const submit = async (event: FormEvent) => { event.preventDefault(); setSaving(true); setError(''); try { await quotesService.addLine(quoteId, values); onSaved(); } catch (err) { setError(formErrorMessage(err, 'No se ha podido añadir la línea.')); } finally { setSaving(false); } };
  return <ModalForm title="Añadir línea" onClose={onClose} onSubmit={submit} saving={saving} error={error}><FormSelect label="Tipo" value={values.line_type} onChange={(value) => set('line_type', value)} options={[['material','Material'],['labor','Horas'],['transport','Transporte'],['travel','Desplazamiento'],['mobile_workshop','Taller móvil'],['lifting_platform','Plataforma elevadora'],['auxiliary_equipment','Medios auxiliares'],['external_cost','Coste externo'],['fee','Fee'],['discount','Descuento'],['other','Otro']].map(([value, label]) => ({ value, label }))} /><label>Descripción *<input value={values.description} onChange={(event) => set('description', event.target.value)} required /></label><div className="form-grid"><label>Cantidad<input type="number" min="0.01" step="0.01" value={values.quantity} onChange={(event) => set('quantity', event.target.value)} required /></label><label>Unidad<input value={values.unit} onChange={(event) => set('unit', event.target.value)} /></label><label>Coste unitario<input type="number" min="0" step="0.01" value={values.unit_cost} onChange={(event) => set('unit_cost', event.target.value)} /></label><label>Venta unitaria<input type="number" min="0" step="0.01" value={values.unit_price} onChange={(event) => set('unit_price', event.target.value)} /></label><label>IVA %<input type="number" min="0" step="0.01" value={values.tax_rate} onChange={(event) => set('tax_rate', event.target.value)} /></label></div></ModalForm>;
}
