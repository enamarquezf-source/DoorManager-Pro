...
function StockOpeningBulkPanel() {
  ...
  const [error, setError] = useState('');
  if (!canOpen) return null;
  const rows = materials.data.map((material: any) => {
    const canonical = stock.data.filter((item: any) => item.material_id === material.id);
    const canonicalTotal = canonical.reduce((sum: number, item: any) => sum + Number(item.quantity ?? 0), 0);
    const legacy = Number(material.stock_quantity ?? 0);
    const status = canonical.length === 0 ? legacy > 0 ? 'LEGACY_ONLY_POSITIVE' : 'LEGACY_ONLY_ZERO' : canonical.length > 1 || canonicalTotal !== legacy ? 'MISMATCH' : 'MATCH';
    return { material, canonical, canonicalTotal, legacy, status };
  });
  const candidates = rows.filter((row) => row.status === 'LEGACY_ONLY_POSITIVE');
  const visibleRows = rows.filter((row) => filter === 'pending' ? row.status === 'LEGACY_ONLY_POSITIVE' : filter === 'opened' ? ['CANONICAL_EXISTS', 'MATCH'].includes(row.status) : filter === 'review' ? row.status === 'MISMATCH' : row.status === 'LEGACY_ONLY_ZERO');
...
function MaterialsModule() { return <><StockOpeningPanel /><StockOpeningBulkPanel /><MaterialsCanonicalSummary /><MaterialsModuleContent /></>; }
