...
function CanonicalMaterialForm({ initial, warehouses, onClose, onSaved }: { initial?: any; warehouses: any[]; onClose: () => void; onSaved: () => void }) {
  const [values, setValues] = useState<Record<string, any>>({ description: '', manufacturer: '', reference: '', unit: 'ud', cost: 0, price: 0, minimum_stock: 0, stock_controlled: true, allow_negative_stock: false, is_specific: false, active: true, initial_quantity: 0, warehouse_id: '', ...initial });
  const suppliers = useLoad(() => suppliersService.list('', 'active'), [], [] as any[]);
