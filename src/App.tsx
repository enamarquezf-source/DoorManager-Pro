...
function MaterialSupplierRow({ row, onChanged }: { row: any; onChanged: () => void }) {
  ...
  return <article className="compact-list"><strong>{row.suppliers?.name ?? 'Proveedor'}</strong>{row.is_preferred && <Badge tone="ok">Preferente</Badge>}<div className="form-grid"><label>Referencia<input value={reference} onChange={(event) => setReference(event.target.value)} /></label><label>Precio habitual de compra<input type="number" min="0" step="0.01" value={price} onChange={(event) => setPrice(event.target.value)} /></label></div><div className="row-actions"><button type="button" onClick={save} disabled={saving}>{saving ? 'Guardando...' : 'Editar relación'}</button><button type="button" onClick={async () => { await suppliersService.setPreferredMaterialSupplier(row.id, row.material_id); onChanged(); }} disabled={saving || row.is_preferred}>Marcar preferente</button><button type="button" onClick={async () => { await suppliersService.deactivateMaterialSupplier(row.id); onChanged(); }}>Desvincular</button></div></article>;
}
...
function MaterialSuppliersEditor({ materialId }: { materialId: string }) {
  ...
  return <Card title="Proveedores"><div className="form-grid"><FormSelect label="Proveedor" value={supplierId} onChange={setSupplierId} options={[{ value: '', label: 'Selecciona proveedor' }, ...available.map((supplier: any) => ({ value: supplier.id, label: `${supplier.name}${supplier.tax_id ? ` · ${supplier.tax_id}` : ''}` }))]} loading={suppliers.loading} /><label>Referencia<input value={reference} onChange={(event) => setReference(event.target.value)} /></label><label>Precio compra<input type="number" min="0" step="0.01" value={price} onChange={(event) => setPrice(event.target.value)} /></label></div><button type="button" onClick={add} disabled={!supplierId || suppliers.loading}>Añadir proveedor</button>{error && <p className="form-error">{error}</p>}<div className="compact-list">{relations.data.filter((row: any) => row.active).map((row: any) => <MaterialSupplierRow key={row.id} row={row} onChanged={relations.reload} />)}{!relations.loading && !relations.data.some((row: any) => row.active) && <p className="large-note">Sin proveedores asociados.</p>}</div></Card>;
}
