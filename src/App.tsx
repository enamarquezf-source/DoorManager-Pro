...
function MultiEquipmentPicker({ values, setValues, filteredEquipment, equipmentTypes, installation }: any) {
  ...
  const addNew = () => { const quantity = Math.max(1, Number(draft.quantity) || 1); if (!draft.equipment_type_id) return; update([...selection, ...Array.from({ length: quantity }, () => ({ kind: 'new', ...draft, quantity: undefined }))]); setDraft({ ...draft, quantity: 1, internal_location: '', serial_number: '' }); };
