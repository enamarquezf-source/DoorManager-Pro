...
import { canAccessModule, canAccessRoute, canArchiveEntity, canAssignTechnician, canCorrectWorkOrderOperationalFields, canCreateAlert, canCreateCheck, canCreateWorkOrder, canEditWorkOrder, canExecuteCheck, canExecuteWorkOrder, canManageCheck, canManageWorkOrderAssignments, canManageWorkOrderCosts, canManageWorkOrderMaterials, canManageWorkOrderStatus, canManageWorkOrderTime, canPermanentlyDeleteEntity, canRestoreEntity, canRole, canViewCheck, canViewWorkOrder, canViewWorkOrderCosts, isSuperadmin, normalizedRoleNames, profileWorkspaces } from './auth/permissions';
...
function ClientsPage() {
  ...
  return <>{workspace === 'superadmin' && <section className="page"><SuperadminCompanyScope /></section>}<ListPage title="Clientes" summary="Listado conectado a public.clients y public.client_contacts." search={search} setSearch={setSearch} archiveFilter={archiveFilter} setArchiveFilter={setArchiveFilter} action={<button className="primary" disabled={workspace === 'superadmin' && !companyId} onClick={() => setCreating(true)}>Crear cliente</button>} loading={loading} error={error} retry={reload} empty={!data.length}><div className="grid half">{data.map((client) => <Card key={client.id} title={client.legal_name} action={<Link to={`${baseRoute}/${client.id}`}>Abrir</Link>}><div className={isArchivedRecord('clients', client) ? 'archived-record' : ''}><InfoGrid items={[[ 'Empresa', client.companies?.name ?? '-' ], [ 'Código', client.code ], [ 'Nombre comercial', client.trade_name ?? '-' ], [ 'NIF', client.tax_id ?? '-' ], [ 'Estado', client.deleted_at ? 'Archivado' : client.status ], [ 'Teléfono', client.phone ?? '-' ], [ 'Correo', client.email ?? '-' ], [ 'Centros', String(client.sites?.length ?? 0) ], [ 'Equipos', String(client.equipment?.length ?? 0) ]]} /><LifecycleActionPanel entity="clients" record={client} onChanged={reload} /></div></Card>)}</div>{creating && <ClientForm title="Crear cliente" initial={{ company_id: companyId }} onClose={() => setCreating(false)} onSaved={() => { setCreating(false); reload(); }} />}</ListPage></>;
}
...
function EquipmentPage() { const { workspace } = useAuth(); const { companyId } = useSuperadminScope(); const scope = workspace === 'superadmin' ? companyId : undefined; const baseRoute = workspace === 'superadmin' ? '/app/superadmin/equipos' : '/app/equipos'; const [search, setSearch] = useState(''); const [archiveFilter, setArchiveFilter] = useState<ArchiveFilter>('active'); const { data, loading, error, reload } = useLoad(() => equipmentService.list(search, scope, archiveFilter), [search, scope, archiveFilter], [] as any[]); const [creating, setCreating] = useState(false); return <>{workspace === 'superadmin' && <section className="page"><SuperadminCompanyScope /></section>}<ListPage title="Equipos" summary="Inventario conectado a public.equipment, tipos y componentes." search={search} setSearch={setSearch} archiveFilter={archiveFilter} setArchiveFilter={setArchiveFilter} action={<button className="primary" disabled={workspace === 'superadmin' && !companyId} onClick={() => setCreating(true)}>Crear equipo</button>} loading={loading} error={error} retry={reload} empty={!data.length}><div className="grid half">{data.map((item) => <Card key={item.id} title={item.code} action={<Link to={`${baseRoute}/${item.id}`}>Abrir</Link>}><div className={isArchivedRecord('equipment', item) ? 'archived-record' : ''}><InfoGrid items={[[ 'Empresa', item.companies?.name ?? '-' ], [ 'Tipo', item.equipment_types?.name ?? '-' ], [ 'Cliente', item.clients?.legal_name ?? '-' ], [ 'Centro', item.sites?.name ?? '-' ], [ 'Ubicación', item.internal_location ?? '-' ], [ 'Marca/modelo', `${item.brand ?? '-'} ${item.model ?? ''}` ], [ 'Estado', item.deleted_at ? 'Archivado' : displayStatus(item.status) ], [ 'Próxima revisión', item.next_review_date ?? '-' ]]} /><LifecycleActionPanel entity="equipment" record={item} onChanged={reload} /></div></Card>)}</div>{creating && <EquipmentForm initial={{ company_id: companyId }} onClose={() => setCreating(false)} onSaved={() => { setCreating(false); reload(); }} />}</ListPage></>; }
...
function WorkOrderOperationalForm({ workOrder, onClose, onSaved }: { workOrder: any; onClose: () => void; onSaved: () => void }) { const summary = interventionSummary(workOrder); const [values, setValues] = useState<Record<string, string>>({ description: workOrder.description ?? '', diagnosis: summary.diagnosis ?? '', work_performed: summary.work ?? '', result: summary.result ?? '', observations: summary.observations ?? '', planned_material: workOrder.planned_material ?? '' }); const [error, setError] = useState(''); const [saving, setSaving] = useState(false); const set = (key: string, value: string) => setValues((current) => ({ ...current, [key]: value })); const submit = async (event: FormEvent) => { event.preventDefault(); setSaving(true); setError(''); try { await workOrdersService.updateOperationalFields(workOrder.id, values); onSaved(); } catch (err: any) { console.error('Error operativo Supabase', { message: err?.message, details: err?.details, hint: err?.hint, code: err?.code }); setError(err instanceof Error ? err.message : 'No se han podido corregir los campos operativos del parte.'); } finally { setSaving(false); } }; return <div className="mini-modal" role="dialog" aria-modal="true"><form onSubmit={submit}><h3>Revisión/corrección de oficina</h3><p className="large-note">Solo se guardan estos campos operativos; no se modifican cliente, centro, equipo ni asignaciones.</p><label>Descripción del problema<textarea value={values.description} onChange={(event) => set('description', event.target.value)} /></label><label>Diagnóstico<textarea value={values.diagnosis} onChange={(event) => set('diagnosis', event.target.value)} /></label><label>Trabajo realizado<textarea value={values.work_performed} onChange={(event) => set('work_performed', event.target.value)} /></label><label>Solución / resultado<textarea value={values.result} onChange={(event) => set('result', event.target.value)} /></label><label>Observaciones<textarea value={values.observations} onChange={(event) => set('observations', event.target.value)} /></label><label>Material previsto<input value={values.planned_material} onChange={(event) => set('planned_material', event.target.value)} /></label>{error && <p className="form-error">{error}</p>}<div className="modal-footer"><button type="button" onClick={onClose} disabled={saving}>Cancelar</button><button className="primary" disabled={saving}>{saving ? 'Guardando...' : 'Guardar cambios operativos'}</button></div></form></div>; }

function WorkOrderStatusSelector({ workOrder, onChanged, onError }: { workOrder: any; onChanged: () => void; onError: (message: string) => void }) { const { profile } = useAuth(); const [value, setValue] = useState(workOrder.status); const [confirm, setConfirm] = useState<any | null>(null); const [saving, setSaving] = useState(false); if (!canManageWorkOrderStatus(profile, workOrder)) return null; const prepare = (next: string) => { if (next === workOrder.status || saving) return; const currentIndex = workOrderStatuses.indexOf(workOrder.status); const nextIndex = workOrderStatuses.indexOf(next); const needsReason = next === 'Cancelado' || next === 'Cerrado' || nextIndex < currentIndex || ['Pendiente','Devuelto por SAT'].includes(next); setValue(next); setConfirm({ next, needsReason, reason: '', warning: next === 'Cerrado' ? 'Cerrar el parte puede bloquear modificaciones posteriores.' : next === 'Cancelado' ? 'Cancelar exige justificar el motivo.' : '' }); }; const change = async () => { if (!confirm || saving) return; if (confirm.needsReason && !confirm.reason.trim()) { onError('El motivo es obligatorio para este cambio de estado.'); return; } setSaving(true); onError(''); try { await workOrdersService.changeStatus(workOrder.id, confirm.next, confirm.reason || 'Cambio directo de estado'); setConfirm(null); onChanged(); } catch (err) { setValue(workOrder.status); onError(err instanceof Error ? err.message : 'No se ha podido cambiar el estado.'); } finally { setSaving(false); } }; return <Card title="Estado del parte"><label className="status-selector">Estado:<select value={value} disabled={saving} onChange={(event) => prepare(event.target.value)}>{workOrderStatuses.map((status) => <option key={status} value={status}>{displayStatus(status)}</option>)}</select></label><p className="large-note">SAT, Gerencia y superadmin pueden seleccionar cualquier estado. Técnicos y Comercial quedan limitados por Supabase.</p>{confirm && <div className="mini-modal" role="dialog" aria-modal="true"><div><h3>Cambiar estado</h3><p>{confirm.warning} Confirmar cambio a {displayStatus(confirm.next)}.</p><label>Motivo {confirm.needsReason ? '*' : 'opcional'}<textarea value={confirm.reason} onChange={(event) => setConfirm({ ...confirm, reason: event.target.value })} /></label><div className="modal-footer"><button onClick={() => { if (!saving) { setValue(workOrder.status); setConfirm(null); } }} disabled={saving}>Cancelar</button><button className="primary" onClick={change} disabled={saving}>{saving ? 'Guardando...' : 'Confirmar estado'}</button></div></div></div>}</Card>; }
...
function WorkOrderCostForm({ workOrder, initial, onClose, onSaved }: { workOrder: any; initial?: any; onClose: () => void; onSaved: () => void }) { const { profile } = useAuth(); const showCosts = canViewWorkOrderCosts(profile); const [values, setValues] = useState<Record<string, string>>({ id: initial?.id ?? '', work_order_id: workOrder.id, cost_type: initial?.cost_type ?? 'desplazamiento', description: initial?.description ?? '', quantity: String(initial?.quantity ?? 1), unit: initial?.unit ?? 'ud', unit_cost: initial?.unit_cost ? String(initial.unit_cost) : '', incurred_at: (initial?.incurred_at ?? new Date().toISOString()).slice(0, 10) }); const [error, setError] = useState(''); const [saving, setSaving] = useState(false); const set = (key: string, value: string) => setValues((current) => ({ ...current, [key]: value })); const submit = async (event: FormEvent) => { event.preventDefault(); if (!values.description.trim()) { setError('validacion del formulario: describe el recurso o coste.'); return; } if (Number(values.quantity) <= 0) { setError('validacion del formulario: la cantidad debe ser mayor que cero.'); return; } if (showCosts && Number(values.unit_cost || 0) < 0) { setError('validacion del formulario: el coste unitario no puede ser negativo.'); return; } setSaving(true); setError(''); try { const payload = Object.fromEntries(Object.entries(values).filter(([, value]) => value !== '')); if (!showCosts) delete payload.unit_cost; await workOrdersService.upsertCostEntry(payload); onSaved(); } catch (err) { setError(err instanceof Error ? err.message : 'respuesta de Supabase: no se ha podido registrar el recurso o coste.'); } finally { setSaving(false); } }; return <div className="mini-modal" role="dialog" aria-modal="true"><form onSubmit={submit}><h3>{initial ? 'Editar recurso/coste' : 'Añadir recurso/coste'}</h3><FormSelect label="Tipo" value={values.cost_type} onChange={(value) => set('cost_type', value)} options={['desplazamiento','taller_movil','plataforma_elevadora','medio_auxiliar','coste_externo','parking_peaje','dieta','otro'].map((value) => ({ value, label: displayStatus(value) }))} /><label>Descripción<textarea value={values.description} onChange={(event) => set('description', event.target.value)} /></label><div className="grid half"><label>Fecha<input type="date" value={values.incurred_at} onChange={(event) => set('incurred_at', event.target.value)} /></label><label>Cantidad<input type="number" min="0.01" step="0.01" value={values.quantity} onChange={(event) => set('quantity', event.target.value)} /></label><label>Unidad<input value={values.unit} onChange={(event) => set('unit', event.target.value)} placeholder="km, h, ud" /></label>{showCosts && <label>Coste unitario<input type="number" min="0" step="0.01" value={values.unit_cost} onChange={(event) => set('unit_cost', event.target.value)} /></label>}</div>{error && <p className="form-error">{error}</p>}<div className="actions"><button type="button" onClick={onClose}>Cancelar</button><button className="primary" disabled={saving}>{saving ? 'Guardando...' : 'Guardar recurso/coste'}</button></div></form></div>; }
...
function TechnicianWorkPage() {
  ...
  const [message, setMessage] = useState('');
  const [actionError, setActionError] = useState('');
  const [mode, setMode] = useState<'time' | 'material' | null>(null);
...
function QuotesModule() {
  ...
  const scope = workspace === 'superadmin' ? companyId : undefined;
  const { data, loading, error, reload } = useLoad(() => quotesService.list(search, 'Todos', scope), [search, scope], [] as any[]);
  ...
  return <>{workspace === 'superadmin' && <section className="page"><SuperadminCompanyScope /></section>}<ListPage title="Presupuestos" summary="Presupuestos de instalación, reparación y mantenimiento con costes, venta y margen." search={search} setSearch={setSearch} action={<button className="primary" disabled={workspace === 'superadmin' && !companyId} onClick={() => setCreating(true)}>Crear presupuesto</button>} loading={loading} error={error} retry={reload} empty={!data.length}><div className="tabs">{quoteStatusFilters.map((item) => <button key={item} className={status === item ? 'active' : ''} onClick={() => setStatus(item)}>{item === 'Enviado' ? 'Enviado/Mandado' : item} ({counts[item] ?? 0})</button>)}</div><div className="grid half">{visibleQuotes.map((quote: any) => <Card key={quote.id} title={`${quote.code} · ${quote.title}`} action={<Link to={`/app/modulos/presupuestos/${quote.id}`}>Abrir</Link>}><InfoGrid items={[[ 'Tipo', displayStatus(quote.quote_type ?? 'reparacion') ], [ 'Estado', displayStatus(quote.status) ], [ 'Cliente', quote.clients?.legal_name ?? '-' ], [ 'Base venta', `${Number(quote.subtotal_sale ?? quote.subtotal ?? 0).toLocaleString('es-ES')} €` ], [ 'Impuestos', `${Number(quote.tax_amount ?? 0).toLocaleString('es-ES')} €` ], [ 'Total', `${Number(quote.total_amount ?? quote.total ?? 0).toLocaleString('es-ES')} €` ], [ 'Margen estimado', `${Number(quote.estimated_margin ?? 0).toLocaleString('es-ES')} €` ], [ 'Validez', quote.valid_until ?? '-' ]]} /></Card>)}</div>{!visibleQuotes.length && <p className="large-note">No hay presupuestos para el estado seleccionado.</p>}{creating && <QuoteForm initial={workspace === 'superadmin' ? { company_id: companyId } : undefined} onClose={() => setCreating(false)} onSaved={(id: string) => { setCreating(false); reload(); navigate(`/app/modulos/presupuestos/${id}`); }} />}</ListPage></>;
}
...
function MaterialsModule() {
  ...
  const [search, setSearch] = useState('');
  const scope = workspace === 'superadmin' ? companyId : undefined;
  const { data, loading, error, reload } = useLoad(() => materialsService.list(search, scope), [search, scope], [] as any[]);
  const [editing, setEditing] = useState<any | null>(null);
  ...
  return <>{workspace === 'superadmin' && <section className="page"><SuperadminCompanyScope /></section>}<ListPage title="Materiales" summary="Catálogo real de materiales reutilizable en partes y presupuestos." search={search} setSearch={setSearch} action={<button className="primary" disabled={workspace === 'superadmin' && !companyId} onClick={() => setEditing(workspace === 'superadmin' ? { company_id: companyId } : {})}>Crear material</button>} loading={loading} error={error} retry={reload} empty={!data.length}><div className="stats-grid"><div className="metric info"><span>Activos</span><strong>{active.length}</strong><small>Materiales disponibles</small></div><div className="metric warn"><span>Bajo stock</span><strong>{low}</strong><small>Por debajo del mínimo</small></div><div className="metric danger"><span>Sin stock</span><strong>{emptyStock}</strong><small>Stock actual cero o inferior</small></div><div className="metric ok"><span>Valor stock</span><strong>{stockValue.toLocaleString('es-ES')} €</strong><small>Stock x coste</small></div></div><div className="grid half">{data.map((item: any) => { const margin = Number(item.price ?? 0) - Number(item.cost ?? 0); const stockStatus = item.active === false ? 'Inactivo' : materialStockStatus(item); return <Card key={item.id} title={`${item.code} · ${item.description}`} action={<Badge tone={stockStatus === 'OK' ? 'ok' : stockStatus === 'Inactivo' ? 'muted' : stockStatus === 'Bajo stock' ? 'warn' : 'danger'}>{stockStatus}</Badge>}><InfoGrid items={[[ 'Activo', item.active === false ? 'No' : 'Sí' ], [ 'Fabricante', item.manufacturer ?? '-' ], [ 'Referencia', item.reference ?? '-' ], [ 'Unidad', item.unit ?? 'ud' ], [ 'Coste', `${Number(item.cost ?? 0).toLocaleString('es-ES')} €` ], [ 'Precio venta', `${Number(item.price ?? 0).toLocaleString('es-ES')} €` ], [ 'Margen aprox.', `${margin.toLocaleString('es-ES')} €` ], [ 'Stock actual', `${Number(item.stock_quantity ?? 0).toLocaleString('es-ES')} ${item.unit ?? 'ud'}` ], [ 'Stock mínimo', `${Number(item.minimum_stock ?? 0).toLocaleString('es-ES')} ${item.unit ?? 'ud'}` ], [ 'Control stock', item.stock_controlled === false ? 'No' : 'Sí' ], [ 'Permite negativo', item.allow_negative_stock ? 'Sí' : 'No' ]]} /><div className="actions"><button onClick={() => setEditing(item)}>Editar material</button><button onClick={() => setStockAction(item)}>Ajustar stock</button><button onClick={() => setMovementItem(item)}>Ver movimientos</button><button className="danger" onClick={() => setRemoving(item)}>Desactivar</button></div></Card>; })}</div>{editing && <MaterialForm initial={editing.id ? editing : workspace === 'superadmin' ? { company_id: companyId } : undefined} onClose={() => setEditing(null)} onSaved={() => { setEditing(null); reload(); }} />}{stockAction && <StockAdjustModal material={stockAction} onClose={() => setStockAction(null)} onSaved={() => { setStockAction(null); reload(); }} />}{movementItem && <StockMovementsModal material={movementItem} onClose={() => setMovementItem(null)} />}{removing && <ReasonConfirmModal title="Desactivar material" text={`Desactivar ${removing.code} · ${removing.description}. El material no se borrará de partes ni presupuestos históricos.`} requiredLabel="Motivo de desactivación" onCancel={() => setRemoving(null)} onConfirm={async () => { await materialsService.deactivate(removing.id); setRemoving(null); reload(); }} />}</ListPage></>;
}

function materialStockStatus(material: any) { if (material.active === false) return 'Inactivo'; if (Number(material.stock_quantity ?? 0) <= 0) return 'Sin stock'; if (Number(material.minimum_stock ?? 0) > 0 && Number(material.stock_quantity ?? 0) <= Number(material.minimum_stock ?? 0)) return 'Bajo stock'; return 'OK'; }

function MaterialForm({ initial, onClose, onSaved }: { initial?: any; onClose: () => void; onSaved: () => void }) {
  const [values, setValues] = useState<Record<string, any>>({ description: '', manufacturer: '', reference: '', unit: 'ud', cost: 0, price: 0, stock_quantity: 0, minimum_stock: 0, stock_controlled: true, allow_negative_stock: false, active: true, ...initial });
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState('');
  const set = (key: string, value: any) => setValues((current) => ({ ...current, [key]: value }));
...
function StockMovementsModal({ material, onClose }: { material: any; onClose: () => void }) {
  ...
  return <SidePanel title={`Movimientos · ${material.code}`} subtitle={material.description} onClose={onClose}><StateBlock loading={loading} error={error} retry={reload} empty={!data.length}><div className="compact-list">{data.map((row: any) => <article key={row.id}><Badge tone={row.movement_type === 'out' ? 'warn' : row.movement_type === 'return' ? 'ok' : 'info'}>{displayStatus(row.movement_type)}</Badge><p><strong>{Number(row.quantity).toLocaleString('es-ES')} {material.unit ?? 'ud'}</strong><br />{formatDate(row.created_at)} · Stock {Number(row.previous_stock).toLocaleString('es-ES')} → {Number(row.new_stock).toLocaleString('es-ES')}</p><p>{row.work_orders?.code ? `Parte: ${row.work_orders.code} · ${row.work_orders.title ?? ''}` : 'Origen manual'} · {fullName(row.profiles) || '-'}</p><p>{row.reason ?? 'Sin motivo informado.'}</p></article>)}</div></StateBlock></SidePanel>;
}
...
function QuoteDetailModal({ quoteId, onClose, onChanged }: { quoteId: string; onClose: () => void; onChanged: () => void }) {
  ...
  const [mode, setMode] = useState<'edit' | 'line' | 'send' | null>(null);
...
function QuoteSendModal({ quote, onClose, onSaved }: { quote: any; onClose: () => void; onSaved: () => void }) {
  ...
  return <ModalForm title="Enviar al cliente" onClose={onClose} onSubmit={submit} saving={saving} error={error} submitLabel="Marcar como enviado"><p className="large-note">No hay integración de email real activa en esta fase. Se registrará el email usado, la fecha de envío y el estado Enviado.</p><label>Email destino *<input type="email" value={email} onChange={(event) => setEmail(event.target.value)} required placeholder="cliente@empresa.com" /></label></ModalForm>;
}
...
const moduleMeta: Record<string, { title: string; description: string; links: { label: string; to: string }[] }> = {
  ...
  materiales: { title: 'Materiales', description: 'Catálogo de materiales para partes y presupuestos.', links: [{ label: 'Partes con material', to: '/app/partes?filtro=material' }] },
