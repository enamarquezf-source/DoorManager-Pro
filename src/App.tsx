...
import { checkProblemStatuses, physicalSectionalZones, sectionalZones, visibleSectionalZones, type CheckBlockId } from './checks/sectionalZones';
...
function CheckDetailPage() {
  ...
  if (loading || error || !data) return <StateBlock loading={loading} error={error} retry={reload} empty={!data} />;
  const zones = visibleSectionalZones(data.equipment);
  const physicalZones = physicalSectionalZones(data.equipment);
...
function ModulePage() { const { moduleId = '' } = useParams(); const { workspace } = useAuth(); const meta = moduleMeta[moduleId] ?? { title: 'Módulo en preparación', description: 'Este módulo tendrá su propia pantalla y no redirige a otro módulo incorrecto.', links: [] }; return <section className="page"><Breadcrumb items={[workspaceTitles[workspace], meta.title]} /><Hero title={meta.title} subtitle={meta.description} tone="info" /><Card title="Estado del módulo"><p className="large-note">Pantalla real reservada para {meta.title}. La navegación ya no abre Partes, Documentación o Gerencia como sustituto incorrecto.</p><div className="actions">{meta.links.map((link: any) => <Link key={link.to} to={link.to}>{link.label}</Link>)}</div></Card></section>; }
...
function CaseForm({ title, initial, onClose, onSaved }: any) { const fields = initial?.id ? [[ 'title','Título',true ],[ 'description','Descripción' ],[ 'type','Tipo',true ],[ 'priority','Prioridad' ],[ 'status','Estado' ],[ 'client_id','Cliente UUID',true ],[ 'site_id','Centro UUID' ],[ 'origin','Origen',true ]] : [[ 'title','Título',true ],[ 'description','Descripción' ],[ 'type','Tipo',true ],[ 'priority','Prioridad' ],[ 'status','Estado' ],[ 'client_id','Cliente UUID',true ],[ 'site_id','Centro UUID' ],[ 'origin','Origen',true ]]; return <EntityForm title={title} fields={fields} initial={{ type: 'Averia', priority: 'Normal', status: 'Abierto', origin: 'SAT', ...initial }} onClose={onClose} onSubmit={(values: any) => initial?.id ? casesService.update(initial.id, values) : casesService.create(values)} onSaved={onSaved} />; }
...
function EntityForm({ title, fields, initial = {}, onClose, onSubmit, onSaved }: any) { useOverlayScrollLock(); const [values, setValues] = useState<Record<string, any>>(initial); const [error, setError] = useState(''); const [saving, setSaving] = useState(false); const submit = async (event: FormEvent) => { event.preventDefault(); setSaving(true); setError(''); try { const result = await onSubmit(values); onSaved?.(result?.id ?? result); } catch (err) { setError(err instanceof Error ? err.message : 'Error al guardar'); } finally { setSaving(false); } }; return <div className="mini-modal" role="dialog" aria-modal="true" onWheel={(event) => event.stopPropagation()} onTouchMove={(event) => event.stopPropagation()}><form onSubmit={submit}><h3>{title}</h3>{fields.map(([key, label, required]: any[]) => <label key={key}>{label}{required ? ' *' : ''}<input value={values[key] ?? ''} onChange={(event) => setValues({ ...values, [key]: event.target.value })} required={Boolean(required)} /></label>)}{error && <p className="form-error">{error}</p>}<div className="modal-footer"><button type="button" onClick={onClose} disabled={saving}>Cancelar</button><button className="primary" disabled={saving}>{saving ? 'Guardando...' : title}</button></div></form></div>; }
