...
import { movementDirection, movementDisplayQuantity, movementLabel, movementLinks, movementOrigin, movementReason } from './services/materialsMovements';
...
import { canAccessModule, canAccessRoute, canArchiveEntity, canAssignTechnician, canCorrectWorkOrderOperationalFields, canCreateAlert, canCreateCheck, canCreateWorkOrder, canDeleteInvoiceDraft, canEditWorkOrder, canExecuteCheck, canExecuteWorkOrder, canManageCheck, canManageEquipmentTypes, canManageHourRates, canManageQuotes, canManagePurchaseOrders, canManageSuppliers, canManageWorkOrderAssignments, canManageWorkOrderCosts, canManageWorkOrderMaterials, canManageWorkOrderStatus, canManageWorkOrderTime, canMarkAdditionalSale, canPermanentlyDeleteEntity, canRestoreEntity, canReviewWorkOrderCommercial, canReviewWorkOrderEconomic, canReviewWorkOrderOffice, canReviewWorkOrderSat, canRole, canViewCheck, canViewInternalEconomics, canViewPurchaseOrders, canViewSuppliers, canViewWorkOrder, canViewWorkOrderCosts, hasPermission, isSuperadmin, normalizedRoleNames, profileWorkspaces } from './auth/permissions';
...
import { technicianDayCounts, technicianDayEmptyMessage, technicianDayRows, technicianDayTabs, type TechnicianDayTab } from './shared/technicianDayFilters';
...
import { resolvePurchaseOrderPrice } from './shared/purchaseOrderPrice';
...
import { formatCenterLabel, formatClientLabel, formatEntityLabel, formatEquipmentLabel, formatMaterialLabel, formatSupplierLabel } from './shared/entityDisplayLabels';
...
const superadminSharedRoutes = ['/app/modulos/presupuestos', '/app/modulos/materiales', '/app/modulos/administracion', '/app/modulos/facturacion', '/app/modulos/cobros', '/app/modulos/compras', '/app/modulos/rentabilidad', '/app/modulos/tarifas-horas', '/app/modulos/tipos-equipo'];
const satReviewFlags = [
  ['materials_entered', 'Se han introducido materiales'],
  ['work_completed', 'El trabajo está terminado'],
  ['non_billable_materials_or_hours', 'Hay materiales u horas no facturables'],
] as const;
...
function SidebarUpdateCheck() {
  ...
  const click = async () => {
    if (available) {
      ...
      sessionStorage.setItem('dmp-last-manual-update', state.latest.version);
      window.location.reload();
      return;
    }
    setChecking(true);
    try { setState(await checkForNewVersion()); } finally { setChecking(false); }
  };

  return <button type="button" className={`sidebar-update${available ? ' needs-update' : ''}`} onClick={click} disabled={checking} role="status" aria-live="polite" title={available ? `Nueva versión ${state.latest.version} disponible. Actualiza para aplicar los cambios.` : checking ? 'Buscando actualizaciones…' : state.status === 'current' ? 'Ya estás usando la última versión de DoorManager Pro' : 'Buscar actualizaciones'}><RefreshCw {...iconProps} /><span>{checking ? 'Buscando…' : available ? `Actualizar ahora (${state.latest.version})` : 'Buscar actualizaciones'}</span></button>;
}
...
function navForWorkspace(workspace: Workspace) {
  const superadmin = [{ id: 'inicio', label: 'Inicio', path: '/app/superadmin', icon: ShieldAlert }, { id: 'usuarios', label: 'Usuarios', path: '/app/superadmin/usuarios', icon: UsersRound }, { id: 'crear-usuario', label: 'Crear usuario', path: '/app/superadmin/usuarios/nuevo', icon: UserRound }, { id: 'roles', label: 'Roles y permisos', path: '/app/superadmin/roles', icon: Settings }, { id: 'clientes', label: 'Clientes', path: '/app/superadmin/clientes', icon: Building2 }, { id: 'centros', label: 'Centros', path: '/app/superadmin/centros', icon: Factory }, { id: 'equipos', label: 'Equipos', path: '/app/superadmin/equipos', icon: Warehouse }, { id: 'tipos-equipo', label: 'Tipos de equipo', path: '/app/modulos/tipos-equipo', icon: Wrench }, { id: 'partes', label: 'Partes', path: '/app/superadmin/partes', icon: ClipboardList }, { id: 'presupuestos', label: 'Presupuestos', path: '/app/modulos/presupuestos', icon: FileText }, { id: 'materiales', label: 'Materiales', path: '/app/modulos/materiales', icon: PackageCheck }, { id: 'tarifas-horas', label: 'Tarifas horas', path: '/app/modulos/tarifas-horas', icon: Settings }, { id: 'checks', label: 'Checks', path: '/app/superadmin/checks', icon: ClipboardCheck }, { id: 'plantillas', label: 'Plantillas de checks', path: '/app/superadmin/plantillas', icon: PackageCheck }, { id: 'sincronizacion', label: 'Sincronización', path: '/app/superadmin/sincronizacion', icon: Truck }, { id: 'auditoria', label: 'Auditoría', path: '/app/superadmin/auditoria', icon: FileText }, { id: 'configuracion', label: 'Configuración', path: '/app/superadmin/configuracion', icon: Settings }];
  const sat = [{ id: 'inicio', label: 'Inicio', path: '/app/inicio', icon: Home }, { id: 'planificacion', label: 'Planificación', path: '/app/modulos/planificacion', icon: CalendarClock }, { id: 'tecnicos', label: 'Técnicos', path: '/app/modulos/tecnicos', icon: UsersRound }, { id: 'clientes', label: 'Clientes', path: '/app/clientes', icon: Building2 }, { id: 'centros', label: 'Centros', path: '/app/centros', icon: Factory }, { id: 'equipos', label: 'Equipos', path: '/app/equipos', icon: Warehouse }, { id: 'tipos-equipo', label: 'Tipos de equipo', path: '/app/modulos/tipos-equipo', icon: Wrench }, { id: 'expedientes', label: 'Expedientes', path: '/app/expedientes', icon: FileText }, { id: 'partes', label: 'Partes', path: '/app/partes', icon: ClipboardList }, { id: 'presupuestos', label: 'Presupuestos', path: '/app/modulos/presupuestos', icon: FileText }, { id: 'materiales', label: 'Materiales', path: '/app/modulos/materiales', icon: PackageCheck }, { id: 'checks', label: 'Checks', path: '/app/checks', icon: ClipboardCheck }, { id: 'deficiencias', label: 'Deficiencias', path: '/app/deficiencias', icon: ShieldAlert }, { id: 'documentos', label: 'Documentación', path: '/app/documentos', icon: FileText }, { id: 'avisos', label: 'Avisos', path: '/app/avisos', icon: Bell }, { id: 'plantillas', label: 'Plantillas', path: '/app/plantillas', icon: PackageCheck }];
...
function ManagementDashboard() {
  ...
  const [mode, setMode] = useState<'work' | 'alert' | null>(null);
  if (loading || error || !data) return <StateBlock loading={loading} error={error} retry={reload} empty={!data} />;
  const done = data.workOrders.filter((w: any) => ['Finalizado tecnicamente', 'Enviado', 'Cerrado'].includes(w.status));
  const pending = data.workOrders.filter((w: any) => !['Enviado', 'Cerrado', 'Cancelado'].includes(w.status));
  const urgent = data.workOrders.filter((w: any) => ['Alta', 'Critica'].includes(w.priority));
  const openDef = data.deficiencies.filter((d: any) => !['Corregida', 'Cerrada', 'Rechazada'].includes(d.status));
  const saleAmount = Number(data.metrics.sale_amount ?? data.metrics.accepted_quote_amount ?? 0);
  const realCost = Number(data.metrics.real_cost ?? 0);
  const marginAmount = Number(data.metrics.margin_amount ?? 0);
  const managementKpis = [kpiCard('Venta sin IVA', `${saleAmount.toLocaleString('es-ES')} €`, 'Partes facturables', 'Venta presupuestada u operativa sin IVA', '/app/modulos/presupuestos?estado=aceptado'), kpiCard('Coste real', `${realCost.toLocaleString('es-ES')} €`, 'Partes visibles', 'Materiales, horas y costes auxiliares', '/app/modulos/rentabilidad'), kpiCard('Margen', `${marginAmount.toLocaleString('es-ES')} €`, 'Sin IVA', 'Venta canónica menos coste real', '/app/modulos/rentabilidad'), kpiCard('Clientes activos', data.metrics.clients ?? data.clients.length, 'Actual', 'Clientes visibles', '/app/clientes'), kpiCard('Equipos', data.metrics.equipment ?? 0, 'Actual', 'Inventario instalado', '/app/equipos'), kpiCard('Partes realizados', done.length, 'Actual', 'Finalizados técnicamente, enviados o cerrados', '/app/partes?estado=realizado'), kpiCard('Pendiente facturar', data.metrics.pending_invoice_work_orders ?? 0, 'Sin cobros reales', 'Trabajos terminados con venta y sin importe facturado', '/app/modulos/rentabilidad'), kpiCard('Garantías', `${Number(data.metrics.warranty_cost ?? 0).toLocaleString('es-ES')} €`, 'Coste real', 'Coste de partes en garantía', '/app/modulos/rentabilidad')];
...
function SuperadminHome() {
  const { data, loading, error, reload } = useLoad(() => superadminService.overview(), [], null as any);
  if (loading || error || !data) return <StateBlock loading={loading} error={error} retry={reload} empty={!data} />;
  ...
  const kpis = [
    kpiCard('Usuarios totales', data.profiles.length, 'Global', 'Perfiles visibles', '/app/superadmin/usuarios', 'info'),
...
function LegacySuperadminUsers() {
  ...
  return <section className="page"><Breadcrumb items={['Propietario DMP', 'Usuarios']} /><div className="page-head"><div><h2>Usuarios</h2><p>Perfiles de la empresa operadora. No se muestran ni se guardan contraseñas.</p></div><Link className="primary" to="/app/superadmin/users/new">Crear usuario</Link></div>{message && <p className="success-note">{message}</p>}<div className="filters local-filters"><label><Search size={16} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar por nombre, email, identificador o rol..." /></label><FormSelect label="Área" value={area} onChange={setArea} options={[{ value: 'todas', label: 'Todas' }, ...areaOptions().map((item) => ({ value: normalizeArea(item), label: areaLabel(item) }))]} /><FormSelect label="Estado" value={active} onChange={setActive} options={[{ value: 'todos', label: 'Todos' }, { value: 'activos', label: 'Activos' }, { value: 'inactivos', label: 'Inactivos' }]} /></div><StateBlock loading={loading} error={error} retry={reload} empty={!rows.length}><div className="table-card"><table><thead><tr><th>Nombre</th><th>Apellidos</th><th>Email</th><th>Área/rol principal</th><th>Activo</th><th>Identificador de acceso</th><th>Fechas</th><th>Acciones</th></tr></thead><tbody>{rows.map((user: any) => <tr key={user.id} className={isArchivedRecord('profiles', user) ? 'archived-row' : ''}><td>{user.first_name}<span>{rolesText(user)}</span></td><td>{user.last_name}</td><td>{user.email}</td><td>{areaLabel(user.primary_area)}</td><td>{user.active ? 'Activo' : 'Inactivo'}</td><td><small>{user.auth_user_id ?? 'Sin enlazar'}</small></td><td>{formatDate(user.created_at)}<span>Actualizado: {formatDate(user.updated_at)}</span>{user.deleted_at && <span>Eliminado: {formatDate(user.deleted_at)}</span>}</td><td><div className="row-actions"><button onClick={() => setEditing(user)}>Ver / editar</button><LifecycleActionPanel entity="profiles" record={user} onChanged={() => { setMessage('Operacion de ciclo de vida registrada.'); reload(); }} /></div></td></tr>)}</tbody></table></div></StateBlock>{editing && <SuperadminProfileForm initial={editing} onClose={() => setEditing(null)} onSaved={() => { setEditing(null); reload(); }} />}</section>;
}

function SuperadminUserCreate() {
  const navigate = useNavigate();
  return <section className="page"><Breadcrumb items={['Propietario DMP', 'Crear usuario']} /><Hero title="Crear usuario" subtitle="Crea el perfil y vincula su identificador de acceso." tone="info" /><SuperadminProfileForm onClose={() => navigate('/app/superadmin/usuarios')} onSaved={() => navigate('/app/superadmin/usuarios')} /></section>;
}

function SuperadminProfileForm({ initial, onClose, onSaved }: any) {
  const [values, setValues] = useState<Record<string, any>>({ first_name: '', last_name: '', email: '', phone: '', primary_area: 'SAT', active: true, auth_user_id: '', roles: initial ? rolesList(initial) : ['SAT'], ...initial });
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      const payload = { first_name: values.first_name, last_name: values.last_name, email: values.email, phone: values.phone || null, primary_area: roles.includes('SAT') ? 'SAT' : values.primary_area, active: values.active === true || values.active === 'true', auth_user_id: values.auth_user_id || null };
  ...
  return <ModalForm title={initial?.id ? 'Editar usuario' : 'Crear usuario'} onClose={onClose} onSubmit={submit} saving={saving} error={error} submitLabel={initial?.id ? 'Guardar usuario' : 'Crear perfil'}><p className="large-note">No se guarda ni se muestra ninguna contraseña. Indica el identificador de acceso cuando el usuario ya esté dado de alta.</p><p className="large-note">SAT y Comercial son incompatibles en el mismo perfil. Al seleccionar uno se desmarca el otro.</p><div className="form-grid"><label>Nombre *<input value={values.first_name ?? ''} onChange={(event) => set('first_name', event.target.value)} required /></label><label>Apellidos *<input value={values.last_name ?? ''} onChange={(event) => set('last_name', event.target.value)} required /></label><label>Email *<input type="email" value={values.email ?? ''} onChange={(event) => set('email', event.target.value)} required /></label><label>Teléfono<input value={values.phone ?? ''} onChange={(event) => set('phone', event.target.value)} /></label><FormSelect label="Área/rol principal" value={values.primary_area} onChange={(value) => { set('primary_area', value); if (!values.roles.includes(value)) set('roles', [...values.roles, value]); }} required options={areaOptions().map((value) => ({ value, label: areaLabel(value) }))} /><FormSelect label="Estado" value={String(values.active)} onChange={(value) => set('active', value)} options={[{ value: 'true', label: 'Activo' }, { value: 'false', label: 'Inactivo' }]} /><label>Identificador de acceso<input value={values.auth_user_id ?? ''} onChange={(event) => set('auth_user_id', event.target.value)} placeholder="Identificador del usuario" /></label></div><Card title="Roles adicionales"><div className="component-select">{areaOptions().map((role) => <label key={role}><input type="checkbox" checked={values.roles.includes(role)} onChange={() => toggleRole(role)} /> {areaLabel(role)}</label>)}</div></Card><Card title="Contraseñas"><p className="large-note">El acceso Auth debe crearse o invitarse desde backend seguro. No se usa ninguna clave de servicio en el navegador.</p></Card></ModalForm>;
}

function SuperadminSettings() {
  const roles = useLoad(() => superadminService.roles(), [], [] as any[]);
  return <section className="page"><Breadcrumb items={['Propietario DMP', 'Configuración']} /><Hero title="Configuración superadmin" subtitle="Ajustes operativos de usuarios, roles y datos de empresa." tone="info" /><div className="grid half"><Card title="Roles disponibles"><InfoGrid items={[[ 'Roles', roles.data.map((role: any) => role.name).join(', ') || '-' ]]} /></Card><Card title="Acciones de gobierno"><div className="actions"><Link to="/app/superadmin/usuarios">Gestionar usuarios</Link><Link to="/app/superadmin/roles">Revisar permisos</Link><Link to="/app/modulos/tipos-equipo">Tipos de equipo</Link><Link to="/app/superadmin/plantillas">Plantillas de checks</Link><Link to="/app/modulos/administracion">Datos de empresa</Link><Link to="/app/superadmin/auditoria">Auditoría</Link></div>{roles.error && <p className="form-error">{roles.error}</p>}</Card></div></section>;
}

function SuperadminRoles() {
  const permissions = ['ver clientes','crear clientes','editar clientes','ver centros','crear centros','editar centros','ver equipos','crear equipos','editar equipos','ver partes','crear partes','editar partes','asignar técnicos','ver checks','crear checks','ejecutar checks','sincronizar trabajo técnico','ver facturación','ver documentación','gestionar usuarios','gestionar plantillas','gestionar tipos de equipo','ver auditoría'];
  const roles = ['superadmin','Gerencia','SAT','Comercial','Oficina','Tecnico'];
  return <section className="page"><Breadcrumb items={['Propietario DMP', 'Roles y permisos']} /><Hero title="Roles y permisos" subtitle="Resumen de accesos por rol." tone="info" /><Card title="Matriz de permisos"><div className="table-card slim"><table><thead><tr><th>Rol</th>{permissions.map((permission) => <th key={permission}>{permission}</th>)}</tr></thead><tbody>{roles.map((role) => <tr key={role}><td>{role === 'superadmin' ? 'Propietario DMP' : role}</td>{permissions.map((permission) => <td key={`${role}-${permission}`}>{permissionForRole(role, permission) ? 'Sí' : 'No'}</td>)}</tr>)}</tbody></table></div><p className="large-note">Los permisos se aplican según el rol activo.</p></Card></section>;
}
...
function EquipmentTypeForm({ initial, onClose, onSaved }: any) {
  const [values, setValues] = useState<Record<string, any>>({ name: '', description: '', active: true, ...initial });
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState('');
  const set = (key: string, value: any) => setValues((current) => ({ ...current, [key]: value }));
  const submit = async (event: FormEvent) => { event.preventDefault(); const name = String(values.name ?? '').trim(); if (!name) { setError('Indica el nombre del tipo de equipo.'); return; } setSaving(true); setError(''); try { const saved = values.id ? await equipmentService.updateType(values.id, { ...values, name }) : await equipmentService.createType({ ...values, name }); onSaved(saved); } catch (err) { setError(err instanceof Error ? err.message : 'No se ha podido guardar el tipo de equipo.'); } finally { setSaving(false); } };
  return <ModalForm title={values.id ? 'Editar tipo de equipo' : 'Crear tipo de equipo'} onClose={onClose} onSubmit={submit} saving={saving} error={error} submitLabel={values.id ? 'Guardar tipo' : 'Crear tipo'}><p className="large-note">Los tipos personalizados se guardan para la empresa actual. No se borran físicamente para conservar equipos, plantillas e históricos.</p><label>Nombre *<input value={values.name ?? ''} onChange={(event) => set('name', event.target.value)} required /></label><label>Descripción<textarea value={values.description ?? ''} onChange={(event) => set('description', event.target.value)} /></label><label className="checkbox-line"><input type="checkbox" checked={values.active !== false} onChange={(event) => set('active', event.target.checked)} /> Activo</label></ModalForm>;
}
...
function SuperadminSync() {
  return <PendingSyncPage />;
}

function SuperadminAudit() {
  const { data, loading, error, reload } = useLoad(() => superadminService.audit(), [], [] as any[]);
  return <section className="page"><Breadcrumb items={['Propietario DMP', 'Auditoría']} /><Hero title="Auditoría" subtitle="Eventos relevantes del sistema." tone="info" /><StateBlock loading={loading} error={error} retry={reload} empty={!data.length}><div className="table-card"><table><thead><tr><th>Evento</th><th>Área</th><th>Usuario</th><th>Fecha</th><th>Detalle</th></tr></thead><tbody>{data.map((row: any) => <tr key={row.id}><td>{row.operation}</td><td>{displayStatus(row.table_name)}</td><td>{fullName(row.profiles) || row.profiles?.email || '-'}</td><td>{formatDate(row.changed_at)}</td><td>{row.record_id ? 'Registro auditado' : 'Sin registro'}</td></tr>)}</tbody></table></div></StateBlock></section>;
}
...
function ClientsPage() {
  ...
  const { data, loading, error, reload } = useLoad(() => clientsService.list(search, scope, archiveFilter), [search, scope, archiveFilter], [] as any[]);
  const [creating, setCreating] = useState(false);
  const baseRoute = workspace === 'superadmin' ? '/app/superadmin/clientes' : '/app/clientes';
  return <><ListPage title="Clientes" search={search} setSearch={setSearch} archiveFilter={archiveFilter} setArchiveFilter={setArchiveFilter} action={<button className="primary" onClick={() => setCreating(true)}>Crear cliente</button>} loading={loading} error={error} retry={reload} empty={!data.length}><div className="record-list">{data.map((client) => <RecordCard key={client.id} archived={isArchivedRecord('clients', client)} title={<strong>{client.legal_name}</strong>} status={[client.deleted_at ? 'Archivado' : client.status, client.deleted_at ? 'muted' : client.status === 'Activo' ? 'ok' : 'warn']} to={`${baseRoute}/${client.id}`} actions={<LifecycleActionPanel entity="clients" record={client} onChanged={reload} />} meta={<RecordMeta items={[[ 'Código', client.code ?? '-' ], [ 'NIF', client.tax_id ?? '-' ], [ 'Teléfono', client.phone ?? '-' ], [ 'Equipos', String(client.equipment?.length ?? 0) ]]} />} />)}</div>{creating && <ClientForm title="Crear cliente" initial={{}} onClose={() => setCreating(false)} onSaved={() => { setCreating(false); reload(); }} />}</ListPage></>;
}
...
function ClientDetailPage({ forcedId }: { forcedId?: string } = {}) {
  ...
  const canonicalEconomics = useLoad(() => economicService.clientSummary(id), [id], null as any);
  const [mode, setMode] = useState<'edit' | 'contact' | 'site' | 'equipment' | 'work' | 'quote' | null>(null);
  if (loading || error || !data) return <StateBlock loading={loading} error={error} retry={reload} empty={!data} />;
  const quoteStats = quotesService.economics(data.quotes ?? []);
  const clientEconomics = {
    estimatedSale: canonicalEconomics.data?.estimated_sale_amount,
    realCost: canonicalEconomics.data?.real_cost_amount,
    margin: canonicalEconomics.data?.margin_amount,
    billableCount: canonicalEconomics.data?.billable_work_orders,
    warrantyCount: canonicalEconomics.data?.warranty_work_orders,
    pendingInvoiceCount: canonicalEconomics.data?.pending_invoice_work_orders,
  };
  return <section className="page"><BackButton /><Hero title={data.legal_name} subtitle={`${data.code} · ${data.status}`} tone={severityForStatus(data.status)} /><div className="actions"><button onClick={() => setMode('edit')}>Modificar cliente</button><button onClick={() => setMode('contact')}>Añadir contacto</button><button onClick={() => setMode('site')}>Crear centro</button><button onClick={() => setMode('equipment')}>Crear equipo</button><button onClick={() => setMode('work')}>Crear parte</button><button className="primary" onClick={() => setMode('quote')}>Crear presupuesto</button></div><div className="grid two-one"><Card title="Datos del cliente"><InfoGrid items={[[ 'Código', data.code ], [ 'Razón social', data.legal_name ], [ 'Nombre comercial', data.trade_name ?? '-' ], [ 'NIF', data.tax_id ?? '-' ], [ 'Dirección', data.address ?? '-' ], [ 'Localidad', data.city ?? '-' ], [ 'Provincia', data.province ?? '-' ], [ 'CP', data.postal_code ?? '-' ], [ 'País', data.country ?? '-' ], [ 'Teléfono', data.phone ?? '-' ], [ 'Correo', data.email ?? '-' ], [ 'Observaciones', data.notes ?? '-' ]]} /></Card><Card title="Contactos"><CompactRows rows={(data.client_contacts ?? []).map((item: any) => [fullName(item), `${item.role ?? '-'} · ${item.email ?? '-'} · ${item.phone ?? '-'}`, item.is_primary ? 'ok' : 'info', `/app/clientes/${data.id}`])} empty="Sin contactos." /></Card></div><Card title="Resumen economico"><InfoGrid items={[[ 'Centros', String(data.sites?.length ?? 0) ], [ 'Equipos', String(data.equipment?.length ?? 0) ], [ 'Partes', String(data.work_orders?.length ?? 0) ], [ 'Presupuestos', String(quoteStats.count) ], [ 'Venta prevista partes', money(clientEconomics.estimatedSale) ], [ 'Coste real partes', money(clientEconomics.realCost) ], [ 'Margen partes', money(clientEconomics.margin) ], [ 'Partes facturables', String(clientEconomics.billableCount) ], [ 'Garantias', String(clientEconomics.warrantyCount) ], [ 'Pendiente facturar', String(clientEconomics.pendingInvoiceCount) ], [ 'Total presupuestado', money(quoteStats.total) ], [ 'Aceptado', money(quoteStats.accepted) ], [ 'Ejecutado cliente', money(quoteStats.executed) ], [ 'Coste estimado presupuestos', money(quoteStats.cost) ], [ 'Venta estimada presupuestos', money(quoteStats.sale) ], [ 'Margen estimado presupuestos', money(quoteStats.margin) ]]} /></Card><Card title="Presupuestos del cliente"><p className="large-note">Presupuestos vinculados y margen se mantienen en el modulo de presupuestos.</p></Card><Related title="Relaciones" groups={[[ 'Centros', data.sites, '/app/centros' ], [ 'Equipos', data.equipment, '/app/equipos' ], [ 'Partes', data.work_orders, '/app/partes' ], [ 'Presupuestos', data.quotes, '/app/modulos/presupuestos' ]]} />{mode === 'edit' && <ClientForm title="Modificar cliente" initial={data} onClose={() => setMode(null)} onSaved={() => { setMode(null); reload(); }} />}{mode === 'contact' && <ContactForm clientId={data.id} onClose={() => setMode(null)} onSaved={() => { setMode(null); reload(); }} />}{mode === 'site' && <SiteForm title="Crear centro" initial={{ client_id: data.id }} onClose={() => setMode(null)} onSaved={() => { setMode(null); reload(); }} />}{mode === 'equipment' && <EquipmentForm initial={{ client_id: data.id }} onClose={() => setMode(null)} onSaved={() => { setMode(null); reload(); }} />}{mode === 'work' && <WorkOrderForm initial={{ client_id: data.id }} onClose={() => setMode(null)} onSaved={() => { setMode(null); reload(); }} />}{mode === 'quote' && <QuoteForm initial={{ client_id: data.id }} onClose={() => setMode(null)} onSaved={() => { setMode(null); reload(); }} />}</section>;
}

function SitesPage() { const { workspace } = useAuth(); const scope = undefined; const baseRoute = workspace === 'superadmin' ? '/app/superadmin/centros' : '/app/centros'; const [search, setSearch] = useState(''); const [archiveFilter, setArchiveFilter] = useState<ArchiveFilter>('active'); const { data, loading, error, reload } = useLoad(() => sitesService.list(search, scope, archiveFilter), [search, scope, archiveFilter], [] as any[]); const [creating, setCreating] = useState(false); return <><ListPage title="Centros" search={search} setSearch={setSearch} archiveFilter={archiveFilter} setArchiveFilter={setArchiveFilter} action={<button className="primary" onClick={() => setCreating(true)}>Crear centro</button>} loading={loading} error={error} retry={reload} empty={!data.length}><div className="record-list">{data.map((site) => <RecordCard key={site.id} archived={isArchivedRecord('sites', site)} title={<strong>{site.name}</strong>} status={[site.deleted_at ? 'Archivado' : site.active === false ? 'Inactivo' : 'Activo', site.deleted_at ? 'muted' : site.active === false ? 'warn' : 'ok']} to={`${baseRoute}/${site.id}`} actions={<LifecycleActionPanel entity="sites" record={site} onChanged={reload} />} meta={<RecordMeta items={[[ 'Código', site.code ?? '-' ], [ 'Cliente', site.clients?.legal_name ?? '-' ], [ 'Dirección', site.address ?? '-' ], [ 'Horario', site.schedule ?? '-' ]]} />} />)}</div>{creating && <SiteForm title="Crear centro" initial={{}} onClose={() => setCreating(false)} onSaved={() => { setCreating(false); reload(); }} />}</ListPage></>; }
...
function EquipmentPage() { const { workspace } = useAuth(); const scope = undefined; const baseRoute = workspace === 'superadmin' ? '/app/superadmin/equipos' : '/app/equipos'; const [search, setSearch] = useState(''); const [archiveFilter, setArchiveFilter] = useState<ArchiveFilter>('active'); const { data, loading, error, reload } = useLoad(() => equipmentService.list(search, scope, archiveFilter), [search, scope, archiveFilter], [] as any[]); const [creating, setCreating] = useState(false); const [purgeTarget, setPurgeTarget] = useState<any>(null); return <><ListPage title="Equipos" search={search} setSearch={setSearch} archiveFilter={archiveFilter} setArchiveFilter={setArchiveFilter} action={<button className="primary" onClick={() => setCreating(true)}>Crear equipo</button>} loading={loading} error={error} retry={reload} empty={!data.length}><div className="record-list">{data.map((item) => <RecordCard key={item.id} archived={isArchivedRecord('equipment', item)} title={<EquipmentOperationalTitle equipment={item} />} status={[item.deleted_at ? 'Archivado' : displayStatus(item.status), item.deleted_at ? 'muted' : severityForStatus(item.status)]} to={`${baseRoute}/${item.id}`} actions={<><LifecycleActionPanel entity="equipment" record={item} onChanged={reload} />{entityPurgeCanShowButton('equipment', item, workspace) && <button className="danger-action" onClick={() => setPurgeTarget(item)}>Eliminar definitivamente</button>}</>} meta={<EquipmentOperationalMeta equipment={item} />} />)}</div>{creating && <EquipmentForm initial={{}} onClose={() => setCreating(false)} onSaved={() => { setCreating(false); reload(); }} />}{purgeTarget && <GenericEntityPurgeModal config={equipmentPurgeConfig} record={purgeTarget} onClose={() => setPurgeTarget(null)} onDeleted={() => { setPurgeTarget(null); reload(); }} />}</ListPage></>; }

function EquipmentDetailPage() { const { id = '' } = useParams(); const navigate = useNavigate(); const { profile, workspace } = useAuth(); const { data, loading, error, reload } = useLoad(() => equipmentService.get(id), [id], null as any); const history = useLoad(() => equipmentService.history(id), [id], [] as any[]); const [mode, setMode] = useState<'edit' | 'component' | 'work' | 'check' | null>(null); const [purgeOpen, setPurgeOpen] = useState(false); if (loading || error || !data) return <StateBlock loading={loading} error={error} retry={reload} empty={!data} />; const label = equipmentOperationalLabel(data); return <section className="page"><BackButton /><Hero title={label.primary} subtitle={`${label.secondary} · ${label.context}`} tone={severityForStatus(data.status)} /><div className="actions"><button onClick={() => setMode('edit')}>Modificar equipo</button><button onClick={() => setMode('component')}>Añadir componente</button><button onClick={() => setMode('work')}>Crear parte desde equipo</button>{canCreateCheck(profile) && <button onClick={() => setMode('check')}>Crear check desde equipo</button>}{entityPurgeCanShowButton('equipment', data, workspace) && <button className="danger-action" onClick={() => setPurgeOpen(true)}>Eliminar definitivamente</button>}</div><Card title="Identificación"><InfoGrid items={[[ 'Ubicación', label.primary ], [ 'Tipo · código', label.secondary ], [ 'Cliente · centro', label.context ], [ 'Fabricante · modelo', label.detail ], [ 'Código', data.code ], [ 'Serie', data.serial_number ?? '-' ], [ 'Estado', displayStatus(data.status) ], [ 'Criticidad', displayStatus(data.criticality) ]]} /></Card><div className="grid half"><Card title="Componentes"><CompactRows rows={(data.equipment_components ?? []).map((c: any) => [c.component_type, `${c.brand ?? '-'} ${c.model ?? ''} · ${c.status}`, severityForStatus(c.status), `/app/equipos/${id}`])} empty="Sin componentes." /></Card><Card title="Historial"><CompactRows rows={history.data.map((h: any) => [displayStatus(h.event_type), `${formatDate(h.event_at)} · ${h.summary ?? ''} · ${h.detail ?? ''}`, severityForStatus(h.detail), h.event_type === 'parte' ? '/app/partes' : `/app/equipos/${id}`])} empty="Sin historial." /></Card></div><Related title="Relaciones" groups={[['Documentos', data.document_links?.map((l: any) => l.documents), '/app/documentos'], ['Expedientes', data.cases, '/app/expedientes'], ['Partes', data.work_orders, '/app/partes'], ['Checks', data.checks, '/app/checks']]} />{mode === 'edit' && <EquipmentForm initial={data} onClose={() => setMode(null)} onSaved={() => { setMode(null); reload(); }} />}{mode === 'component' && <ComponentForm equipmentId={data.id} onClose={() => setMode(null)} onSaved={() => { setMode(null); reload(); }} />}{mode === 'work' && <WorkOrderForm initial={{ client_id: data.client_id, site_id: data.site_id, main_equipment_id: data.id }} onClose={() => setMode(null)} onSaved={(newId) => { setMode(null); if (newId) navigate(`/app/partes/${newId}`); }} />}{mode === 'check' && <CheckForm initial={{ equipment_id: data.id }} onClose={() => setMode(null)} onSaved={(newId) => { setMode(null); if (newId) navigate(`/app/checks/${newId}`); }} />}{purgeOpen && <GenericEntityPurgeModal config={equipmentPurgeConfig} record={data} onClose={() => setPurgeOpen(false)} onDeleted={() => navigate(workspace === 'superadmin' ? '/app/superadmin/equipos' : '/app/equipos')} />}</section>; }

function CasesPage() { const { workspace } = useAuth(); const scope = undefined; const baseRoute = workspace === 'superadmin' ? '/app/superadmin/expedientes' : '/app/expedientes'; const [search, setSearch] = useState(''); const [archiveFilter, setArchiveFilter] = useState<ArchiveFilter>('active'); const { data, loading, error, reload } = useLoad(() => casesService.list(search, scope, archiveFilter), [search, scope, archiveFilter], [] as any[]); const [creating, setCreating] = useState(false); return <><ListPage title="Expedientes" search={search} setSearch={setSearch} archiveFilter={archiveFilter} setArchiveFilter={setArchiveFilter} action={<button className="primary" onClick={() => setCreating(true)}>Crear expediente</button>} loading={loading} error={error} retry={reload} empty={!data.length}><WorkTable rows={data} columns={['code', 'title', 'clients.legal_name', 'status', 'priority']} route={baseRoute} lifecycleEntity="cases" onLifecycleChanged={reload} />{creating && <CaseForm title="Crear expediente" initial={{}} onClose={() => setCreating(false)} onSaved={() => { setCreating(false); reload(); }} />}</ListPage></>; }
...
function WorkOrdersPage() {
  ...
  const rows = data.filter((work: any) => {
   ...
   return <><ListPage title="Partes" search={search} setSearch={setSearch} archiveFilter={archiveFilter} setArchiveFilter={setArchiveFilter} action={canCreateWorkOrder(profile) ? <button className="primary" onClick={() => setCreating(true)}>Crear parte</button> : null} loading={loading} error={error} retry={reload} empty={!rows.length}><div className="filters sat-filters"><FormSelect label="Filtro" value={filter} onChange={changeFilter} options={[['todos','Todos'],['sin-asignar','Sin asignar'],['checks-pendientes','Checks pendientes'],['pendientes-validacion','Pendientes de validación'],['material','Material pendiente'],['no-terminados','No terminados'],['en-curso','En curso'],['finalizados','Finalizados'],['pendientes','Pendientes operativos'],['urgentes','Alta o crítica'],['hoy','Hoy']].map(([value, label]) => ({ value, label }))} /><FormSelect label="Técnico o comercial" value={person} onChange={(value) => { setPerson(value); const updated = new URLSearchParams(params); value ? updated.set('tecnico', value) : updated.delete('tecnico'); setParams(updated, { replace: true }); }} options={[...data.flatMap((work: any) => (work.assignments ?? []).map((item: any) => ({ value: item.profiles?.id ?? fullName(item.profiles), label: fullName(item.profiles) }))), ...people.map((name) => ({ value: name, label: name }))].filter((item, index, all) => item.value && all.findIndex((other) => other.value === item.value) === index)} /><button className="link-button" onClick={clearFilters}>Limpiar filtros</button></div><div className="sat-work-list">{rows.map((work: any) => <SatWorkOrderCard key={work.id} work={work} onChanged={reload} onPurge={setPurgeTarget} />)}</div>{workspace === 'sat' && rows.length === 0 && <p className="large-note">No hay partes para este filtro. Cambia el filtro o la búsqueda.</p>}{creating && <WorkOrderForm initial={{}} onClose={() => setCreating(false)} onSaved={() => { setCreating(false); reload(); }} />}{purgeTarget && <WorkOrderPurgeModal workOrder={purgeTarget} onClose={() => setPurgeTarget(null)} onDeleted={() => { setPurgeTarget(null); reload(); }} />}</ListPage></>;
}

function SatWorkOrderCard({ work, onChanged, onPurge }: { work: any; onChanged?: () => void; onPurge?: (work: any) => void }) {
  const { workspace } = useAuth();
  ...
  const archived = isArchivedRecord('work_orders', work);
  return <article className={`sat-work-card ${archived ? 'archived-record' : ''}`}><header><div><strong>{work.code} · {work.title}</strong><span>{work.client_name} · {work.site_name} · {work.equipment_code ?? 'Sin equipo'}</span></div><Badge tone={archived ? 'muted' : severityForStatus(work.status)}>{archived ? 'Archivado' : displayStatus(work.status)}</Badge></header><div className="sat-meta-row"><span><b>Técnico</b> {principal ? fullName(principal) : work.main_technician_name ?? 'Sin asignar'}</span>{support.length > 0 && <span><b>Apoyo</b> {support.join(', ')}</span>}<span><b>Comercial</b> {work.commercial_name ?? work.creator_name ?? 'No informado'}</span><span><b>Carga</b> {load}</span><span><b>Agenda</b> {work.scheduled_date ?? 'Sin fecha'} · {work.scheduled_time ?? 'Sin hora'} · Prioridad {displayStatus(work.priority ?? 'Normal')}</span></div><footer><div className="row-actions"><Link className="primary" to={`${workspace === 'superadmin' ? '/app/superadmin/partes' : '/app/partes'}/${work.id}`}>Abrir</Link>{onChanged && <LifecycleActionPanel entity="work_orders" record={work} onChanged={onChanged} />}{onPurge && workOrderPurgeCanShowButton(work, workspace) && <button className="danger-action" onClick={() => onPurge(work)}>Eliminar definitivamente</button>}</div></footer></article>;
}
...
function WorkOrderDetailPageV2({ forcedId }: { forcedId?: string } = {}) {
  ...
  const totalMinutes = (data.time_entries ?? []).reduce((sum: number, row: any) => sum + Number(row.duration_minutes ?? 0), 0);
  const reloadKeepingContext = (text: string) => { const scrollY = window.scrollY; setMessage(text); reload(); window.requestAnimationFrame(() => window.scrollTo({ top: scrollY, behavior: 'auto' })); };
  const changed = () => reloadKeepingContext('Asignaciones actualizadas.');
  const operationalChanged = (text: string) => reloadKeepingContext(text);
...
function formatMinutes(minutes: number) { const safe = Number.isFinite(minutes) ? Math.max(0, minutes) : 0; const hours = Math.floor(safe / 60); const mins = safe % 60; return `${hours} h ${mins} min`; }
function pendingWorkOrderCheckCount(workOrder: any) { const checks = (workOrder?.checks ?? []).filter((check: any) => check.status !== 'Realizado' && !check.deleted_at); const generatedEquipmentIds = new Set((workOrder?.checks ?? []).filter((check: any) => !check.deleted_at).map((check: any) => check.equipment_id)); const pendingTemplateRows = (workOrder?.associated_equipment ?? []).filter((row: any) => row.check_status === 'pending_template' && !generatedEquipmentIds.has((row.equipment ?? row).id)); return checks.length + pendingTemplateRows.length; }
function plannedMaterialRows(workOrder: any) { return workOrder.planned_material_lines ?? []; }
function plannedMaterialDecision(workOrder: any, lineId: string) { return (workOrder.planned_material_decisions ?? workOrder.work_order_planned_material_decisions ?? []).find((item: any) => item.quote_line_id === lineId && !item.deleted_at); }
function plannedQuoteLines(workOrder: any) { return workOrder.planned_quote_lines ?? []; }
function plannedQuoteLineDecision(workOrder: any, lineId: string) { return (workOrder.planned_quote_line_decisions ?? workOrder.work_order_quote_line_decisions ?? []).find((item: any) => item.quote_line_id === lineId && !item.deleted_at); }
...
function WorkOrderOperationalCard({ workOrder, onChanged }: { workOrder: any; onChanged: () => void }) { const { profile } = useAuth(); const [editing, setEditing] = useState(false); const allowed = canCorrectWorkOrderOperationalFields(profile, workOrder); const summary = interventionSummary(workOrder); const closed = ['Cerrado','Cancelado'].includes(workOrder.status); return <Card title="Corrección operativa" action={allowed ? <button className="primary" onClick={() => setEditing(true)}>Editar/guardar trabajo</button> : null}><InfoGrid items={[[ 'Descripción del problema', workOrder.description ?? '-' ], [ 'Diagnóstico', summary.diagnosis ?? '-' ], [ 'Trabajo realizado', summary.work ?? '-' ], [ 'Solución / resultado', summary.result ?? '-' ], [ 'Observaciones', summary.observations ?? '-' ], [ 'Material previsto', workOrder.planned_material ?? '-' ], [ 'Última modificación', workOrder.updated_at ? `${formatDate(workOrder.updated_at)} · ${fullName(workOrder.updated_by_profile) || workOrder.updated_by || 'Usuario no informado'}` : '-' ]]}/>{!allowed && <p className="large-note">No tienes permiso para corregir estos campos del parte.</p>}{closed && <p className="state-warning">Este parte está cerrado. Solo perfiles autorizados pueden corregirlo o reabrirlo.</p>}{editing && <WorkOrderOperationalForm workOrder={workOrder} onClose={() => setEditing(false)} onSaved={() => { setEditing(false); onChanged(); }} />}</Card>; }

function WorkOrderOperationalForm({ workOrder, onClose, onSaved }: { workOrder: any; onClose: () => void; onSaved: () => void }) { const summary = interventionSummary(workOrder); const [values, setValues] = useState<Record<string, string>>({ description: workOrder.description ?? '', diagnosis: summary.diagnosis ?? '', work_performed: summary.work ?? '', result: summary.result ?? '', observations: summary.observations ?? '', planned_material: workOrder.planned_material ?? '' }); const [error, setError] = useState(''); const [saving, setSaving] = useState(false); const set = (key: string, value: string) => setValues((current) => ({ ...current, [key]: value })); const submit = async (event: FormEvent) => { event.preventDefault(); setSaving(true); setError(''); try { await workOrdersService.updateOperationalFields(workOrder.id, values); onSaved(); } catch (err: any) { console.error('DMP operational update failed', { message: err?.message, details: err?.details, hint: err?.hint, code: err?.code }); setError(err instanceof Error ? err.message : 'No se han podido corregir los campos operativos del parte.'); } finally { setSaving(false); } }; return <div className="mini-modal" role="dialog" aria-modal="true"><form onSubmit={submit}><h3>Revisión/corrección de oficina</h3><p className="large-note">Solo se guardan estos campos operativos; no se modifican cliente, centro, equipo ni asignaciones.</p><label>Descripción del problema<textarea value={values.description} onChange={(event) => set('description', event.target.value)} /></label><label>Diagnóstico<textarea value={values.diagnosis} onChange={(event) => set('diagnosis', event.target.value)} /></label><label>Trabajo realizado<textarea value={values.work_performed} onChange={(event) => set('work_performed', event.target.value)} /></label><label>Solución / resultado<textarea value={values.result} onChange={(event) => set('result', event.target.value)} /></label><label>Observaciones<textarea value={values.observations} onChange={(event) => set('observations', event.target.value)} /></label><label>Material previsto<input value={values.planned_material} onChange={(event) => set('planned_material', event.target.value)} /></label>{error && <p className="form-error">{error}</p>}<div className="modal-footer"><button type="button" onClick={onClose} disabled={saving}>Cancelar</button><button className="primary" disabled={saving}>{saving ? 'Guardando...' : 'Guardar cambios operativos'}</button></div></form></div>; }
...
function WorkOrderStatusSelector({ workOrder, onChanged, onError }: { workOrder: any; onChanged: () => void; onError: (message: string) => void }) {
  ...
  const prepare = (next: string) => {
    ...
    setValue(next);
    setConfirm({ next, needsReason, reason: '', warning: next === 'Cerrado' ? 'Cerrar el parte puede bloquear modificaciones posteriores.' : next === 'Cancelado' ? 'Cancelar exige justificar el motivo.' : '' });
  };
  const change = async () => {
    if (!confirm || saving) return;
    if (confirm.needsReason && !confirm.reason.trim()) { onError('El motivo es obligatorio para este cambio de estado.'); return; }
    setSaving(true); onError('');
    try { await workOrdersService.changeStatus(workOrder.id, confirm.next, confirm.reason || 'Cambio directo de estado'); setConfirm(null); onChanged(); }
...
function WorkOrderSatReviewCard({ workOrder, onChanged, onError }: { workOrder: any; onChanged: () => void; onError: (message: string) => void }) {
  ...
  const submit = async () => {
    ...
    setSaving(true); onError('');
    try { await workOrdersService.reviewWorkOrderSat(workOrder.id, decision, decision === 'approved' ? destination : null, decision === 'approved' && destination === 'comercial' ? commercialProfileId : null, flags, reason); setDecision(null); setReason(''); setCommercialProfileId(''); onChanged(); }
...
function WorkOrderCommercialReviewCard({ workOrder, onChanged, onError }: { workOrder: any; onChanged: () => void; onError: (message: string) => void }) {
  ...
  return <Card title="REVISIÓN COMERCIAL" action={<Badge tone="warn">Pendiente de revisión</Badge>}><InfoGrid items={[[ 'Responsable', fullName(workOrder.responsible) || workOrder.current_responsible_id || 'No informado' ], [ 'Estado', 'Pendiente de revisión' ], [ 'Cliente', workOrder.clients?.legal_name ?? 'No informado' ], [ 'Centro', workOrder.sites?.name ?? 'No informado' ], [ 'Equipo', workOrder.primary_equipment?.code ?? 'Sin equipo' ], [ 'Presupuesto', workOrder.quotes?.code ?? workOrder.quote_id ?? 'Sin presupuesto' ], [ 'Venta calculada', canViewWorkOrderCosts(profile) ? money(workOrder.sale_amount) : 'No disponible' ]]}/><section className="commercial-review-sat"><h4>REVISIÓN SAT</h4><p><strong>Indicadores:</strong> {satReviewFlagLabels(workOrder.sat_review_flags).join(' · ') || 'Ninguno'}</p><p><strong>Observaciones SAT:</strong> {workOrder.sat_review_reason ?? 'Sin observaciones'}</p><p><strong>Fecha:</strong> {workOrder.sat_reviewed_at ? formatDate(workOrder.sat_reviewed_at) : 'No informada'}</p></section><p className="large-note">Revisa el parte, los materiales, las horas y los desplazamientos antes de enviarlo a Facturación.</p>{canApprove ? <><label>Observaciones comerciales *<textarea value={reason} onChange={(event) => setReason(event.target.value)} placeholder="Comentario de aprobación" /></label><button className="primary" onClick={approve} disabled={saving || !reason.trim()} title="Aprobar importe y enviar a Facturación">{saving ? 'Enviando...' : 'ENVIAR A FACTURACIÓN'}</button></> : <p className="state-warning">Parte asignado a otro Comercial. Solo el responsable o un supervisor puede enviarlo a Facturación.</p>}</Card>;
}
...
function WorkOrderOfficeValidationCard({ workOrder, onChanged, onError }: { workOrder: any; onChanged: () => void; onError: (message: string) => void }) {
  ...
  const submit = async () => {
    ...
    setSaving(true); onError('');
    await submitOfficeValidationReview({ decision, reason, saving, review: (nextDecision, nextReason) => workOrdersService.reviewWorkOrderOffice(workOrder.id, nextDecision, nextReason), onSuccess: () => { setDecision(null); setReason(''); onChanged(); }, onError: (err) => onError(formErrorMessage(err, 'No se ha podido registrar la validación de oficina.')) });
    setSaving(false);
  };
...
function OfficeValidationModal({ decision, reason, saving, onReasonChange, onCancel, onSubmit }: { decision: 'validated' | 'rejected'; reason: string; saving: boolean; onReasonChange: (value: string) => void; onCancel: () => void; onSubmit: () => void }) {
  useOverlayScrollLock();
  useEffect(() => {
    const onKey = (event: KeyboardEvent) => { if (event.key === 'Escape' && canCloseOfficeValidationModal(saving)) onCancel(); };
    window.addEventListener('keydown', onKey);
    return () => window.removeEventListener('keydown', onKey);
  }, [onCancel, saving]);
  return <div className="mini-modal" role="dialog" aria-modal="true"><div><h3>{decision === 'validated' ? 'Validar parte en oficina' : 'Devolver parte'}</h3><label>{decision === 'validated' ? 'Comentario / motivo *' : 'Motivo de devolución *'}<textarea value={reason} onChange={(event) => onReasonChange(event.target.value)} required /></label><div className="modal-footer"><button onClick={onCancel} disabled={saving}>Cancelar</button><button className={decision === 'validated' ? 'primary' : 'danger'} onClick={onSubmit} disabled={saving || !reason.trim()}>{saving ? 'Guardando...' : decision === 'validated' ? 'Validar' : 'Devolver'}</button></div></div></div>;
}

function timeWorkerLabel(profile: any, currentProfileId?: string, assignmentRole?: string | null) {
  const tags = [profile?.id === currentProfileId ? 'Tú' : '', assignmentRole === 'Principal' ? 'Técnico principal' : assignmentRole ? 'Apoyo' : '', profile?.primary_area ?? ''].filter(Boolean);
  return fullName(profile) + (tags.length ? ' · ' + tags.join(' · ') : '');
}

function WorkOrderFinalizeModal({ workOrder, onClose, onDone, onError }: { workOrder: any; onClose: () => void; onDone: () => void; onError: (message: string) => void }) {
  const { profile } = useAuth();
  const [saving, setSaving] = useState(false);
  ...
  const blocked = pending.length > 0 || pendingChecks > 0;
  ...
  return <div className="mini-modal finalize-modal" role="dialog" aria-modal="true"><div>
    ...
    {pendingChecks > 0 && <p className="form-error">Resuelve {pendingChecks} check(s) pendiente(s) antes de cerrar el parte.</p>}
...
function PlannedMaterialList({ workOrder, onChanged, readOnly = false }: { workOrder: any; onChanged: () => void; readOnly?: boolean }) {
  const { profile } = useAuth();
  const [usingLine, setUsingLine] = useState<any | null>(null);
  const [error, setError] = useState('');
  const rows = plannedMaterialRows(workOrder);
  const canAct = !readOnly && canManageWorkOrderMaterials(profile, workOrder);
  const markUnused = async (line: any) => { try { setError(''); await workOrdersService.setPlannedMaterialDecision({ work_order_id: workOrder.id, quote_line_id: line.id, decision: 'no_utilizado', quantity: line.quantity, unit: line.unit ?? 'ud', notes: 'Material previsto no utilizado' }); onChanged(); } catch (err) { setError(err instanceof Error ? err.message : 'No se ha podido marcar el material previsto.'); } };
  return <div className="planned-materials"><div className="compact-list">{rows.map((line: any) => { const decision = plannedMaterialDecision(workOrder, line.id); return <article key={line.id}><Badge tone={decision?.decision === 'utilizado' ? 'ok' : decision?.decision === 'no_utilizado' ? 'muted' : 'warn'}>{decision?.decision === 'utilizado' ? 'Utilizado' : decision?.decision === 'no_utilizado' ? 'No utilizado' : 'Pendiente'}</Badge><p><strong>{line.description}</strong><br />{Number(line.quantity ?? 0).toLocaleString('es-ES')} {line.unit ?? 'ud'}{line.material_id ? ' · Catálogo vinculado' : ' · Sin material de catálogo vinculado'}</p>{canAct && !decision && <div className="row-actions"><button className="primary" onClick={() => setUsingLine(line)}>Marcar como utilizado</button><button onClick={() => markUnused(line)}>No utilizado</button></div>}</article>; })}</div>{!rows.length && <p className="large-note">Sin materiales previstos. Material indicado: {workOrder.planned_material ?? '-'}</p>}{error && <p className="form-error">{error}</p>}{canAct && usingLine && <PlannedMaterialUseForm workOrder={workOrder} line={usingLine} onClose={() => setUsingLine(null)} onSaved={() => { setUsingLine(null); onChanged(); }} />}</div>;
}

function PlannedMaterialUseForm({ workOrder, line, onClose, onSaved }: { workOrder: any; line: any; onClose: () => void; onSaved: () => void }) {
  ...
  const [notes, setNotes] = useState('Material previsto confirmado como utilizado');
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState('');
  const requiresWarehouse = Boolean(line.material_id && line.materials?.stock_controlled !== false);
  const selectedStock = warehouseStock.data.find((item: any) => item.material_id === line.material_id && item.warehouse_id === warehouseId);
  const submit = async (event: FormEvent) => { event.preventDefault(); if (Number(quantity) <= 0) { setError('validacion del formulario: la cantidad debe ser mayor que cero.'); return; } if (requiresWarehouse && !warehouseId) { setError('stock: selecciona el almacen de origen para este material previsto.'); return; } setSaving(true); setError(''); try { const materialUsageId = await workOrdersService.upsertMaterial({ work_order_id: workOrder.id, quote_line_id: line.id, material_id: line.material_id ?? '', description: line.material_id ? '' : line.description, quantity, unit: line.unit ?? 'ud', warehouse_id: warehouseId, notes }); await workOrdersService.setPlannedMaterialDecision({ work_order_id: workOrder.id, quote_line_id: line.id, decision: 'utilizado', work_order_material_id: materialUsageId, quantity, unit: line.unit ?? 'ud', notes }); onSaved(); } catch (err) { setError(err instanceof Error ? err.message : 'No se ha podido confirmar el material previsto.'); } finally { setSaving(false); } };
  return <div className="mini-modal" role="dialog" aria-modal="true"><form onSubmit={submit}><h3>Marcar material previsto como utilizado</h3><p><strong>{line.description}</strong></p><p className="large-note">Se registrará como material utilizado real. El material queda pendiente de validación de stock y conserva el origen del presupuesto.</p>{requiresWarehouse && <><FormSelect label="Almacén de origen" value={warehouseId} onChange={setWarehouseId} options={[{ value: '', label: warehouses.loading ? 'Cargando almacenes...' : 'Selecciona almacén' }, ...warehouses.data.map((item: any) => ({ value: item.id, label: `${item.code} · ${item.name}` }))]} loading={warehouses.loading} />{warehouseId && <p className="large-note">Saldo disponible en el almacén seleccionado: {selectedStock ? `${Number(selectedStock.quantity ?? 0).toLocaleString('es-ES')} ${line.unit ?? 'ud'}` : 'Sin apertura'}</p>}</>}<label>Cantidad real<input type="number" step="0.01" min="0.01" value={quantity} onChange={(event) => setQuantity(event.target.value)} required /></label><label>Observaciones<textarea value={notes} onChange={(event) => setNotes(event.target.value)} /></label>{error && <p className="form-error">{error}</p>}<div className="modal-footer"><button type="button" onClick={onClose} disabled={saving}>Cancelar</button><button className="primary" disabled={saving || warehouses.loading}>{saving ? 'Confirmando...' : 'Confirmar utilizado'}</button></div></form></div>;
}

function PlannedQuoteConceptsCard({ workOrder, onChanged }: { workOrder: any; onChanged: () => void }) {
  const { profile } = useAuth();
  const [editing, setEditing] = useState<any | null>(null);
  const lines = plannedQuoteLines(workOrder);
  const groups = ['MATERIALES PREVISTOS', 'MANO DE OBRA PREVISTA', 'DESPLAZAMIENTOS', 'TALLER MÓVIL', 'PLATAFORMA / MEDIOS AUXILIARES', 'COSTES EXTERNOS / OTROS', 'CONCEPTOS COMERCIALES'];
  const realMinutes = realHoursMinutes(workOrder);
  const canAct = canResolvePlannedConcept(profile, workOrder);
  return <Card title="CONCEPTOS PREVISTOS DEL PRESUPUESTO"><p className="large-note">Revisa los conceptos aceptados y confirma qué se ha realizado.</p>{groups.map((group) => { const groupLines = lines.filter((line: any) => quoteLineCategory(line) === group); if (!groupLines.length) return null; if (group === 'MATERIALES PREVISTOS') return <section className="planned-group" key={group}><h4>{group}</h4><PlannedMaterialList workOrder={workOrder} onChanged={onChanged} /></section>; return <section className="planned-group" key={group}><h4>{group}</h4><div className="compact-list">{groupLines.map((line: any) => { const decision = plannedQuoteLineDecision(workOrder, line.id); const realCost = costEntryForQuoteLine(workOrder, line.id); const commercial = ['fee','discount'].includes(line.line_type); const labor = line.line_type === 'labor'; return <article key={line.id}><Badge tone={decision?.decision === 'confirmado' ? 'ok' : decision?.decision === 'no_realizado' ? 'muted' : commercial || labor ? 'info' : 'warn'}>{decision?.decision === 'confirmado' ? 'Confirmado' : decision?.decision === 'no_realizado' ? 'No realizado' : labor ? 'Comparativo' : commercial ? 'Comercial' : 'Pendiente'}</Badge><p><strong>{line.description}</strong><br />Previsto: {Number(line.quantity ?? 0).toLocaleString('es-ES')} {line.unit ?? 'ud'}{labor ? ` · Horas reales registradas: ${formatMinutes(realMinutes)}` : ''}{realCost ? ` · Real: ${Number(realCost.quantity ?? 0).toLocaleString('es-ES')} ${realCost.unit ?? line.unit ?? 'ud'}` : ''}</p>{canViewWorkOrderCosts(profile) && <p>Coste previsto: {money(line.total_cost)} · Venta aceptada: {money(line.total_price ?? line.total)}{realCost ? ` · Coste real: ${money(realCost.total_cost ?? Number(realCost.quantity ?? 0) * Number(realCost.unit_cost ?? 0))}` : ''}</p>}{commercial && <p className="large-note">Concepto comercial sin coste operativo asociado.</p>}{labor && <p className="large-note">La mano de obra prevista no crea horas. Las horas reales proceden de registros técnicos.</p>}{canAct && !commercial && !labor && <div className="row-actions"><button className="primary" onClick={() => setEditing(line)}>{decision?.decision === 'confirmado' ? 'Modificar cantidad' : 'Confirmar'}</button><button onClick={async () => { await workOrdersService.setPlannedQuoteLineDecision({ work_order_id: workOrder.id, quote_line_id: line.id, decision: 'no_realizado', notes: 'Concepto previsto no realizado' }); onChanged(); }}>No realizado</button></div>}</article>; })}</div></section>; })}{!lines.length && <p className="large-note">Este parte no tiene presupuesto origen con líneas estructuradas.</p>}{editing && <PlannedQuoteCostForm workOrder={workOrder} line={editing} initial={costEntryForQuoteLine(workOrder, editing.id)} onClose={() => setEditing(null)} onSaved={() => { setEditing(null); onChanged(); }} />}</Card>;
}

function PlannedQuoteCostForm({ workOrder, line, initial, onClose, onSaved }: { workOrder: any; line: any; initial?: any; onClose: () => void; onSaved: () => void }) {
  const { profile } = useAuth();
  const showCosts = canViewWorkOrderCosts(profile);
  const [quantity, setQuantity] = useState(String(initial?.quantity ?? line.quantity ?? 1));
  const [unitCost, setUnitCost] = useState(String(initial?.unit_cost ?? line.unit_cost ?? 0));
  const [notes, setNotes] = useState('Concepto previsto confirmado');
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState('');
  const submit = async (event: FormEvent) => { event.preventDefault(); if (Number(quantity) <= 0) { setError('validacion del formulario: la cantidad debe ser mayor que cero.'); return; } if (showCosts && Number(unitCost || 0) < 0) { setError('validacion del formulario: el coste unitario no puede ser negativo.'); return; } setSaving(true); setError(''); try { if (showCosts) await workOrdersService.setPlannedQuoteLineDecision({ work_order_id: workOrder.id, quote_line_id: line.id, decision: 'confirmado', quantity, unit: line.unit ?? 'ud', unit_cost: unitCost, notes }); else await workOrdersService.setTechnicalPlannedQuoteLineDecision({ work_order_id: workOrder.id, quote_line_id: line.id, decision: 'confirmado', actual_quantity: quantity, technical_notes: notes }); onSaved(); } catch (err) { setError(err instanceof Error ? err.message : 'No se ha podido confirmar el concepto previsto.'); } finally { setSaving(false); } };
  return <div className="mini-modal" role="dialog" aria-modal="true"><form onSubmit={submit}><h3>Confirmar concepto previsto</h3><p><strong>{line.description}</strong></p><InfoGrid items={[[ 'Tipo', displayStatus(line.line_type) ], [ 'Cantidad prevista', `${Number(line.quantity ?? 0).toLocaleString('es-ES')} ${line.unit ?? 'ud'}` ], [ 'Coste previsto', showCosts ? money(line.total_cost) : '-' ], [ 'Venta aceptada', showCosts ? money(line.total_price ?? line.total) : '-' ]]} /><label>Cantidad real<input type="number" step="0.01" min="0.01" value={quantity} onChange={(event) => setQuantity(event.target.value)} required /></label>{showCosts && <label>Coste unitario real<input type="number" step="0.01" min="0" value={unitCost} onChange={(event) => setUnitCost(event.target.value)} /></label>}<label>Observaciones<textarea value={notes} onChange={(event) => setNotes(event.target.value)} /></label>{error && <p className="form-error">{error}</p>}<div className="modal-footer"><button type="button" onClick={onClose} disabled={saving}>Cancelar</button><button className="primary" disabled={saving}>{saving ? 'Confirmando...' : 'Confirmar concepto'}</button></div></form></div>;
}
...
function WorkOrderCostForm({ workOrder, initial, onClose, onSaved }: { workOrder: any; initial?: any; onClose: () => void; onSaved: () => void }) { const selectedId = initial?.concept_id ?? initial?.rate_id; const catalog = useLoad(() => hourRatesService.catalog('', undefined, 'cost', false, selectedId), [], [] as any[]); const [values, setValues] = useState<Record<string, string>>({ id: initial?.id ?? '', work_order_id: workOrder.id, concept_id: initial?.concept_id ?? initial?.rate_id ?? '', rate_id: initial?.rate_id ?? initial?.concept_id ?? '', quantity: String(initial?.quantity ?? 1), description: initial?.description ?? '' }); const [error, setError] = useState(''); const [saving, setSaving] = useState(false); const [pendingAdditional, setPendingAdditional] = useState(false); const set = (key: string, value: string) => setValues((current) => ({ ...current, [key]: value })); useEffect(() => { if (!values.concept_id && initial?.cost_type && catalog.data.length) { const legacy = catalog.data.find((item: any) => item.code === initial.cost_type); if (legacy) setValues((current) => ({ ...current, concept_id: legacy.id, rate_id: legacy.id })); } }, [catalog.data.length, values.concept_id, initial?.cost_type]); const selected = catalog.data.find((item: any) => item.id === values.concept_id); const saveCost = async (asAdditional = false) => { if (saving) return; if (!values.concept_id) { setError('validacion del formulario: selecciona un concepto.'); return; } if (Number(values.quantity) <= 0) { setError('validacion del formulario: la cantidad debe ser mayor que cero.'); return; } const planned = !values.id && (workOrder.cost_entries ?? workOrder.work_order_cost_entries ?? []).some((row: any) => (row.concept_id === values.concept_id || row.rate_id === values.rate_id) && row.quote_line_id && !row.deleted_at); if (planned && !asAdditional) { setPendingAdditional(true); return; } setSaving(true); setError(''); try { const payload: Record<string, any> = { id: values.id, work_order_id: workOrder.id, concept_id: values.concept_id, rate_id: values.rate_id, quantity: values.quantity }; if (values.description.trim()) payload.description = values.description.trim(); if (asAdditional) payload.additional_to_planned = true; await workOrdersService.upsertCostEntry(payload); onSaved(); } catch (err) { const message = err instanceof Error ? err.message : ''; if (!asAdditional && message.includes('adicional:')) setPendingAdditional(true); else setError(message || 'No se ha podido registrar el recurso o coste.'); } finally { setSaving(false); } }; const submit = (event: FormEvent) => { event.preventDefault(); saveCost(false); }; return <div className="mini-modal" role="dialog" aria-modal="true"><form onSubmit={submit}><h3>{initial ? 'Editar recurso/coste' : 'Añadir recurso/coste'}</h3><FormSelect label="Concepto de catalogo" value={values.concept_id} onChange={(value) => { set('concept_id', value); set('rate_id', value); }} loading={catalog.loading} options={catalog.data.map((item: any) => ({ value: item.id, label: `${item.name} · ${item.unit} · ${item.billing_mode}` }))} emptyLabel="Selecciona un concepto" /><label>Cantidad<input type="number" min="0.01" step="0.01" value={values.quantity} onChange={(event) => set('quantity', event.target.value)} required /></label><label>Observaciones opcionales<textarea value={values.description} onChange={(event) => set('description', event.target.value)} /></label>{selected && <p className="large-note">Tarifa: {selected.name} · unidad {selected.unit}. El importe lo resuelve el servidor desde la version vigente.</p>}{error && <p className="form-error">{error}</p>}<div className="actions"><button type="button" onClick={onClose}>Cancelar</button><button className="primary" disabled={saving}>{saving ? 'Guardando...' : 'Guardar recurso/coste'}</button></div></form>{pendingAdditional && <div className="mini-modal" role="dialog" aria-modal="true"><div><h3>Coste adicional al presupuestado</h3><p>Este concepto procedente del presupuesto ya esta contabilizado.</p><p>Confirma solo si es un coste adicional realmente realizado.</p><div className="actions"><button onClick={() => setPendingAdditional(false)}>Cancelar</button><button className="primary" onClick={() => { setPendingAdditional(false); saveCost(true); }}>Añadir como coste adicional</button></div></div></div>}</div>; }
...
function CheckSummaryCard({ check }: { check: any }) {
  ...
  const equipment = equipmentOperationalLabel(check.equipment);
  return <article className="check-summary-card"><div><strong>{check.code} · {check.check_templates?.name ?? 'Plantilla no informada'}</strong><small><b>{equipment.primary}</b> · {equipment.secondary} · {reviewed}/{blocks.length} secciones · {(check.check_photos ?? []).length} fotos</small>{templateTypeMismatch(check) && <p className="form-error">La plantilla no coincide con el tipo del equipo.</p>}</div><Badge tone={severityForStatus(check.global_result)}>{displayStatus(check.global_result ?? check.status)}</Badge><Link to={route}>Ver check completo</Link></article>;
}
...
function PendingSyncPage() {
  ...
  const [confirm, setConfirm] = useState<{ title: string; text: string; ids?: string[]; failedOnly?: boolean } | null>(null);
  const selectedItems = pending.filter((item) => selected.includes(item.id));
  const selectedCount = selectedItems.length;
  const failedIds = pending.filter((item) => item.status === 'failed').map((item) => item.id);
  const runPendingAction = async (action: string, ids: string[], task: () => Promise<void>) => { try { setSyncing(true); await task(); } catch (error: any) { console.error('DMP pending sync action failed', { action, ids, message: error?.message, details: error?.details, hint: error?.hint, code: error?.code, name: error?.name }); setMessage(error instanceof Error ? error.message : 'No se ha podido completar la acción de sincronización.'); } finally { setSyncing(false); await reload(); } };
  const sync = async () => runPendingAction('sync all', pending.map((item) => item.id), async () => { const result = await technicianOfflineService.sync(setMessage); setMessage(`Sincronizados: ${result.synced}. Fallidos: ${result.failed}. Quedan por enviar: ${result.pending}.`); setSelected([]); });
  const retryOne = async (changeId: string) => runPendingAction('retry one', [changeId], async () => { const result = await technicianOfflineService.syncOne(changeId, setMessage); setMessage(`Fila sincronizada: ${result.synced}. Fallidos: ${result.failed}.`); setSelected((current) => current.filter((id) => id !== changeId)); });
  const retrySelected = async () => { const ids = [...selected]; if (!ids.length) return; await runPendingAction('retry selected', ids, async () => { await technicianOfflineService.resetForRetry(ids); const result = await technicianOfflineService.syncSelected(ids, setMessage); setMessage(`Seleccionados sincronizados: ${result.synced}. Fallidos: ${result.failed}. Quedan seleccionados por enviar: ${result.pending}.`); setSelected([]); }); };
  const toggle = (id: string) => setSelected((current) => current.includes(id) ? current.filter((item) => item !== id) : [...current, id]);
  const confirmDelete = (ids: string[], title: string) => setConfirm({ title, ids, text: `Vas a eliminar ${ids.length} elemento(s) de la cola local de sincronización. No se enviarán. Esta acción no borra datos ya sincronizados. ¿Continuar?` });
  const deleteConfirmed = async () => { if (!confirm) return; const ids = confirm.failedOnly ? failedIds : [...(confirm.ids ?? [])]; await runPendingAction(confirm.failedOnly ? 'delete failed' : 'delete selected', ids, async () => { const count = confirm.failedOnly ? await technicianOfflineService.deleteFailedQueueItems() : await technicianOfflineService.deleteQueueItems(ids); setMessage(`Eliminados de este dispositivo: ${count}.`); setSelected([]); setConfirm(null); }); };
  return <section className="page technician-page"><BackButton /><div className="page-head"><div><h2>Pendientes de sincronizar</h2><p>Cambios guardados en este dispositivo pendientes de enviar. Eliminar de este dispositivo solo borra la copia local pendiente.</p></div><div className="actions"><button disabled={!pending.length || syncing} onClick={() => setSelected(selectedCount === pending.length ? [] : pending.map((item) => item.id))}>{selectedCount === pending.length ? 'Quitar selección' : 'Seleccionar todo'}</button><button className="primary" disabled={!pending.length || syncing} onClick={sync}>{syncing ? 'Sincronizando...' : 'Sincronizar todo'}</button></div></div><Card title="Resumen"><InfoGrid items={[[ 'Total por enviar', summary.total ], [ 'Pendientes', summary.pending ], [ 'Bloqueados', summary.blocked ], [ 'Fallidos', summary.failed ], [ 'Seleccionados', selectedCount ], [ 'Bloques', summary.blocks ], [ 'Incidencias', summary.incidences ], [ 'Materiales', summary.materials ], [ 'Fotos', summary.photos ], [ 'Firmas', summary.signatures ]]} />{message && <p className="success-note">{message}</p>}<div className="actions"><button disabled={!selectedCount || syncing} onClick={retrySelected}>Reintentar seleccionados</button><button disabled={!selectedCount || syncing} onClick={() => confirmDelete(selected, 'Borrar seleccionados')}>Borrar seleccionados</button><button className="danger" disabled={!failedIds.length || syncing} onClick={() => setConfirm({ title: 'Borrar fallidos', failedOnly: true, text: `Vas a eliminar ${failedIds.length} elemento(s) de la cola local de sincronización. No se enviarán. Esta acción no borra datos ya sincronizados. ¿Continuar?` })}>Borrar fallidos</button></div></Card><div className="compact-list">{pending.map((item) => <article key={item.id}><label className="check-consent"><input type="checkbox" checked={selected.includes(item.id)} onChange={() => toggle(item.id)} /> <Badge tone={item.status === 'failed' ? 'danger' : item.status === 'blocked' ? 'warn' : 'info'}>{item.status === 'failed' ? 'Fallido' : item.status === 'blocked' ? 'Bloqueado' : item.status === 'syncing' ? 'Sincronizando' : 'Pendiente'}</Badge></label><div><p><strong>{displayStatus(item.type)}</strong><br /><small>ID cola: {item.id}</small><br /><small>Parte: {item.workOrderId ?? '-'} · Check: {item.checkId ?? '-'} · Bloque: {item.blockId ?? '-'}</small><br /><small>Fecha: {formatDate(item.updatedAt)} · Creado: {formatDate(item.createdAt)} · Intentos: {item.attempts ?? 0}</small></p>{item.error && <p className="form-error">{technicianOfflineService.safeDetail(item.error)}</p>}<details><summary>Detalle técnico local</summary><pre>{technicianOfflineService.safeDetail(item.payload)}</pre></details></div><div className="row-actions"><button onClick={() => retryOne(item.id)} disabled={syncing}>Reintentar</button><button className="danger" onClick={() => confirmDelete([item.id], 'Borrar pendiente local')} disabled={syncing}>Borrar</button></div></article>)}</div>{!pending.length && <Card title="Sin pendientes"><p className="large-note">No hay cambios locales pendientes, fallidos o bloqueados.</p></Card>}{confirm && <ConfirmModal title={confirm.title} text={confirm.text} onCancel={() => setConfirm(null)} onConfirm={deleteConfirmed} />}</section>;
}

function TechnicianDayPage() {
  const navigate = useNavigate();
  const [tab, setTab] = useState<TechnicianDayTab>('todos');
  const [refreshMessage, setRefreshMessage] = useState('');
  const active = useLoad(() => assignmentsService.assignedActiveWork(), [], [] as any[]);
  const history = useLoad(() => assignmentsService.assignmentHistory(), [], [] as any[]);
  const { pending } = useOfflineQueue();
  const today = localDateKey();
  const source = tab === 'finalizados' ? history : active;
  const counts = technicianDayCounts(active.data, history.data, today);
  const rows = technicianDayRows(tab, active.data, history.data, today);
  const refresh = async () => { const [freshActive] = await Promise.all([assignmentsService.assignedActiveWork(), active.reload(), history.reload()]); const result = await technicianOfflineService.reconcileActiveWork(freshActive.map((row) => row.work_order_id)); setRefreshMessage(`Última actualización: ${new Date().toLocaleTimeString()}. Trabajos activos: ${freshActive.length}. Cambios pendientes: ${pending.length}. Obsoletos bloqueados: ${result.blocked}.`); };
  const emptyMessage = technicianDayEmptyMessage(tab, active.data.length, rows.length, source.error);
...
function TechnicianTravelCard({ workOrder, onChanged }: { workOrder: any; onChanged: () => void }) { const { profile } = useAuth(); const [editing, setEditing] = useState<any | null>(null); const [removing, setRemoving] = useState<any | null>(null); const rows = (workOrder.cost_entries ?? []).filter((row: any) => row.cost_type === 'desplazamiento' && !row.deleted_at); const planned = (workOrder.planned_quote_lines ?? []).filter((line: any) => ['transport', 'travel'].includes(line.line_type)); return <Card title="Desplazamientos" action={canManageWorkOrderCosts(profile, workOrder) ? <button className="primary" onClick={() => setEditing({ cost_type: 'desplazamiento', quantity: 1, unit: 'km', description: 'Desplazamiento' })}>Añadir desplazamiento</button> : null}><p className="large-note">Los desplazamientos previstos se distinguen de los realizados.</p>{planned.length > 0 && <div className="compact-list"><article><strong>Previsto / planificado</strong><span>{planned.map((line: any) => `${line.description} · ${line.quantity ?? 0} ${line.unit ?? 'ud'}`).join(' · ')}</span></article></div>}<div className="compact-list">{rows.map((row: any) => <article key={row.id}><strong>Real · {row.quantity ?? 0} {row.unit ?? 'km'}</strong><span>{row.description || 'Desplazamiento'} · {formatDate(row.incurred_at ?? row.created_at)}</span>{canManageWorkOrderCosts(profile, workOrder, row) && <div className="row-actions"><button onClick={() => setEditing(row)}>Editar</button><button onClick={() => setRemoving(row)}>Eliminar</button></div>}</article>)}</div>{!rows.length && <p className="large-note">Sin desplazamientos reales registrados.</p>}{editing && <WorkOrderCostForm workOrder={workOrder} initial={editing} onClose={() => setEditing(null)} onSaved={() => { setEditing(null); onChanged(); }} />}{removing && <ReasonConfirmModal title="Eliminar desplazamiento" text="Se eliminará este desplazamiento." requiredLabel="Motivo de eliminación" onCancel={() => setRemoving(null)} onConfirm={async (reason) => { await workOrdersService.deleteCostEntry(removing.id, reason); setRemoving(null); onChanged(); }} />}</Card>; }
...
function TechnicianLegacyWorkPage() {
  ...
  const [message, setMessage] = useState('');
  const [actionError, setActionError] = useState('');
  const [mode, setMode] = useState<'time' | 'material' | 'finalize' | null>(null);
  if (loading) return <StateBlock loading={loading} retry={reload} empty={false} />;
  if (error || !data) return <section className="page"><Card title="Sin permiso"><p className="form-error">{error ? String(error) : 'No tienes permiso para acceder a este parte'}</p><Link className="primary" to="/app/tecnico">Volver a Mi jornada</Link></Card></section>;
  ...
  const advance = async (status: string) => { if (status === 'Finalizado tecnicamente') { setMode('finalize'); return; } try { setActionError(''); await workOrdersService.changeStatus(data.id, status, 'Cambio operativo desde modo técnico'); setMessage('Estado actualizado: ' + displayStatus(status)); reload(); } catch (err) { setActionError(err instanceof Error ? err.message : 'No se ha podido cambiar el estado.'); } };
  return <section className="page technician-page"><BackButton /><Hero title={data.code + ' · ' + data.title} subtitle={(data.clients?.legal_name ?? '') + ' · ' + (data.sites?.name ?? '')} tone={severityForStatus(data.status)} /><p className="state-warning"><CheckCircle2 size={17} /> Estado actual del parte: {displayStatus(data.status)}</p>{message && <p className="success-note">{message}</p>}{actionError && <p className="form-error">{actionError}</p>}<div className="actions"><button onClick={() => advance(nextWorkOrderStatus(data.status))}>Iniciar / avanzar</button><button onClick={() => advance('Pausado')}>Pausar</button>{canFinalizeWorkOrderTechnical(profile, data) && <button className="primary" onClick={() => advance('Finalizado tecnicamente')}>FINALIZAR PARTE TÉCNICO</button>}<button onClick={() => setMode('time')} disabled={!canManageWorkOrderTime(profile, data)}>Añadir horas</button><button onClick={() => setMode('material')} disabled={!canManageWorkOrderMaterials(profile, data)}>Añadir material</button><SyncButton workOrderId={id} /></div><Card title="Qué verá SAT"><p className="large-note">Las horas, materiales, checks, observaciones, fotos y firmas que guardes quedan vinculadas al parte. Horas y materiales se guardan al confirmar. Si no hay conexión, verás el aviso y podrás reintentarlo.</p></Card><div className="grid half"><Card title="Datos para la intervención"><InfoGrid items={[[ 'Prioridad', displayStatus(data.priority) ], [ 'Cliente', data.clients?.legal_name ?? '-' ], [ 'Centro', data.sites?.name ?? '-' ], [ 'Dirección', data.sites?.address ?? '-' ], [ 'Equipo principal', data.primary_equipment?.code ?? '-' ], [ 'Tipo de trabajo', displayStatus(data.type) ], [ 'Horario', (data.scheduled_date ?? '-') + ' ' + (data.scheduled_time ?? '') ], [ 'Acceso', data.access_requirement?.description ?? '-' ], [ 'Material previsto', data.planned_material ?? '-' ], [ 'Descripción', data.description ?? '-' ]]}/></Card><Card title="Checks vinculados"><CompactRows rows={(data.checks ?? []).map((check: any) => [check.code, (check.equipment?.code ?? 'Equipo') + ' · ' + displayStatus(check.status) + ' · ' + displayStatus(check.global_result), severityForStatus(check.global_result), '/app/checks/' + check.id])} empty="Sin checks vinculados." /></Card></div><div className="grid half"><TechnicianLocalForm workOrderId={id} type="work-note" title="Diagnóstico e intervención" fields={[[ 'diagnosis','Diagnóstico' ], [ 'work','Trabajo realizado' ], [ 'observations','Observaciones' ]]} /><WorkOrderTimeCard workOrder={data} onChanged={reload} /></div><WorkOrderMaterialsCard workOrder={data} onChanged={reload} /><div className="grid half"><WorkOrderPhotoForm workOrderId={id} /><WorkOrderSignatureForm workOrderId={id} /></div><WorkOrderDeficiencyForm workOrderId={id} checks={data.checks ?? []} />{mode === 'time' && <WorkOrderTimeForm workOrder={data} onClose={() => setMode(null)} onSaved={() => { setMode(null); setMessage('Horas guardadas y visibles para SAT.'); reload(); }} />}{mode === 'material' && <WorkOrderMaterialForm workOrder={data} onClose={() => setMode(null)} onSaved={() => { setMode(null); setMessage('Material guardado y visible para SAT.'); reload(); }} />}{mode === 'finalize' && <WorkOrderFinalizeModal workOrder={data} onClose={() => setMode(null)} onDone={() => { setMode(null); setMessage('FINALIZADO TÉCNICAMENTE · ' + economicStatusLabel(data)); reload(); }} onError={setActionError} />}</section>;
}
  ...
  return (
    <section className="check-mobile">
      ...
      <header>
        ...
        <h2>
          {equipmentLabel.primary}
        </h2>
        <p>
          {equipmentLabel.secondary} · {reviewed} de {zones.length} bloques revisados
        </p>
        {!typeName && (
...
function CheckBlockPageV2({
  ...
  const save = async () => {
    ...
    try {
      ...
      setSaveState("saved");
      navigate(workspace === "superadmin" ? `/app/superadmin/checks/${id}` : `/app/checks/${id}`);
...
function ManagementPage() { return <CanonicalManagementPage />; }

function economicDateRange(period: string, from: string, to: string) { const now = new Date(); const start = new Date(now); if (period === 'month') start.setDate(1); else if (period === '3m') start.setMonth(start.getMonth() - 3); else if (period === 'year') { start.setMonth(0); start.setDate(1); } const customStart = period === 'custom' && from ? from : start.toISOString().slice(0, 10); const customEnd = period === 'custom' && to ? to : now.toISOString().slice(0, 10); return { from: customStart, to: customEnd }; }
function economicInRange(value: string | null | undefined, range: { from: string; to: string }) { if (!value) return true; const day = String(value).slice(0, 10); return day >= range.from && day <= range.to; }
function EconomicMetric({ label, value, tone, help }: { label: string; value: string; tone: Severity; help: string }) { return <div className={`metric ${tone}`}><div><Gauge {...iconProps} /><span>{label}</span></div><strong>{value}</strong><small>{help}</small></div>; }
function EconomicBars({ values }: { values: [string, number][] }) { const max = Math.max(1, ...values.map(([, value]) => value)); return <div className="interactive-bars">{values.map(([label, value]) => <div key={label} title={label}><span style={{ height: `${Math.max(12, (value / max) * 120)}px` }} /><strong>{money(value)}</strong><small>{label}</small></div>)}</div>; }
...
function QuotesModule() {
  ...
  const [creating, setCreating] = useState(false);
  const [archiveFilter, setArchiveFilter] = useState<ArchiveFilter>('active');
  const status = quoteStatusFilters.find((item) => normalizeParam(item) === normalizeParam(params.get('estado'))) ?? 'Todos';
  const scope = undefined;
   const dateFilters: DateRangeFilters = { createdFrom: params.get('creado_desde') ?? undefined, createdTo: params.get('creado_hasta') ?? undefined, updatedFrom: params.get('actualizado_desde') ?? undefined, updatedTo: params.get('actualizado_hasta') ?? undefined };
   const { data, loading, error, reload } = useLoad(() => quotesService.list(search, 'Todos', scope, archiveFilter, dateFilters), [search, scope, archiveFilter, dateFilters.createdFrom, dateFilters.createdTo, dateFilters.updatedFrom, dateFilters.updatedTo], [] as any[]);
  const visibleQuotes = status === 'Todos' ? data : data.filter((quote: any) => quote.status === status);
  const counts = Object.fromEntries(quoteStatusFilters.map((item) => [item, item === 'Todos' ? data.length : data.filter((quote: any) => quote.status === item).length]));
  const setStatus = (next: string) => { const updated = new URLSearchParams(params); next === 'Todos' ? updated.delete('estado') : updated.set('estado', normalizeParam(next)); setParams(updated); };
...
function StockReconciliationModal({ row, warehouses, onClose, onResolved }: { row: any; warehouses: any[]; onClose: () => void; onResolved: () => Promise<void> }) {
  return <StockReconciliationSpanishView row={row} warehouses={warehouses} onClose={onClose} onResolved={onResolved} />;
  const canonical = row.canonical[0];
  const warehouse = warehouses.find((item: any) => item.id === canonical.warehouse_id);
  const movements = useLoad(() => workOrdersService.warehouseStockMovements(canonical.warehouse_id, row.material.id), [canonical.warehouse_id, row.material.id], [] as any[]);
  const [quantity, setQuantity] = useState(String(canonical.quantity ?? 0));
  const [reason, setReason] = useState('');
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState('');
  useOverlayScrollLock();
  const resolve = async (action: 'accept_canonical' | 'adjust_canonical') => {
    if (!reason.trim()) { setError('Indica el motivo de la conciliacion.'); return; }
    const confirmed = Number(quantity);
    if (action === 'adjust_canonical' && (!Number.isFinite(confirmed) || confirmed < 0)) { setError('La cantidad confirmada no puede ser negativa.'); return; }
    setSaving(true); setError('');
    try {
      await workOrdersService.resolveInitialStockReview({ warehouse_id: canonical.warehouse_id, material_id: row.material.id, action, confirmed_quantity: action === 'accept_canonical' ? Number(canonical.quantity) : confirmed, reason: reason.trim(), idempotency_key: crypto.randomUUID() });
      await onResolved();
      window.dispatchEvent(new Event('warehouse-stock-reconciliation-changed'));
    } catch (err) { setError(formErrorMessage(err, 'No se ha podido resolver la discrepancia.')); }
    finally { setSaving(false); }
  };
...
function CanonicalStockMovementsModal({ material, onClose }: { material: any; onClose: () => void }) {
  return <Navigate to={`/app/modulos/materiales/${material.id}?tab=movimientos`} replace />;
  return <MaterialMovementsView material={material} onClose={onClose} />;
  const { data, loading, error, reload } = useLoad(() => materialsService.movements(material.id), [material.id], [] as any[]);
  const [filter, setFilter] = useState<'all' | 'in' | 'out' | 'consumption' | 'adjustment'>('all');
  const visible = data.filter((movement: any) => filter === 'all' || filter === 'in' && movementDirection(movement.movement_type) === 'Entrada' || filter === 'out' && movementDirection(movement.movement_type) === 'Salida' || filter === 'consumption' && movement.movement_type === 'Consumo en parte' || filter === 'adjustment' && movement.movement_type === 'Ajuste');
  return <SidePanel title={`Movimientos · ${material.code}`} subtitle={material.description} onClose={onClose}><StateBlock loading={loading} error={error} retry={reload} empty={!data.length}><div className="tabs movement-filters" aria-label="Filtro de movimientos"><button className={filter === 'all' ? 'active' : ''} onClick={() => setFilter('all')}>Todos</button><button className={filter === 'in' ? 'active' : ''} onClick={() => setFilter('in')}>Entradas</button><button className={filter === 'out' ? 'active' : ''} onClick={() => setFilter('out')}>Salidas</button><button className={filter === 'consumption' ? 'active' : ''} onClick={() => setFilter('consumption')}>Consumos de parte</button><button className={filter === 'adjustment' ? 'active' : ''} onClick={() => setFilter('adjustment')}>Ajustes</button></div><div className="compact-list stock-movements-list">{visible.map((movement: any) => { const workOrder = movement.work_orders; const quote = workOrder?.quotes; const links = movementLinks(movement); const direction = movementDirection(movement.movement_type); const tone = direction === 'Salida' ? 'warn' : direction === 'Entrada' ? 'ok' : 'info'; const displayQuantity = movementDisplayQuantity(movement.movement_type, movement.quantity); const context = [workOrder?.clients?.legal_name, workOrder?.sites?.name, workOrder?.equipment?.code].filter(Boolean).join(' · '); return <article key={movement.id}><div className="stock-movement-main"><strong>{formatDate(movement.created_at)}</strong><span><Badge tone={tone}>{direction} · {movementLabel(movement.movement_type)}</Badge> <strong>{displayQuantity > 0 ? '+' : ''}{displayQuantity.toLocaleString('es-ES')} {material.unit ?? 'ud'}</strong></span>{workOrder ? <span>{links.workOrder ? <Link to={links.workOrder.to}>{links.workOrder.label}</Link> : workOrder.code} · {workOrder.title}{links.quote && <> · <Link to={links.quote.to}>{links.quote.label}</Link></>}</span> : <span>Movimiento sin parte de origen</span>}{context && <small>{context}</small>}<small>{movement.warehouses ? `Almacén: ${movement.warehouses.code} · ${movement.warehouses.name}` : 'Almacén no informado'} · {movement.profiles ? `Actor: ${fullName(movement.profiles)}` : 'Actor no informado'}</small><small>{movement.notes ?? 'Sin motivo informado.'}</small></div><div className="stock-movement-qty"><strong>{movement.current_quantity === null ? 'Saldo actual no disponible' : `Saldo actual: ${Number(movement.current_quantity).toLocaleString('es-ES')} ${material.unit ?? 'ud'}`}</strong><span>Stock antes/después no persistido en el ledger; no se reconstruye de forma ambigua.</span><div className="row-actions">{links.workOrder && <Link to={links.workOrder.to}>Ver parte {links.workOrder.label}</Link>}</div></div></article>; })}</div>{!visible.length && <p className="large-note">No hay movimientos para este filtro.</p>}</StateBlock></SidePanel>;
}

function QuoteDetailPage() {
  const { id = '' } = useParams();
  const navigate = useNavigate();
  return <QuoteDetailModal quoteId={id} onClose={() => navigate('/app/modulos/presupuestos')} onChanged={() => undefined} />;
}

function quotePlannedMaterial(lines: any[]) {
  const materialLines = lines.filter((line) => line.line_type === 'material' || line.material_id);
  return materialLines.map((line) => `${Number(line.quantity ?? 0).toLocaleString('es-ES')} ${line.unit ?? 'ud'} · ${line.description}`).join('\n');
}
...
function QuoteDetailModal({ quoteId, onClose, onChanged }: { quoteId: string; onClose: () => void; onChanged: () => void }) {
  const { profile, workspace } = useAuth();
  const { data, loading, error, reload } = useLoad(() => quotesService.get(quoteId), [quoteId], null as any);
  const [mode, setMode] = useState<'edit' | 'line' | 'send' | 'work' | null>(null);
  const [purgeOpen, setPurgeOpen] = useState(false);
  const [printMode, setPrintMode] = useState<'client' | 'internal'>('client');
  const [editingLine, setEditingLine] = useState<any | null>(null);
  const [removingLine, setRemovingLine] = useState<any | null>(null);
  const [message, setMessage] = useState('');
...
function WorkOrderPurgeModal({ workOrder, onClose, onDeleted }: { workOrder: any; onClose: () => void; onDeleted: () => void }) {
  const opener = useRef<HTMLElement | null>(typeof document === 'undefined' ? null : document.activeElement as HTMLElement | null);
  const [plan, setPlan] = useState<any>(null);
  const [planScopeKey, setPlanScopeKey] = useState<WorkOrderPurgeScopeKey | null>(null);
  const [loadError, setLoadError] = useState('');
  const [includeDocuments, setIncludeDocuments] = useState(false);
  const [includeStockMovements, setIncludeStockMovements] = useState(false);
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState('');
  const expected = workOrderPurgeExpectedConfirmation(workOrder.code ?? workOrder.id);
  const blocks = workOrderPurgeBlocks(plan);
  const dryRunId = useRef(0);
  const runDryRun = (documents: boolean, stockMovements: boolean, scopeKey: WorkOrderPurgeScopeKey) => {
    const id = ++dryRunId.current;
    setPlan(null); setLoadError(''); setError('');
    entityLifecycleService.purge('work_orders', workOrder.id, { confirmation: expected, scope: workOrderPurgeScope(documents, stockMovements), dryRun: true })
      .then((result) => { if (id !== dryRunId.current) return; setPlan(result?.plan ?? result); setPlanScopeKey(scopeKey); })
      .catch((err) => { if (id !== dryRunId.current) return; const raw = err as any; console.error('[WorkOrderPurgeModal] dry-run', { message: err instanceof Error ? err.message : String(err), details: raw?.details, hint: raw?.hint, code: raw?.code }); setLoadError(err instanceof Error ? err.message : 'No se ha podido consultar el plan de purga.'); });
  };
  const onDocumentDecision = (checked: boolean) => { setIncludeDocuments(checked); runDryRun(checked, includeStockMovements, workOrderPurgeScopeKey(checked, includeStockMovements)); };
  const onStockDecision = (checked: boolean) => { setIncludeStockMovements(checked); runDryRun(includeDocuments, checked, workOrderPurgeScopeKey(includeDocuments, checked)); };
  const canExecute = Boolean(plan) && !blocks.hardBlock && workOrderPurgePlanMatchesScope(planScopeKey, includeDocuments, includeStockMovements) && (blocks.documents === 0 || includeDocuments) && (blocks.stockMovements === 0 || includeStockMovements);
  useOverlayScrollLock();
  useEffect(() => { runDryRun(false, false, 'baseline'); }, [workOrder.id, expected]);
  useEffect(() => {
    const onKey = (event: KeyboardEvent) => { if (event.key === 'Escape' && !saving) onClose(); };
    window.addEventListener('keydown', onKey);
    return () => { window.removeEventListener('keydown', onKey); opener.current?.focus(); };
  ...
  const submit = async () => {
    ...
    setSaving(true); setError('');
    try {
      const result = await entityLifecycleService.purge('work_orders', workOrder.id, { confirmation: expected, scope: workOrderPurgeScope(includeDocuments, includeStockMovements), dryRun: false });
      if (workOrderPurgeResultOk(result)) { onDeleted(); return; }
      setError('La purga no devolvió un resultado esperado.');
    } catch (err) { const raw = err as any; console.error('[WorkOrderPurgeModal] ejecución', { message: err instanceof Error ? err.message : String(err), details: raw?.details, hint: raw?.hint, code: raw?.code }); setError(err instanceof Error ? err.message : 'No se ha podido completar la purga. Revisa las dependencias indicadas.'); } finally { setSaving(false); }
  };
  return <div className="mini-modal lifecycle-modal" role="dialog" aria-modal="true" aria-labelledby="work-order-purge-title"><div><h3 id="work-order-purge-title">Eliminar definitivamente el parte</h3><p className="large-note">{workOrder.code ?? workOrder.id} · {workOrder.title ?? 'Parte'} · {workOrder.client_name ?? workOrder.clients?.legal_name ?? 'Cliente no informado'}</p>{loadError ? <p className="form-error">{loadError}</p> : !plan ? <p className="large-note">Analizando dependencias…</p> : <>{blocks.hardBlock && <p className="form-error">El parte tiene dependencias no resueltas ({blocks.externalDeficiencies} deficiencias externas, {blocks.unclassifiedDeficiencyReferences} referencias a deficiencias). No se puede purgar definitivamente.</p>}<InfoGrid items={workOrderPurgePlanItems(plan)} /><Card title="Se eliminará en cascada"><DependencyRows dependencies={plan.cascade_dependencies ?? {}} /></Card>{workOrder.quote_id ? <p className="large-note">El presupuesto relacionado se conservará.</p> : null}{workOrder.main_equipment_id ? <p className="large-note">Los equipos asociados no se eliminarán.</p> : null}{blocks.documents > 0 && <label><input type="checkbox" checked={includeDocuments} onChange={(event) => onDocumentDecision(event.target.checked)} disabled={saving} /> Eliminar también los {blocks.documents} documento(s) vinculados</label>}{blocks.documents > 0 && !includeDocuments && <p className="form-error">Debes decidir explícitamente qué hacer con los {blocks.documents} documentos vinculados para poder purgar este parte.</p>}{blocks.stockMovements > 0 && <label><input type="checkbox" checked={includeStockMovements} onChange={(event) => onStockDecision(event.target.checked)} disabled={saving} /> Eliminar también los {blocks.stockMovements} movimientos de stock históricos</label>}{blocks.stockMovements > 0 && !includeStockMovements && <p className="form-error">Debes decidir explícitamente qué hacer con los {blocks.stockMovements} movimientos de stock históricos para poder purgar este parte.</p>}{!blocks.hardBlock && (includeDocuments || includeStockMovements) && planScopeKey === workOrderPurgeScopeKey(includeDocuments, includeStockMovements) && <p className="success-note">Plan recalculado con las decisiones indicadas.</p>}{error && <p className="form-error">{error}</p>}<div className="modal-footer"><button type="button" onClick={onClose} disabled={saving}>Cancelar</button>{!blocks.hardBlock && <button type="button" className="danger-action" onClick={submit} disabled={saving || !canExecute}>{saving ? 'Eliminando…' : 'Eliminar definitivamente'}</button>}</div></>}</div></div>;
}

function GenericEntityPurgeModal({ config, record, onClose, onDeleted }: { config: EntityPurgeConfig; record: any; onClose: () => void; onDeleted: () => void }) {
  const opener = useRef<HTMLElement | null>(typeof document === 'undefined' ? null : document.activeElement as HTMLElement | null);
  const [plan, setPlan] = useState<any>(null);
  const [planScopeKey, setPlanScopeKey] = useState<string | null>(null);
  const [loadError, setLoadError] = useState('');
  const [enabledScopeKeys, setEnabledScopeKeys] = useState<PurgeScopeKey[]>([]);
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState('');
  const expected = entityPurgeExpectedConfirmation(record.code ?? record.id);
  const blocks = entityPurgeBlockers(config, plan);
  const dryRunId = useRef(0);
  const runDryRun = (scopeKeys: PurgeScopeKey[], scopeKey: string) => {
    const id = ++dryRunId.current;
    setPlan(null); setLoadError(''); setError('');
    entityLifecycleService.purge(config.entity, record.id, { confirmation: expected, scope: entityPurgeScope(config, scopeKeys), dryRun: true })
      .then((result) => { if (id !== dryRunId.current) return; setPlan(result?.plan ?? result); setPlanScopeKey(scopeKey); })
      .catch((err) => { if (id !== dryRunId.current) return; const raw = err as any; console.error('[GenericEntityPurgeModal] dry-run', { entity: config.entity, message: err instanceof Error ? err.message : String(err), details: raw?.details, hint: raw?.hint, code: raw?.code }); setLoadError(err instanceof Error ? err.message : 'No se ha podido consultar el plan de purga.'); });
  };
  const toggleDecision = (decision: EntityPurgeDecision, checked: boolean) => {
    const next = checked ? [...enabledScopeKeys, decision.scopeKey] : enabledScopeKeys.filter((key) => key !== decision.scopeKey);
    setEnabledScopeKeys(next);
    runDryRun(next, entityPurgeScopeKey(next));
  };
  const canExecute = Boolean(plan) && blocks.hard.length === 0 && entityPurgePlanMatchesScope(planScopeKey, enabledScopeKeys) && blocks.decisions.every((item) => enabledScopeKeys.includes(item.config.scopeKey));
  useOverlayScrollLock();
  useEffect(() => { runDryRun([], 'baseline'); }, [config.entity, record.id, expected]);
  useEffect(() => {
    const onKey = (event: KeyboardEvent) => { if (event.key === 'Escape' && !saving) onClose(); };
    window.addEventListener('keydown', onKey);
    return () => { window.removeEventListener('keydown', onKey); opener.current?.focus(); };
  ...
  const submit = async () => {
    ...
    setSaving(true); setError('');
    try {
      const result = await entityLifecycleService.purge(config.entity, record.id, { confirmation: expected, scope: entityPurgeScope(config, enabledScopeKeys), dryRun: false });
      if (entityPurgeResultOk(result)) { onDeleted(); return; }
      setError('La purga no devolvió un resultado esperado.');
    } catch (err) { const raw = err as any; console.error('[GenericEntityPurgeModal] ejecución', { entity: config.entity, message: err instanceof Error ? err.message : String(err), details: raw?.details, hint: raw?.hint, code: raw?.code }); setError(err instanceof Error ? err.message : 'No se ha podido completar la purga. Revisa las dependencias indicadas.'); } finally { setSaving(false); }
  };
...
function QuotePrintableReport({ quote, lines, variant }: { quote: any; lines: any[]; variant: 'client' | 'internal' }) {
  ...
  const money = (value: any) => `${Number(value ?? 0).toLocaleString('es-ES', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} €`;
  const marginPercent = Number(quote.taxable_base ?? 0) > 0 ? Number(quote.estimated_margin ?? 0) / Number(quote.taxable_base ?? 0) * 100 : 0;
  return <article className={`quote-report print-only ${internal ? 'internal-report' : 'client-report'}`}><header className="quote-report-hero"><div><p className="eyebrow">{internal ? 'Informe interno DMP' : 'Presupuesto cliente'}</p><h1>{quote.code}</h1><p>{quote.title}</p></div><div><strong>DoorManager Pro</strong><span>{quote.issue_date ?? '-'}</span><span>Validez: {quote.valid_until ?? '-'}</span></div></header><section className="quote-report-grid"><div><h3>Cliente</h3><p><strong>{quote.clients?.legal_name ?? 'Cliente no informado'}</strong><br />{quote.clients?.email ?? 'Email no informado'}</p></div><div><h3>Ubicación y equipo</h3><p>{quote.sites?.name ?? 'Centro no informado'}<br />{quote.equipment?.code ?? 'Equipo no informado'}{quote.work_orders?.code ? ` · Parte ${quote.work_orders.code}` : ''}</p></div><div><h3>Estado</h3><p>{displayStatus(quote.status)}<br />Tipo: {displayStatus(quote.quote_type)}</p></div></section><section className="quote-report-section"><h3>Descripción</h3><p>{quote.description ?? 'Sin descripción.'}</p></section><section className="quote-report-section"><h3>{internal ? 'Líneas con control económico' : 'Detalle económico'}</h3><div className="quote-report-table"><table><thead><tr><th>Descripción</th><th>Cant.</th><th>Ud.</th>{internal && <th>Coste ud.</th>}<th>Precio ud.</th><th>IVA</th>{internal && <th>Coste total</th>}<th>Total venta</th>{internal && <th>Margen línea</th>}</tr></thead><tbody>{lines.map((line: any) => { const lineMargin = Number(line.total_price ?? 0) - Number(line.total_cost ?? 0); return <tr key={line.id}><td>{line.description}</td><td>{Number(line.quantity ?? 0).toLocaleString('es-ES')}</td><td>{line.unit ?? 'ud'}</td>{internal && <td>{money(line.unit_cost)}</td>}<td>{money(line.unit_price)}</td><td>{Number(line.tax_rate ?? 0).toLocaleString('es-ES')}%</td>{internal && <td>{money(line.total_cost)}</td>}<td>{money(line.total_price ?? line.total)}</td>{internal && <td>{money(lineMargin)}</td>}</tr>; })}</tbody></table></div></section><section className="quote-report-totals"><div><span>Subtotal venta sin IVA</span><strong>{money(quote.subtotal_sale ?? quote.subtotal)}</strong></div><div><span>Descuento {quote.discount_type === 'amount' ? '€' : '%'}</span><strong>{Number(quote.discount_value ?? quote.discount_amount ?? 0).toLocaleString('es-ES')} {quote.discount_type === 'amount' ? '€' : '%'}</strong><small>Aplicado: {money(quote.discount_amount)}</small></div><div><span>Base imponible</span><strong>{money(quote.taxable_base)}</strong></div><div><span>IVA</span><strong>{money(quote.tax_amount)}</strong></div><div className="quote-report-total"><span>Total cliente con IVA</span><strong>{money(quote.total_amount ?? quote.total)}</strong></div>{internal && <><div><span>Subtotal coste interno</span><strong>{money(quote.subtotal_cost)}</strong></div><div><span>Beneficio estimado sin IVA</span><strong>{money(quote.estimated_margin)}</strong></div><div><span>Margen %</span><strong>{marginPercent.toLocaleString('es-ES', { maximumFractionDigits: 2 })}%</strong></div></>}</section><section className="quote-report-section"><h3>Condiciones</h3><p>{quote.conditions ?? 'Condiciones no informadas.'}</p></section>{internal && <section className="quote-report-section"><h3>Control interno</h3><p>Estados candidatos para fases futuras: Aceptado y Ejecutado en cliente. Ejecutado en cliente será base para factura/cobro en siguientes fases.</p></section>}</article>;
}

function QuoteForm({ initial, onClose, onSaved }: { initial?: any; onClose: () => void; onSaved: (id: string) => void }) {
  const clients = useLoad(() => clientsService.list('', initial?.company_id), [initial?.company_id], [] as any[]);
  const [values, setValues] = useState<Record<string, any>>({ quote_type: 'reparacion', status: 'Borrador', title: '', description: '', conditions: '', discount_type: 'percentage', discount_value: 0, ...initial });
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState('');
  const set = (key: string, value: any) => setValues((current) => ({ ...current, [key]: value }));
  ...
  const clientOptions = clients.data.map((client: any) => ({ value: client.id, label: formatClientLabel(client) }));
  if (values.client_id && !clientOptions.some((option: any) => option.value === values.client_id)) clientOptions.unshift({ value: values.client_id, label: formatClientLabel(initial?.clients ?? { code: 'Cliente', legal_name: 'Cliente asociado actualmente' }) });
...
function QuoteSendModal({ quote, onClose, onSaved }: { quote: any; onClose: () => void; onSaved: () => void }) {
  const [email, setEmail] = useState(quote.clients?.email ?? quote.sent_to_email ?? '');
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState('');
...
function SalesModule({ mode }: { mode: 'ventas' | 'oportunidades' | 'presupuestos' }) {
  ...
  return <section className="page"><Breadcrumb items={['Gerencia', mode === 'ventas' ? 'Ventas' : mode === 'presupuestos' ? 'Presupuestos' : 'Oportunidades']} /><Hero title={mode === 'ventas' ? 'Ventas ejecutivas' : mode === 'presupuestos' ? 'Presupuestos' : 'Oportunidades'} subtitle="Seguimiento comercial de oportunidades y presupuestos." tone="commercial" /><StateBlock loading={loading} error={error} retry={reload} empty={!data.quotes.length && !data.opportunities.length}><div className="stats-grid"><div className="metric ok"><div><Gauge {...iconProps} /><span>Presupuestos aceptados</span></div><strong>{accepted.length}</strong><small>Total: {total.toLocaleString('es-ES')} €</small></div><div className="metric info"><div><Gauge {...iconProps} /><span>Oportunidades</span></div><strong>{opportunities.length}</strong><small>Registros visibles</small></div></div>{showQuotes && <Card title="Presupuestos"><div className="table-card"><table><thead><tr><th>Código</th><th>Estado</th><th>Cliente</th><th>Oportunidad</th><th>Fecha</th><th>Base</th><th>Impuestos</th><th>Total</th><th>Responsable</th></tr></thead><tbody>{quotes.map((quote: any) => <tr key={quote.id}><td>{quote.code}</td><td>{displayStatus(quote.status)}</td><td>{quote.clients?.legal_name ?? '-'}</td><td>{quote.opportunities?.title ?? quote.opportunities?.code ?? '-'}</td><td>{quote.issue_date ?? '-'}</td><td>{Number(quote.subtotal ?? 0).toLocaleString('es-ES')} €</td><td>{Number(quote.tax_amount ?? 0).toLocaleString('es-ES')} €</td><td>{Number(quote.total ?? 0).toLocaleString('es-ES')} €</td><td>{fullName(quote.profiles) || '-'}</td></tr>)}</tbody></table></div>{!quotes.length && <p className="large-note">No hay presupuestos para el filtro actual.</p>}</Card>}{showOpps && <Card title="Oportunidades"><div className="table-card"><table><thead><tr><th>Código</th><th>Título</th><th>Cliente</th><th>Responsable</th><th>Estado</th><th>Importe estimado</th><th>Origen</th><th>Fecha</th></tr></thead><tbody>{opportunities.map((opp: any) => <tr key={opp.id}><td>{opp.code}</td><td>{opp.title}</td><td>{opp.clients?.legal_name ?? '-'}</td><td>{fullName(opp.profiles) || '-'}</td><td>{displayStatus(opp.status)}</td><td>{Number(opp.estimated_amount ?? 0).toLocaleString('es-ES')} €</td><td>{displayStatus(opp.origin)}</td><td>{formatDate(opp.created_at)}</td></tr>)}</tbody></table></div>{!opportunities.length && <p className="large-note">No hay oportunidades para el filtro actual.</p>}</Card>}</StateBlock></section>;
}

function ProfitabilityModule() { const { workspace } = useAuth(); const scope = undefined; const [search, setSearch] = useState(''); const works = useLoad(() => economicService.workOrderSummaries(search, scope), [search, scope], [] as any[]); const clients = useLoad(() => economicService.clientSummaries(scope), [scope], [] as any[]); const totalCost = works.data.reduce((sum: number, row: any) => sum + Number(row.real_cost_amount ?? 0), 0); const totalSale = works.data.reduce((sum: number, row: any) => sum + Number(row.estimated_sale_amount ?? 0), 0); const margin = totalSale - totalCost; return <section className="page"><Breadcrumb items={['Gerencia', 'Rentabilidad']} /><Hero title="Rentabilidad" subtitle="Coste real de partes, venta prevista y margen calculados desde horas, materiales y recursos." tone="commercial" /><div className="filters sat-filters"><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar parte, cliente o estado economico" /></div><div className="stats-grid"><div className="metric info"><span>Venta prevista</span><strong>{money(totalSale)}</strong><small>Partes visibles</small></div><div className="metric warn"><span>Coste real</span><strong>{money(totalCost)}</strong><small>Materiales, horas y recursos</small></div><div className={margin >= 0 ? 'metric ok' : 'metric danger'}><span>Margen previsto</span><strong>{money(margin)}</strong><small>{totalSale > 0 ? `${((margin / totalSale) * 100).toLocaleString('es-ES', { maximumFractionDigits: 2 })}% sobre venta` : 'Sin venta prevista'}</small></div><div className="metric info"><span>Clientes con economia</span><strong>{clients.data.length}</strong><small>Resumen agregado</small></div></div><StateBlock loading={works.loading || clients.loading} error={works.error || clients.error} retry={() => { works.reload(); clients.reload(); }} empty={!works.data.length}><div className="grid half"><Card title="Partes por rentabilidad"><div className="table-card"><table><thead><tr><th>Parte</th><th>Cliente</th><th>Estado</th><th>Coste</th><th>Venta</th><th>Margen</th><th></th></tr></thead><tbody>{works.data.map((row: any) => <tr key={row.id}><td>{row.code}</td><td>{row.client_name ?? '-'}</td><td>{displayStatus(row.economic_status ?? row.status)}</td><td>{money(row.real_cost_amount)}</td><td>{money(row.estimated_sale_amount)}</td><td>{money(row.estimated_margin_amount)}</td><td><Link to={`/app/partes/${row.id}`}>Abrir</Link></td></tr>)}</tbody></table></div></Card><Card title="Clientes"><div className="table-card"><table><thead><tr><th>Cliente</th><th>Coste</th><th>Venta</th><th>Margen</th><th>Facturables</th></tr></thead><tbody>{clients.data.map((row: any) => <tr key={row.id}><td>{row.legal_name}</td><td>{money(row.real_cost_amount)}</td><td>{money(row.estimated_sale_amount)}</td><td>{money(row.estimated_margin_amount)}</td><td>{row.pending_invoice_work_orders ?? 0}</td></tr>)}</tbody></table></div></Card></div></StateBlock></section>; }
...
function HourRatesModule() { const { profile, workspace } = useAuth(); const scope = undefined; const [search, setSearch] = useState(''); const [editing, setEditing] = useState<any | null>(null); const rates = useLoad(() => hourRatesService.list(search, scope), [search, scope], [] as any[]); const canManage = canManageHourRates(profile); return <section className="page"><Breadcrumb items={['Configuracion', 'Tarifas horas']} /><Hero title="Tarifas de horas tecnicas" subtitle="Coste interno y precio de venta vigentes para calcular partes, informes y rentabilidad." tone="commercial" /><div className="filters sat-filters"><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar categoria o notas" />{canManage && <button className="primary" onClick={() => setEditing({})}>Nueva tarifa</button>}</div><StateBlock loading={rates.loading} error={rates.error} retry={rates.reload} empty={!rates.data.length}><div className="grid half">{rates.data.map((rate: any) => <Card key={rate.id} title={rate.technician_profile_id ? fullName(rate.profiles) : rate.category || 'Tarifa por defecto'} action={<Badge tone={rate.active ? 'ok' : 'muted'}>{rate.active ? 'Activa' : 'Inactiva'}</Badge>}><InfoGrid items={[[ 'Coste hora interno', money(rate.hourly_cost) ], [ 'Precio hora venta', money(rate.hourly_price) ], [ 'Categoria', rate.category ?? '-' ], [ 'Desde', rate.valid_from ?? '-' ], [ 'Hasta', rate.valid_to ?? '-' ], [ 'Notas', rate.notes ?? '-' ]]} />{canManage && <div className="row-actions"><button onClick={() => setEditing(rate)}>Editar</button><button onClick={async () => { await hourRatesService.archive(rate.id); rates.reload(); }}>Archivar</button></div>}</Card>)}</div></StateBlock>{editing && <HourRateForm initial={editing} scope={scope} onClose={() => setEditing(null)} onSaved={() => { setEditing(null); rates.reload(); }} />}</section>; }

function HourRateForm({ initial, scope, onClose, onSaved }: { initial: any; scope?: string | null; onClose: () => void; onSaved: () => void }) { const technicians = useLoad(() => hourRatesService.technicianOptions(scope), [scope], [] as any[]); const [values, setValues] = useState<Record<string, any>>({ technician_profile_id: '', category: 'tecnico', hourly_cost: 0, hourly_price: 0, valid_from: new Date().toISOString().slice(0, 10), valid_to: '', active: true, notes: '', ...initial }); const [saving, setSaving] = useState(false); const [error, setError] = useState(''); const set = (key: string, value: any) => setValues((current) => ({ ...current, [key]: value })); const submit = async (event: FormEvent) => { event.preventDefault(); if (Number(values.hourly_cost ?? 0) < 0 || Number(values.hourly_price ?? 0) < 0) { setError('Coste y precio no pueden ser negativos.'); return; } setSaving(true); setError(''); try { initial?.id ? await hourRatesService.update(initial.id, values) : await hourRatesService.create(values); onSaved(); } catch (err) { setError(formErrorMessage(err, initial?.id ? 'No se ha podido editar la tarifa.' : 'No se ha podido crear la tarifa.')); } finally { setSaving(false); } }; return <ModalForm title={initial?.id ? 'Editar tarifa hora' : 'Crear tarifa hora'} onClose={onClose} onSubmit={submit} saving={saving} error={error}><FormSelect label="Tecnico especifico" value={values.technician_profile_id ?? ''} onChange={(value) => set('technician_profile_id', value)} loading={technicians.loading} emptyLabel="Tarifa por categoria" options={technicians.data.map((item: any) => ({ value: item.id, label: `${fullName(item)} · ${item.primary_area ?? '-'}` }))} /><div className="form-grid"><label>Categoria<input value={values.category ?? ''} onChange={(event) => set('category', event.target.value)} placeholder="tecnico, oficial, ayudante" /></label><label>Coste hora interno<input type="number" min="0" step="0.01" value={values.hourly_cost ?? 0} onChange={(event) => set('hourly_cost', event.target.value)} required /></label><label>Precio hora venta<input type="number" min="0" step="0.01" value={values.hourly_price ?? 0} onChange={(event) => set('hourly_price', event.target.value)} required /></label><label>Fecha desde<input type="date" value={values.valid_from ?? ''} onChange={(event) => set('valid_from', event.target.value)} required /></label><label>Fecha hasta<input type="date" value={values.valid_to ?? ''} onChange={(event) => set('valid_to', event.target.value)} /></label></div><label className="check-consent"><input type="checkbox" checked={values.active !== false} onChange={(event) => set('active', event.target.checked)} /> Tarifa activa</label><label>Notas<textarea value={values.notes ?? ''} onChange={(event) => set('notes', event.target.value)} /></label></ModalForm>; }

function RateCatalogModule() { const { profile } = useAuth(); const canManage = canManageHourRates(profile); const [search, setSearch] = useState(''); const [catalog, setCatalog] = useState<any | null>(null); const rates = useLoad(() => hourRatesService.catalog(search, undefined, undefined, true), [search], [] as any[]); return <section className="page"><Breadcrumb items={['Gerencia', 'Catalogo de tarifas']} /><Hero title="Catalogo generico de tarifas" subtitle="Conceptos, unidades y modalidad. En dia/periodo la cantidad es el numero explicito de dias o periodos; no se calcula automaticamente desde fechas." tone="commercial" /><div className="filters sat-filters"><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar concepto" />{canManage && <button className="primary" onClick={() => setCatalog({})}>Nuevo concepto</button>}</div><StateBlock loading={rates.loading} error={rates.error} retry={rates.reload} empty={!rates.data.length}><div className="grid half">{rates.data.map((item: any) => <Card key={item.id} title={item.name} action={<Badge tone={item.active ? 'ok' : 'muted'}>{item.active ? 'Activo' : 'Archivado'}</Badge>}><InfoGrid items={[[ 'Codigo', item.code ], [ 'Unidad', item.unit ], [ 'Cobro', item.billing_mode ], [ 'Dias periodo', item.period_days ?? '-' ], [ 'Venta adicional', item.contributes_to_sale ? 'Sí' : 'No' ], [ 'Versiones', String(item.rate_versions?.length ?? 0) ]]} /><div className="compact-list">{(item.rate_versions ?? []).map((version: any) => <p key={version.id} className="large-note">Version {version.valid_from} - {version.valid_to ?? 'abierta'} · {version.active && !version.deleted_at ? 'Activa' : 'Archivada'} · Coste {money(version.cost_amount)} · Venta {money(version.sale_amount)}{canManage && version.active && <button onClick={() => { hourRatesService.archiveVersion(version.id).then(rates.reload); }}>Archivar</button>}</p>)}</div>{canManage && <div className="row-actions"><button onClick={() => setCatalog(item)}>Nueva version</button><button onClick={() => { hourRatesService.archiveCatalog(item.id).then(rates.reload); }}>Archivar concepto</button></div>}</Card>)}</div></StateBlock>{catalog && <RateCatalogForm initial={catalog} onClose={() => setCatalog(null)} onSaved={() => { setCatalog(null); rates.reload(); }} />}</section>; }
function RateCatalogForm({ initial, mode = 'version', onClose, onSaved }: { initial: any; mode?: 'concept' | 'version'; onClose: () => void; onSaved: () => void }) { const isConcept = mode === 'concept'; const [values, setValues] = useState<Record<string, any>>({ code: '', name: '', kind: 'cost', classification: 'cost', unit: 'ud', billing_mode: 'unit', period_days: '', contributes_to_sale: false, cost_amount: 0, sale_amount: 0, valid_from: new Date().toISOString().slice(0, 10), valid_to: '', notes: '', ...initial }); const [saving, setSaving] = useState(false); const [error, setError] = useState(''); const set = (key: string, value: any) => setValues((current) => ({ ...current, [key]: value })); const submit = async (event: FormEvent) => { event.preventDefault(); if (!isConcept && (Number(values.cost_amount ?? 0) < 0 || Number(values.sale_amount ?? 0) < 0)) { setError('Coste y precio no pueden ser negativos.'); return; } if (!isConcept && values.valid_to && values.valid_to < values.valid_from) { setError('La fecha final no puede ser anterior a la fecha inicial.'); return; } setSaving(true); setError(''); try { if (isConcept) { const payload = { kind: values.kind === 'labor' ? 'labor' : 'cost', classification: values.classification === 'labor' ? 'labor' : 'cost', unit: values.unit, billing_mode: values.billing_mode, period_days: values.period_days === '' ? null : Number(values.period_days), contributes_to_sale: values.contributes_to_sale, notes: values.notes }; if (initial?.id) await hourRatesService.updateCatalog(initial.id, payload); else await hourRatesService.createCatalog({ code: values.code, name: values.name, ...payload }); } else { await hourRatesService.createVersion({ rate_id: initial.id, technician_profile_id: values.technician_profile_id, category: values.category, cost_amount: Number(values.cost_amount ?? 0), sale_amount: Number(values.sale_amount ?? 0), valid_from: values.valid_from, valid_to: values.valid_to || null, active: true, notes: values.notes }); } onSaved(); } catch (err) { setError(formErrorMessage(err, isConcept ? 'Error al editar el concepto.' : 'Error al crear la nueva versión de tarifa.')); } finally { setSaving(false); } }; return <ModalForm title={isConcept ? (initial?.id ? 'Editar concepto' : 'Nuevo concepto de tarifa') : 'Nueva versión de tarifa'} onClose={onClose} onSubmit={submit} saving={saving} error={error}><div className="form-grid">{isConcept ? <><label>Código<input value={values.code ?? ''} onChange={(event) => set('code', event.target.value)} disabled={Boolean(initial?.id)} required /></label><label>Nombre<input value={values.name ?? ''} onChange={(event) => set('name', event.target.value)} disabled={Boolean(initial?.id)} required /></label><label>Clasificación<select value={values.classification} onChange={(event) => set('classification', event.target.value)}><option value="labor">Mano de obra</option><option value="cost">Recurso / coste</option></select></label><label>Unidad<input value={values.unit ?? ''} onChange={(event) => set('unit', event.target.value)} required /></label><label>Modalidad<select value={values.billing_mode} onChange={(event) => set('billing_mode', event.target.value)}><option value="unit">Unidad</option><option value="hour">Hora</option><option value="day">Día</option><option value="period">Periodo</option></select></label><label>Días del periodo<input type="number" min="1" step="1" value={values.period_days ?? ''} onChange={(event) => set('period_days', event.target.value)} /></label><label><input type="checkbox" checked={values.contributes_to_sale === true} onChange={(event) => set('contributes_to_sale', event.target.checked)} /> Facturable como venta adicional</label></> : <><label>Concepto<input value={values.name ?? ''} readOnly /></label><label>Código<input value={values.code ?? ''} readOnly /></label><label>Unidad<input value={values.unit ?? ''} readOnly /></label><label>Modalidad<input value={values.billing_mode ?? ''} readOnly /></label><label>Válida desde<input type="date" value={values.valid_from ?? ''} onChange={(event) => set('valid_from', event.target.value)} required /></label><label>Válida hasta<input type="date" value={values.valid_to ?? ''} onChange={(event) => set('valid_to', event.target.value)} /></label><label>Coste interno<input type="number" min="0" step="0.01" value={values.cost_amount ?? 0} onChange={(event) => set('cost_amount', event.target.value)} required /></label><label>Precio venta<input type="number" min="0" step="0.01" value={values.sale_amount ?? 0} onChange={(event) => set('sale_amount', event.target.value)} required /></label></>}</div><label>Observaciones<textarea value={values.notes ?? ''} onChange={(event) => set('notes', event.target.value)} /></label></ModalForm>; }

function RateCatalogModuleV2() { const { profile } = useAuth(); const canManage = canManageHourRates(profile); const [search, setSearch] = useState(''); const [status, setStatus] = useState<'active' | 'archived' | 'all'>('active'); const [catalog, setCatalog] = useState<any | null>(null); const rates = useLoad(() => hourRatesService.catalog(search, undefined, undefined, true), [search], [] as any[]); const activeRates = rates.data.filter((item: any) => item.active === true && !item.deleted_at); const archivedRates = rates.data.filter((item: any) => item.active !== true || Boolean(item.deleted_at)); const visibleRates = status === 'active' ? activeRates : status === 'archived' ? archivedRates : rates.data; const statusOptions = [['active', `Activas (${activeRates.length})`], ['archived', `Archivadas (${archivedRates.length})`], ['all', `Todas (${rates.data.length})`]] as const; return <section className="page"><Breadcrumb items={['Gerencia', 'Catalogo de tarifas']} /><Hero title="Catalogo generico de tarifas" subtitle="Conceptos, unidades y modalidad." tone="commercial" /><div className="filters sat-filters"><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar concepto" /><div className="rate-status-filter" role="group" aria-label="Estado de tarifas">{statusOptions.map(([value, label]) => <button key={value} type="button" className={status === value ? 'active' : ''} aria-pressed={status === value} onClick={() => setStatus(value)}>{label}</button>)}</div>{canManage && <button className="primary" onClick={() => setCatalog({ formMode: 'concept' })}>Nuevo concepto</button>}</div><StateBlock loading={rates.loading} error={rates.error} retry={rates.reload} empty={!visibleRates.length}><div className="grid half">{visibleRates.map((item: any) => <Card key={item.id} title={item.name} action={<Badge tone={item.active && !item.deleted_at ? 'ok' : 'muted'}>{item.active && !item.deleted_at ? 'Activo' : 'Archivado'}</Badge>}><InfoGrid items={[[ 'Codigo', item.code ], [ 'Unidad', item.unit ], [ 'Cobro', item.billing_mode ], [ 'Dias periodo', item.period_days ?? '-' ], [ 'Venta adicional', item.contributes_to_sale ? 'Si' : 'No' ], [ 'Versiones', String(item.rate_versions?.length ?? 0) ]]} /><div className="compact-list">{(item.rate_versions ?? []).map((version: any) => <p key={version.id} className="large-note">Version {version.valid_from} - {version.valid_to ?? 'abierta'} · {version.active && !version.deleted_at ? 'Activa' : 'Archivada'} · Coste {money(version.cost_amount)} · Venta {money(version.sale_amount)}</p>)}</div>{canManage && <div className="row-actions"><button onClick={() => setCatalog({ ...item, formMode: 'version' })}>Nueva version</button><button onClick={() => setCatalog({ ...item, formMode: 'concept' })}>Editar concepto</button><button onClick={() => { hourRatesService.archiveCatalog(item.id).then(rates.reload); }}>Archivar concepto</button></div>}</Card>)}</div></StateBlock>{catalog && <RateCatalogForm initial={catalog} mode={catalog.formMode ?? 'concept'} onClose={() => setCatalog(null)} onSaved={() => { setCatalog(null); rates.reload(); }} />}</section>; }
...
function CompanySettingsForm({ initial, onClose, onSaved }: { initial: any; onClose: () => void; onSaved: () => void }) {
  const [values, setValues] = useState<Record<string, any>>(initial);
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState('');
  const set = (key: string, value: any) => setValues((current) => ({ ...current, [key]: value }));
  const submit = async (event: FormEvent) => { event.preventDefault(); if (!values.name?.trim()) { setError('La razón social es obligatoria.'); return; } setSaving(true); setError(''); try { await superadminService.updateOperatingCompany(values); onSaved(); } catch (err) { setError(formErrorMessage(err, 'No se han podido guardar los datos de empresa.')); } finally { setSaving(false); } };
...
function OperationalProfilePage({ profileId, role }: { profileId: string; role: 'Tecnico' | 'Comercial' }) {
  ...
  const canManage = canManageWorkOrderAssignments(profile);
  ...
  return <section className="page"><BackButton /><Hero title={`${role === 'Tecnico' ? 'Ficha técnico' : 'Ficha comercial'} · ${fullName(person)}`} subtitle={`${person.email ?? '-'} · ${person.phone ?? '-'} · ${person.active ? 'Activo' : 'Inactivo'}`} tone={role === 'Tecnico' ? 'maintenance' : 'commercial'} /><div className="actions">{canManage && <button className="primary" onClick={() => setAssigning(true)}>{role === 'Tecnico' ? 'Asignar parte' : 'Asignar visita/parte'}</button>}<Link to="/app/partes">Ver partes</Link><Link to="/app/checks">Ver checks</Link></div><div className="stats-grid"><Card title="Datos"><InfoGrid items={[[ 'Nombre', fullName(person) ], [ 'Rol', areaLabel(person.primary_area) ], [ 'Email', person.email ?? '-' ], [ 'Teléfono', person.phone ?? '-' ], [ 'Estado', person.active ? 'Activo' : 'Inactivo' ]]} /></Card><Card title="Carga de trabajo"><InfoGrid items={[[ 'Partes asignados', assignedWorks.length ], [ 'Abiertos', openWorks.length ], [ 'Hoy', activeToday.length ], [ 'Checks pendientes', assignedChecks.length ], [ 'Disponibilidad', !person.active ? 'Fuera de servicio' : openWorks.some((work: any) => ['En desplazamiento','En intervencion','Pausado'].includes(work.status)) ? 'Ocupado' : 'Disponible' ]]} /></Card></div><Card title={role === 'Tecnico' ? 'Partes asignados' : 'Visitas y partes comerciales'}><div className="compact-list">{assignedWorks.map((work: any) => <article key={work.id}><Badge tone={severityForStatus(work.status)}>{work.code}</Badge><p>{work.title} · {work.client_name ?? '-'} · {displayStatus(work.status)} · {work.scheduled_date ?? 'Sin fecha'}</p><div className="row-actions"><Link to={`/app/partes/${work.id}`}>Abrir parte</Link>{canManage && <Link to={`/app/partes/${work.id}`}>Gestionar asignaciones</Link>}</div></article>)}{!assignedWorks.length && <p className="large-note">Sin partes asignados.</p>}</div></Card><Card title="Checks asignados"><CompactRows rows={assignedChecks.map((check: any) => [check.code, `${check.equipment_code ?? 'Equipo'} · ${displayStatus(check.status)} · ${displayStatus(check.global_result)}`, severityForStatus(check.global_result), `/app/checks/${check.id}`])} empty="Sin checks pendientes." /></Card>{assigning && <OperationalAssignForm role={role} profileId={profileId} workOrders={works.data} onClose={() => setAssigning(false)} onSaved={() => { setAssigning(false); works.reload(); }} />}</section>;
}
...
function SuperadminUserForm({ initial, onClose, onSaved }: any) {
  ...
  return <ModalForm title={initial?.id ? 'Editar usuario' : 'Crear usuario'} onClose={onClose} onSubmit={submit} saving={saving} error={error} submitLabel={initial?.id ? 'Guardar usuario' : 'Crear perfil'}><p className="large-note">No se guarda ni se muestra ninguna contraseña. Indica el identificador de acceso cuando el usuario ya esté dado de alta.</p><p className="large-note">SAT y Comercial son incompatibles en el mismo perfil. Al seleccionar uno se desmarca el otro.</p><div className="form-grid"><label>Nombre *<input value={values.first_name ?? ''} onChange={(event) => set('first_name', event.target.value)} required /></label><label>Apellidos *<input value={values.last_name ?? ''} onChange={(event) => set('last_name', event.target.value)} required /></label><label>Email *<input type="email" value={values.email ?? ''} onChange={(event) => set('email', event.target.value)} required /></label><label>Teléfono<input value={values.phone ?? ''} onChange={(event) => set('phone', event.target.value)} /></label><label>Identificador de acceso<input value={values.auth_user_id ?? ''} onChange={(event) => set('auth_user_id', event.target.value)} placeholder="Identificador del usuario" /></label><FormSelect label="Rol principal" value={values.primary_area} onChange={(value) => set('primary_area', value)} required options={roleOptions.map((value) => ({ value, label: value === 'superadmin' ? 'Propietario DMP' : value }))} /><FormSelect label="Estado" value={String(values.active)} onChange={(value) => set('active', value)} options={[{ value: 'true', label: 'Activo' }, { value: 'false', label: 'Inactivo' }]} /></div><Card title="Roles adicionales"><div className="component-select">{roleOptions.map((role) => <label key={role}><input type="checkbox" checked={values.roles.includes(role)} onChange={() => toggleRole(role)} /> {role === 'superadmin' ? 'Propietario DMP' : role}</label>)}</div></Card><Card title="Vinculaciones"><p className="large-note">Vincula el perfil con su ficha de técnico o comercial cuando corresponda. Los roles seleccionados determinan qué áreas puede usar.</p></Card><p className="state-warning"><ShieldAlert size={16} />Contraseña protegida. No se mostrará después de crear, no se guarda en tablas públicas y no se registra en consola.</p></ModalForm>;
}
...
function ComponentForm({ equipmentId, initial, onClose, onSaved }: any) { return <EntityForm title={initial?.id ? 'Editar componente' : 'Añadir componente'} fields={[[ 'component_type','Tipo',true ],[ 'brand','Marca' ],[ 'model','Modelo' ],[ 'serial_number','Serie' ],[ 'installed_at','Fecha instalación' ],[ 'status','Estado' ],[ 'notes','Observaciones' ]]} initial={{ status: 'Operativo', ...initial }} errorMessage={initial?.id ? 'No se ha podido modificar el componente.' : 'No se ha podido crear el componente.'} onClose={onClose} onSubmit={(values: any) => initial?.id ? equipmentService.updateComponent(initial.id, values) : equipmentService.addComponent(equipmentId, values)} onSaved={onSaved} />; }
...
function MultiEquipmentPicker({ values, setValues, filteredEquipment, equipmentTypes, installation: initialInstallation }: any) {
  ...
  const update = (next: any[]) => setValues((current: any) => ({ ...current, equipment_selection: next, main_equipment_id: next.find((item) => item.kind === 'existing')?.equipment_id ?? current.main_equipment_id ?? '' }));
  const addExisting = () => { if (!existingId || selection.some((item: any) => item.kind === 'existing' && item.equipment_id === existingId)) return; update([...selection, { kind: 'existing', equipment_id: existingId }]); setExistingId(''); };
  const addNew = () => { const quantity = Number(draft.quantity); if (!draft.equipment_type_id || !Number.isInteger(quantity) || quantity < 1) return; update([...selection, ...Array.from({ length: quantity }, () => ({ kind: 'new', ...draft, quantity: undefined }))]); setDraft({ equipment_type_id: '', quantity: 1, brand: '', model: '', internal_location: '', serial_number: '', notes: '' }); };
  const remove = (index: number) => update(selection.filter((_: any, itemIndex: number) => itemIndex !== index));
...
function MaterialBulkImportPanel() {
  const [fileName, setFileName] = useState('');
  const [preview, setPreview] = useState<string[][]>([]);
  const readFile = async (file?: File) => {
    if (!file) return;
    const rows = (await file.text()).split(/\r?\n/).filter(Boolean).slice(0, 6).map((line) => line.split(',').map((cell) => cell.trim()));
    setFileName(file.name);
    setPreview(rows);
  };
  return <Card title="Importación masiva" action={<FileUp {...iconProps} />}><p className="large-note">Carga catálogo, coste, precio, referencia y stock inicial para una futura implantación. La importación operativa registrará el stock por almacén en <code>warehouse_stock</code> y su trazabilidad en <code>stock_movements</code>.</p><label className="file-input">Archivo CSV<input id="material-bulk-import" type="file" accept=".csv,text/csv" onChange={(event) => { void readFile(event.target.files?.[0]); }} /></label>{fileName && <><p className="success-note">Archivo seleccionado: {fileName}. Vista previa limitada a 5 filas.</p><div className="table-card"><table><tbody>{preview.map((row, index) => <tr key={`${fileName}-${index}`}>{row.map((cell, cellIndex) => <td key={`${index}-${cellIndex}`}>{cell}</td>)}</tr>)}</tbody></table></div></>}<p className="large-note">La escritura definitiva requiere validación de columnas, almacén de destino, duplicados y movimientos idempotentes; no se ejecuta parcialmente desde esta vista.</p></Card>;
}
...
function CanonicalMaterialsModule() {
  ...
  const stats = materialDashboardStats(data, stock.data);
...
function MaterialSupplierRow({ row, onChanged }: { row: any; onChanged: () => void }) {
  const [reference, setReference] = useState(row.supplier_reference ?? '');
  const [price, setPrice] = useState(String(row.purchase_unit_price ?? ''));
  const [saving, setSaving] = useState(false);
  const save = async () => { setSaving(true); try { await suppliersService.updateMaterialSupplier(row.id, { supplier_reference: reference, purchase_unit_price: price, is_preferred: row.is_preferred }); onChanged(); } finally { setSaving(false); } };
  return <div className="material-supplier-row"><strong>{row.suppliers?.name ?? 'Proveedor'}</strong><span>{reference || 'Sin referencia'}</span><span>{price === '' ? 'Sin precio' : `${Number(price).toLocaleString('es-ES', { minimumFractionDigits: 2 })} €`}</span><span>{row.is_preferred ? <Badge tone="ok">Preferente</Badge> : 'No'}</span><span>{row.active === false ? 'Inactivo' : 'Activo'}</span><div className="row-actions"><button type="button" onClick={save} disabled={saving}>{saving ? 'Guardando...' : 'Editar relación'}</button><button type="button" onClick={async () => { await suppliersService.setPreferredMaterialSupplier(row.id, row.material_id); onChanged(); }} disabled={saving || row.is_preferred}>Preferente</button><button type="button" onClick={async () => { await suppliersService.deactivateMaterialSupplier(row.id); onChanged(); }}>Desvincular</button></div></div>;
}

function MaterialMovementsView({ material, onClose }: { material: any; onClose: () => void }) {
  const movements = useLoad(() => materialsService.movements(material.id), [material.id], [] as any[]);
  const warehouses = useLoad(() => workOrdersService.warehousesCatalog(), [], [] as any[]);
  const [type, setType] = useState('all'); const [warehouseId, setWarehouseId] = useState('all'); const [search, setSearch] = useState('');
  const filtered = movements.data.filter((row: any) => { const origin: any = movementOrigin(row); const haystack = [origin.text, row.notes, row.source_reference, row.work_orders?.code, row.purchase_receipts?.code, row.purchase_orders?.code].filter(Boolean).join(' ').toLowerCase(); return (type === 'all' || row.movement_type === type || movementDirection(row.movement_type) === type) && (warehouseId === 'all' || row.warehouse_id === warehouseId) && (!search.trim() || haystack.includes(search.trim().toLowerCase())); });
  const visible = filtered.slice(0, 80); const formatMovementDate = (value: string) => value ? new Date(value).toLocaleString('es-ES', { dateStyle: 'short', timeStyle: 'short' }) : '-';
  return <SidePanel title={`Movimientos · ${material.code}`} subtitle={material.description} onClose={onClose}><div className="movement-view-toolbar"><select value={type} onChange={(event) => setType(event.target.value)}><option value="all">Todos los tipos</option><option value="Entrada">Entradas</option><option value="Salida">Salidas</option><option value="Consumo en parte">Consumos</option><option value="Ajuste">Ajustes</option></select><select value={warehouseId} onChange={(event) => setWarehouseId(event.target.value)}><option value="all">Todos los almacenes</option>{warehouses.data.map((item: any) => <option key={item.id} value={item.id}>{item.name}</option>)}</select><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar documento o motivo" /></div><StateBlock loading={movements.loading} error={movements.error} retry={movements.reload} empty={!visible.length}><div className="movement-view-table"><table><thead><tr><th>Fecha</th><th>Tipo</th><th>Cantidad</th><th>Almacén</th><th>Origen</th><th>Usuario</th><th>Motivo</th><th>Acción</th></tr></thead><tbody>{visible.map((row: any) => { const origin: any = movementOrigin(row); const quantity = movementDisplayQuantity(row.movement_type, row.quantity); const direction = movementDirection(row.movement_type); const actor = row.profiles ? fullName(row.profiles) : row.created_by ? 'Usuario no informado' : 'Sistema'; return <tr key={row.id}><td data-label="Fecha">{formatMovementDate(row.created_at)}</td><td data-label="Tipo"><Badge tone={direction === 'Salida' ? 'warn' : direction === 'Entrada' ? 'ok' : 'info'}>{movementLabel(row.movement_type)}</Badge></td><td data-label="Cantidad"><strong className={quantity < 0 ? 'movement-negative' : 'movement-positive'}>{quantity > 0 ? '+' : ''}{quantity.toLocaleString('es-ES')} {material.unit ?? 'ud'}</strong></td><td data-label="Almacén">{row.warehouses?.name ?? 'Almacén no informado'}</td><td data-label="Origen">{origin.text}</td><td data-label="Usuario">{actor}</td><td data-label="Motivo" className="movement-reason" title={movementReason(row)}>{movementReason(row)}</td><td data-label="Acción"><div className="movement-actions">{origin.to && <Link className="record-open" to={origin.to}>Abrir</Link>}<details><summary>Detalle</summary><small>{[row.source, row.source_reference, row.idempotency_key, row.id].filter(Boolean).join(' · ') || 'Sin metadatos técnicos'}</small></details></div></td></tr>; })}</tbody></table></div></StateBlock></SidePanel>;
}
...
function PurchaseOrderForm({ initial, suppliers, warehouses, onClose, onSaved }: any) {
  const [values, setValues] = useState<Record<string, any>>({ order_date: new Date().toISOString().slice(0, 10), supplier_id: '', destination_warehouse_id: '', internal_reference: '', supplier_reference: '', notes: '', ...initial });
  const [saving, setSaving] = useState(false); const [error, setError] = useState('');
  const set = (key: string, value: any) => setValues((current) => ({ ...current, [key]: value }));
  const submit = async (event: FormEvent) => { event.preventDefault(); if (!values.supplier_id) { setError('Selecciona un proveedor.'); return; } setSaving(true); setError(''); try { const saved = initial?.id ? await purchaseOrdersService.update(initial.id, values) : await purchaseOrdersService.create(values); onSaved(initial?.id ?? saved); } catch (err) { setError(formErrorMessage(err, 'No se ha podido guardar el pedido de compra.')); } finally { setSaving(false); } };
  return <ModalForm title={initial?.id ? 'Editar pedido de compra' : 'Nuevo pedido de compra'} onClose={onClose} onSubmit={submit} saving={saving} error={error}><div className="form-grid"><FormSelect label="Proveedor" value={values.supplier_id} onChange={(value) => set('supplier_id', value)} required options={[{ value: '', label: 'Selecciona proveedor' }, ...suppliers.map((item: any) => ({ value: item.id, label: item.name }))]} /><label>Fecha pedido<input type="date" value={values.order_date ?? ''} onChange={(event) => set('order_date', event.target.value)} required /></label><FormSelect label="Almacén previsto" value={values.destination_warehouse_id ?? ''} onChange={(value) => set('destination_warehouse_id', value)} options={[{ value: '', label: 'Sin almacén previsto' }, ...warehouses.map((item: any) => ({ value: item.id, label: `${item.code} · ${item.name}` }))]} /></div><div className="form-grid"><label>Referencia interna<input value={values.internal_reference ?? ''} onChange={(event) => set('internal_reference', event.target.value)} /></label><label>Referencia proveedor<input value={values.supplier_reference ?? ''} onChange={(event) => set('supplier_reference', event.target.value)} /></label></div><label>Observaciones<textarea value={values.notes ?? ''} onChange={(event) => set('notes', event.target.value)} /></label></ModalForm>;
}

function MaterialSupplierLinkForm({ order, material, onCancel, onSaved }: { order: any; material: any; onCancel: () => void; onSaved: (relation: any) => void }) {
  const [reference, setReference] = useState('');
  const [price, setPrice] = useState('');
  const [preferred, setPreferred] = useState(false);
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState('');
  const submit = async () => { setSaving(true); setError(''); try { const relation = await suppliersService.addMaterialSupplier(material.id, { supplier_id: order.supplier_id, supplier_reference: reference, purchase_unit_price: price, is_preferred: preferred }); onSaved(relation); } catch (err) { setError(formErrorMessage(err, 'No se ha podido vincular el material con el proveedor.')); } finally { setSaving(false); } };
  return <Card title="Vincular material con proveedor"><div className="form-grid"><label>Proveedor<input value={order.suppliers?.name ?? 'Proveedor del pedido'} disabled /></label><label>Material<input value={`${material.code ?? '-'} · ${material.description}`} disabled /></label><label>Referencia del proveedor<input value={reference} onChange={(event) => setReference(event.target.value)} /></label><label>Precio habitual de compra<input type="number" min="0" step="0.01" value={price} onChange={(event) => setPrice(event.target.value)} /></label></div><label className="checkbox-line"><input type="checkbox" checked={preferred} onChange={(event) => setPreferred(event.target.checked)} /> Marcar como proveedor preferente</label>{error && <p className="form-error">{error}</p>}<div className="modal-footer"><button type="button" onClick={onCancel} disabled={saving}>Cancelar</button><button type="button" className="primary" onClick={submit} disabled={saving}>{saving ? 'Guardando...' : 'Guardar relación y continuar'}</button></div></Card>;
}

function PurchaseOrderLineForm({ order, materials, initial, onClose, onSaved }: any) {
  const [values, setValues] = useState<Record<string, any>>({ material_id: initial?.material_id ?? '', material_supplier_id: initial?.material_supplier_id ?? '', ordered_quantity: initial?.ordered_quantity == null ? '1' : String(initial.ordered_quantity), unit_purchase_price: initial?.unit_purchase_price == null ? '' : String(initial.unit_purchase_price) });
  const [saving, setSaving] = useState(false); const [error, setError] = useState('');
  const relations = useLoad(() => values.material_id ? suppliersService.listMaterialSuppliers(values.material_id) : Promise.resolve([]), [values.material_id], [] as any[]);
  const matching = relations.data.filter((row: any) => row.supplier_id === order.supplier_id && row.active === true);
  const selected = matching.find((row: any) => row.id === values.material_supplier_id);
  const selectedMaterial = materials.find((item: any) => item.id === values.material_id);
  const [linking, setLinking] = useState(false);
  const set = (key: string, value: any) => setValues((current) => ({ ...current, [key]: value }));
  const priceResolution = resolvePurchaseOrderPrice(relations.data, order.supplier_id);
  useEffect(() => { if (!initial?.id && values.material_id && !relations.loading && !priceResolution.ambiguous && priceResolution.relation && !values.material_supplier_id) setValues((current) => ({ ...current, material_supplier_id: priceResolution.relation.id, unit_purchase_price: priceResolution.unitPurchasePrice })); }, [initial?.id, values.material_id, values.material_supplier_id, relations.loading, priceResolution.ambiguous, priceResolution.relation?.id, priceResolution.unitPurchasePrice]);
  const selectMaterial = (value: string) => setValues((current) => ({ ...current, material_id: value, material_supplier_id: '', ...(initial?.id ? {} : { unit_purchase_price: '' }) }));
  const selectRelation = (value: string) => { const relation = matching.find((row: any) => row.id === value); setValues((current) => ({ ...current, material_supplier_id: value, unit_purchase_price: relation?.purchase_unit_price == null ? '' : String(relation.purchase_unit_price) })); };
  const submit = async (event: FormEvent) => { event.preventDefault(); if (!values.material_id) { setError('Selecciona un material.'); return; } setSaving(true); setError(''); try { if (initial?.id) await purchaseOrdersService.updateLine(initial.id, values); else await purchaseOrdersService.addLine(order.id, values); onSaved(); } catch (err) { setError(formErrorMessage(err, 'No se ha podido guardar la línea.')); } finally { setSaving(false); } };
  return <ModalForm title={initial?.id ? 'Editar línea del pedido' : 'Añadir línea al pedido'} onClose={onClose} onSubmit={submit} saving={saving} error={error}><FormSelect label="Material" value={values.material_id} onChange={selectMaterial} required options={[{ value: '', label: 'Selecciona material' }, ...materials.map((item: any) => ({ value: item.id, label: `${item.code} · ${item.description}` }))]} loading={relations.loading} /><FormSelect label="Relación proveedor-material" value={values.material_supplier_id ?? ''} onChange={selectRelation} options={[{ value: '', label: matching.length ? 'Línea manual' : 'Sin relación para este proveedor' }, ...matching.map((row: any) => ({ value: row.id, label: `${row.suppliers?.name ?? 'Proveedor'} · ${row.supplier_reference ?? 'Sin referencia'} · ${row.purchase_unit_price == null ? 'Precio no configurado' : `${row.purchase_unit_price} €`}` }))]} />{values.material_id && !matching.length && <div className="state-warning"><strong>Este material no está vinculado a {order.suppliers?.name ?? 'este proveedor'}.</strong><p>La vinculación permitirá reutilizar referencia y precio habitual en futuros pedidos.</p><div className="row-actions"><button type="button" onClick={() => setLinking(true)} disabled={linking}>Vincular material con este proveedor</button><span className="large-note">También puedes continuar como línea manual. Se permite una línea manual; la relación no se creará automáticamente.</span></div></div>}{selected && <p className="large-note">{selected.suppliers?.name ?? order.suppliers?.name ?? 'Proveedor'} · Referencia: {selected.supplier_reference ?? 'Sin referencia'} · Precio habitual: {selected.purchase_unit_price == null ? 'No configurado' : `${selected.purchase_unit_price} €`}. El precio acordado se copia al pedido y puede editarse.</p>}<div className="form-grid"><label>Cantidad<input type="number" min="0.01" step="0.01" value={values.ordered_quantity} onChange={(event) => set('ordered_quantity', event.target.value)} required /></label><label>Precio acordado<input type="number" min="0" step="0.01" value={values.unit_purchase_price} onChange={(event) => set('unit_purchase_price', event.target.value)} placeholder="Obligatorio al pedir" /></label></div>{linking && selectedMaterial && <MaterialSupplierLinkForm order={order} material={selectedMaterial} onCancel={() => setLinking(false)} onSaved={(relation: any) => { setValues((current) => ({ ...current, material_supplier_id: relation.id, unit_purchase_price: relation.purchase_unit_price == null ? '' : String(relation.purchase_unit_price) })); setLinking(false); relations.reload(); }} />}</ModalForm>;
}

function LegacyPurchaseReceiptForm({ order, warehouses, onClose, onSaved }: any) {
  const initialLines = order.purchase_receipt_lines ?? [];
  const [values, setValues] = useState<Record<string, any>>({ receipt_date: order.receipt_date ?? new Date().toISOString().slice(0, 10), warehouse_id: order.warehouse_id ?? order.destination_warehouse_id ?? warehouses[0]?.id ?? '', supplier_document_reference: order.supplier_document_reference ?? '', notes: order.notes ?? '' });
  const lines = order.purchase_order_lines ?? [];
  const existingByOrderLine = Object.fromEntries(initialLines.map((line: any) => [line.purchase_order_line_id, line]));
  const [quantities, setQuantities] = useState<Record<string, string>>(() => Object.fromEntries(initialLines.map((line: any) => [line.purchase_order_line_id, String(line.received_quantity ?? '')])));
  const [costs, setCosts] = useState<Record<string, string>>(() => Object.fromEntries(lines.map((line: any) => [line.id, initialReceiptCost(line, existingByOrderLine[line.id])] )));
  const [saving, setSaving] = useState(false); const [error, setError] = useState('');
  const set = (key: string, value: any) => setValues((current) => ({ ...current, [key]: value }));
  const receivedBefore = (line: any) => (order.purchase_receipts ?? []).filter((receipt: any) => receipt.status === 'confirmed').flatMap((receipt: any) => receipt.purchase_receipt_lines ?? []).filter((item: any) => item.purchase_order_line_id === line.id).reduce((sum: number, item: any) => sum + Number(item.received_quantity ?? 0), 0);
  const progress = lines.map((line: any) => receiptLineProgress(line, receivedBefore(line)));
  const save = async (confirmAfter: boolean) => { const selected = lines.filter((line: any, index: number) => progress[index].pending > 0 && Number(quantities[line.id] ?? 0) > 0); if (!values.warehouse_id || (!selected.length && !order.id)) { setError('Selecciona almacén y al menos una línea con cantidad.'); return; } if (lines.some((line: any, index: number) => { const value = Number(quantities[line.id] ?? 0); return value < 0 || value > progress[index].pending; })) { setError('La cantidad a recibir debe estar entre cero y la cantidad pendiente.'); return; } setSaving(true); setError(''); try { const id = order.purchase_receipt_id ?? await purchaseReceiptsService.createDraft(order.id, values); if (order.purchase_receipt_id) await purchaseReceiptsService.updateDraft(id, values); for (const line of lines) { const existing = existingByOrderLine[line.id]; const quantity = Number(quantities[line.id] ?? 0); if (existing && quantity <= 0) await purchaseReceiptsService.removeLine(existing.id); else if (quantity > 0) { const payload = { purchase_order_line_id: line.id, received_quantity: quantity, actual_unit_cost: costs[line.id] === '' ? null : costs[line.id] }; if (existing) await purchaseReceiptsService.updateLine(existing.id, payload); else await purchaseReceiptsService.addLine(id, payload); } } if (confirmAfter) await purchaseReceiptsService.confirm(id); onSaved(); } catch (err) { setError(formErrorMessage(err, 'No se ha podido guardar la recepción.')); } finally { setSaving(false); } };
  useOverlayScrollLock();
  const pendingUnits = progress.reduce((sum, item) => sum + item.pending, 0); const receiveUnits = lines.reduce((sum, line) => sum + Math.max(0, Number(quantities[line.id] ?? 0)), 0); const materialLabel = (line: any) => [line.materials?.code ?? line.material_code, line.material_description_snapshot].filter(Boolean).join(' · ') || 'Material no informado';
  return <div className="mini-modal receipt-modal" role="dialog" aria-modal="true" onWheel={(event) => event.stopPropagation()} onTouchMove={(event) => event.stopPropagation()}><form onSubmit={(event) => { event.preventDefault(); void save(false); }}><h3>Registrar recepción · {order.code}</h3><div className="receipt-header-grid"><label>Fecha<input type="date" value={values.receipt_date} onChange={(event) => set('receipt_date', event.target.value)} required /></label><FormSelect label="Almacén real" value={values.warehouse_id} onChange={(value) => set('warehouse_id', value)} options={[{ value: '', label: 'Selecciona almacén' }, ...warehouses.map((warehouse: any) => ({ value: warehouse.id, label: `${warehouse.code} · ${warehouse.name}` }))]} /><label>Referencia / albarán proveedor<input value={values.supplier_document_reference} onChange={(event) => set('supplier_document_reference', event.target.value)} /></label></div><label>Observaciones<textarea value={values.notes} onChange={(event) => set('notes', event.target.value)} /></label><div className="receipt-toolbar"><strong>{lines.length} líneas pendientes: {pendingUnits} uds</strong><span>A recibir ahora: {receiveUnits} uds</span><button type="button" onClick={() => setQuantities(Object.fromEntries(lines.map((line: any, index: number) => [line.id, String(progress[index].pending)])))} disabled={saving}>Recibir todo lo pendiente</button><button type="button" onClick={() => setQuantities({})} disabled={saving}>Limpiar cantidades</button></div><div className="receipt-lines-scroll"><table className="receipt-lines-table"><thead><tr><th>Material</th><th>Pedido</th><th>Recibido</th><th>Pendiente</th><th>Recibir ahora</th><th>Precio acordado</th><th>Coste real</th></tr></thead><tbody>{lines.map((line: any, index: number) => { const item = progress[index]; const complete = item.pending === 0; return <tr key={line.id} className={complete ? 'receipt-line-complete' : ''}><td><strong>{materialLabel(line)}</strong></td><td>{item.ordered}</td><td>{item.received}</td><td>{item.pending}</td><td>{complete ? <span className="receipt-complete">Completo</span> : <input aria-label={`Recibir ahora ${materialLabel(line)}`} className="receipt-number" type="number" min="0" max={item.pending} step="0.01" value={quantities[line.id] ?? ''} onChange={(event) => setQuantities((current) => ({ ...current, [line.id]: event.target.value }))} />}</td><td>{line.unit_purchase_price == null ? '-' : `${Number(line.unit_purchase_price).toLocaleString('es-ES', { minimumFractionDigits: 2 })} €`}</td><td>{complete ? <span>{costs[line.id] ? `${Number(costs[line.id]).toLocaleString('es-ES', { minimumFractionDigits: 2 })} €` : '-'}</span> : <input aria-label={`Coste real ${materialLabel(line)}`} className="receipt-number" type="number" min="0" step="0.01" value={costs[line.id] ?? ''} onChange={(event) => setCosts((current) => ({ ...current, [line.id]: event.target.value }))} />}</td></tr>; })}</tbody></table></div><p className="large-note receipt-help">Se propone el precio acordado o habitual. Puedes modificarlo para esta recepción. Guardar borrador no genera stock; confirmar sí genera stock real. Las recepciones confirmadas quedan congeladas.</p>{error && <p className="form-error">{error}</p>}<div className="modal-footer"><button type="button" onClick={onClose} disabled={saving}>Cancelar</button><button type="button" onClick={() => void save(false)} disabled={saving}>{saving ? 'Guardando...' : 'Guardar borrador'}</button><button type="button" className="primary" onClick={() => void save(true)} disabled={saving || !receiveUnits}>{saving ? 'Guardando...' : 'Confirmar recepción'}</button></div></form></div>;
}

function PurchaseReceiptForm({ order, warehouses, onClose, onSaved }: any) {
  const currentReceipt = order.current_receipt ?? order.purchase_receipts?.find((receipt: any) => receipt.status === 'draft') ?? order.purchase_receipts?.[0];
  const receiptOrder = { ...order, supplier_document_reference: currentReceipt?.supplier_document_reference ?? '' };
  return <div><div className="form-grid"><label>Referencia interna<input value={order.internal_reference ?? ''} readOnly /></label><label>Referencia proveedor<input value={order.supplier_reference ?? ''} readOnly /></label><label>Referencia documento proveedor<input value={receiptOrder.supplier_document_reference ?? ''} readOnly /></label></div><LegacyPurchaseReceiptForm order={receiptOrder} warehouses={warehouses} onClose={onClose} onSaved={onSaved} /></div>;
}

function PurchaseOrderDetail({ id, suppliers, warehouses, materials, canManage, onChanged }: any) {
  const order = useLoad(() => purchaseOrdersService.get(id), [id], null as any);
  const receipts = useLoad(() => purchaseReceiptsService.listForOrder(id), [id], [] as any[]);
  const [edit, setEdit] = useState(false); const [addingLine, setAddingLine] = useState(false); const [editingLine, setEditingLine] = useState<any | null>(null); const [registeringReceipt, setRegisteringReceipt] = useState(false); const [actionError, setActionError] = useState('');
  if (order.loading || !order.data) return <StateBlock loading={order.loading} error={order.error} retry={order.reload} empty={!order.data} />;
  const data = { ...order.data, purchase_receipts: receipts.data }; const internalReference = data.internal_reference; const supplierReference = data.supplier_reference; const lines = data.purchase_order_lines ?? []; const refresh = () => { order.reload(); receipts.reload(); onChanged(); };
  const run = async (action: () => Promise<any>) => { setActionError(''); try { await action(); refresh(); } catch (err) { setActionError(formErrorMessage(err, 'No se ha podido actualizar el pedido.')); } };
  return <Card title={`${data.code} · ${data.suppliers?.name ?? 'Proveedor'}`} action={<Badge tone={data.status === 'ordered' ? 'ok' : data.status === 'cancelled' ? 'muted' : 'warn'}>{displayStatus(data.status)}</Badge>}><InfoGrid items={[[ 'Fecha', data.order_date ], [ 'Almacén previsto', data.warehouses ? `${data.warehouses.code} · ${data.warehouses.name}` : 'Sin especificar' ], [ 'Creado por', fullName(data.profiles) || '-' ], [ 'Total', `${Number(data.total_amount ?? 0).toLocaleString('es-ES')} €` ], [ 'Observaciones', data.notes ?? '-' ]]} /><div className="form-grid"><label>Referencia interna<input value={internalReference ?? '-'} readOnly /></label><label>Referencia proveedor<input value={supplierReference ?? '-'} readOnly /></label></div>{actionError && <p className="form-error">{actionError}</p>}<div className="row-actions">{canManage && ['ordered','partially_received'].includes(data.status) && <button className="primary" onClick={() => setRegisteringReceipt(true)}>Registrar recepción</button>}{canManage && data.status === 'draft' && <><button onClick={() => setEdit(true)}>Editar cabecera</button><button onClick={() => setAddingLine(true)}>Añadir línea</button><button className="primary" onClick={() => run(() => purchaseOrdersService.markOrdered(data.id))}>Marcar pedido</button></>}{canManage && ['draft','ordered'].includes(data.status) && <button onClick={() => run(() => purchaseOrdersService.cancel(data.id))}>Cancelar</button>}</div><div className="compact-list">{lines.map((line: any) => <article key={line.id}><strong>{line.material_description_snapshot}</strong><span>{line.ordered_quantity} {line.unit_snapshot} · {line.unit_purchase_price == null ? 'Precio pendiente' : `${line.unit_purchase_price} €/ud`} · {line.subtotal == null ? 'Subtotal pendiente' : `${line.subtotal} €`}</span><small>Referencia: {line.supplier_reference_snapshot ?? 'Sin referencia'}</small>{canManage && data.status === 'draft' && <><button type="button" onClick={() => setEditingLine(line)}>Editar línea</button><button type="button" onClick={() => run(() => purchaseOrdersService.removeLine(line.id))}>Quitar línea</button></>}</article>)}{!lines.length && <p className="large-note">Sin líneas. Añade al menos una antes de marcar el pedido.</p>}</div><Card title="Recepciones"><div className="compact-list">{receipts.data.map((receipt: any) => <article key={receipt.id}><strong>{receipt.code}</strong><span>{receipt.receipt_date} · {receipt.warehouses?.name ?? '-'} · {displayStatus(receipt.status)}</span><small>Referencia documento proveedor: {receipt.supplier_document_reference ?? 'Sin referencia'} · {receipt.purchase_receipt_lines?.reduce((sum: number, line: any) => sum + Number(line.received_quantity ?? 0), 0) ?? 0} unidades · {receipt.purchase_receipt_lines?.reduce((sum: number, line: any) => sum + Number(line.subtotal ?? 0), 0).toLocaleString('es-ES')} € · {fullName(receipt.profiles) || '-'}</small></article>)}{!receipts.loading && !receipts.data.length && <p className="large-note">Sin recepciones registradas.</p>}</div></Card>{edit && <PurchaseOrderForm initial={data} suppliers={suppliers} warehouses={warehouses} onClose={() => setEdit(false)} onSaved={() => { setEdit(false); refresh(); }} />}{addingLine && <PurchaseOrderLineForm order={data} materials={materials} onClose={() => setAddingLine(false)} onSaved={() => { setAddingLine(false); refresh(); }} />}{editingLine && <PurchaseOrderLineForm order={data} materials={materials} initial={editingLine} onClose={() => setEditingLine(null)} onSaved={() => { setEditingLine(null); refresh(); }} />}{registeringReceipt && <PurchaseReceiptForm order={{ ...data, purchase_receipts: receipts.data }} warehouses={warehouses} onClose={() => setRegisteringReceipt(false)} onSaved={() => { setRegisteringReceipt(false); refresh(); }} />}</Card>;
}

function LegacyPurchaseOrdersModule() {
  const { profile } = useAuth(); const canView = canViewPurchaseOrders(profile); const canManage = canManagePurchaseOrders(profile);
  const [filters, setFilters] = useState<any>({ status: '', supplierId: '', warehouseId: '', from: '', to: '' }); const [search, setSearch] = useState(''); const [selectedId, setSelectedId] = useState<string | null>(null); const [creating, setCreating] = useState(false);
  const orders = useLoad(() => canView ? purchaseOrdersService.list({ ...filters, search }) : Promise.resolve([]), [canView, search, filters.status, filters.supplierId, filters.warehouseId, filters.from, filters.to], [] as any[]);
  const suppliers = useLoad(() => suppliersService.list('', 'active'), [], [] as any[]); const warehouses = useLoad(() => workOrdersService.warehousesCatalog(), [], [] as any[]); const materials = useLoad(() => materialsService.list('', undefined, 'active'), [], [] as any[]);
  if (!canView) return <section className="page"><Card title="Pedidos de compra"><p className="large-note">No tienes permisos para consultar pedidos de compra.</p></Card></section>;
  const setFilter = (key: string, value: string) => setFilters((current: any) => ({ ...current, [key]: value }));
  return <section className="page"><Breadcrumb items={['Oficina', 'Compras']} /><Hero title="Pedidos de compra" subtitle="Pedidos a proveedores, precios acordados y trazabilidad previa a la recepción." tone="commercial" /><div className="filters sat-filters"><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar código o referencia" /><FormSelect label="Estado" value={filters.status} onChange={(value) => setFilter('status', value)} options={[{ value: '', label: 'Todos' }, ...purchaseOrderStatuses.map((value) => ({ value, label: displayStatus(value) }))]} /><FormSelect label="Proveedor" value={filters.supplierId} onChange={(value) => setFilter('supplierId', value)} options={[{ value: '', label: 'Todos' }, ...suppliers.data.map((item: any) => ({ value: item.id, label: item.name }))]} /><FormSelect label="Almacén" value={filters.warehouseId} onChange={(value) => setFilter('warehouseId', value)} options={[{ value: '', label: 'Todos' }, ...warehouses.data.map((item: any) => ({ value: item.id, label: `${item.code} · ${item.name}` }))]} /><label>Desde<input type="date" value={filters.from} onChange={(event) => setFilter('from', event.target.value)} /></label><label>Hasta<input type="date" value={filters.to} onChange={(event) => setFilter('to', event.target.value)} /></label>{canManage && <button className="primary" onClick={() => setCreating(true)}>Nuevo pedido</button>}</div><StateBlock loading={orders.loading} error={orders.error} retry={orders.reload} empty={!orders.data.length}><div className="grid half">{orders.data.map((order: any) => <Card key={order.id} title={order.code} action={<Badge tone={order.status === 'ordered' ? 'ok' : order.status === 'cancelled' ? 'muted' : 'warn'}>{displayStatus(order.status)}</Badge>}><button className="link-button" onClick={() => setSelectedId(order.id)}>Abrir pedido</button><InfoGrid items={[[ 'Proveedor', order.suppliers?.name ?? '-' ], [ 'Fecha', order.order_date ], [ 'Almacén', order.warehouses?.name ?? '-' ], [ 'Total', `${Number(order.total_amount ?? 0).toLocaleString('es-ES')} €` ], [ 'Creado por', fullName(order.profiles) || '-' ]]} /></Card>)}</div></StateBlock>{creating && <PurchaseOrderForm suppliers={suppliers.data} warehouses={warehouses.data} onClose={() => setCreating(false)} onSaved={(id: string) => { setCreating(false); setSelectedId(id); orders.reload(); }} />}{selectedId && <PurchaseOrderDetail id={selectedId} suppliers={suppliers.data} warehouses={warehouses.data} materials={materials.data} canManage={canManage} onChanged={orders.reload} />}</section>;
}

function LegacyContextualPurchaseFlow({ material, orders, suppliers, warehouses, onClose }: any) {
  const navigate = useNavigate();
  const relations = useLoad(() => suppliersService.listMaterialSuppliers(material.id).then((rows) => rows.filter((row: any) => row.active === true)), [material.id], [] as any[]);
  const drafts = orders.filter((order: any) => order.status === 'draft' && !order.archived_at);
  const [mode, setMode] = useState<'new' | 'existing'>('new'); const [supplierId, setSupplierId] = useState(''); const [orderId, setOrderId] = useState(drafts[0]?.id ?? ''); const [quantity, setQuantity] = useState('1'); const [price, setPrice] = useState(''); const [warehouseId, setWarehouseId] = useState(''); const [saving, setSaving] = useState(false); const [error, setError] = useState('');
  const preferredSupplier = relations.data.find((row: any) => row.active === true && row.is_preferred && !resolvePurchaseOrderPrice(relations.data, row.supplier_id).ambiguous);
  const selectedDraft = drafts.find((order: any) => order.id === orderId);
  const effectiveSupplierId = mode === 'new' ? supplierId : selectedDraft?.supplier_id;
  const priceResolution = resolvePurchaseOrderPrice(relations.data, effectiveSupplierId);
  const preferred = priceResolution.relation;
  useEffect(() => { if (mode === 'new' && !supplierId && preferredSupplier) setSupplierId(preferredSupplier.supplier_id); }, [mode, supplierId, preferredSupplier?.id]);
  useEffect(() => { if (!relations.loading) setPrice(priceResolution.unitPurchasePrice); }, [mode, effectiveSupplierId, priceResolution.relation?.id, priceResolution.unitPurchasePrice, relations.loading]);
  const submit = async (event: FormEvent) => { event.preventDefault(); if (mode === 'existing' && !orderId) { setError('Selecciona un borrador editable.'); return; } if (mode === 'new' && !supplierId) { setError('Selecciona un proveedor.'); return; } if (Number(quantity) <= 0) { setError('La cantidad debe ser mayor que cero.'); return; } setSaving(true); setError(''); try { const id = mode === 'new' ? await purchaseOrdersService.create({ supplier_id: supplierId, destination_warehouse_id: warehouseId || null, order_date: new Date().toISOString().slice(0, 10), notes: `Compra iniciada desde ${material.code}` }) : orderId; await purchaseOrdersService.addLine(id, { material_id: material.id, ordered_quantity: quantity, unit_purchase_price: price === '' ? null : price, material_supplier_id: priceResolution.relation?.id ?? null }); onClose(); navigate(`/app/modulos/compras?pedido=${id}`); } catch (err) { setError(formErrorMessage(err, 'No se ha podido preparar la compra.')); } finally { setSaving(false); } };
  return <div className="mini-modal contextual-purchase" role="dialog" aria-modal="true"><form onSubmit={submit}><h3>Comprar material</h3><p className="purchase-context"><strong>{material.code} · {material.description}</strong><span>El material se conservará como línea del pedido.</span></p><div className="tabs"><button type="button" className={mode === 'new' ? 'active' : ''} onClick={() => setMode('new')}>Nuevo borrador</button><button type="button" className={mode === 'existing' ? 'active' : ''} onClick={() => setMode('existing')}>Añadir a borrador</button></div>{mode === 'new' ? <><FormSelect label="Proveedor" value={supplierId} onChange={(value) => { setSupplierId(value); const relation = relations.data.find((row: any) => row.supplier_id === value && row.active !== false); setPrice(relation?.purchase_unit_price == null ? '' : String(relation.purchase_unit_price)); }} options={[{ value: '', label: relations.loading ? 'Cargando proveedores...' : 'Selecciona proveedor' }, ...suppliers.map((item: any) => ({ value: item.id, label: item.name }))]} loading={relations.loading} /><FormSelect label="Almacén previsto" value={warehouseId} onChange={setWarehouseId} options={[{ value: '', label: 'Sin almacén previsto' }, ...warehouses.map((item: any) => ({ value: item.id, label: `${item.code} · ${item.name}` }))]} /></> : <FormSelect label="Borrador editable" value={orderId} onChange={setOrderId} options={[{ value: '', label: drafts.length ? 'Selecciona borrador' : 'No hay borradores' }, ...drafts.map((order: any) => ({ value: order.id, label: `${order.code} · ${order.suppliers?.name ?? 'Sin proveedor'}` }))]} />}{preferred && <p className="large-note">Precio habitual preseleccionado: {Number(preferred.purchase_unit_price ?? 0).toLocaleString('es-ES', { minimumFractionDigits: 2 })} €.</p>}<div className="form-grid"><label>Cantidad<input type="number" min="0.01" step="0.01" value={quantity} onChange={(event) => setQuantity(event.target.value)} required /></label><label>Precio acordado<input type="number" min="0" step="0.01" value={price} onChange={(event) => setPrice(event.target.value)} /></label></div>{error && <p className="form-error">{error}</p>}<div className="modal-footer"><button type="button" onClick={onClose} disabled={saving}>Cancelar</button><button className="primary" disabled={saving || relations.loading}>{saving ? 'Preparando...' : 'Continuar al pedido'}</button></div></form></div>;
}

function ContextualPurchaseFlow({ material, orders, suppliers, warehouses, onClose }: any) {
  const navigate = useNavigate();
  const { profile } = useAuth();
  const relations = useLoad(() => suppliersService.listMaterialSuppliers(material.id), [material.id], [] as any[]);
  const drafts = orders.filter((order: any) => order.status === 'draft' && !order.archived_at);
  const [mode, setMode] = useState<'new' | 'existing'>('new');
  const [supplierId, setSupplierId] = useState('');
  const [orderId, setOrderId] = useState(drafts[0]?.id ?? '');
  ...
  const effectiveSupplierId = mode === 'new' ? supplierId : selectedDraft?.supplier_id;
  const priceResolution = resolvePurchaseOrderPrice(relations.data, effectiveSupplierId);
  const canEditOrder = hasPermission(profile, 'purchase_orders.update');
  useEffect(() => { setInternalReference(mode === 'existing' ? selectedDraft?.internal_reference ?? '' : ''); setSupplierReference(mode === 'existing' ? selectedDraft?.supplier_reference ?? '' : ''); }, [mode, orderId, selectedDraft?.internal_reference, selectedDraft?.supplier_reference]);
  useEffect(() => { if (!relations.loading) setPrice(priceResolution.unitPurchasePrice); }, [effectiveSupplierId, priceResolution.relation?.id, priceResolution.unitPurchasePrice, relations.loading]);
  const submit = async (event: FormEvent) => { event.preventDefault(); if ((mode === 'new' && !supplierId) || (mode === 'existing' && !orderId)) { setError('Completa el proveedor o selecciona un borrador.'); return; } if (Number(quantity) <= 0) { setError('La cantidad debe ser mayor que cero.'); return; } setSaving(true); setError(''); try { const id = mode === 'new' ? await purchaseOrdersService.create({ supplier_id: supplierId, internal_reference: internalReference, supplier_reference: supplierReference, destination_warehouse_id: warehouseId || null, order_date: new Date().toISOString().slice(0, 10), notes: `Compra iniciada desde ${material.code}` }) : orderId; if (mode === 'existing' && !selectedDraft?.internal_reference && internalReference.trim() && canEditOrder) await purchaseOrdersService.update(id, { supplier_id: selectedDraft.supplier_id, order_date: selectedDraft.order_date, destination_warehouse_id: selectedDraft.destination_warehouse_id ?? null, supplier_reference: selectedDraft.supplier_reference, internal_reference: internalReference, notes: selectedDraft.notes }); await purchaseOrdersService.addLine(id, { material_id: material.id, ordered_quantity: quantity, unit_purchase_price: price === '' ? null : price, material_supplier_id: priceResolution.relation?.id ?? null }); onClose(); navigate(`/app/modulos/compras?pedido=${id}`); } catch (err) { setError(formErrorMessage(err, 'No se ha podido preparar la compra.')); } finally { setSaving(false); } };
  return <div className="mini-modal contextual-purchase" role="dialog" aria-modal="true"><form onSubmit={submit}><h3>Comprar material</h3><p className="purchase-context"><strong>{material.code} · {material.description}</strong><span>El material se conservará como línea del pedido.</span></p><div className="tabs"><button type="button" className={mode === 'new' ? 'active' : ''} onClick={() => setMode('new')}>Nuevo borrador</button><button type="button" className={mode === 'existing' ? 'active' : ''} onClick={() => setMode('existing')}>Añadir a borrador</button></div>{mode === 'new' ? <><FormSelect label="Proveedor" value={supplierId} onChange={setSupplierId} options={[{ value: '', label: 'Selecciona proveedor' }, ...suppliers.map((item: any) => ({ value: item.id, label: item.name }))]} /><div className="form-grid"><label>Referencia interna<input value={internalReference} onChange={(event) => setInternalReference(event.target.value)} /></label><label>Referencia proveedor<input value={supplierReference} onChange={(event) => setSupplierReference(event.target.value)} /></label></div></> : <><FormSelect label="Borrador editable" value={orderId} onChange={setOrderId} options={[{ value: '', label: 'Selecciona borrador' }, ...drafts.map((order: any) => ({ value: order.id, label: `${order.code} · ${order.suppliers?.name ?? 'Sin proveedor'} · ${displayStatus(order.status)} · ${order.internal_reference ?? 'Sin referencia interna'}` }))]} />{selectedDraft?.internal_reference ? <p className="large-note">Referencia interna: {selectedDraft.internal_reference}</p> : canEditOrder ? <label>Referencia interna<input value={internalReference} onChange={(event) => setInternalReference(event.target.value)} /></label> : <p className="large-note">El borrador no tiene referencia interna y no tienes permiso para completarla.</p>}{selectedDraft?.supplier_reference && <p className="large-note">Referencia proveedor: {selectedDraft.supplier_reference}</p>}</>}<div className="form-grid"><label>Cantidad<input type="number" min="0.01" step="0.01" value={quantity} onChange={(event) => setQuantity(event.target.value)} required /></label><label>Precio acordado<input type="number" min="0" step="0.01" value={price} onChange={(event) => setPrice(event.target.value)} /></label></div>{error && <p className="form-error">{error}</p>}<div className="modal-footer"><button type="button" onClick={onClose} disabled={saving}>Cancelar</button><button className="primary" disabled={saving}>{saving ? 'Preparando...' : 'Añadir al pedido'}</button></div></form></div>;
}

function LegacyContextualPurchaseFlow013({ material, orders, suppliers, warehouses, onClose }: any) {
  const navigate = useNavigate();
  const { profile } = useAuth();
  const relations = useLoad(() => suppliersService.listMaterialSuppliers(material.id), [material.id], [] as any[]);
  const drafts = orders.filter((order: any) => order.status === 'draft' && !order.archived_at);
  ...
  const [supplierId, setSupplierId] = useState('');
  const [orderId, setOrderId] = useState(drafts[0]?.id ?? '');
  const [internalReference, setInternalReference] = useState(''); const [supplierReference, setSupplierReference] = useState('');
  const reference = internalReference; const setReference = setInternalReference;
  const [quantity, setQuantity] = useState('1');
  const [price, setPrice] = useState('');
  const [warehouseId, setWarehouseId] = useState('');
  const [saving, setSaving] = useState(false);
  ...
  const submit = async (event: FormEvent) => {
    event.preventDefault();
    if (mode === 'existing' && !orderId) { setError('Selecciona un borrador editable.'); return; }
    if (mode === 'new' && !supplierId) { setError('Selecciona un proveedor.'); return; }
    if (Number(quantity) <= 0) { setError('La cantidad debe ser mayor que cero.'); return; }
    setSaving(true); setError('');
    try {
      let id: string;
      if (mode === 'new') {
         id = await purchaseOrdersService.create({ supplier_id: supplierId, internal_reference: internalReference, supplier_reference: supplierReference, destination_warehouse_id: warehouseId || null, order_date: new Date().toISOString().slice(0, 10), notes: `Compra iniciada desde ${material.code}` });
      } else {
        id = orderId;
        if (!selectedDraft?.internal_reference && internalReference.trim() && canEditOrder) {
          await purchaseOrdersService.update(id, { supplier_id: selectedDraft.supplier_id, order_date: selectedDraft.order_date, destination_warehouse_id: selectedDraft.destination_warehouse_id ?? null, supplier_reference: selectedDraft.supplier_reference, internal_reference: internalReference, notes: selectedDraft.notes });
        }
      }
      await purchaseOrdersService.addLine(id, { material_id: material.id, ordered_quantity: quantity, unit_purchase_price: price === '' ? null : price, material_supplier_id: priceResolution.relation?.id ?? null });
      onClose(); navigate(`/app/modulos/compras?pedido=${id}`);
    } catch (err) { setError(formErrorMessage(err, 'No se ha podido preparar la compra.')); } finally { setSaving(false); }
  };
  return <div className="mini-modal contextual-purchase" role="dialog" aria-modal="true"><form onSubmit={submit}><h3>Comprar material</h3><p className="purchase-context"><strong>{material.code} · {material.description}</strong><span>El material se conservará como línea del pedido.</span></p><div className="tabs"><button type="button" className={mode === 'new' ? 'active' : ''} onClick={() => setMode('new')}>Nuevo borrador</button><button type="button" className={mode === 'existing' ? 'active' : ''} onClick={() => setMode('existing')}>Añadir a borrador</button></div>{mode === 'new' ? <><FormSelect label="Proveedor" value={supplierId} onChange={(value) => { setSupplierId(value); const resolved = resolvePurchaseOrderPrice(relations.data, value); setPrice(resolved.unitPurchasePrice); }} options={[{ value: '', label: relations.loading ? 'Cargando proveedores...' : 'Selecciona proveedor' }, ...suppliers.map((item: any) => ({ value: item.id, label: item.name }))]} loading={relations.loading} /><label>Referencia de compra<input value={reference} onChange={(event) => setReference(event.target.value)} /></label><FormSelect label="Almacén previsto" value={warehouseId} onChange={setWarehouseId} options={[{ value: '', label: 'Sin almacén previsto' }, ...warehouses.map((item: any) => ({ value: item.id, label: `${item.code} · ${item.name}` }))]} /></> : <><FormSelect label="Borrador editable" value={orderId} onChange={setOrderId} options={[{ value: '', label: drafts.length ? 'Selecciona borrador' : 'No hay borradores' }, ...drafts.map((order: any) => ({ value: order.id, label: `${order.code} · ${order.suppliers?.name ?? 'Sin proveedor'}` }))]} />{selectedDraft?.supplier_reference ? <p className="large-note">Referencia de compra: {selectedDraft.supplier_reference}</p> : canEditOrder ? <label>Referencia de compra<input value={reference} onChange={(event) => setReference(event.target.value)} /></label> : <p className="large-note">El borrador no tiene referencia de compra y no tienes permiso para editarla.</p>}</>}{priceResolution.relation && <p className="large-note">Precio habitual preseleccionado: {Number(priceResolution.relation.purchase_unit_price ?? 0).toLocaleString('es-ES', { minimumFractionDigits: 2 })} €.</p>}<div className="form-grid"><label>Cantidad<input type="number" min="0.01" step="0.01" value={quantity} onChange={(event) => setQuantity(event.target.value)} required /></label><label>Precio acordado<input type="number" min="0" step="0.01" value={price} onChange={(event) => setPrice(event.target.value)} /></label></div>{error && <p className="form-error">{error}</p>}<div className="modal-footer"><button type="button" onClick={onClose} disabled={saving}>Cancelar</button><button className="primary" disabled={saving}>{saving ? 'Guardando...' : 'Añadir compra'}</button></div></form></div>;
}

function PurchaseOrdersModule() { return <PurchaseOrdersCompactModule />; }

function PurchaseOrdersCompactModule() {
  const { profile } = useAuth(); const canView = canViewPurchaseOrders(profile); const canManage = canManagePurchaseOrders(profile);
  const [searchParams] = useSearchParams(); const [filters, setFilters] = useState<any>({ status: '', supplierId: '', warehouseId: '', from: '', to: '' }); const [archiveFilter, setArchiveFilter] = useState<'active' | 'archived' | 'all'>('active'); const [search, setSearch] = useState(''); const [selectedId, setSelectedId] = useState<string | null>(() => searchParams.get('pedido')); const [creating, setCreating] = useState(false); const [actionError, setActionError] = useState('');
  useEffect(() => { const orderId = searchParams.get('pedido'); if (orderId) setSelectedId(orderId); }, [searchParams]);
  const orders = useLoad(() => canView ? purchaseOrdersService.list({ ...filters, search, archiveFilter }) : Promise.resolve([]), [canView, search, archiveFilter, filters.status, filters.supplierId, filters.warehouseId, filters.from, filters.to], [] as any[]);
  const suppliers = useLoad(() => suppliersService.list('', 'active'), [], [] as any[]); const warehouses = useLoad(() => workOrdersService.warehousesCatalog(), [], [] as any[]); const materials = useLoad(() => materialsService.list('', undefined, 'active'), [], [] as any[]);
  if (!canView) return <section className="page"><Card title="Pedidos de compra"><p className="large-note">No tienes permisos para consultar pedidos de compra.</p></Card></section>;
  const setFilter = (key: string, value: string) => setFilters((current: any) => ({ ...current, [key]: value }));
  const archive = async (order: any) => { if (!window.confirm(`Archivar el pedido ${order.code}? Se conservará su trazabilidad.`)) return; setActionError(''); try { await purchaseOrdersService.archive(order.id); orders.reload(); } catch (err) { setActionError(formErrorMessage(err, 'No se ha podido archivar el pedido.')); } };
  if (selectedId) return <section className="page"><BackButton /><PurchaseOrderDetail id={selectedId} suppliers={suppliers.data} warehouses={warehouses.data} materials={materials.data} canManage={canManage} onChanged={orders.reload} /></section>;
  const contextualMaterial = materials.data.find((item: any) => item.id === searchParams.get('material'));
  if (canManage && contextualMaterial) return <ContextualPurchaseFlow material={contextualMaterial} orders={orders.data} suppliers={suppliers.data} warehouses={warehouses.data} onClose={() => window.location.assign('/app/modulos/compras')} />;
  const statusOptions = [{ value: '', label: 'Todos los estados' }, { value: 'draft', label: 'Borrador' }, { value: 'ordered', label: 'Emitido' }, { value: 'partially_received', label: 'Recibido parcialmente' }, { value: 'received', label: 'Recibido' }, { value: 'cancelled', label: 'Cancelado' }];
  return <section className="page purchase-orders-page"><Breadcrumb items={['Oficina', 'Compras']} /><Hero title="Pedidos de compra" subtitle="Pedidos a proveedores, precios acordados y trazabilidad previa a la recepción." tone="commercial" /><div className="filters sat-filters"><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar código, referencia o proveedor" /><FormSelect label="Estado" value={filters.status} onChange={(value) => setFilter('status', value)} options={statusOptions} /><FormSelect label="Proveedor" value={filters.supplierId} onChange={(value) => setFilter('supplierId', value)} options={[{ value: '', label: 'Todos' }, ...suppliers.data.map((item: any) => ({ value: item.id, label: item.name }))]} /><div className="tabs archive-tabs" aria-label="Filtro de pedidos"><button type="button" className={archiveFilter === 'active' ? 'active' : ''} onClick={() => setArchiveFilter('active')}>Todos activos</button><button type="button" className={archiveFilter === 'archived' ? 'active' : ''} onClick={() => setArchiveFilter('archived')}>Archivados</button><button type="button" className={archiveFilter === 'all' ? 'active' : ''} onClick={() => setArchiveFilter('all')}>Todos</button></div>{canManage && <button className="primary" onClick={() => setCreating(true)}>Nuevo pedido</button>}</div>{actionError && <p className="form-error">{actionError}</p>}<StateBlock loading={orders.loading} error={orders.error} retry={orders.reload} empty={!orders.data.length}><div className="table-card purchase-orders-table"><table><thead><tr><th>Código</th><th>Proveedor</th><th>Fecha</th><th>Estado</th><th>Importe</th><th>Recepción</th><th>Acciones</th></tr></thead><tbody>{orders.data.map((order: any) => { const archived = Boolean(order.archived_at); const status = archived ? 'Archivado' : displayStatus(order.status); const tone = archived ? 'muted' : order.status === 'cancelled' ? 'muted' : order.status === 'received' ? 'ok' : order.status === 'partially_received' ? 'info' : 'warn'; return <tr key={order.id} className={archived ? 'archived-row' : ''}><td><strong>{order.code}</strong></td><td>{order.suppliers?.name ?? '-'}</td><td>{order.order_date ?? '-'}</td><td><Badge tone={tone}>{status}</Badge></td><td>{Number(order.total_amount ?? 0).toLocaleString('es-ES')} €</td><td>{order.status === 'received' ? 'Completada' : order.status === 'partially_received' ? 'Parcial' : order.status === 'ordered' ? 'Pendiente' : '-'}</td><td><div className="row-actions"><button type="button" className="primary" onClick={() => setSelectedId(order.id)}>Abrir</button>{canManage && order.status === 'draft' && !archived && <button type="button" onClick={() => setSelectedId(order.id)}>Editar</button>}{canManage && ['cancelled', 'received'].includes(order.status) && !archived && <button type="button" onClick={() => archive(order)}>Archivar</button>}</div></td></tr>; })}</tbody></table></div></StateBlock>{creating && <PurchaseOrderForm initial={undefined} suppliers={suppliers.data} warehouses={warehouses.data} onClose={() => setCreating(false)} onSaved={(id: string) => { setCreating(false); orders.reload(); setSelectedId(id); }} />}</section>;
}

function MaterialSuppliersEditor({ materialId }: { materialId: string }) {
  const suppliers = useLoad(() => suppliersService.list('', 'active'), [], [] as any[]);
  const relations = useLoad(() => suppliersService.listMaterialSuppliers(materialId), [materialId], [] as any[]);
  const [supplierId, setSupplierId] = useState('');
  const [reference, setReference] = useState('');
  const [price, setPrice] = useState('');
  const [error, setError] = useState('');
  const add = async () => { if (!supplierId) return; setError(''); try { await suppliersService.addMaterialSupplier(materialId, { supplier_id: supplierId, supplier_reference: reference, purchase_unit_price: price }); setSupplierId(''); setReference(''); setPrice(''); relations.reload(); } catch (err) { setError(formErrorMessage(err, 'No se ha podido asociar el proveedor.')); } };
  const available = suppliers.data.filter((supplier: any) => !relations.data.some((row: any) => row.supplier_id === supplier.id && row.active));
  return <Card title="Proveedores"><div className="supplier-link-form"><h4>Añadir proveedor</h4><div className="form-grid"><FormSelect label="Proveedor" value={supplierId} onChange={setSupplierId} options={[{ value: '', label: 'Selecciona proveedor' }, ...available.map((supplier: any) => ({ value: supplier.id, label: `${supplier.name}${supplier.tax_id ? ` · ${supplier.tax_id}` : ''}` }))]} loading={suppliers.loading} /><label>Referencia<input value={reference} onChange={(event) => setReference(event.target.value)} /></label><label>Precio habitual<input type="number" min="0" step="0.01" value={price} onChange={(event) => setPrice(event.target.value)} /></label></div><button type="button" onClick={add} disabled={!supplierId || suppliers.loading}>Añadir proveedor</button></div>{error && <p className="form-error">{error}</p>}<div className="material-supplier-table"><div className="material-supplier-head"><span>Proveedor</span><span>Referencia</span><span>Precio habitual</span><span>Preferente</span><span>Estado</span><span>Acciones</span></div>{relations.data.filter((row: any) => row.active).map((row: any) => <MaterialSupplierRow key={row.id} row={row} onChanged={relations.reload} />)}{!relations.loading && !relations.data.some((row: any) => row.active) && <p className="large-note">Sin proveedores asociados.</p>}</div></Card>;
}

function CanonicalMaterialForm({ initial, warehouses, onClose, onSaved }: { initial?: any; warehouses: any[]; onClose: () => void; onSaved: () => void }) {
  return <MaterialMasterForm initial={initial} warehouses={warehouses} onClose={onClose} onSaved={onSaved} />;
  const [values, setValues] = useState<Record<string, any>>({ description: '', manufacturer: '', reference: '', unit: 'ud', cost: 0, price: 0, minimum_stock: 0, stock_controlled: true, allow_negative_stock: false, is_specific: false, made_to_measure: false, single_use: false, active: true, initial_quantity: 0, warehouse_id: '', ...initial });
  const suppliers = useLoad(() => suppliersService.list('', 'active'), [], [] as any[]);
  const [supplierDraft, setSupplierDraft] = useState({ supplier_id: '', supplier_reference: '', purchase_unit_price: '', is_preferred: true });
  const [saving, setSaving] = useState(false); const [error, setError] = useState(''); const set = (key: string, value: any) => setValues((current) => ({ ...current, [key]: value }));
  const submit = async (event: FormEvent) => { event.preventDefault(); if (!values.description?.trim()) { setError('Indica la descripción del material.'); return; } if (Number(values.initial_quantity) > 0 && !values.warehouse_id) { setError('Selecciona el almacén del stock inicial.'); return; } setSaving(true); setError(''); try { const saved = initial?.id ? await materialsService.update(initial.id, values) : await materialsService.create(values); if (!initial?.id && supplierDraft.supplier_id) await suppliersService.addMaterialSupplier(saved.id, supplierDraft); onSaved(); return saved; } catch (err) { setError(formErrorMessage(err, 'No se ha podido guardar el material.')); } finally { setSaving(false); } };
  return <ModalForm title={initial?.id ? 'Editar material' : 'Crear material'} onClose={onClose} onSubmit={submit} saving={saving} error={error}><div className="form-grid"><label>Descripción *<input value={values.description} onChange={(event) => set('description', event.target.value)} /></label><label>Unidad<input value={values.unit} onChange={(event) => set('unit', event.target.value)} /></label><label>Coste<input type="number" min="0" step="0.01" value={values.cost} onChange={(event) => set('cost', event.target.value)} /></label><label>Precio venta<input type="number" min="0" step="0.01" value={values.price} onChange={(event) => set('price', event.target.value)} /></label><label>Stock mínimo<input type="number" min="0" step="0.01" value={values.minimum_stock} onChange={(event) => set('minimum_stock', event.target.value)} /></label>{!initial?.id && <label>Stock inicial<input type="number" min="0" step="0.01" value={values.initial_quantity} onChange={(event) => set('initial_quantity', event.target.value)} /></label>}{!initial?.id && Number(values.initial_quantity) > 0 && <FormSelect label="Almacén" value={values.warehouse_id} onChange={(value) => set('warehouse_id', value)} options={[{ value: '', label: 'Selecciona almacén' }, ...warehouses.map((warehouse: any) => ({ value: warehouse.id, label: `${warehouse.code} · ${warehouse.name}` }))]} />}</div>{!initial?.id && <Card title="Proveedor inicial"><FormSelect label="Proveedor" value={supplierDraft.supplier_id} onChange={(value) => setSupplierDraft((current) => ({ ...current, supplier_id: value }))} options={[{ value: '', label: 'Sin proveedor inicial' }, ...suppliers.data.map((supplier: any) => ({ value: supplier.id, label: supplier.name }))]} loading={suppliers.loading} /><div className="form-grid"><label>Referencia del proveedor<input value={supplierDraft.supplier_reference} onChange={(event) => setSupplierDraft((current) => ({ ...current, supplier_reference: event.target.value }))} /></label><label>Precio de compra<input type="number" min="0" step="0.01" value={supplierDraft.purchase_unit_price} onChange={(event) => setSupplierDraft((current) => ({ ...current, purchase_unit_price: event.target.value }))} /></label></div><label><input type="checkbox" checked={supplierDraft.is_preferred} onChange={(event) => setSupplierDraft((current) => ({ ...current, is_preferred: event.target.checked }))} /> Proveedor preferente</label></Card>}{initial?.id && <MaterialSuppliersEditor materialId={initial.id} />}</ModalForm>;
}

function CanonicalStockAdjustForm({ material, warehouses, onClose, onSaved }: { material: any; warehouses: any[]; onClose: () => void; onSaved: () => void }) {
  const [values, setValues] = useState({ warehouse_id: '', movement_type: 'Entrada', quantity: '', reason: '' }); const [saving, setSaving] = useState(false); const [error, setError] = useState(''); const set = (key: string, value: string) => setValues((current) => ({ ...current, [key]: value }));
  const submit = async (event: FormEvent) => { event.preventDefault(); if (!values.warehouse_id || !values.reason.trim() || Number(values.quantity) <= 0) { setError('Selecciona almacén, cantidad y motivo.'); return; } setSaving(true); setError(''); try { await materialsService.adjustStock(material.id, values); onSaved(); } catch (err) { setError(formErrorMessage(err, 'No se ha podido ajustar el stock.')); } finally { setSaving(false); } };
...
function LegacyMaterialsUxModule() {
  ...
  const warehouses = useLoad(() => workOrdersService.warehousesCatalog(), [], [] as any[]);
  const data = materialsQuery.data ?? [];
  const visible = filterMaterials(data.filter((item: any) => !item.deleted_at && item.active !== false), stock.data, stockFilter);
  const stats = materialDashboardStats(data, stock.data);
  const cards = [{ key: 'all' as const, label: 'Todos', value: stats.total, tone: 'info' as Severity }, { key: 'in_stock' as const, label: 'Con stock', value: stats.inStock, tone: 'ok' as Severity }, { key: 'low_stock' as const, label: 'Bajo stock', value: stats.lowStock, tone: 'warn' as Severity }, { key: 'out_of_stock' as const, label: 'Sin stock', value: stats.outOfStock, tone: 'danger' as Severity }];
  const canAdjust = hasPermission(profile, 'stock.adjust');
  return <ListPage title="Materiales" summary="Catálogo y disponibilidad con acciones directas por material." search={search} setSearch={setSearch} action={<button className="primary" onClick={() => setEditing({})}>Crear material</button>} loading={materialsQuery.isPending || stock.loading} error={materialsQuery.error?.message ?? stock.error} retry={() => { void materialsQuery.refetch(); stock.reload(); }} empty={!data.length}><div className="stats-grid materials-ux-stats">{cards.map((card) => <button type="button" key={card.key} className={`metric ${card.tone}${stockFilter === card.key ? ' selected' : ''}`} onClick={() => setStockFilter(stockFilter === card.key && card.key !== 'all' ? 'all' : card.key)}><span>{card.label}</span><strong>{card.value}</strong><small>Filtrar listado</small></button>)}</div><div className="material-ux-list">{visible.map((material: any) => { const quantity = canonicalQuantity(material.id, stock.data); const state = materialStockState(material, quantity); return <article className="material-ux-row" key={material.id}><div className="material-ux-main"><Link to={`/app/modulos/materiales/${material.id}`}><strong>{material.code}</strong><span>{material.description}</span></Link><small>{[material.manufacturer, material.reference, `Unidad: ${material.unit ?? 'ud'}`].filter(Boolean).join(' · ')}</small></div><div className="material-ux-stock"><strong>{quantity.toLocaleString('es-ES')} {material.unit ?? 'ud'}</strong><Badge tone={state === 'out_of_stock' ? 'danger' : state === 'low_stock' ? 'warn' : 'ok'}>{state === 'out_of_stock' ? 'Sin stock' : state === 'low_stock' ? 'Bajo stock' : 'Disponible'}</Badge></div><div className="material-ux-actions"><Link to={`/app/modulos/materiales/${material.id}`}>Abrir</Link><Link to={`/app/modulos/materiales/${material.id}?editar=1`}>Editar</Link>{canAdjust && <button type="button" onClick={() => setAdjusting(material)}>Ajustar stock</button>}<Link className="primary" to={`/app/modulos/compras?material=${material.id}`}>Comprar</Link></div></article>; })}</div>{!visible.length && <p className="large-note">No hay materiales para este filtro.</p>}{editing && <MaterialMasterForm initial={editing.id ? editing : undefined} warehouses={warehouses.data} onClose={() => setEditing(null)} onSaved={() => { setEditing(null); void materialsQuery.refetch(); }} />}{adjusting && <CanonicalStockAdjustForm material={adjusting} warehouses={warehouses.data} onClose={() => setAdjusting(null)} onSaved={() => { setAdjusting(null); stock.reload(); }} />}</ListPage>;
}

function MaterialsUxModule() {
  const { companyId } = useAuth();
  const [search, setSearch] = useState('');
  const [stockFilter, setStockFilter] = useState<MaterialStockFilter>('all');
  const materialsQuery = useMaterialsCatalog(companyId, search);
  ...
  const stats = materialDashboardStats(data, stock.data);
  const cards = [{ key: 'all' as const, label: 'Todos', value: stats.total, tone: 'info' as Severity }, { key: 'in_stock' as const, label: 'Con stock', value: stats.inStock, tone: 'ok' as Severity }, { key: 'low_stock' as const, label: 'Bajo stock', value: stats.lowStock, tone: 'warn' as Severity }, { key: 'out_of_stock' as const, label: 'Sin stock', value: stats.outOfStock, tone: 'danger' as Severity }];
  return <ListPage title="Materiales" summary="Catálogo y disponibilidad por almacén." search={search} setSearch={setSearch} loading={materialsQuery.isPending || stock.loading} error={materialsQuery.error?.message ?? stock.error} retry={() => { void materialsQuery.refetch(); stock.reload(); }} empty={!data.length}><div className="stats-grid materials-ux-stats">{cards.map((card) => <button type="button" key={card.key} className={`metric ${card.tone}${stockFilter === card.key ? ' selected' : ''}`} onClick={() => setStockFilter(stockFilter === card.key && card.key !== 'all' ? 'all' : card.key)}><span>{card.label}</span><strong>{card.value}</strong><small>Filtrar listado</small></button>)}</div><div className="material-ux-list">{visible.map((material: any) => { const quantity = canonicalQuantity(material.id, stock.data); const state = materialStockState(material, quantity); return <article className="material-ux-row" key={material.id}><div className="material-ux-main"><Link to={`/app/modulos/materiales/${material.id}`}><strong>{material.code}</strong><span>{material.description}</span></Link><small>{[material.manufacturer, material.reference, `Unidad: ${material.unit ?? 'ud'}`].filter(Boolean).join(' · ')}</small></div><div className="material-ux-stock"><strong>{quantity.toLocaleString('es-ES')} {material.unit ?? 'ud'}</strong><Badge tone={state === 'out_of_stock' ? 'danger' : state === 'low_stock' ? 'warn' : 'ok'}>{state === 'out_of_stock' ? 'Sin stock' : state === 'low_stock' ? 'Bajo stock' : 'Disponible'}</Badge></div><div className="material-ux-actions"><Link className="primary" to={`/app/modulos/materiales/${material.id}`}>Detalles</Link></div></article>; })}</div></ListPage>;
}

function SupplierForm({ initial, onClose, onSaved, canManageBanking }: { initial?: any; onClose: () => void; onSaved: () => void; canManageBanking: boolean }) {
  const primaryBank = (initial?.supplier_bank_accounts ?? []).find((account: any) => account.is_primary) ?? initial?.supplier_bank_accounts?.[0] ?? {};
  const [values, setValues] = useState<Record<string, any>>({ active: true, ...initial });
  const [bank, setBank] = useState<Record<string, any>>({ account_holder: primaryBank.account_holder ?? '', iban: primaryBank.iban ?? '', bic_swift: primaryBank.bic_swift ?? '', notes: primaryBank.notes ?? '' });
  const [saving, setSaving] = useState(false); const [error, setError] = useState('');
  const set = (key: string, value: any) => setValues((current) => ({ ...current, [key]: value }));
  const setBankValue = (key: string, value: string) => setBank((current) => ({ ...current, [key]: value }));
  const submit = async (event: FormEvent) => { event.preventDefault(); if (!values.name?.trim()) { setError('Indica la razón social.'); return; } setSaving(true); setError(''); try { const saved = initial?.id ? await suppliersService.update(initial.id, values) : await suppliersService.create(values); if (canManageBanking && Object.values(bank).some((value) => String(value ?? '').trim())) await suppliersService.savePrimaryBankAccount(saved.id, bank, saved.company_id); onSaved(); } catch (err) { setError(formErrorMessage(err, initial?.id ? 'No se ha podido modificar el proveedor.' : 'No se ha podido crear el proveedor.')); } finally { setSaving(false); } };
  return <div className="mini-modal" role="dialog" aria-modal="true" onWheel={(event) => event.stopPropagation()} onTouchMove={(event) => event.stopPropagation()}><form onSubmit={submit}><h3>{initial?.id ? 'Editar proveedor' : 'Crear proveedor'}</h3><Card title="Datos generales"><div className="form-grid"><label>Razón social *<input value={values.name ?? ''} onChange={(event) => set('name', event.target.value)} required /></label><label>Nombre comercial<input value={values.trade_name ?? ''} onChange={(event) => set('trade_name', event.target.value)} /></label><label>Código interno<input value={values.internal_code ?? ''} onChange={(event) => set('internal_code', event.target.value)} /></label><label>NIF/CIF<input value={values.tax_id ?? ''} onChange={(event) => set('tax_id', event.target.value)} /></label></div></Card><Card title="Dirección fiscal"><div className="form-grid"><label>Dirección<input value={values.fiscal_address ?? ''} onChange={(event) => set('fiscal_address', event.target.value)} /></label><label>Código postal<input value={values.postal_code ?? ''} onChange={(event) => set('postal_code', event.target.value)} /></label><label>Población<input value={values.city ?? ''} onChange={(event) => set('city', event.target.value)} /></label><label>Provincia / región<input value={values.province ?? ''} onChange={(event) => set('province', event.target.value)} /></label><label>País<input value={values.country ?? ''} onChange={(event) => set('country', event.target.value)} /></label></div></Card><Card title="Contacto"><div className="form-grid"><label>Persona de contacto<input value={values.contact_name ?? ''} onChange={(event) => set('contact_name', event.target.value)} /></label><label>Correo<input type="email" value={values.email ?? ''} onChange={(event) => set('email', event.target.value)} /></label><label>Teléfono<input value={values.phone ?? ''} onChange={(event) => set('phone', event.target.value)} /></label><label>Web<input type="url" value={values.website ?? ''} onChange={(event) => set('website', event.target.value)} placeholder="https://" /></label></div></Card><Card title="Condiciones de pago"><div className="form-grid"><FormSelect label="Forma de pago" value={values.payment_method ?? ''} onChange={(value) => set('payment_method', value)} options={[{ value: '', label: 'No configurada' }, ...['transferencia', 'domiciliacion', 'confirming', 'tarjeta', 'efectivo', 'otra'].map((value) => ({ value, label: value[0].toUpperCase() + value.slice(1) }))]} /><label>Plazo de pago (días)<input type="number" min="0" step="1" value={values.payment_terms_days ?? ''} onChange={(event) => set('payment_terms_days', event.target.value)} placeholder="Ej. 30" /></label><label>Día de pago<input type="number" min="1" max="31" step="1" value={values.payment_due_day ?? ''} onChange={(event) => set('payment_due_day', event.target.value)} placeholder="Opcional" /></label><label>Moneda (ISO)<input maxLength={3} value={values.currency_code ?? ''} onChange={(event) => set('currency_code', event.target.value.toUpperCase())} placeholder="EUR" /></label><label>Descuento habitual (%)<input type="number" min="0" max="100" step="0.01" value={values.usual_discount ?? ''} onChange={(event) => set('usual_discount', event.target.value)} /></label><label>Referencia de cliente con proveedor<input value={values.supplier_customer_reference ?? ''} onChange={(event) => set('supplier_customer_reference', event.target.value)} /></label></div></Card>{canManageBanking && <Card title="Datos bancarios"><p className="large-note">Acceso administrativo. El IBAN no es obligatorio y puede completarse más adelante.</p><div className="form-grid"><label>Titular<input value={bank.account_holder} onChange={(event) => setBankValue('account_holder', event.target.value)} /></label><label>IBAN<input value={bank.iban} onChange={(event) => setBankValue('iban', event.target.value)} /></label><label>BIC / SWIFT<input value={bank.bic_swift} onChange={(event) => setBankValue('bic_swift', event.target.value)} /></label></div><label>Notas de la cuenta<textarea value={bank.notes} onChange={(event) => setBankValue('notes', event.target.value)} /></label></Card>}<Card title="Facturación y notas"><div className="form-grid"><label>Email de facturación<input type="email" value={values.billing_email ?? ''} onChange={(event) => set('billing_email', event.target.value)} /></label><label>Observaciones de facturación<textarea value={values.billing_notes ?? ''} onChange={(event) => set('billing_notes', event.target.value)} /></label></div><label>Observaciones internas<textarea value={values.internal_notes ?? ''} onChange={(event) => set('internal_notes', event.target.value)} /></label></Card>{error && <p className="form-error">{error}</p>}<div className="modal-footer"><button type="button" onClick={onClose} disabled={saving}>Cancelar</button><button className="primary" disabled={saving}>{saving ? 'Guardando...' : initial?.id ? 'Guardar proveedor' : 'Crear proveedor'}</button></div></form></div>;
}
...
function LegacyMultiEquipmentPicker({ values, setValues, filteredEquipment, equipmentTypes, installation: initialInstallation }: any) {
  ...
  const update = (next: any[]) => setValues((current: any) => ({ ...current, equipment_selection: next, main_equipment_id: next.find((item) => item.kind === 'existing')?.equipment_id ?? current.main_equipment_id ?? '' }));
  const addExisting = () => { if (!existingId || selection.some((item: any) => item.kind === 'existing' && item.equipment_id === existingId)) return; update([...selection, { kind: 'existing', equipment_id: existingId }]); setExistingId(''); };
  const addNew = () => { const quantity = Number(draft.quantity); if (!draft.equipment_type_id || !Number.isInteger(quantity) || quantity < 1) return; update([...selection, ...Array.from({ length: quantity }, () => ({ kind: 'new', ...draft, quantity: undefined }))]); setDraft({ ...draft, equipment_type_id: '', quantity: 1, internal_location: '', serial_number: '' }); };
  const remove = (index: number) => update(selection.filter((_: any, itemIndex: number) => itemIndex !== index));
  const edit = (index: number, key: string, value: any) => update(selection.map((item: any, itemIndex: number) => itemIndex === index ? { ...item, [key]: value } : item));
  return <Card title="EQUIPOS DEL PARTE"><p className="large-note">Cada unidad nueva se guarda como un equipo físico independiente. Puedes mezclar equipos existentes y nuevos.</p><div className="form-grid"><FormSelect label="Equipo existente" value={existingId} onChange={setExistingId} options={filteredEquipment.filter((item: any) => !selection.some((selected: any) => selected.kind === 'existing' && selected.equipment_id === item.id)).map((item: any) => ({ value: item.id, label: `${item.code} · ${item.equipment_types?.name ?? item.model ?? 'Equipo'} · ${item.internal_location ?? ''}` }))} disabled={!values.site_id} /><button type="button" onClick={addExisting} disabled={!existingId}>+ Añadir existente</button></div>{installation && <div className="nested-form"><h4>Crear equipo nuevo</h4><div className="form-grid"><FormSelect label="Tipo de equipo" value={draft.equipment_type_id} onChange={(value) => setDraft({ ...draft, equipment_type_id: value })} required options={equipmentTypes.data.map((type: any) => ({ value: type.id, label: type.name }))} loading={equipmentTypes.loading} /><label>Cantidad *<input type="number" min="1" value={draft.quantity} onChange={(event) => setDraft({ ...draft, quantity: event.target.value })} required /></label><label>Marca<input value={draft.brand} onChange={(event) => setDraft({ ...draft, brand: event.target.value })} /></label><label>Modelo<input value={draft.model} onChange={(event) => setDraft({ ...draft, model: event.target.value })} /></label><label>Referencia / ubicación<input value={draft.internal_location} onChange={(event) => setDraft({ ...draft, internal_location: event.target.value })} /></label><label>Número de serie<input value={draft.serial_number} onChange={(event) => setDraft({ ...draft, serial_number: event.target.value })} /></label></div><label>Observaciones<textarea value={draft.notes} onChange={(event) => setDraft({ ...draft, notes: event.target.value })} /></label><button type="button" onClick={addNew} disabled={!draft.equipment_type_id}>+ Añadir equipos nuevos</button></div>}<div className="compact-list">{selection.map((item: any, index: number) => { const existing = item.kind === 'existing' ? filteredEquipment.find((equipment: any) => equipment.id === item.equipment_id) : null; const type = existing?.equipment_types?.name ?? equipmentTypes.data.find((equipmentType: any) => equipmentType.id === item.equipment_type_id)?.name ?? 'Equipo'; return <article key={`${item.kind}-${item.equipment_id ?? index}`}><Badge tone={item.kind === 'existing' ? 'info' : 'ok'}>{item.kind === 'existing' ? 'EXISTENTE' : 'NUEVO'}</Badge><strong>{existing?.code ?? type}</strong>{item.kind === 'existing' ? <span>{existing?.brand ?? '-'} {existing?.model ?? ''} · {existing?.internal_location ?? '-'}</span> : <div className="form-grid"><label>Ubicación<input value={item.internal_location ?? ''} onChange={(event) => edit(index, 'internal_location', event.target.value)} /></label><label>Serie<input value={item.serial_number ?? ''} onChange={(event) => edit(index, 'serial_number', event.target.value)} /></label></div>}<button type="button" className="danger" onClick={() => remove(index)}>Eliminar</button></article>; })}</div>{!selection.length && <p className="large-note">Todavía no hay equipos añadidos.</p>}</Card>;
}
...
function LegacyWorkOrderForm({ initial, sourceQuote, onClose, onSaved }: any) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      if (initial?.quote_id) console.error('DMP generate work order from quote failed', { quoteId: initial.quote_id, quoteStatus: sourceQuote?.status, quoteCompanyId: initial.company_id, quoteClientId: initial.client_id, quoteSiteId: initial.site_id, quoteEquipmentId: initial.main_equipment_id, quoteCaseId: initial.case_id, payload, rpcName: 'dmp_create_work_order_full', message: err?.message, details: err?.details, hint: err?.hint, code: err?.code, name: err?.name });
      const codeSuffix = err?.code ? ` Codigo: ${err.code}.` : '';
      setError(formErrorMessage(err, initial?.id ? 'No se ha podido modificar el parte.' : initial?.quote_id ? `No se pudo generar el parte desde el presupuesto.${codeSuffix}` : 'No se ha podido crear el parte.'));
    }
    finally { setSaving(false); }
  };
...
function WorkOrderForm({ initial, sourceQuote, onClose, onSaved }: any) {
  ...
  const changeContext = (key: 'client_id' | 'site_id', value: string) => {
    ...
    if (selected.length && !window.confirm('Cambiar cliente o centro eliminará los equipos seleccionados para evitar relaciones incompatibles. ¿Continuar?')) return;
    setValues((current) => ({ ...current, [key]: value, ...(key === 'client_id' ? { site_id: '', case_id: '' } : { case_id: '' }), equipment_selection: [], main_equipment_id: '' }));
  };
  const submit = async (event: FormEvent) => {
    event.preventDefault();
    if (saving) return;
  ...
  return <ModalForm title={initial?.id ? 'Modificar parte' : 'Crear parte'} onClose={onClose} onSubmit={submit} saving={saving} error={error}><FormSelect label="Cliente" value={values.client_id} onChange={(value) => changeContext('client_id', value)} required options={clients.data.map((client) => ({ value: client.id, label: `${client.code} · ${client.legal_name}` }))} loading={clients.loading} /><FormSelect label="Centro" value={values.site_id} onChange={(value) => changeContext('site_id', value)} required options={filteredSites.map((site) => ({ value: site.id, label: `${site.code} · ${site.name}` }))} loading={sites.loading} disabled={!values.client_id} /><FormSelect label="Expediente" value={values.case_id} onChange={(value) => set('case_id', value)} options={filteredCases.map((item) => ({ value: item.id, label: `${item.code} · ${item.title}` }))} disabled={!values.client_id} /><div className="form-grid"><FormSelect label="Tipo" value={values.type} onChange={(value) => set('type', value)} required options={['Averia urgente','Correctivo','Preventivo','Mantenimiento','Inspeccion','Instalacion','Visita tecnica','Visita comercial','Garantia'].map((value) => ({ value, label: displayStatus(value) }))} /><FormSelect label="Prioridad" value={values.priority} onChange={(value) => set('priority', value)} options={['Baja','Normal','Alta','Critica'].map((value) => ({ value, label: displayStatus(value) }))} /><label>Título *<input value={values.title ?? ''} onChange={(event) => set('title', event.target.value)} required /></label><label>Fecha<input type="date" value={values.scheduled_date ?? ''} onChange={(event) => set('scheduled_date', event.target.value)} /></label><label>Hora<input type="time" value={values.scheduled_time ?? ''} onChange={(event) => set('scheduled_time', event.target.value)} /></label><label>Duración (minutos)<input type="number" min="1" value={values.estimated_duration_minutes ?? ''} onChange={(event) => set('estimated_duration_minutes', event.target.value)} /></label></div><FormSelect label="Técnico principal" value={values.technician_id} onChange={(value) => set('technician_id', value)} options={technicians.data.map((person) => ({ value: person.id, label: fullName(person) }))} /><MultiEquipmentPicker values={values} setValues={setValues} filteredEquipment={filteredEquipment} equipmentTypes={equipmentTypes} installation /><label>Descripción<textarea value={values.description ?? ''} onChange={(event) => set('description', event.target.value)} /></label><label>Material previsto<textarea value={values.planned_material ?? ''} onChange={(event) => set('planned_material', event.target.value)} /></label><label>Observaciones<textarea value={values.observations ?? ''} onChange={(event) => set('observations', event.target.value)} /></label></ModalForm>;
}

function assignmentRows(workOrder: any) {
  const rows = (workOrder.assignments ?? []).map((item: any) => ({ id: item.technician_id, assignmentId: item.id, person: item.profiles, type: item.role === 'Principal' ? 'Técnico principal' : 'Técnico de apoyo', assignmentType: 'technical', assignment_date: item.assignment_date, planned_start_time: item.planned_start_time, planned_end_time: item.planned_end_time, status: item.status, route: `/app/modulos/tecnicos/${item.technician_id}` }));
  if (workOrder.responsible || workOrder.current_responsible_id) {
    const person = workOrder.responsible ?? { first_name: workOrder.commercial_name ?? 'Responsable', last_name: '', primary_area: workOrder.commercial_role };
    const commercial = isCommercialProfile(person);
    rows.push({ id: workOrder.current_responsible_id, person, type: commercial ? 'Comercial' : 'Responsable', assignmentType: 'commercial', assignment_date: workOrder.scheduled_date, planned_start_time: workOrder.scheduled_time, planned_end_time: null, status: workOrder.status, route: commercial ? `/app/modulos/comerciales/${workOrder.current_responsible_id}` : `/app/partes/${workOrder.id}` });
  }
  return rows.filter((row: any) => row.id);
}
...
function AssignmentsCard({ workOrder, canManage, onManage, onChanged }: { workOrder: any; canManage: boolean; onManage: () => void; onChanged: () => void }) {
  ...
  const [message, setMessage] = useState('');
  const [error, setError] = useState('');
  const rows = assignmentRows(workOrder);
  const unassign = async () => {
    if (!confirm || savingId) return;
    setSavingId(confirm.id); setError(''); setMessage('');
    try { await workOrdersService.unassign(workOrder.id, confirm.id, confirm.reason, confirm.assignmentType); setMessage(`${fullName(confirm.person)} desasignado.`); setConfirm(null); onChanged(); }
    catch (err) { setError(err instanceof Error ? err.message : 'No se ha podido desasignar.'); }
    finally { setSavingId(''); }
  };
  return <Card title="Equipo asignado" action={canManage ? <button className="primary" onClick={onManage}>Gestionar asignaciones</button> : null}>{rows.length ? <div className="assignment-list">{rows.map((row: any) => <article key={`${row.type}-${row.id}`} className="assignment-row"><div><strong>{fullName(row.person)}</strong><span>{row.type} · {row.assignment_date ?? 'Sin fecha'} · {row.planned_start_time ?? 'Sin hora'}{row.planned_end_time ? `-${row.planned_end_time}` : ''}</span><small>{displayStatus(row.status)}</small></div><div className="row-actions"><Link to={row.route}>Ver ficha</Link>{canManage && <button onClick={() => onManage()}>Cambiar/Reasignar</button>}{canManage && <button className="danger" disabled={savingId === row.id} onClick={() => setConfirm({ ...row, reason: '' })}>{savingId === row.id ? 'Desasignando...' : 'Desasignar'}</button>}</div></article>)}</div> : <p className="large-note">Sin técnico principal, apoyos ni comercial asignado.</p>}{message && <p className="success-note">{message}</p>}{error && <p className="form-error">{error}</p>}{confirm && <div className="mini-modal" role="dialog" aria-modal="true"><div><h3>{`¿Desasignar a ${fullName(confirm.person)} del parte ${workOrder.code}?`}</h3><p>{`${confirm.type}. Esta acción conserva el historial y no asigna el parte a quien ejecuta la operación.`}</p><label>Motivo opcional<textarea value={confirm.reason ?? ''} onChange={(event) => setConfirm({ ...confirm, reason: event.target.value })} placeholder="Motivo de la desasignación" /></label><div className="modal-footer"><button onClick={() => savingId ? null : setConfirm(null)} disabled={Boolean(savingId)}>Cancelar</button><button className="primary danger" onClick={unassign} disabled={Boolean(savingId)}>{savingId ? 'Desasignando...' : 'Confirmar desasignación'}</button></div></div></div>}</Card>;
}

function AssignmentForm({ workOrder, workOrderId, companyId, onClose, onSaved }: any) {
  const currentRows = assignmentRows(workOrder ?? {});
  const [values, setValues] = useState<Record<string, any>>({ assignment_date: workOrder?.scheduled_date ?? new Date().toISOString().slice(0, 10), planned_start_time: workOrder?.scheduled_time ?? '', planned_end_time: '', main_technician_id: workOrder?.main_technician_id ?? currentRows.find((row: any) => row.type === 'Técnico principal')?.id ?? '', support_technician_ids: currentRows.filter((row: any) => row.type === 'Técnico de apoyo').map((row: any) => row.id), commercial_id: isCommercialProfile(workOrder?.responsible) ? workOrder?.current_responsible_id : '' });
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState('');
  const technicians = useLoad(() => profilesService.listTechnicians(companyId), [companyId], [] as any[]);
  const commercials = useLoad(() => profilesService.listCommercials(companyId), [companyId], [] as any[]);
  const set = (key: string, value: any) => setValues((current) => ({ ...current, [key]: value }));
  const toggleSupport = (id: string) => setValues((current) => ({ ...current, support_technician_ids: current.support_technician_ids?.includes(id) ? current.support_technician_ids.filter((item: string) => item !== id) : [...(current.support_technician_ids ?? []), id] }));
  const submit = async (event: FormEvent) => {
    event.preventDefault();
    if (saving) return;
    setSaving(true); setError('');
    try { await workOrdersService.manageAssignments(workOrderId, { main_technician_id: values.main_technician_id || null, support_technician_ids: (values.support_technician_ids ?? []).filter((id: string) => id !== values.main_technician_id), commercial_id: values.commercial_id || null, assignment_date: values.assignment_date, planned_start_time: values.planned_start_time || null, planned_end_time: values.planned_end_time || null, reason: values.reason || null }); onSaved?.(); }
    catch (err) { setError(err instanceof Error ? err.message : 'No se han podido guardar las asignaciones.'); }
    finally { setSaving(false); }
  };
  return <ModalForm title="Gestionar asignaciones" onClose={onClose} onSubmit={submit} saving={saving} error={error} submitLabel={saving ? 'Guardando...' : 'Guardar cambios'}><p className="large-note">Puedes reasignar en un único flujo. Desasignar al principal o al comercial dejando el campo vacío conserva el historial y deja el campo sin responsable.</p><FormSelect label="Técnico principal" value={values.main_technician_id} onChange={(value) => set('main_technician_id', value)} loading={technicians.loading} options={[{ value: '', label: 'Sin técnico principal' }, ...technicians.data.map((item) => ({ value: item.id, label: `${fullName(item)} · ${item.active ? 'Activo' : 'Inactivo'}` }))]} /><Card title="Técnicos de apoyo"><div className="component-select">{technicians.data.filter((item) => item.id !== values.main_technician_id).map((item) => <label key={item.id}><input type="checkbox" checked={(values.support_technician_ids ?? []).includes(item.id)} onChange={() => toggleSupport(item.id)} /> {fullName(item)}</label>)}</div></Card><FormSelect label="Comercial/responsable" value={values.commercial_id} onChange={(value) => set('commercial_id', value)} loading={commercials.loading} options={[{ value: '', label: 'Sin comercial asignado' }, ...commercials.data.map((item) => ({ value: item.id, label: `${fullName(item)} · ${item.active ? 'Activo' : 'Inactivo'}` }))]} /><div className="form-grid"><label>Fecha *<input type="date" value={values.assignment_date ?? ''} onChange={(event) => set('assignment_date', event.target.value)} required /></label><label>Horario inicio<input type="time" value={values.planned_start_time ?? ''} onChange={(event) => set('planned_start_time', event.target.value)} /></label><label>Horario fin<input type="time" value={values.planned_end_time ?? ''} onChange={(event) => set('planned_end_time', event.target.value)} /></label></div><label>Motivo opcional<textarea value={values.reason ?? ''} onChange={(event) => set('reason', event.target.value)} placeholder="Motivo del cambio, reasignación o desasignación" /></label></ModalForm>;
}
function CheckForm({ initial, onClose, onSaved }: any) {
  ...
  if (!canCreateCheck(profile)) return null;
  const set = (key: string, value: any) => setValues((current) => ({ ...current, [key]: value }));
  ...
  return <ModalForm title={initial?.id ? 'Modificar check' : 'Crear check'} onClose={onClose} onSubmit={submit} saving={saving} error={error}><p className="large-note">El código de check se generará automáticamente al guardar. Se selecciona una plantilla activa compatible con el tipo de equipo.</p><FormSelect label="Parte relacionado" value={values.work_order_id} onChange={selectWorkOrder} options={workOrders.data.map((item) => ({ value: item.id, label: `${item.code} · ${item.title}` }))} loading={workOrders.loading} /><FormSelect label="Equipo" value={values.equipment_id} onChange={(value) => setValues((current) => ({ ...current, equipment_id: value, template_id: '' }))} required options={equipment.data.map((item) => ({ value: item.id, label: `${item.code} · ${item.equipment_types?.name ?? item.model ?? 'Equipo'}` }))} loading={equipment.loading} /><FormSelect label="Plantilla" value={values.template_id} onChange={(value) => set('template_id', value)} required options={templates.data.map((item) => ({ value: item.id, label: `${item.name} · v${item.version} · ${item.companies?.name ?? 'Global'} · ${item.equipment_types?.name ?? 'Tipo global'}` }))} loading={templates.loading} disabled={!values.equipment_id || !selectedEquipment?.equipment_type_id} />{values.equipment_id && !selectedEquipment?.equipment_type_id && <p className="form-error">El equipo seleccionado no tiene tipo de equipo. Empresa: {selectedEquipment?.companies?.name ?? selectedEquipment?.company_id ?? 'no informada'}. No se puede validar compatibilidad de plantilla.</p>}{values.equipment_id && selectedEquipment?.equipment_type_id && !templates.loading && !templates.data.length && <p className="form-error">No hay plantilla compatible activa. Empresa: {selectedEquipment?.companies?.name ?? selectedEquipment?.company_id ?? 'no informada'}. Tipo del equipo: {selectedEquipment?.equipment_types?.name ?? selectedEquipment?.equipment_type_id}. Plantillas activas visibles: {activeTemplateCount.loading ? 'consultando...' : activeTemplateCount.data}.</p>}<FormSelect label="Estado" value={values.status} onChange={(value) => set('status', value)} options={['Por realizar','En curso','Realizado','Cancelado'].map((value) => ({ value, label: value }))} /><FormSelect label="Resultado global" value={values.global_result} onChange={(value) => set('global_result', value)} options={['Sin revisar','Todo favorable','Problema leve','No favorable','Favorable tras intervencion','No aplicable'].map((value) => ({ value, label: displayStatus(value) }))} /><label>Observaciones<textarea value={values.observations ?? ''} onChange={(event) => set('observations', event.target.value)} /></label></ModalForm>;
}
...
function FormSelect({ label, value, onChange, options, required, loading, disabled, emptyLabel = 'Seleccionar' }: { label: string; value?: string; onChange: (value: string) => void; options: { value: string; label: string }[]; required?: boolean; loading?: boolean; disabled?: boolean; emptyLabel?: string }) {
  const explicitEmpty = options.find((option) => option.value === '');
...
function LifecycleConfirmModal({ entity, record, action, onClose, onChanged, skipDependencyLoad = false }: { entity: LifecycleEntity; record: any; action: 'archive' | 'restore' | 'delete' | 'dependencies'; onClose: () => void; onChanged: () => void; skipDependencyLoad?: boolean }) { const opener = useRef<HTMLElement | null>(typeof document === 'undefined' ? null : document.activeElement as HTMLElement | null); const [mode, setMode] = useState(action); const [summary, setSummary] = useState<LifecycleSummary | null>(null); const [reason, setReason] = useState(''); const [confirmation, setConfirmation] = useState(''); const [saving, setSaving] = useState(false); const [error, setError] = useState(''); const target = summary ?? { code: record.code ?? record.email ?? record.name ?? record.title ?? record.id, name: record.legal_name ?? record.name ?? record.title ?? record.email ?? record.code ?? record.id, dependencies: {}, dependency_total: 0, can_restore: true, can_permanently_delete: false, can_controlled_cascade_delete: false, cascade_dependencies: {}, blocking_dependencies: {}, physical_delete_blocker: null, restore_blocker: null } as LifecycleSummary; const canDelete = Boolean(target.can_permanently_delete || target.can_controlled_cascade_delete); const expected = `ELIMINAR ${target.code}`; useOverlayScrollLock(); useEffect(() => { if (skipDependencyLoad) return; entityLifecycleService.dependencies(entity, record.id).then(setSummary).catch((err) => setError(err instanceof Error ? err.message : 'No se han podido consultar las dependencias.')); }, [entity, record.id, skipDependencyLoad]); useEffect(() => { const onKey = (event: KeyboardEvent) => { if (event.key === 'Escape' && !saving) onClose(); }; window.addEventListener('keydown', onKey); return () => { window.removeEventListener('keydown', onKey); opener.current?.focus(); }; }, [onClose, saving]); const submit = async () => { if (saving || mode === 'dependencies') return; if (!reason.trim()) { setError('El motivo es obligatorio.'); return; } if (mode === 'delete' && confirmation !== expected) { setError(`Para borrar definitivamente escribe: ${expected}`); return; } if (mode === 'delete' && !window.confirm(`Confirmacion destructiva final: se eliminara ${target.code}. Esta accion no se puede deshacer.`)) return; setSaving(true); setError(''); try { if (mode === 'archive') await entityLifecycleService.archive(entity, record.id, reason.trim()); if (mode === 'restore') await entityLifecycleService.restore(entity, record.id, reason.trim()); if (mode === 'delete') await entityLifecycleService.permanentlyDelete(entity, record.id, reason.trim(), confirmation); onChanged(); } catch (err) { setError(err instanceof Error ? err.message : 'No se ha podido completar la operación. Revisa las dependencias indicadas.'); } finally { setSaving(false); } }; const title = mode === 'archive' ? 'Archivar o desactivar registro' : mode === 'restore' ? 'Restaurar registro' : mode === 'delete' ? 'Eliminar definitivamente' : 'Dependencias del registro'; return <div className="mini-modal lifecycle-modal" role="dialog" aria-modal="true" aria-labelledby="lifecycle-title"><div><h3 id="lifecycle-title">{title}</h3><InfoGrid items={[[ 'Tipo', entityLabels[entity] ], [ 'Código', target.code ], [ 'Nombre', target.name ], [ 'Registros relacionados', String(target.dependency_total ?? 0) ], [ 'Restaurable', target.can_restore ? 'Sí' : `No. ${target.restore_blocker ?? ''}` ], [ 'Borrado definitivo', canDelete ? (target.can_controlled_cascade_delete ? 'Sí, con cascada controlada' : 'Sí, sin dependencias') : `No. ${target.physical_delete_blocker ?? 'Hay dependencias bloqueantes'}` ]]} /><Card title="Dependencias bloqueantes"><DependencyRows dependencies={target.blocking_dependencies ?? target.dependencies ?? {}} /></Card><Card title="Dependencias eliminables en cascada"><DependencyRows dependencies={target.cascade_dependencies ?? {}} /></Card><p className="large-note">Archivar conserva historial, auditoría y relaciones. Eliminar definitivamente se valida en Supabase y, para partes/checks, puede borrar dependencias operativas controladas si no hay bloqueos.</p>{mode !== 'dependencies' && <label>Motivo obligatorio<textarea value={reason} onChange={(event) => setReason(event.target.value)} placeholder="Explica por qué se realiza esta operación" /></label>}{mode === 'delete' && <label>Confirmación definitiva<input value={confirmation} onChange={(event) => setConfirmation(event.target.value)} placeholder={expected} /></label>}{error && <p className="form-error">{error}</p>}<div className="modal-footer"><button type="button" onClick={onClose} disabled={saving}>Cerrar</button>{mode === 'dependencies' && canDelete && <button type="button" className="danger-action" onClick={() => { setMode('delete'); setError(''); }} disabled={saving}>Eliminar definitivamente</button>}{mode === 'dependencies' && summary && !canDelete && <span className="large-note">No puede eliminarse; archívalo.</span>}{mode !== 'dependencies' && <button type="button" className="primary" onClick={submit} disabled={saving}>{saving ? 'Procesando...' : mode === 'archive' ? 'Archivar' : mode === 'restore' ? 'Restaurar' : 'Eliminar definitivamente'}</button>}</div></div></div>; }
function formErrorMessage(error: unknown, fallback?: string) { return userFacingErrorMessage(error, fallback); }

function DateRangeFiltersPanel({ params, setParams }: { params: URLSearchParams; setParams: (next: URLSearchParams, options?: { replace?: boolean }) => void }) {
...
function normalizeEntityOptionLabel(value: string) {
  const match = value.match(/^((?:CLI|CEN|MAT|EQ)-[A-Z0-9-]+) · (.+)$/i);
  return match ? `${match[2]} · ${match[1]}` : value;
}

function formatEntityTableValue(row: any, column: string) {
  const value = String(readPath(row, column) ?? '-');
  if (column === 'clients.legal_name' && row.clients) return formatClientLabel({ ...row.clients, code: row.client_code ?? row.clients.code });
  if (column === 'sites.name' && row.sites) return formatCenterLabel({ ...row.sites, code: row.site_code ?? row.sites.code, clients: row.clients });
  if (column === 'materials.description' && row.materials) return formatMaterialLabel({ ...row.materials, code: row.material_code ?? row.materials.code });
  return normalizeEntityOptionLabel(displayStatus(value));
}
...
function CanonicalManagementPage() {
  const { profile } = useAuth();
  const canSeeCosts = canViewInternalEconomics(profile);
  const { data, loading, error, reload } = useLoad(() => economicService.dashboardData(undefined), [], { workOrders: [], clients: [], quotes: [], metrics: {} } as any);
  ...
  const range = economicDateRange(period, from, to);
  const workOrders = data.workOrders.filter((work: any) => economicInRange(work.scheduled_date ?? work.created_at, range) && (!workType || work.work_type === workType) && (!economicStatus || work.economic_status === economicStatus));
  if (!canSeeCosts) return <section className="page"><Breadcrumb items={['Gerencia', 'Control económico']} /><Card title="Acceso económico restringido"><p className="large-note">Tu rol puede ver actividad comercial u operativa, pero no costes internos ni margen económico.</p></Card></section>;
  return <section className="page economic-dashboard"><Breadcrumb items={['Gerencia', 'Control económico']} /><Hero title="Panel de control económico" subtitle="Métricas calculadas por las vistas económicas canónicas de 060." tone="commercial" /><div className="filters sat-filters"><label>Periodo<select value={period} onChange={(event) => setPeriod(event.target.value)}><option value="month">Mes actual</option><option value="3m">Últimos 3 meses</option><option value="year">Año actual</option><option value="custom">Rango personalizado</option></select></label>{period === 'custom' && <><label>Desde<input type="date" value={from} onChange={(event) => setFrom(event.target.value)} /></label><label>Hasta<input type="date" value={to} onChange={(event) => setTo(event.target.value)} /></label></>}<label>Tipo trabajo<input value={workType} onChange={(event) => setWorkType(event.target.value)} placeholder="Todos" /></label><label>Estado económico<input value={economicStatus} onChange={(event) => setEconomicStatus(event.target.value)} placeholder="Todos" /></label></div><StateBlock loading={loading} error={error} retry={reload} empty={false}><div className="stats-grid dashboard-kpis"><EconomicMetric label="Venta total" value={money(metrics.sale_amount)} tone="info" help="Vista v_management_metrics" /><EconomicMetric label="Coste real de trabajos" value={money(metrics.real_cost)} tone="warn" help="Vista v_management_metrics" /><EconomicMetric label="Margen" value={money(metrics.margin_amount)} tone={Number(metrics.margin_amount ?? 0) >= 0 ? 'ok' : 'danger'} help="Venta total - coste real" /><EconomicMetric label="Venta adicional" value={money(metrics.additional_sale_amount)} tone="commercial" help="Snapshots facturables" /><EconomicMetric label="Garantías" value={String(metrics.warranty_cost ?? 0)} tone="danger" help="Coste de partes en garantía" /><EconomicMetric label="Pendiente facturar" value={String(metrics.pending_invoice_work_orders ?? 0)} tone="warn" help="Vista v_management_metrics" /></div><Card title="Desglose de costes"><p className="large-note">Venta sin IVA, coste real y margen; costes adicionales y snapshots de venta procedentes de las vistas económicas canónicas.</p></Card><Card title="Partes económicos"><div className="compact-list dashboard-list">{workOrders.slice(0, 8).map((work: any) => <article key={work.id}><Badge tone={Number(work.margin_amount ?? 0) >= 0 ? 'ok' : 'danger'}>{work.code}</Badge><p>{work.client_name ?? '-'} · Venta {money(work.sale_amount)} · Coste {money(work.real_cost_amount)} · Margen {money(work.margin_amount)}</p><Link to={`/app/partes/${work.id}`}>Abrir parte</Link></article>)}</div></Card><Card title="Clientes más rentables"><div className="compact-list">{data.clients.slice(0, 5).map((client: any) => <p key={client.client_id}><Link to={client.id ? `/app/clientes/${client.id}` : `/app/clientes/${client.client_id}`}>{client.client_name ?? client.legal_name ?? 'Cliente'}</Link> · Margen {money(client.estimated_margin_amount)}</p>)}</div></Card><Card title="Trabajos con margen negativo"><div className="compact-list">{workOrders.filter((work: any) => Number(work.margin_amount ?? 0) < 0).map((work: any) => <p key={work.id}>{work.code} · {money(work.margin_amount)}</p>)}</div></Card></StateBlock></section>;
}

function MaterialMasterForm({ initial, warehouses, onClose, onSaved, redirectExisting = true }: { initial?: any; warehouses: any[]; onClose: () => void; onSaved: () => void; redirectExisting?: boolean }) {
  const suppliers = useLoad(() => suppliersService.list('', 'active'), [], [] as any[]);
  const [values, setValues] = useState<Record<string, any>>({ description: '', manufacturer: '', reference: '', unit: 'ud', cost: 0, price: 0, minimum_stock: 0, made_to_measure: false, single_use: false, active: true, initial_quantity: 0, warehouse_id: '', ...initial });
  const [supplier, setSupplier] = useState({ supplier_id: '', supplier_reference: '', purchase_unit_price: '', is_preferred: true });
  const [saving, setSaving] = useState(false); const [error, setError] = useState('');
  const set = (key: string, value: any) => setValues((current) => ({ ...current, [key]: value }));
  const submit = async (event: FormEvent) => { event.preventDefault(); if (!values.description?.trim()) { setError('Indica la descripción del material.'); return; } if (Number(values.initial_quantity) > 0 && !values.warehouse_id) { setError('Selecciona el almacén del stock inicial.'); return; } setSaving(true); setError(''); try { const saved = initial?.id ? await materialsService.update(initial.id, values) : await materialsService.create(values); if (!initial?.id && supplier.supplier_id) await suppliersService.addMaterialSupplier(saved.id, supplier); onSaved(); } catch (err) { setError(formErrorMessage(err, 'No se ha podido guardar el material.')); } finally { setSaving(false); } };
  if (redirectExisting && initial?.id) return <Navigate to={`/app/modulos/materiales/${initial.id}?editar=1`} replace />;
  return <ModalForm title={initial?.id ? 'Editar material' : 'Crear material'} onClose={onClose} onSubmit={submit} saving={saving} error={error} submitLabel={initial?.id ? 'Guardar material' : 'Crear material'}><Card title="Datos del material"><div className="form-grid"><label>Descripción *<input value={values.description} onChange={(event) => set('description', event.target.value)} required /></label><label>Fabricante<input value={values.manufacturer ?? ''} onChange={(event) => set('manufacturer', event.target.value)} /></label><label>Referencia<input value={values.reference ?? ''} onChange={(event) => set('reference', event.target.value)} /></label><label>Unidad<input value={values.unit} onChange={(event) => set('unit', event.target.value)} /></label><label>Coste<input type="number" min="0" step="0.01" value={values.cost} onChange={(event) => set('cost', event.target.value)} /></label><label>Precio de venta<input type="number" min="0" step="0.01" value={values.price} onChange={(event) => set('price', event.target.value)} /></label><label>Stock mínimo<input type="number" min="0" step="0.01" value={values.minimum_stock} onChange={(event) => set('minimum_stock', event.target.value)} /></label></div><div className="form-grid"><label className="checkbox-line"><input type="checkbox" checked={Boolean(values.made_to_measure)} onChange={(event) => set('made_to_measure', event.target.checked)} /> Material a medida</label><label className="checkbox-line"><input type="checkbox" checked={Boolean(values.single_use)} onChange={(event) => set('single_use', event.target.checked)} /> Uso único</label>{initial?.id && <label className="checkbox-line"><input type="checkbox" checked={values.active !== false} onChange={(event) => set('active', event.target.checked)} /> Activo</label>}</div></Card>{!initial?.id && <Card title="Stock inicial"><div className="form-grid"><label>Cantidad inicial<input type="number" min="0" step="0.01" value={values.initial_quantity} onChange={(event) => set('initial_quantity', event.target.value)} /></label>{Number(values.initial_quantity) > 0 && <FormSelect label="Almacén" value={values.warehouse_id} onChange={(value) => set('warehouse_id', value)} options={[{ value: '', label: 'Selecciona un almacén' }, ...warehouses.map((warehouse: any) => ({ value: warehouse.id, label: `${warehouse.code} · ${warehouse.name}` }))]} />}</div><p className="large-note">Con cantidad mayor que cero debes seleccionar un almacén. El saldo se registra por el circuito canónico de existencias.</p></Card>}{!initial?.id && <Card title="Proveedor inicial"><FormSelect label="Proveedor" value={supplier.supplier_id} onChange={(value) => setSupplier((current) => ({ ...current, supplier_id: value }))} options={[{ value: '', label: 'Sin proveedor inicial' }, ...suppliers.data.map((item: any) => ({ value: item.id, label: item.name }))]} loading={suppliers.loading} />{supplier.supplier_id && <><div className="form-grid"><label>Referencia del proveedor<input value={supplier.supplier_reference} onChange={(event) => setSupplier((current) => ({ ...current, supplier_reference: event.target.value }))} /></label><label>Precio de compra<input type="number" min="0" step="0.01" value={supplier.purchase_unit_price} onChange={(event) => setSupplier((current) => ({ ...current, purchase_unit_price: event.target.value }))} /></label></div><label className="checkbox-line"><input type="checkbox" checked={supplier.is_preferred} onChange={(event) => setSupplier((current) => ({ ...current, is_preferred: event.target.checked }))} /> Marcar como proveedor preferente</label></>}</Card>}</ModalForm>;
}

function MaterialMovementsSection({ material }: { material: any }) {
  const movements = useLoad(() => materialsService.movements(material.id), [material.id], [] as any[]);
  const [type, setType] = useState('all'); const [warehouseId, setWarehouseId] = useState('all'); const [search, setSearch] = useState('');
  const filtered = movements.data.filter((row: any) => { const origin: any = movementOrigin(row); const haystack = [origin.text, row.notes, row.source_reference, row.work_orders?.code, row.purchase_receipts?.code, row.purchase_orders?.code].filter(Boolean).join(' ').toLowerCase(); return (type === 'all' || row.movement_type === type || movementDirection(row.movement_type) === type) && (warehouseId === 'all' || row.warehouse_id === warehouseId) && (!search.trim() || haystack.includes(search.trim().toLowerCase())); });
  return <Card title="Movimientos"><div className="material-page-filters"><select value={type} onChange={(event) => setType(event.target.value)}><option value="all">Todos los tipos</option><option value="Entrada">Entradas</option><option value="Salida">Salidas</option><option value="Consumo en parte">Consumos</option><option value="Ajuste">Ajustes</option></select><select value={warehouseId} onChange={(event) => setWarehouseId(event.target.value)}><option value="all">Todos los almacenes</option>{[...new Map(movements.data.map((row: any) => [row.warehouse_id, row.warehouses])).entries()].filter(([id, warehouse]) => id && warehouse).map(([id, warehouse]: any) => <option key={id} value={id}>{warehouse.name}</option>)}</select><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar origen o motivo" /></div><StateBlock loading={movements.loading} error={movements.error} retry={movements.reload} empty={!filtered.length}><div className="material-page-movements"><table><thead><tr><th>Fecha</th><th>Tipo</th><th>Cantidad</th><th>Almacén</th><th>Origen</th><th>Usuario</th><th>Motivo</th><th>Acción</th></tr></thead><tbody>{filtered.map((row: any) => { const origin: any = movementOrigin(row); const quantity = movementDisplayQuantity(row.movement_type, row.quantity); const direction = movementDirection(row.movement_type); return <tr key={row.id}><td>{row.created_at ? new Date(row.created_at).toLocaleString('es-ES', { dateStyle: 'short', timeStyle: 'short' }) : '-'}</td><td><Badge tone={direction === 'Salida' ? 'warn' : direction === 'Entrada' ? 'ok' : 'info'}>{movementLabel(row.movement_type)}</Badge></td><td className={quantity < 0 ? 'movement-negative' : 'movement-positive'}>{quantity > 0 ? '+' : ''}{quantity.toLocaleString('es-ES')} {material.unit ?? 'ud'}</td><td>{row.warehouses?.name ?? 'Almacén no informado'}</td><td>{origin.text}</td><td>{row.profiles ? fullName(row.profiles) : row.created_by ? 'Usuario no informado' : 'Sistema'}</td><td className="movement-reason" title={movementReason(row)}>{movementReason(row)}</td><td>{origin.to && <Link className="record-open" to={origin.to}>Abrir</Link>}<details><summary>Detalle</summary><small>{[row.source, row.source_reference, row.idempotency_key, row.id].filter(Boolean).join(' · ') || 'Sin metadatos técnicos'}</small></details></td></tr>; })}</tbody></table></div></StateBlock></Card>;
}

function MaterialDetailPage({ forcedId }: { forcedId?: string } = {}) { return <><MaterialDetailLifecycleActions forcedId={forcedId} /><MaterialDetailPageLegacyRuntime forcedId={forcedId} /></>; }

function MaterialDetailLifecycleActions({ forcedId }: { forcedId?: string }) {
  const { id: routeId = '' } = useParams();
  const id = forcedId ?? routeId;
  const navigate = useNavigate();
  const validId = isValidMaterialId(id);
  const material = useLoad(() => validId ? materialsService.get(id) : Promise.resolve(null), [id, validId], null as any);
  const [target, setTarget] = useState<HTMLElement | null>(null);
  useEffect(() => {
    setTarget(null);
    const findTarget = () => {
      const nextTarget = document.querySelector<HTMLElement>('.material-detail-actions');
  ...
  if (material.loading || material.error || !material.data) return null;
  return target ? createPortal(<LifecycleActionPanel entity="materials" record={material.data} onChanged={material.reload} />, target) : null;
}

function MaterialDetailPageLegacyRuntime({ forcedId }: { forcedId?: string } = {}) {
  const { id: routeId = '' } = useParams(); const id = forcedId ?? routeId; const validId = isValidMaterialId(id); const routeNavigate = useNavigate(); const navigate = (route: string) => route === '/app/modulos/compras' ? routeNavigate(`/app/modulos/compras?material=${id}`) : routeNavigate(route); const [searchParams] = useSearchParams();
  const material = useLoad(() => validId ? materialsService.get(id) : Promise.resolve(null), [id, validId], null as any); const stock = useLoad(() => workOrdersService.warehouseStockCatalog(), [], [] as any[]); const warehouses = useLoad(() => workOrdersService.warehousesCatalog(), [], [] as any[]); const relations = useLoad(() => validId ? suppliersService.listMaterialSuppliers(id) : Promise.resolve([]), [id, validId], [] as any[]); const { profile } = useAuth();
  const [editing, setEditing] = useState(searchParams.get('editar') === '1'); const [adjusting, setAdjusting] = useState(false); const tab = searchParams.get('tab') === 'movimientos' ? 'movimientos' : 'resumen';
  useEffect(() => { setEditing(searchParams.get('editar') === '1'); }, [searchParams]);
  if (!validId) return <section className="page material-detail-page"><Link className="back-link" to="/app/modulos/materiales">← Volver a materiales</Link><Card title="Material no identificado"><p className="form-error">No se ha podido identificar el material.</p></Card></section>;
  if (material.loading || material.error || !material.data) return <section className="page material-detail-page"><Link className="back-link" to="/app/modulos/materiales">← Volver a materiales</Link><StateBlock loading={material.loading} error={material.error} retry={material.reload} empty={!material.data} /></section>;
  const data = material.data; const balances = stock.data.filter((row: any) => row.material_id === data.id); const totalStock = balances.reduce((sum: number, row: any) => sum + Number(row.quantity ?? 0), 0); const preferred = relations.data.find((row: any) => row.active !== false && row.is_preferred);
  const canEdit = hasPermission(profile, 'materials.update'); const canAdjust = hasPermission(profile, 'stock.adjust'); const refresh = () => { material.reload(); stock.reload(); relations.reload(); };
  return <section className="page material-detail-page"><div className="material-detail-top"><div><Link className="back-link" to="/app/modulos/materiales">← Volver a materiales</Link><p className="eyebrow">Ficha de material</p><h2>{data.code}</h2><p className="material-detail-name">{data.description}</p><div className="material-detail-meta"><Badge tone={data.active === false ? 'muted' : 'ok'}>{data.active === false ? 'Inactivo' : 'Activo'}</Badge><span>Unidad: {data.unit ?? 'ud'}</span>{data.manufacturer && <span>Fabricante: {data.manufacturer}</span>}</div></div><div className="actions material-detail-actions">{canEdit && <button type="button" onClick={() => setEditing(true)}>Editar material</button>}{canAdjust && <button type="button" onClick={() => setAdjusting(true)}>Ajustar stock</button>}<button type="button" className="primary" onClick={() => navigate('/app/modulos/compras')}>Comprar</button></div></div><div className="material-summary-grid"><div className="material-summary-card stock"><span>Stock actual</span><strong>{totalStock.toLocaleString('es-ES')} {data.unit ?? 'ud'}</strong><small>Saldo de almacenes</small></div><div className="material-summary-card"><span>Stock mínimo</span><strong>{Number(data.minimum_stock ?? 0).toLocaleString('es-ES')} {data.unit ?? 'ud'}</strong><small>Umbral configurado</small></div><div className="material-summary-card"><span>Coste</span><strong>{Number(data.cost ?? 0).toLocaleString('es-ES')} €</strong><small>Coste de catálogo</small></div><div className="material-summary-card"><span>Precio de venta</span><strong>{Number(data.price ?? 0).toLocaleString('es-ES')} €</strong><small>Precio de catálogo</small></div><div className="material-summary-card"><span>Proveedor preferente</span><strong>{preferred?.suppliers?.name ?? 'Sin proveedor preferente'}</strong><small>{relations.data.filter((row: any) => row.active !== false).length} proveedor(es) asociado(s)</small></div></div><nav className="material-detail-tabs" aria-label="Secciones del material"><Link className={tab === 'resumen' ? 'active' : ''} to={`/app/modulos/materiales/${id}`}>Resumen</Link><Link className={tab === 'movimientos' ? 'active' : ''} to={`/app/modulos/materiales/${id}?tab=movimientos`}>Movimientos</Link></nav>{tab === 'resumen' ? <><div className="material-detail-columns"><Card title="Stock y almacenes"><div className="material-page-stock">{balances.map((row: any) => { const warehouse = warehouses.data.find((item: any) => item.id === row.warehouse_id); return <div className="material-page-stock-row" key={row.warehouse_id}><strong>{warehouse?.name ?? 'Almacén no informado'}</strong><span>{Number(row.quantity ?? 0).toLocaleString('es-ES')} {data.unit ?? 'ud'}</span></div>; })}{!balances.length && <p className="large-note">Sin existencias abiertas por almacén.</p>}</div></Card><Card title="Información del material"><InfoGrid items={[[ 'Código', data.code ], [ 'Descripción', data.description ], [ 'Fabricante', data.manufacturer ?? '-' ], [ 'Referencia', data.reference ?? '-' ], [ 'Unidad', data.unit ?? 'ud' ], [ 'Estado', data.active === false ? 'Inactivo' : 'Activo' ], [ 'A medida', data.made_to_measure ? 'Sí' : 'No' ], [ 'Uso único', data.single_use ? 'Sí' : 'No' ]]} /></Card></div><MaterialSuppliersEditor materialId={data.id} /></> : <MaterialMovementsSection material={data} />}<div className="material-detail-footer"><Link className="back-link" to="/app/modulos/materiales">← Volver a materiales</Link><span>Los movimientos son históricos y el stock actual procede de warehouse_stock.</span></div>{editing && <MaterialMasterForm initial={data} warehouses={warehouses.data} redirectExisting={false} onClose={() => setEditing(false)} onSaved={() => { setEditing(false); refresh(); }} />}{adjusting && <CanonicalStockAdjustForm material={data} warehouses={warehouses.data} onClose={() => setAdjusting(false)} onSaved={() => { setAdjusting(false); refresh(); }} />}</section>;
}

function MaterialDetailPanelLegacy({ material, onClose }: { material: any; onClose: () => void }) {
  const movements = useLoad(() => materialsService.movements(material.id), [material.id], [] as any[]);
  const relations = useLoad(() => suppliersService.listMaterialSuppliers(material.id), [material.id], [] as any[]);
  const stock = useLoad(() => workOrdersService.warehouseStockCatalog(), [], [] as any[]);
  const warehouses = useLoad(() => workOrdersService.warehousesCatalog(), [], [] as any[]);
  const balances = stock.data.filter((row: any) => row.material_id === material.id);
  return <SidePanel title={`Ficha de material · ${material.code}`} subtitle={material.description} onClose={onClose}><Card title="Datos del material"><InfoGrid items={[[ 'Descripción', material.description ], [ 'Unidad', material.unit ?? 'ud' ], [ 'Coste', `${Number(material.cost ?? 0).toLocaleString('es-ES')} €` ], [ 'Precio de venta', `${Number(material.price ?? 0).toLocaleString('es-ES')} €` ], [ 'Stock mínimo', `${Number(material.minimum_stock ?? 0).toLocaleString('es-ES')} ${material.unit ?? 'ud'}` ], [ 'Material a medida', material.made_to_measure ? 'Sí' : 'No' ], [ 'Uso único', material.single_use ? 'Sí' : 'No' ], [ 'Estado', material.active === false ? 'Inactivo' : 'Activo' ]]} /></Card><Card title="Existencias por almacén"><div className="compact-list">{balances.map((row: any) => { const warehouse = warehouses.data.find((item: any) => item.id === row.warehouse_id); const low = Number(row.quantity ?? 0) <= Number(material.minimum_stock ?? 0); return <article key={row.warehouse_id}><strong>{warehouse ? `${warehouse.code} · ${warehouse.name}` : row.warehouse_id}</strong><span>{Number(row.quantity ?? 0).toLocaleString('es-ES')} {material.unit ?? 'ud'} · {low ? 'Bajo stock' : 'Disponible'}</span></article>; })}{!stock.loading && !balances.length && <p className="large-note">Sin existencias abiertas por almacén.</p>}</div></Card><MaterialSuppliersEditor materialId={material.id} /><Card title="Movimientos recientes"><div className="compact-list">{movements.data.slice(0, 20).map((row: any) => <article key={row.id}><strong>{formatDate(row.created_at)} · {displayStatus(row.movement_type)}</strong><span>{row.quantity} {material.unit ?? 'ud'} · {row.warehouses?.name ?? 'Almacén no informado'}</span><small>{row.notes ?? 'Sin motivo'}{row.work_orders?.code ? ` · Parte ${row.work_orders.code}` : ''}</small></article>)}{!movements.loading && !movements.data.length && <p className="large-note">Sin movimientos recientes.</p>}</div></Card></SidePanel>;
}
...
function MaterialDetailPanel({ material, onClose }: { material: any; onClose: () => void }) {
  ...
  const stock = useLoad(() => workOrdersService.warehouseStockCatalog(), [], [] as any[]);
  const warehouses = useLoad(() => workOrdersService.warehousesCatalog(), [], [] as any[]);
  const [showAll, setShowAll] = useState(false); const [type, setType] = useState('all'); const [warehouseId, setWarehouseId] = useState('all'); const [search, setSearch] = useState('');
  const balances = stock.data.filter((row: any) => row.material_id === material.id); const totalStock = balances.reduce((sum: number, row: any) => sum + Number(row.quantity ?? 0), 0);
  const filtered = movements.data.filter((row: any) => { const origin = movementOrigin(row); const haystack = [origin.text, row.notes, row.source_reference, row.work_orders?.code, row.purchase_receipts?.code, row.purchase_orders?.code].filter(Boolean).join(' ').toLowerCase(); return (type === 'all' || movementDirection(row.movement_type) === type || row.movement_type === type) && (warehouseId === 'all' || row.warehouse_id === warehouseId) && (!search.trim() || haystack.includes(search.trim().toLowerCase())); });
   const displayed = showAll ? filtered : filtered.slice(0, 20); const formatMovementDate = (value: string) => value ? new Date(value).toLocaleString('es-ES', { dateStyle: 'short', timeStyle: 'short' }) : '-';
  return <SidePanel title={`Ficha de material · ${material.code}`} subtitle={`${material.description} · Stock total: ${totalStock.toLocaleString('es-ES')} ${material.unit ?? 'ud'}`} onClose={onClose}><div className="material-detail-grid"><Card title="Identidad"><InfoGrid items={[[ 'Código', material.code ], [ 'Descripción', material.description ], [ 'Fabricante', material.manufacturer ], [ 'Referencia', material.reference ], [ 'Unidad', material.unit ?? 'ud' ], [ 'Estado', material.active === false ? 'Inactivo' : 'Activo' ]].filter((item: any) => item[1] != null && item[1] !== '') as [string, any][]} /></Card><Card title="Economía"><InfoGrid items={[[ 'Coste', `${Number(material.cost ?? 0).toLocaleString('es-ES')} €` ], [ 'Precio venta', `${Number(material.price ?? 0).toLocaleString('es-ES')} €` ]]} /></Card><Card title="Características"><InfoGrid items={[[ 'A medida', material.made_to_measure ? 'Sí' : 'No' ], [ 'Uso único', material.single_use ? 'Sí' : 'No' ]]} /></Card></div><Card title={`Existencias · ${totalStock.toLocaleString('es-ES')} ${material.unit ?? 'ud'} disponibles`}><div className="material-stock-table"><div className="material-stock-head"><span>Almacén</span><span>Disponible</span><span>Estado</span></div>{balances.map((row: any) => { const warehouse = warehouses.data.find((item: any) => item.id === row.warehouse_id); const low = Number(row.quantity ?? 0) <= Number(material.minimum_stock ?? 0); return <div className="material-stock-row" key={row.warehouse_id}><strong>{warehouse?.name ?? 'Almacén no informado'}</strong><span>{Number(row.quantity ?? 0).toLocaleString('es-ES')} {material.unit ?? 'ud'}</span><span>{low ? 'Bajo stock' : 'Disponible'}</span></div>; })}{!stock.loading && !balances.length && <p className="large-note">Sin existencias abiertas por almacén.</p>}</div></Card><MaterialSuppliersEditor materialId={material.id} /><Card title="Movimientos recientes" action={<button type="button" onClick={() => setShowAll((current) => !current)}>{showAll ? 'Ver resumen' : 'Ver todos los movimientos'}</button>}><div className="movement-filter-bar"><select value={type} onChange={(event) => setType(event.target.value)}><option value="all">Todos los tipos</option><option value="Entrada">Entradas</option><option value="Salida">Salidas</option><option value="Consumo en parte">Consumos</option><option value="Ajuste">Ajustes</option></select><select value={warehouseId} onChange={(event) => setWarehouseId(event.target.value)}><option value="all">Todos los almacenes</option>{warehouses.data.map((item: any) => <option key={item.id} value={item.id}>{item.name}</option>)}</select><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar origen o motivo" /></div><div className="material-movements-table"><div className="material-movement-head"><span>Fecha</span><span>Tipo</span><span>Cantidad</span><span>Almacén</span><span>Origen</span><span>Usuario</span><span>Motivo</span><span>Acción</span></div>{displayed.map((row: any) => { const origin: any = movementOrigin(row); const quantity = movementDisplayQuantity(row.movement_type, row.quantity); const direction = movementDirection(row.movement_type); const actor = row.profiles ? fullName(row.profiles) : row.created_by ? 'Usuario no informado' : 'Sistema'; return <div className="material-movement-row" key={row.id}><span>{formatMovementDate(row.created_at)}</span><span><Badge tone={direction === 'Salida' ? 'warn' : direction === 'Entrada' ? 'ok' : 'info'}>{movementLabel(row.movement_type)}</Badge></span><strong className={quantity < 0 ? 'movement-negative' : 'movement-positive'}>{quantity > 0 ? '+' : ''}{quantity.toLocaleString('es-ES')} {material.unit ?? 'ud'}</strong><span>{row.warehouses?.name ?? 'Almacén no informado'}</span><span>{origin.text}</span><span>{actor}</span><span>{movementReason(row)}</span><span>{origin.to && <Link className="record-open" to={origin.to}>Abrir</Link>}<details><summary>Detalle técnico</summary><small>{[row.source, row.source_reference, row.idempotency_key].filter(Boolean).join(' · ') || 'Sin metadatos técnicos'}</small></details></span></div>; })}{!movements.loading && !displayed.length && <p className="large-note">Sin movimientos para el filtro actual.</p>}</div></Card></SidePanel>;
}

function StockReconciliationSpanishView({ row, warehouses, onClose, onResolved }: { row: any; warehouses: any[]; onClose: () => void; onResolved: () => Promise<void> }) {
  const canonical = row.canonical[0];
  const warehouse = warehouses.find((item: any) => item.id === canonical.warehouse_id);
  const movements = useLoad(() => workOrdersService.warehouseStockMovements(canonical.warehouse_id, row.material.id), [canonical.warehouse_id, row.material.id], [] as any[]);
  const [quantity, setQuantity] = useState(String(canonical.quantity ?? 0));
  const [reason, setReason] = useState(''); const [saving, setSaving] = useState(false); const [error, setError] = useState('');
  const resolve = async (action: 'accept_canonical' | 'adjust_canonical') => { const confirmed = Number(quantity); if (!reason.trim()) { setError('Indica el motivo de la conciliación.'); return; } if (action === 'adjust_canonical' && (!Number.isFinite(confirmed) || confirmed < 0)) { setError('La cantidad confirmada no puede ser negativa.'); return; } setSaving(true); setError(''); try { await workOrdersService.resolveInitialStockReview({ warehouse_id: canonical.warehouse_id, material_id: row.material.id, action, confirmed_quantity: action === 'accept_canonical' ? Number(canonical.quantity) : confirmed, reason: reason.trim(), idempotency_key: crypto.randomUUID() }); await onResolved(); } catch (err) { setError(formErrorMessage(err, 'No se ha podido resolver la discrepancia.')); } finally { setSaving(false); } };
  return <div className="mini-modal" role="dialog" aria-modal="true"><form onSubmit={(event) => event.preventDefault()}><h3>Resolver discrepancia de existencias</h3><InfoGrid items={[[ 'Material', `${row.material.code} · ${row.material.description}` ], [ 'Existencia anterior', `${row.legacy.toLocaleString('es-ES')} ${row.material.unit ?? 'ud'}` ], [ 'Existencia canónica', `${Number(canonical.quantity ?? 0).toLocaleString('es-ES')} ${row.material.unit ?? 'ud'}` ], [ 'Almacén', warehouse ? `${warehouse.code} · ${warehouse.name}` : canonical.warehouse_id ]]} /><label>Cantidad física confirmada<input type="number" min="0" step="0.01" value={quantity} onChange={(event) => setQuantity(event.target.value)} /></label><label>Motivo obligatorio<textarea value={reason} onChange={(event) => setReason(event.target.value)} required placeholder="Recuento físico o conciliación documental" /></label><p className="large-note">Movimientos existentes: {movements.loading ? 'Cargando...' : movements.data.length}. Aceptar la existencia canónica no crea movimiento; ajustar registra la diferencia.</p>{error && <p className="form-error">{error}</p>}<div className="modal-footer"><button type="button" onClick={onClose} disabled={saving}>Cancelar</button><button type="button" onClick={() => resolve('accept_canonical')} disabled={saving}>Aceptar existencia canónica</button><button type="button" className="primary" onClick={() => resolve('adjust_canonical')} disabled={saving}>{saving ? 'Guardando...' : 'Ajustar cantidad confirmada'}</button></div></form></div>;
}

export { LifecycleActionPanel, LifecycleConfirmModal, DependencyRows };
export default App;
