...
function SatDashboard() {
  ...
  return <RoleDashboard title="Inicio SAT" subtitle="Centro operativo de planificación, partes, técnicos, checks y avisos." kpis={kpis} quickActions={<><button onClick={() => setMode('work')}>Crear parte</button><Link to="/app/partes?filtro=sin-asignar">Asignar técnico</Link><button onClick={() => setMode('check')}>Crear check</button><button onClick={() => setMode('alert')}>Crear aviso</button><Link to="/app/partes?fecha=hoy">Abrir planificación</Link><Link to="/app/deficiencias?estado=pendiente">Revisar deficiencia</Link></>}><DashboardList title="Trabajos de hoy" rows={todayWorks.slice(0, 8).map((w: any) => [`${w.scheduled_time ?? 'Sin hora'} · ${w.code}`, `${displayStatus(w.priority)} · ${w.type} · ${w.client_name} · ${w.site_name} · ${w.main_technician_name ?? 'Sin técnico'} · ${displayStatus(w.status)}`, severityForPriority(w.priority), `/app/partes/${w.id}`])} empty="No hay trabajos planificados hoy." /><DashboardList title="Técnicos" rows={data.technicians.slice(0, 6).map((t: any) => [fullName(t), 'Perfil técnico activo · disponibilidad según partes asignados', 'info', `/app/modulos/tecnicos/`])} empty="Sin técnicos visibles." /><DashboardList title="Alertas operativas" rows={[...delayed, ...unassigned, ...criticalAlerts, ...openDef].slice(0, 8).map((item: any) => [item.code ?? item.title, item.description ?? item.title ?? item.status, severityForStatus(item.status ?? item.priority), item.related_id ? routeForAlert(item) : '/app/partes'])} empty="Sin alertas operativas destacadas." /><DashboardList title="Actividad reciente" rows={data.assignments.slice(0, 8).map((a: any) => [a.work_orders?.code ?? 'Asignación', `${a.assignment_date} · ${fullName(a.profiles)} · ${displayStatus(a.status)}`, severityForStatus(a.status), `/app/partes/${a.work_order_id}`])} empty="Sin actividad reciente." />{mode === 'work' && <WorkOrderForm onClose={() => setMode(null)} onSaved={() => { setMode(null); reload(); }} />}{mode === 'alert' && <AlertForm onClose={() => setMode(null)} onSaved={() => { setMode(null); reload(); }} />}{mode === 'check' && <CheckForm onClose={() => setMode(null)} onSaved={() => { setMode(null); reload(); }} />}</RoleDashboard>;
}
