...
import { canAccessModule, canAccessRoute, canArchiveEntity, canAssignTechnician, canCreateAlert, canCreateCheck, canCreateWorkOrder, canEditWorkOrder, canExecuteCheck, canExecuteWorkOrder, canManageCheck, canManageWorkOrderAssignments, canPermanentlyDeleteEntity, canRestoreEntity, canRole, canViewCheck, canViewWorkOrder, isSuperadmin, normalizedRoleNames, profileWorkspaces } from './auth/permissions';
...
function LifecycleActionPanel({ entity, record, onChanged }: { entity: LifecycleEntity; record: any; onChanged: () => void }) { const { profile } = useAuth(); const [action, setAction] = useState<'archive' | 'restore' | 'delete' | 'dependencies' | null>(null); const archived = isArchivedRecord(entity, record); const allowedArchive = canArchiveEntity(profile, record); const allowedRestore = canRestoreEntity(profile, record); const allowedDelete = canPermanentlyDeleteEntity(profile, record); if (!allowedArchive && !allowedRestore && !allowedDelete) return null; return <div className="lifecycle-panel"><div className="actions"><button type="button" onClick={() => setAction('dependencies')}>Ver dependencias</button>{!archived && allowedArchive && <button type="button" onClick={() => setAction('archive')}>Archivar/desactivar</button>}{archived && allowedRestore && <button type="button" onClick={() => setAction('restore')}>Restaurar</button>}{allowedDelete && <button type="button" className="danger-action" onClick={() => setAction('delete')}>Eliminar definitivamente</button>}</div>{action && <LifecycleConfirmModal entity={entity} record={record} action={action} onClose={() => setAction(null)} onChanged={() => { setAction(null); onChanged(); }} />}</div>; }

function LifecycleConfirmModal({ entity, record, action, onClose, onChanged }: { entity: LifecycleEntity; record: any; action: 'archive' | 'restore' | 'delete' | 'dependencies'; onClose: () => void; onChanged: () => void }) { const opener = useRef<HTMLElement | null>(document.activeElement as HTMLElement | null); const [summary, setSummary] = useState<LifecycleSummary | null>(null); const [reason, setReason] = useState(''); const [confirmation, setConfirmation] = useState(''); const [saving, setSaving] = useState(false); const [error, setError] = useState(''); const target = summary ?? { code: record.code ?? record.email ?? record.name ?? record.title ?? record.id, name: record.legal_name ?? record.name ?? record.title ?? record.email ?? record.code ?? record.id, dependencies: {}, dependency_total: 0, can_restore: true, can_permanently_delete: false, physical_delete_blocker: null, restore_blocker: null } as LifecycleSummary; const expected = `ELIMINAR ${target.code}`; useOverlayScrollLock(); useEffect(() => { entityLifecycleService.dependencies(entity, record.id).then(setSummary).catch((err) => setError(err instanceof Error ? err.message : 'No se han podido consultar las dependencias.')); }, [entity, record.id]); useEffect(() => { const onKey = (event: KeyboardEvent) => { if (event.key === 'Escape' && !saving) onClose(); }; window.addEventListener('keydown', onKey); return () => { window.removeEventListener('keydown', onKey); opener.current?.focus(); }; }, [onClose, saving]); const submit = async () => { if (saving || action === 'dependencies') return; if (!reason.trim()) { setError('El motivo es obligatorio.'); return; } if (action === 'delete' && confirmation !== expected) { setError(`Para borrar definitivamente escribe: ${expected}`); return; } setSaving(true); setError(''); try { if (action === 'archive') await entityLifecycleService.archive(entity, record.id, reason.trim()); if (action === 'restore') await entityLifecycleService.restore(entity, record.id, reason.trim()); if (action === 'delete') await entityLifecycleService.permanentlyDelete(entity, record.id, reason.trim(), confirmation); onChanged(); } catch (err) { setError(err instanceof Error ? err.message : 'No se ha podido completar la operación. Revisa las dependencias indicadas.'); } finally { setSaving(false); } }; const title = action === 'archive' ? 'Archivar o desactivar registro' : action === 'restore' ? 'Restaurar registro' : action === 'delete' ? 'Eliminar definitivamente' : 'Dependencias del registro'; return <div className="mini-modal lifecycle-modal" role="dialog" aria-modal="true" aria-labelledby="lifecycle-title"><div><h3 id="lifecycle-title">{title}</h3><InfoGrid items={[[ 'Tipo', entityLabels[entity] ], [ 'Código', target.code ], [ 'Nombre', target.name ], [ 'Registros relacionados', String(target.dependency_total ?? 0) ], [ 'Restaurable', target.can_restore ? 'Sí' : `No. ${target.restore_blocker ?? ''}` ], [ 'Borrado definitivo legal', target.can_permanently_delete ? 'Sí, si confirmas expresamente' : `No. ${target.physical_delete_blocker ?? 'Hay dependencias'}` ]]} /><Card title="Dependencias detectadas"><CompactRows rows={Object.entries(target.dependencies ?? {}).map(([name, count]) => [displayStatus(name), `${count} registro(s)`, Number(count) > 0 ? 'warn' : 'ok', '#'])} empty="Sin dependencias." /></Card><p className="large-note">Archivar conserva historial, auditoría y relaciones. Eliminar definitivamente solo se permite para registros nuevos sin dependencias y se vuelve a validar en Supabase.</p>{action !== 'dependencies' && <label>Motivo obligatorio<textarea value={reason} onChange={(event) => setReason(event.target.value)} placeholder="Explica por qué se realiza esta operación" /></label>}{action === 'delete' && <label>Confirmación definitiva<input value={confirmation} onChange={(event) => setConfirmation(event.target.value)} placeholder={expected} /></label>}{error && <p className="form-error">{error}</p>}<div className="modal-footer"><button type="button" onClick={onClose} disabled={saving}>Cerrar</button>{action !== 'dependencies' && <button type="button" className="primary" onClick={submit} disabled={saving}>{saving ? 'Procesando...' : action === 'archive' ? 'Archivar' : action === 'restore' ? 'Restaurar' : 'Eliminar definitivamente'}</button>}</div></div></div>; }
