...
function AssociatedEquipmentCard({ workOrder, onChanged }: { workOrder: any; onChanged?: () => void }) {
  ...
  return <Card title="EQUIPOS ASOCIADOS"><div className="compact-list">{rows.map((row: any) => { const equipment = row.equipment ?? row; const check = (workOrder.checks ?? []).find((item: any) => item.equipment_id === equipment.id); return <article key={equipment.id}><Badge tone={row.is_primary ? 'ok' : 'info'}>{row.is_primary ? 'PRINCIPAL' : 'ASOCIADO'}</Badge><strong>{equipment.code ?? 'Equipo'}</strong><span>{equipment.equipment_types?.name ?? 'Tipo no informado'} · {equipment.brand ?? '-'} {equipment.model ?? ''} · {equipment.internal_location ?? '-'}{equipment.serial_number ? ` · Serie ${equipment.serial_number}` : ''}</span><span>{check ? `✓ Check ${check.code} · ${displayStatus(check.status)}` : row.check_status === 'pending_template' ? '⚠ Sin plantilla compatible' : row.check_status === 'not_applicable' ? 'No aplica check de instalación' : 'Sin check'}</span>{row.check_status === 'pending_template' && <button type="button" onClick={() => generate(equipment.id)} disabled={saving === equipment.id}>{saving === equipment.id ? 'Generando...' : 'Generar check pendiente'}</button>}</article>; })}</div>{!rows.length && <p className="large-note">No hay equipos asociados.</p>}</Card>;
}
