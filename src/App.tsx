...
function CheckBlockPage() {
  ...
  return <section className="check-mobile"><button className="link-button sticky-back" onClick={() => navigate(`/app/checks/${id}`)}><ChevronLeft size={16} /> Volver a la puerta</button><header><p className="eyebrow">Detalle del bloque</p><h2>{zone.name}</h2><Badge tone={severityForStatus(status)}>{displayStatus(status)}</Badge></header><div className="status-grid">{['Todo favorable', ...checkProblemStatuses].map((item) => <button key={item} className={status === item ? 'active' : ''} onClick={() => setStatus(item)}>{item}</button>)}</div>{['Todo favorable', 'No aplicable'].includes(status) && <button className="primary wide big" disabled={saving} onClick={() => save(status)}>Confirmar selección</button>}{status === 'Sin revisar' && <p className="large-note">Selecciona un estado y pulsa Confirmar selección para guardar el bloque.</p>}{!['Todo favorable', 'Sin revisar', 'No aplicable'].includes(status) && <Card title="Componentes afectados"><div className="component-select">{zone.components.map((component) => <label key={component}><input type="checkbox" /> {component}</label>)}</div><label>Observación<textarea value={observations} onChange={(event) => setObservations(event.target.value)} required /></label><label>Intervención<textarea placeholder="Intervención realizada si aplica" /></label><div className="photo-strip"><button type="button">Añadir fotografía</button></div><button className="primary wide" disabled={saving || !observations.trim()} onClick={() => save()}>Guardar bloque</button></Card>}</section>;
}
