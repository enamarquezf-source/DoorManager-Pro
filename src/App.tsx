...
function WorkOrderOperationalCard({ workOrder, onChanged }: { workOrder: any; onChanged: () => void }) { const { profile } = useAuth(); const [editing, setEditing] = useState(false); const allowed = canCorrectWorkOrderOperationalFields(profile, workOrder); const closed = ['Cerrado','Cancelado'].includes(workOrder.status); return <Card title="Corrección operativa" action={allowed ? <button className="primary" onClick={() => setEditing(true)}>Editar/guardar trabajo</button> : null}><InfoGrid items={[[ 'Descripción del problema', workOrder.description ?? '-' ], [ 'Diagnóstico', workOrder.diagnosis ?? '-' ], [ 'Trabajo realizado', workOrder.work_performed ?? '-' ], [ 'Solución / resultado', workOrder.result ?? '-' ], [ 'Material previsto', workOrder.planned_material ?? '-' ], [ 'Última modificación', workOrder.updated_at ? `${formatDate(workOrder.updated_at)} · ${fullName(workOrder.updated_by_profile) || workOrder.updated_by || 'Usuario no informado'}` : '-' ]]}/>{!allowed && <p className="large-note">No tienes permiso para corregir estos campos del parte.</p>}{closed && <p className="state-warning">Este parte está cerrado. Solo perfiles autorizados pueden corregirlo o reabrirlo.</p>}{editing && <WorkOrderOperationalForm workOrder={workOrder} onClose={() => setEditing(false)} onSaved={() => { setEditing(false); onChanged(); }} />}</Card>; }

function WorkOrderOperationalForm({ workOrder, onClose, onSaved }: { workOrder: any; onClose: () => void; onSaved: () => void }) { const [values, setValues] = useState<Record<string, string>>({ description: workOrder.description ?? '', diagnosis: workOrder.diagnosis ?? '', work_performed: workOrder.work_performed ?? '', result: workOrder.result ?? '', planned_material: workOrder.planned_material ?? '' }); const [error, setError] = useState(''); const [saving, setSaving] = useState(false); const set = (key: string, value: string) => setValues((current) => ({ ...current, [key]: value })); const submit = async (event: FormEvent) => { event.preventDefault(); setSaving(true); setError(''); try { await workOrdersService.updateOperationalFields(workOrder.id, values); onSaved(); } catch (err) { setError(err instanceof Error ? err.message : 'No se han podido corregir los campos operativos del parte.'); } finally { setSaving(false); } }; return <div className="mini-modal" role="dialog" aria-modal="true"><form onSubmit={submit}><h3>Revisión/corrección de oficina</h3><p className="large-note">Solo se guardan estos campos operativos; no se modifican cliente, centro, equipo ni asignaciones.</p><label>Descripción del problema<textarea value={values.description} onChange={(event) => set('description', event.target.value)} /></label><label>Diagnóstico<textarea value={values.diagnosis} onChange={(event) => set('diagnosis', event.target.value)} /></label><label>Trabajo realizado<textarea value={values.work_performed} onChange={(event) => set('work_performed', event.target.value)} /></label><label>Solución / resultado<textarea value={values.result} onChange={(event) => set('result', event.target.value)} /></label><label>Material previsto<input value={values.planned_material} onChange={(event) => set('planned_material', event.target.value)} /></label>{error && <p className="form-error">{error}</p>}<div className="modal-footer"><button type="button" onClick={onClose} disabled={saving}>Cancelar</button><button className="primary" disabled={saving}>{saving ? 'Guardando...' : 'Guardar cambios operativos'}</button></div></form></div>; }
