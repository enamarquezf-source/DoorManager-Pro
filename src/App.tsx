...
function RateCatalogForm({ initial, onClose, onSaved }: { initial: any; onClose: () => void; onSaved: () => void }) { const [values, setValues] = useState<Record<string, any>>({ code: '', name: '', kind: 'cost', classification: 'cost', unit: 'ud', billing_mode: 'unit', period_days: '', cost_amount: 0, sale_amount: 0, valid_from: new Date().toISOString().slice(0, 10), valid_to: '', active: true, contributes_to_sale: false, ...initial }); const [saving, setSaving] = useState(false); const [error, setError] = useState(''); const set = (key: string, value: any) => setValues((current) => ({ ...current, [key]: value })); const submit = async (event: FormEvent) => { event.preventDefault(); setSaving(true); setError(''); try { const classification = values.classification === 'labor' ? 'labor' : 'cost'; if (initial?.id) { await hourRatesService.updateCatalog(initial.id, { kind: classification, classification, unit: values.unit, billing_mode: values.billing_mode, period_days: values.period_days === '' ? null : Number(values.period_days), contributes_to_sale: values.contributes_to_sale }); await hourRatesService.createVersion({ rate_id: initial.id, ...values }); } else { const created = await hourRatesService.createCatalog({ ...values, kind: classification, classification }); await hourRatesService.createVersion({ rate_id: created.id, ...values }); } onSaved(); } catch (err) { setError(formErrorMessage(err, 'No se ha podido guardar el concepto o version.')); } finally { setSaving(false); } }; return <ModalForm title={initial?.id ? 'Nueva version de tarifa' : 'Nuevo concepto de tarifa'} onClose={onClose} onSubmit={submit} saving={saving} error={error}><div className="form-grid"><label>Codigo<input value={values.code ?? ''} onChange={(event) => set('code', event.target.value)} disabled={Boolean(initial?.id)} required /></label><label>Nombre<input value={values.name ?? ''} onChange={(event) => set('name', event.target.value)} disabled={Boolean(initial?.id)} required /></label><label>Clasificacion<select value={values.classification} onChange={(event) => set('classification', event.target.value)}><option value="labor">Mano de obra</option><option value="cost">Recurso / coste</option></select></label><label>Unidad<input value={values.unit ?? ''} onChange={(event) => set('unit', event.target.value)} disabled={Boolean(initial?.id)} required /></label><label>Modalidad<select value={values.billing_mode} onChange={(event) => set('billing_mode', event.target.value)} disabled={Boolean(initial?.id)}><option value="unit">Unidad</option><option value="hour">Hora</option><option value="day">Dia</option><option value="period">Periodo</option></select></label><label>Dias del periodo<input type="number" min="1" step="1" value={values.period_days ?? ''} onChange={(event) => set('period_days', event.target.value)} /></label><label>Valida desde<input type="date" value={values.valid_from ?? ''} onChange={(event) => set('valid_from', event.target.value)} required /></label><label>Valida hasta<input type="date" value={values.valid_to ?? ''} onChange={(event) => set('valid_to', event.target.value)} /></label><label>Coste interno<input type="number" min="0" step="0.01" value={values.cost_amount ?? 0} onChange={(event) => set('cost_amount', event.target.value)} /></label><label>Precio venta<input type="number" min="0" step="0.01" value={values.sale_amount ?? 0} onChange={(event) => set('sale_amount', event.target.value)} /></label><label>Version activa<input type="checkbox" checked={values.active !== false} onChange={(event) => set('active', event.target.checked)} /></label><label><input type="checkbox" checked={values.contributes_to_sale === true} onChange={(event) => set('contributes_to_sale', event.target.checked)} /> Facturable como venta adicional<small>Solo afecta a costes con origen adicional; no cambia si el coste procede del presupuesto.</small></label></div><label>Observaciones<textarea value={values.notes ?? ''} onChange={(event) => set('notes', event.target.value)} /></label></ModalForm>; }

function ManagementPage060() { return <CanonicalManagementPage />; }

function ModulePage() { const { moduleId = '' } = useParams(); if (moduleId === 'planificacion') return <PlanningModule />; if (moduleId === 'tecnicos') return <TechniciansModule />; if (moduleId === 'comerciales') return <CommercialProfilesModule />; if (moduleId === 'ventas') return <SalesModule mode="ventas" />; if (moduleId === 'oportunidades') return <SalesModule mode="oportunidades" />; if (moduleId === 'presupuestos') return <QuotesModule />; if (moduleId === 'materiales') return <MaterialsModule />; if (moduleId === 'tarifas-horas') return <RateCatalogModule />; if (moduleId === 'operaciones') return <OperationsModule />; if (moduleId === 'informes') return <ManagementPage060 />; if (moduleId === 'personal') return <TechniciansModule />; if (moduleId === 'rentabilidad') return <ProfitabilityModule />; if (moduleId === 'administracion') return <CompanySettingsModule />; return <OperationalModule moduleId={moduleId} />; }
