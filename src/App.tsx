...
function PurchaseReceiptForm({ order, warehouses, onClose, onSaved }: any) {
  const [values, setValues] = useState<Record<string, any>>({ receipt_date: new Date().toISOString().slice(0, 10), warehouse_id: order.destination_warehouse_id ?? warehouses[0]?.id ?? '', supplier_document_reference: '', notes: '' });
  const [quantities, setQuantities] = useState<Record<string, string>>({}); const [costs, setCosts] = useState<Record<string, string>>({}); const [saving, setSaving] = useState(false); const [error, setError] = useState('');
  ...
  const submit = async (event: FormEvent) => { event.preventDefault(); const selected = lines.filter((line: any) => Number(quantities[line.id] ?? 0) > 0); if (!values.warehouse_id || !selected.length) { setError('Selecciona almacén y al menos una línea con cantidad.'); return; } setSaving(true); setError(''); try { const id = await purchaseReceiptsService.createDraft(order.id, values); for (const line of selected) await purchaseReceiptsService.addLine(id, { purchase_order_line_id: line.id, received_quantity: quantities[line.id], actual_unit_cost: costs[line.id] === '' ? line.unit_purchase_price : costs[line.id] }); if (window.confirm('Confirmar recepción y generar entrada real en stock?')) await purchaseReceiptsService.confirm(id); onSaved(); } catch (err) { setError(formErrorMessage(err, 'No se ha podido guardar la recepción.')); } finally { setSaving(false); } };
  return <ModalForm title={`Registrar recepción · ${order.code}`} onClose={onClose} onSubmit={submit} saving={saving} error={error}><div className="form-grid"><label>Fecha<input type="date" value={values.receipt_date} onChange={(event) => set('receipt_date', event.target.value)} required /></label><FormSelect label="Almacén real" value={values.warehouse_id} onChange={(value) => set('warehouse_id', value)} options={[{ value: '', label: 'Selecciona almacén' }, ...warehouses.map((warehouse: any) => ({ value: warehouse.id, label: `${warehouse.code} · ${warehouse.name}` }))]} /><label>Referencia / albarán proveedor<input value={values.supplier_document_reference} onChange={(event) => set('supplier_document_reference', event.target.value)} /></label></div><label>Observaciones<textarea value={values.notes} onChange={(event) => set('notes', event.target.value)} /></label><div className="compact-list"><h3>Líneas</h3>{lines.map((line: any) => { const before = receivedBefore(line); const pending = Math.max(0, Number(line.ordered_quantity) - before); return <article key={line.id}><strong>{line.material_description_snapshot}</strong><span>Pedido: {line.ordered_quantity} · Recibido anteriormente: {before} · Pendiente: {pending} · Precio acordado: {line.unit_purchase_price ?? '-'} €</span><div className="form-grid"><label>Recibir ahora<input type="number" min="0" max={pending} step="0.01" value={quantities[line.id] ?? ''} onChange={(event) => setQuantities((current) => ({ ...current, [line.id]: event.target.value }))} /></label><label>Coste real<input type="number" min="0" step="0.01" value={costs[line.id] ?? String(line.unit_purchase_price ?? '')} onChange={(event) => setCosts((current) => ({ ...current, [line.id]: event.target.value }))} /></label></div></article>; })}</div><p className="large-note">Guardar borrador y confirmar genera stock real. Las recepciones confirmadas quedan congeladas.</p></ModalForm>;
}
