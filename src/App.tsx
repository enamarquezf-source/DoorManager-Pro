...
import { canAccessModule, canAccessRoute, canAssignTechnician, canCreateAlert, canCreateCheck, canCreateWorkOrder, canEditWorkOrder, canExecuteWorkOrder, canViewWorkOrder, isSuperadmin } from './auth/permissions';
...
function AccessDeniedZone() { return <section className="page"><Card title="No tienes permiso para acceder a esta zona"><p className="form-error">No tienes permiso para acceder a esta zona</p><Link className="primary" to="/app/inicio">Volver al inicio</Link></Card></section>; }
...
function ModulePage() { const { moduleId = '' } = useParams(); const { workspace } = useAuth(); if (moduleId === 'tecnicos') return <TechniciansModule />; if (moduleId === 'comerciales') return <CommercialProfilesModule />; const meta = moduleMeta[moduleId] ?? { title: 'Módulo en preparación', description: 'Este módulo tendrá su propia pantalla y no redirige a otro módulo incorrecto.', links: [] }; return <section className="page"><Breadcrumb items={[workspaceTitles[workspace], meta.title]} /><Hero title={meta.title} subtitle={meta.description} tone="info" /><Card title="Estado del módulo"><p className="large-note">Pantalla real reservada para {meta.title}. La navegación ya no abre Partes, Documentación o Gerencia como sustituto incorrecto.</p><div className="actions">{meta.links.map((link: any) => <Link key={link.to} to={link.to}>{link.label}</Link>)}</div></Card></section>; }
...
function NotFound() {
  ...
  const superadminRoutes: Record<string, ReactNode> = {
    '/app/superadmin/usuarios': <SuperadminUsersV2 />,
...
function permissionForRole(role: string, permission: string) {
  ...
  if (role === 'SAT') return ['ver clientes','crear clientes','editar clientes','ver centros','crear centros','editar centros','ver equipos','crear equipos','editar equipos','ver partes','crear partes','editar partes','asignar técnicos','ver checks','crear checks','ejecutar checks','sincronizar trabajo técnico','ver documentación','gestionar plantillas'].includes(permission);
  if (role === 'Comercial') return ['ver clientes','crear clientes','editar clientes','ver centros','ver equipos','ver partes','crear partes','ver checks','ver documentación'].includes(permission);
  if (role === 'Oficina') return ['ver clientes','ver centros','ver equipos','ver partes','editar partes','ver checks','ver facturación','ver documentación'].includes(permission);
  if (role === 'Tecnico') return ['ver partes','editar partes','ver checks','ejecutar checks','sincronizar trabajo técnico','ver documentación'].includes(permission);
  return false;
}
