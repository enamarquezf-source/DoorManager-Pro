...
function StockMovementsModal({ material, onClose }: { material: any; onClose: () => void }) {
  const { data, loading, error, reload } = useLoad(() => materialsService.movements(material.id), [material.id], [] as any[]);
  return <SidePanel title={`Movimientos · ${material.code}`} subtitle={material.description} onClose={onClose}><StateBlock loading={loading} error={error} retry={reload} empty={!data.length}><div className="compact-list stock-movements-list">{data.map((row: any) => <article key={row.id}><div className="stock-movement-main"><strong>{formatDate(row.created_at)}</strong><span>{row.work_orders?.code ? `Parte: ${row.work_orders.code}${row.work_orders.title ? ` · ${row.work_orders.title}` : ''}` : 'Movimiento manual'}</span><small className="stock-movement-meta">{fullName(row.profiles) || 'Usuario no informado'}</small><small>{row.reason ?? 'Sin motivo informado.'}</small></div><div className="stock-movement-qty"><Badge tone={row.movement_type === 'out' ? 'warn' : row.movement_type === 'return' ? 'ok' : 'info'}>{displayStatus(row.movement_type)}</Badge><strong>{Number(row.quantity).toLocaleString('es-ES')} {material.unit ?? 'ud'}</strong><span>Stock: {Number(row.previous_stock).toLocaleString('es-ES')} → {Number(row.new_stock).toLocaleString('es-ES')}</span></div></article>)}</div></StateBlock></SidePanel>;
}
...
function MaterialsCanonicalSummary() {
  const stock = useLoad(() => workOrdersService.warehouseStockCatalog(), [], [] as any[]);
  const total = stock.data.reduce((sum: number, item: any) => sum + Number(item.quantity ?? 0), 0);
  return <Card title="Lectura de stock"><p className="large-note">Los indicadores legacy del catálogo se mantienen solo como referencia de compatibilidad. El saldo canónico por almacén es la autoridad operativa.</p><p><strong>Stock canónico total informativo:</strong> {stock.loading ? 'Cargando...' : `${total.toLocaleString('es-ES')} unidades`}. No implica disponibilidad en un almacén concreto.</p></Card>;
}
...
function CanonicalMaterialsModule() {
  ...
  const [search, setSearch] = useState('');
  const [archiveFilter, setArchiveFilter] = useState<MaterialFilter>('active');
  const { data, loading, error, reload } = useLoad(() => materialsService.list(search, undefined, archiveFilter), [search, archiveFilter], [] as any[]);
  const stock = useLoad(() => workOrdersService.warehouseStockCatalog(), [], [] as any[]);
  const warehouses = useLoad(() => workOrdersService.warehousesCatalog(), [], [] as any[]);
  const [editing, setEditing] = useState<any | null>(null);
  const [adjusting, setAdjusting] = useState<any | null>(null);
  const quantity = (materialId: string) => stock.data.filter((row: any) => row.material_id === materialId).reduce((sum: number, row: any) => sum + Number(row.quantity ?? 0), 0);
  const status = (material: any) => { const current = quantity(material.id); if (current <= 0) return 'Sin stock'; if (material.minimum_stock > 0 && current <= Number(material.minimum_stock)) return 'Bajo stock'; return 'Activo'; };
  return <><ListPage title="Materiales" search={search} setSearch={setSearch} archiveFilter={archiveFilter} setArchiveFilter={setArchiveFilter} action={<button className="primary" onClick={() => setEditing({})}>Crear material</button>} loading={loading} error={error} retry={reload} empty={!data.length}><div className="record-list">{data.map((material: any) => <RecordCard key={material.id} archived={Boolean(material.deleted_at)} title={<strong>{material.code} · {material.description}</strong>} status={[status(material), status(material) === 'Activo' ? 'ok' : status(material) === 'Bajo stock' ? 'warn' : 'danger']} actions={<><button onClick={() => setEditing(material)}>Editar material</button><button onClick={() => setAdjusting(material)}>Ajustar stock</button><LifecycleActionPanel entity="materials" record={material} onChanged={reload} /></>} meta={<RecordMeta items={[[ 'Unidad', material.unit ?? 'ud' ], [ 'Coste', `${Number(material.cost ?? 0).toLocaleString('es-ES')} €` ], [ 'Precio venta', `${Number(material.price ?? 0).toLocaleString('es-ES')} €` ], [ 'Stock', `${quantity(material.id).toLocaleString('es-ES')} ${material.unit ?? 'ud'}` ]]} />} />)}</div></ListPage>{editing && <CanonicalMaterialForm initial={editing.id ? editing : undefined} warehouses={warehouses.data} onClose={() => setEditing(null)} onSaved={() => { setEditing(null); reload(); stock.reload(); }} />}{adjusting && <CanonicalStockAdjustForm material={adjusting} warehouses={warehouses.data} onClose={() => setAdjusting(null)} onSaved={() => { setAdjusting(null); stock.reload(); }} />}</>;
}
