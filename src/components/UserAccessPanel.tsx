...
export function UserAccessPanel({ user, onSaved }: Props) {
  ...
  const inherited = effectivePermissionKeys({ ...user, primary_area: null, roles: access.roles ?? [], permission_grants: [] } as any);
  ...
  return <Card title={`Permisos y visibilidad · ${user.first_name} ${user.last_name}`}><p className="large-note">Los permisos heredados por rol no se duplican. Los cambios de esta pantalla son permisos adicionales y preferencias de navegación.</p><h4>Permisos funcionales</h4><div className="component-select permission-grid">{permissionCatalog.map((code) => { const inheritedByRole = inherited.has(code); const checked = inheritedByRole || selected[code] === true; return <label key={code}><input type="checkbox" checked={checked} disabled={inheritedByRole} onChange={(event) => { setDirty(true); setSelected((current) => ({ ...current, [code]: event.target.checked })); }} /> {permissionLabels[code]} <small>{inheritedByRole ? 'Permiso heredado por rol' : checked ? 'Permiso adicional' : ''}</small></label>; })}</div><h4>Visibilidad del menú</h4><div className="component-select permission-grid">{Object.keys(moduleLabels).map((code) => <label key={code}><input type="checkbox" checked={modules[code] !== false} onChange={(event) => { setDirty(true); setModules((current) => ({ ...current, [code]: event.target.checked })); }} /> {moduleLabels[code]}</label>)}</div>{error && <p className="form-error">{error}</p>}<div className="actions"><button className="primary" onClick={save} disabled={saving || !dirty}>{saving ? 'Guardando...' : 'Guardar cambios'}</button></div></Card>;
}
