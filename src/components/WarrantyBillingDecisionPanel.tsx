...
export function WarrantyBillingDecisionPanel({ workOrder, lines, canDecide, onChanged }: { workOrder: any; lines: any[]; canDecide: boolean; onChanged: () => void }) {
  ...
  const setDecision = async (line: any, billingDecision: BillingDecision) => {
    ...
    await runWarrantyDecisionOnce(pendingRef.current, key, async () => {
      setSaving(new Set(pendingRef.current)); setError('');
    ...
    setSaving(new Set(pendingRef.current));
  };
  return <section className="warranty-billing-decisions"><h4>DECISIÓN DE GARANTÍA</h4><p className="large-note">NULL significa cubierto por garantía por defecto.</p><div className="compact-list">{lines.filter((line: any) => !['fee', 'discount', 'labor'].includes(line.line_type)).map((line: any) => { const material = line.line_type === 'material' || line.material_id; const row = (material ? materialDecisions : lineDecisions).find((item: any) => item.quote_line_id === line.id); const decision = row?.billing_decision; const key = `${material ? 'planned_material' : 'quote_line'}:${line.id}`; const isSaving = saving.has(key); return <article key={line.id}><strong>{line.description}</strong><p>{decision === 'facturable' ? 'Facturable' : 'Cubierto por garantía (por defecto)'}</p><div className="row-actions"><button disabled={isSaving} onClick={() => setDecision(line, 'cubierto_garantia')}>Cubierto por garantía</button><button className="primary" disabled={isSaving} onClick={() => setDecision(line, 'facturable')}>Facturable</button></div></article>; })}</div>{error && <p className="form-error">{error}</p>}</section>;
}
