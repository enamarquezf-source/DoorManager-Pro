...
describe('audit_log operations 076', () => {
  ...
  it('proves all literal audit operations are admitted by 076', () => {
    ...
    const admitted = finalConstraintOperations(migration099);
    expect([...operations].filter((operation) => !admitted.has(operation))).toEqual([]);
    expect([...admitted].sort()).toEqual([...new Set([...legacyOperations, ...officeOperations, ...futureOperations, 'INVOICE_DRAFT_CREATE', 'INVOICE_DRAFT_UPDATE', 'WAREHOUSE_STOCK_RECONCILE', 'ECONOMIC_REVIEW_APPROVE', 'INVOICE_ISSUE_OVERRIDE'])].sort());
