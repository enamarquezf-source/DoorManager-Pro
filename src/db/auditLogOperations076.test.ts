...
describe('audit_log operations 076', () => {
  ...
  it('proves all literal audit operations are admitted by 076', () => {
    ...
    const admitted = new Set([...finalConstraintOperations(migration099), ...finalConstraintOperations(migration100)]);
    expect([...operations].filter((operation) => !admitted.has(operation))).toEqual([]);
    expect([...admitted].sort()).toEqual([...new Set([...legacyOperations, ...officeOperations, ...futureOperations, 'INVOICE_DRAFT_CREATE', 'INVOICE_DRAFT_UPDATE', 'WAREHOUSE_STOCK_RECONCILE', 'ECONOMIC_REVIEW_APPROVE', 'ECONOMIC_REVIEW_REOPEN', 'INVOICE_ISSUE_OVERRIDE'])].sort());
