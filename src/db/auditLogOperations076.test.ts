...
const migration099 = readFileSync(new URL('../../supabase/migrations/099_economic_review_and_structured_billing.sql', import.meta.url), 'utf8');
const migration100 = readFileSync(new URL('../../supabase/migrations/100_economic_review_billing_hardening.sql', import.meta.url), 'utf8');
const migration131 = readFileSync(new URL('../../supabase/migrations/131_supplier_invoice_payments.sql', import.meta.url), 'utf8');
...
describe('audit_log operations 076', () => {
  ...
  it('proves all literal audit operations are admitted by 076', () => {
    ...
    const admitted = new Set([...finalConstraintOperations(migration099), ...finalConstraintOperations(migration100), ...finalConstraintOperations(migration131)]);
    expect([...operations].filter((operation) => !admitted.has(operation))).toEqual([]);
    expect([...admitted].sort()).toEqual([...new Set([...legacyOperations, ...officeOperations, ...futureOperations, 'INVOICE_DRAFT_CREATE', 'INVOICE_DRAFT_UPDATE', 'WAREHOUSE_STOCK_RECONCILE', 'ECONOMIC_REVIEW_APPROVE', 'ECONOMIC_REVIEW_REOPEN', 'INVOICE_ISSUE_OVERRIDE', 'PAYMENT_REVERSE'])].sort());
