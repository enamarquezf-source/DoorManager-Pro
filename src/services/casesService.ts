...
export const casesService = {
  async list(search = '', companyScope?: string | null, archiveFilter: ArchiveFilter = 'active') {
    ...
    let query = applyArchiveFilter(supabase.from('cases').select('id,company_id,client_id,site_id,code,title,description,type,priority,status,origin,created_at,deleted_at,delete_reason,clients!cases_client_id_fkey(code,legal_name),sites!cases_site_id_fkey(code,name)'), archiveFilter).order('created_at', { ascending: false });
