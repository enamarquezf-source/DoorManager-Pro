...
export const dashboardService = {
  async getSatDashboardData() {
    ...
    const [workOrders, assignments, technicians, pendingChecks, completedChecks, deficiencies, alerts, materials] = await Promise.all([
      expectStep('Inicio SAT / partes', () => expectData<any[]>(supabase.from('v_work_order_full_detail').select('id,company_id,code,title,status,scheduled_date,scheduled_time,priority,type,description,client_name,site_name,equipment_code,main_technician_name,creator_name,planned_material,deleted_at').eq('company_id', companyId).order('scheduled_time', { ascending: true }), { service: 'dashboardService', operation: 'Inicio SAT / partes', resource: 'v_work_order_full_detail' })),
