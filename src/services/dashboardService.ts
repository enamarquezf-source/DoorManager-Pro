...
export const dashboardService = {
  async getSatDashboardData() {
    ...
    const [workOrders, assignments, technicians, pendingChecks, completedChecks, deficiencies, alerts, materials] = await Promise.all([
      expectStep('Inicio SAT / partes', () => expectData<any[]>(supabase.from('v_work_order_full_detail').select('id,company_id,code,title,status,scheduled_date,scheduled_time,priority,type,description,client_name,site_name,equipment_code,main_technician_name,created_by_name,deleted_at').eq('company_id', companyId).order('scheduled_time', { ascending: true }), { service: 'dashboardService', operation: 'Inicio SAT / partes', resource: 'v_work_order_full_detail' })),
      ...
      expectStep('Inicio SAT / tecnicos', () => expectData<any[]>(supabase.from('profiles').select('id,first_name,last_name,primary_area,active,profile_roles!profile_roles_profile_id_fkey(roles!profile_roles_role_id_fkey(name))').eq('company_id', companyId).eq('active', true).is('deleted_at', null).order('first_name'), { service: 'dashboardService', operation: 'Inicio SAT / tecnicos', resource: 'profiles' })),
      ...
      expectStep('Inicio SAT / deficiencias', () => expectData<any[]>(supabase.from('deficiencies').select('id,code,status,severity,description,created_at,clients!deficiencies_client_id_fkey(code,legal_name),equipment!deficiencies_equipment_id_fkey(code),work_orders!deficiencies_work_order_id_fkey(code)').eq('company_id', companyId).is('deleted_at', null).order('created_at', { ascending: false }), { service: 'dashboardService', operation: 'Inicio SAT / deficiencias', resource: 'deficiencies' })),
      expectStep('Inicio SAT / avisos', () => expectData<any[]>(supabase.from('alerts').select('id,code,title,description,status,priority,type,related_entity,related_id,created_at').eq('company_id', companyId).is('deleted_at', null).order('created_at', { ascending: false }), { service: 'dashboardService', operation: 'Inicio SAT / avisos', resource: 'alerts' })),
      expectStep('Inicio SAT / materiales', () => expectData<any[]>(supabase.from('work_order_materials').select('id,work_order_id,created_at,work_orders!work_order_materials_work_order_id_fkey(code,title,status),materials!work_order_materials_material_id_fkey(code,description)').eq('company_id', companyId).order('created_at', { ascending: false }), { service: 'dashboardService', operation: 'Inicio SAT / materiales', resource: 'work_order_materials' })),
    ]);
  ...
  async getCommercialDashboardData() {
    const companyId = await currentProfileCompanyId();
    const [opportunities, quotes, deficiencies, clients] = await Promise.all([
    ...
    return { opportunities, quotes, deficiencies, clients, alerts: [], workOrders: [] };
  },

  async getOfficeDashboardData() {
    const companyId = await currentProfileCompanyId();
    const [documents, materialRequests, workOrderMaterials, alerts, workOrders, suppliers] = await Promise.all([
      ...
      expectData<any[]>(supabase.from('alerts').select('id,code,title,description,status,priority,type,related_entity,related_id,created_at').eq('company_id', companyId).is('deleted_at', null).order('created_at', { ascending: false })),
      expectData<any[]>(supabase.from('v_work_order_full_detail').select('id,code,status,scheduled_date,client_name').eq('company_id', companyId).order('scheduled_date', { ascending: false })),
      expectData<any[]>(supabase.from('suppliers').select('id,code,name,status').eq('company_id', companyId).is('deleted_at', null).order('name')),
    ]);
  ...
  async getManagementDashboardData() {
    const companyId = await currentProfileCompanyId();
    const [metrics, workOrders, deficiencies, alerts, clients] = await Promise.all([
      expectData<any[]>(supabase.from('v_management_metrics').select('*').eq('company_id', companyId)),
      expectData<any[]>(supabase.from('v_work_order_full_detail').select('id,code,status,priority,scheduled_date,client_name').eq('company_id', companyId).order('scheduled_date', { ascending: false })),
