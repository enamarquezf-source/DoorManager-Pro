...
export function workOrderEconomicSummary(workOrder: any) {
  ...
  const materialCost = materials.reduce((sum: number, row: any) => sum + Number(row.used_quantity ?? row.quantity ?? 0) * Number(row.unit_cost ?? row.materials?.cost ?? row.unit_price ?? 0), 0);
  const materialSale = materials.reduce((sum: number, row: any) => sum + Number(row.used_quantity ?? row.quantity ?? 0) * Number(row.unit_price ?? row.materials?.price ?? 0), 0);
