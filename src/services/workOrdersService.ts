...
export const workOrdersService = {
  ...
  async getWorkOrderFullDetail(workOrderId: string): Promise<WorkOrderFullDetail> {
    try {
      ...
      const [associated, compatibleCheckTemplates, assignments, history, timeEntries, notes, materials, plannedQuoteLines, plannedQuoteLineDecisions, plannedMaterialDecisions, costEntries, checks, deficiencies, alertRows, documents, signatures, photos] = await Promise.all([
        expectStep('Detalle parte / equipos asociados', () => expectData<any[]>(supabase.from('work_order_equipment').select('*, equipment!work_order_equipment_equipment_id_fkey(*, equipment_types!equipment_equipment_type_id_fkey(*))').eq('work_order_id', workOrderId).order('is_primary', { ascending: false }).order('created_at'))),
        expectStep('Detalle parte / plantillas compatibles', () => expectData<any[]>(supabase.from('check_templates').select('id, equipment_type_id, company_id, name').eq('active', true).or(`company_id.eq.${workOrder.company_id},company_id.is.null`))),
        expectStep('Detalle parte / asignaciones', () => expectData<any[]>(supabase.from('work_order_assignments').select('*, profiles!work_order_assignments_technician_id_fkey(*)').eq('work_order_id', workOrderId).is('deleted_at', null).order('planned_start_time'))),
        expectStep('Detalle parte / historial estados', () => expectData<any[]>(supabase.from('work_order_status_history').select('*, profiles!work_order_status_history_changed_by_fkey(first_name,last_name)').eq('work_order_id', workOrderId).order('changed_at', { ascending: true }))),
        expectStep('Detalle parte / horas', () => expectData<any[]>(supabase.from('work_order_time_entries').select('*, profiles!work_order_time_entries_profile_id_fkey(first_name,last_name,primary_area), created_by_profile:profiles!work_order_time_entries_created_by_fkey(first_name,last_name,primary_area), updated_by_profile:profiles!work_order_time_entries_updated_by_fkey(first_name,last_name,primary_area)').eq('work_order_id', workOrderId).order('work_date', { ascending: true }))),
        expectStep('Detalle parte / notas', () => expectData<any[]>(supabase.from('work_order_notes').select('*, profiles!work_order_notes_created_by_fkey(first_name,last_name)').eq('work_order_id', workOrderId).order('created_at', { ascending: true }))),
        expectStep('Detalle parte / materiales', () => expectData<any[]>(supabase.from('work_order_materials').select('*, materials!work_order_materials_material_id_fkey(*), profiles!work_order_materials_registered_by_fkey(first_name,last_name,primary_area)').eq('work_order_id', workOrderId).is('deleted_at', null).order('created_at', { ascending: true }))),
        workOrder.quote_id ? expectStep('Detalle parte / conceptos previstos', () => expectData<any[]>(supabase.from('quote_lines').select('*, materials!quote_lines_material_id_fkey(*)').eq('quote_id', workOrder.quote_id).is('deleted_at', null).order('position', { ascending: true }))) : Promise.resolve([]),
        workOrder.quote_id ? expectStep('Detalle parte / decisiones conceptos previstos', () => expectData<any[]>(supabase.from('work_order_quote_line_decisions').select('*').eq('work_order_id', workOrderId).is('deleted_at', null))) : Promise.resolve([]),
        workOrder.quote_id ? expectStep('Detalle parte / decisiones materiales previstos', () => expectData<any[]>(supabase.from('work_order_planned_material_decisions').select('*').eq('work_order_id', workOrderId).is('deleted_at', null))) : Promise.resolve([]),
        expectStep('Detalle parte / recursos y costes', () => expectData<any[]>(supabase.from('work_order_cost_entries').select('*, profiles!work_order_cost_entries_registered_by_fkey(first_name,last_name,primary_area)').eq('work_order_id', workOrderId).is('deleted_at', null).order('incurred_at', { ascending: true }))),
        expectStep('Detalle parte / checks', () => expectData<any[]>(supabase.from('checks').select('*, check_templates!checks_template_id_fkey(*, check_template_sections!check_template_sections_template_id_fkey(*, check_template_items!check_template_items_section_id_fkey(*))), equipment!checks_equipment_id_fkey(*, equipment_types!equipment_equipment_type_id_fkey(*)), profiles!checks_technician_id_fkey(first_name,last_name), check_section_results!check_section_results_check_id_fkey(*), check_photos!check_photos_check_id_fkey(*)').eq('work_order_id', workOrderId).is('deleted_at', null).order('created_at', { ascending: false }))),
        expectStep('Detalle parte / deficiencias', () => expectData<any[]>(supabase.from('deficiencies').select('*, equipment!deficiencies_equipment_id_fkey(*), checks!deficiencies_check_id_fkey(code), profiles!deficiencies_responsible_profile_id_fkey(first_name,last_name)').eq('work_order_id', workOrderId).is('deleted_at', null).order('created_at', { ascending: false }))),
        expectStep('Detalle parte / avisos', () => expectData<any[]>(supabase.from('alerts').select('*, alert_recipients!alert_recipients_alert_id_fkey(*)').eq('related_entity', 'work_orders').eq('related_id', workOrderId).is('deleted_at', null).order('created_at', { ascending: false }))),
        expectStep('Detalle parte / documentos', () => expectData<any[]>(supabase.from('document_links').select('*, documents!document_links_document_id_fkey(*)').eq('related_type', 'Parte').eq('related_id', workOrderId).order('created_at', { ascending: false }))),
        expectStep('Detalle parte / firmas', () => expectData<any[]>(supabase.from('work_order_signatures').select('*, files!work_order_signatures_file_id_fkey(*)').eq('work_order_id', workOrderId).order('signed_at', { ascending: false }))),
        expectStep('Detalle parte / fotos', () => expectData<any[]>(supabase.from('work_order_photos').select('*, files!work_order_photos_file_id_fkey(*), profiles!work_order_photos_taken_by_fkey(first_name,last_name)').eq('work_order_id', workOrderId).order('taken_at', { ascending: false }))),
      ]);
