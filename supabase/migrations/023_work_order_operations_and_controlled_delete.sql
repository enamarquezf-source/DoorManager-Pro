...
begin
  ...
  if p_entity = 'work_orders' then
    ...
    delete from public.files f where f.id = any(v_file_ids) and not exists (select 1 from public.work_order_photos p where p.file_id = f.id) and not exists (select 1 from public.work_order_signatures s where s.file_id = f.id) and not exists (select 1 from public.check_photos cp where cp.file_id = f.id);
