...
alter table public.work_order_cost_entries add column if not exists updated_by uuid references public.profiles(id);
alter table public.work_order_cost_entries add column if not exists validated_at timestamptz;
alter table public.work_order_cost_entries add column if not exists validated_by uuid references public.profiles(id);
alter table public.work_order_cost_entries add column if not exists deleted_at timestamptz;
alter table public.work_order_cost_entries add column if not exists deleted_by uuid references public.profiles(id);
alter table public.work_order_cost_entries add column if not exists delete_reason text;
...
create index if not exists work_order_cost_entries_active_idx on public.work_order_cost_entries(work_order_id, incurred_at) where deleted_at is null;
drop index if exists public.work_order_cost_entries_work_order_local_change_unique;
create unique index if not exists work_order_cost_entries_work_order_local_change_unique on public.work_order_cost_entries(company_id, work_order_id, local_change_id) where local_change_id is not null and deleted_at is null;
...
begin
  ...
  insert into public.work_order_cost_entries(company_id, work_order_id, cost_type, description, quantity, unit, unit_cost, incurred_at, registered_by, updated_by, local_change_id)
  values (v_work.company_id, v_work.id, v_cost_type, trim(p_payload->>'description'), v_quantity, coalesce(nullif(p_payload->>'unit', ''), 'ud'), case when public.has_any_role(array['superadmin','SAT','Gerencia','Oficina']) then v_unit_cost else 0 end, coalesce(nullif(p_payload->>'incurred_at', '')::date, current_date), v_profile.id, v_profile.id, v_local)
...
begin
  ...
  if v_entry.deleted_at is not null then raise exception 'Recurso o coste ya eliminado'; end if;
  ...
  if v_entry.validated_at is not null and not public.has_any_role(array['superadmin','SAT','Gerencia','Oficina']) then raise exception 'No tienes permisos para eliminar costes validados'; end if;
  update public.work_order_cost_entries set deleted_at = now(), deleted_by = v_profile.id, delete_reason = trim(p_reason), updated_at = now(), updated_by = v_profile.id where id = p_cost_entry_id;
  insert into public.audit_log(company_id, table_name, record_id, operation, changed_by, old_data, new_data) values (v_entry.company_id, 'work_order_cost_entries', v_entry.id, 'SOFT_DELETE', v_profile.id, to_jsonb(v_entry), jsonb_build_object('reason', p_reason, 'deleted_at', now(), 'deleted_by', v_profile.id));
  return p_cost_entry_id;
end;
$$;
