...
begin
  ...
  if v_quote_id is not null then
    ...
    if lower(coalesce(v_quote.status, '')) not in ('aceptado','accepted','ejecutado en cliente') then raise exception 'validacion del formulario: presupuesto no aceptado para generar parte'; end if;
  end if;
