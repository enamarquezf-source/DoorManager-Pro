-- DoorManager Pro - canonical work-order sale model.
-- A quoted work order uses the accepted quote plus explicit additional sales.
-- An unquoted work order uses billable operational snapshots.
begin;
...
with mat as (
  ...
  select distinct on (wo.company_id,wo.id) wo.company_id,wo.id work_order_id,
    round(coalesce(q.taxable_base,q.subtotal_sale,q.subtotal,0),2) quoted_sale_amount, true has_accepted_quote
  ...
  select wo.*,c.legal_name client_name,s.name site_name,e.code equipment_code,
    ...
    coalesce(aux.additional_sale_amount,coalesce(wo.additional_sale_amount,0)) additional_calc,
  ...
  select b.*,round(b.material_cost+b.time_cost+b.auxiliary_cost,2) real_cost_calc,
    case when b.warranty or not b.billable or b.economic_status in ('garantia','no_facturable') then 0
      when b.has_accepted_quote then round(b.quoted_calc+b.additional_calc,2)
      else round(b.material_sale+b.time_sale+b.auxiliary_sale,2) end sale_calc
  from base b
)
...
w as (select company_id,count(*) work_orders,count(*) filter(where scheduled_date>=date_trunc('month',current_date)::date) work_orders_this_month,
  ...
  count(*) filter(where status in ('Finalizado tecnicamente','Enviado','Cerrado') and sale_amount>0 and coalesce(invoiced_amount,0)=0 and not warranty) pending_invoice_work_orders,
  round(sum(real_cost_amount) filter(where warranty or economic_status='garantia'),2) warranty_cost,round(sum(real_cost_amount),2) real_cost,round(sum(sale_amount),2) sale_amount,
...
q as (select q.company_id,count(*) filter(where q.status='Aceptado') accepted_quotes,count(*) filter(where q.status='Ejecutado en cliente') executed_quotes,
  round(sum(coalesce(q.tax_amount,0)),2) tax_amount,round(sum(coalesce(q.total_amount,q.total,0)),2) total_amount
  from public.quotes q where q.deleted_at is null and q.status in ('Aceptado','Ejecutado en cliente') group by q.company_id)
select c.id company_id,coalesce(cc.clients,0) clients,coalesce(ec.equipment,0) equipment,coalesce(w.work_orders_this_month,0) work_orders_this_month,
  ...
  round(coalesce(w.sale_amount,0)-coalesce(w.real_cost,0),2) margin_amount,
  case when coalesce(w.sale_amount,0)>0 then round((coalesce(w.sale_amount,0)-coalesce(w.real_cost,0))/coalesce(w.sale_amount,0)*100,2) else null end margin_percentage,
