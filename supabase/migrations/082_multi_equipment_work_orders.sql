-- DoorManager Pro - partes con multiples equipos.
-- Reutiliza work_order_equipment y conserva main_equipment_id como principal.
-- No consume stock ni factura y la falta de plantilla es un aviso no bloqueante.

begin;

alter table public.work_order_equipment
  add column if not exists check_status text not null default 'not_applicable',
  add column if not exists check_message text;

create index if not exists work_order_equipment_work_order_idx
  on public.work_order_equipment(work_order_id, created_at);

insert into public.work_order_equipment (company_id, work_order_id, equipment_id, is_primary)
select wo.company_id, wo.id, wo.main_equipment_id, true
from public.work_orders wo
where wo.main_equipment_id is not null
  and not exists (
    select 1 from public.work_order_equipment woe
    where woe.work_order_id = wo.id and woe.equipment_id = wo.main_equipment_id
  );
...
begin
  ...
  for v_item in select value from jsonb_array_elements(v_selection) loop
    ...
    if v_type = 'Instalacion' then
      select id into v_template_id from public.check_templates where active = true and (company_id = v_company_id or company_id is null) and equipment_type_id = v_equipment_type_id and (lower(name) like '%instal%' or lower(name) like '%puesta en marcha%') order by company_id nulls last, updated_at desc, created_at desc limit 1;
...
declare v_company_id uuid; v_type_id uuid; v_template_id uuid; v_check_id uuid; v_technician_id uuid; v_type text; v_code text;
begin
  ...
  select id into v_template_id from public.check_templates where active and (company_id = v_company_id or company_id is null) and equipment_type_id = v_type_id and (lower(name) like '%instal%' or lower(name) like '%puesta en marcha%') order by company_id nulls last, updated_at desc, created_at desc limit 1;
  ...
  if v_check_id is null then insert into public.checks(company_id, code, work_order_id, equipment_id, template_id, technician_id) values (v_company_id, public.next_dmp_code(v_company_id, 'checks', 'CHK', true, 6), p_work_order_id, p_equipment_id, v_template_id, coalesce(v_technician_id, public.current_profile_id())) returning id into v_check_id; end if;
