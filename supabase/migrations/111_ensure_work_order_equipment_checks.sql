-- DoorManager Pro - asegura checks por equipo en partes de mantenimiento e instalacion.
-- La operacion sigue siendo transaccional dentro de create_work_order_full.
...
declare
  ...
  v_check_id uuid;
begin
  if p_work_order_type not in ('Instalacion', 'Mantenimiento') then
...
begin
  ...
  if v_main_equipment_id is not null then
    ...
    perform public.dmp_ensure_work_order_equipment_check(v_company_id, v_id, v_main_equipment_id, coalesce(v_technician_id, v_created_by), v_type);
    v_is_first := false;
  end if;
  ...
  for v_item in select value from jsonb_array_elements(v_selection) loop
    ...
    perform public.dmp_ensure_work_order_equipment_check(v_company_id, v_id, v_equipment_id, coalesce(v_technician_id, v_created_by), v_type);
...
begin
  ...
  if v_type not in ('Instalacion', 'Mantenimiento') then raise exception 'El parte no admite checks por equipo'; end if;
